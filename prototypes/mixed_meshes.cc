 ...
 * Quads and triangles use the same Gauss quadrature on the faces (which is in
 * 2D lines)
 *
 * Does not work in 3D out of the box, need to adopt hp_vertex_dof_identities()
 * and hp_quad_dof_identities() for FE_Q and FE_SimplexP elements.
 *
...
public:
  ...
  void
  reinit(hp::MappingCollection<dim> const &mapping,
         DoFHandler<dim> const            &dof_handler,
         hp::QCollection<dim> const       &quad,
         AffineConstraints<number> const  &constraints,
         unsigned int const mg_level         = numbers::invalid_unsigned_int,
         bool const         ones_on_diagonal = false)
  {
  ...
  rhs(VectorType &vec) const
  {
    ...
    matrix_free.template cell_loop<VectorType, int>(
      [&](auto const &matrix_free, auto &dst, auto const &, auto const range) {
        (void)matrix_free;
...
void
do_test(unsigned int const fe_degree)
{
  ...
  unsigned int const subdivisions = dim == 2 ? 25 : 4;

  if (dim == 2)
    GridGenerator::subdivided_hyper_cube_with_simplices_mix(tria, subdivisions);
  else
    GridGenerator::mixed_3d_mesh(tria, subdivisions, false);
  ...
  if (false)
    {
      ...
      // data_out.build_patches(mappings, 1);
  ...
  op.reinit(mappings,
            ...
            true);
  LinearAlgebra::distributed::Vector<Number> vec1, vec2, x, b;
      ...
      for (unsigned int r = 0; r < 5; ++r)
        {
          ...
          double const run_time = time.wall_time();
