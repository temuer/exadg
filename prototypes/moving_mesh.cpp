...
do_test()
{
  ...
  position.reinit(dof_handler.locally_owned_dofs(),
                  relevant_dofs,
                  MPI_COMM_SELF);
  displacement.reinit(dof_handler.locally_owned_dofs(),
                      relevant_dofs,
                      MPI_COMM_SELF);
  ...
  data_out.build_patches(mapping_original,
                         fe.degree,
  ...
  dealii::
    MappingFEField<dim, dim, dealii::LinearAlgebra::distributed::Vector<double>>
      mapping(dof_handler, vector);
