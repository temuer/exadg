...
namespace ExaDG
{
  ...
  public:
    ...
    {
      AssertThrow(i < M,
                  dealii::ExcMessage("Index exceeds matrix dimensions."));
  ...
  public:
    ...
    void
    set_value(value_type const   value,
              unsigned int const i,
              unsigned int const j)
    {
      AssertThrow(i < M and j < M,
                  dealii::ExcMessage("Index exceeds matrix dimensions."));
  ...
  {
    std::cout << std::endl
              << "GMRES solver (double), size M=3:" << std::endl
              << std::endl;
    ...
    Elementwise::SolverGMRES<double, Matrix, Preconditioner> gmres_solver(
      M, solver_data);
    ...
    std::cout << "L2 norm of initial residual = " << std::scientific
              << std::setprecision(1) << resi.l2_norm() << std::endl;
    ...
    std::cout << "L2 norm of final residual = " << std::scientific
              << std::setprecision(1) << res.l2_norm() << std::endl;
  }
  ...
  {
    std::cout << std::endl
              << "GMRES solver (double), size M=10000:" << std::endl
              << std::endl;
  ...
  gmres_test_2a()
  {
    ...
    typedef Elementwise::PreconditionerIdentity<dealii::VectorizedArray<double>>
                                                      Preconditioner;
    typedef MyMatrix<dealii::VectorizedArray<double>> Matrix;
    Preconditioner                                    preconditioner(M);
    Elementwise::
      SolverGMRES<dealii::VectorizedArray<double>, Matrix, Preconditioner>
        gmres_solver(M, solver_data);
    ...
    for(unsigned int v = 0; v < dealii::VectorizedArray<double>::size(); ++v)
      std::cout << "L2 norm of initial residual[" << v << "] = " <<
    l2_norm_initial_res[v] << std::endl;
     */
  ...
  gmres_test_2c()
  {
    ...
    dealii::VectorizedArray<double> inhom_array =
      dealii::make_vectorized_array<double>(1.0);
    for (unsigned int v = 0; v < dealii::VectorizedArray<double>::size(); ++v)
      if (v > 1)
        inhom_array[v] = 0.0;
