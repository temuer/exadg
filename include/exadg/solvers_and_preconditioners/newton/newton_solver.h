...
namespace ExaDG
{
  namespace Newton
  {
    ...
    public:
      ...
      std::tuple<unsigned int /* Newton iter */,
                 unsigned int /* accumulated linear iter */>
      solve(VectorType &solution, UpdateData const &update)
      {
        ...
        while (norm_r > this->solver_data.abs_tol and
               norm_r / norm_r_0 > solver_data.rel_tol and
               newton_iterations < solver_data.max_iter)
          {
            // reset increment
            increment = 0.0;

            // multiply by -1.0 since the linearized problem is "linear_operator
            // * increment = - residual"
            residual *= -1.0;
            ...
            // determine whether to update the operator/preconditioner of the
            // linearized problem
            bool const update_now =
              update.do_update and
              (newton_iterations % update.update_every_newton_iter == 0);
            ...
            unsigned int const n_iter_linear =
              linear_solver.solve(increment, residual);
            ...
            unsigned int const max_iter_damp =
              10;                   // max iterations of damping scheme
            double const tau = 0.5; // a parameter (has to be smaller than 1)
            do
              {
                // add increment to solution vector but scale by a factor omega
                // <= 1
                temporary = solution;
            ...
            while (norm_r_damp >= (1.0 - tau * omega) * norm_r and
                   n_iter_damp < max_iter_damp);
        ...
        AssertThrow(
          norm_r <= this->solver_data.abs_tol or
        ...
        return std::tuple<unsigned int, unsigned int>(newton_iterations,
                                                      linear_iterations);
      }
