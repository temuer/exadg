...
template<int dim, int n_components>
ConstantModes<dim, n_components>::ConstantModes(AMGOperatorType    amg_operator_type,
                                                unsigned int const mode_index)
  : dealii::Function<dim>(n_components),
    amg_operator_type(amg_operator_type),
...
template<int dim, int n_components>
double
ConstantModes<dim, n_components>::value(dealii::Point<dim> const & p,
  ...
  get_constant_modes()
  {
    ...
    for(unsigned int i = 0; i < n_constant_modes; ++i)
    {
      ...
      unsigned int const n_components =
        pde_operator.get_matrix_free().get_dof_handler(dof_index).get_fe().n_components();
