...
namespace ExaDG
{
  ...
  create_subcommunicator(dealii::DoFHandler<dim, spacedim> const &dof_handler)
  {
    ...
    if (dealii::Utilities::MPI::min(n_locally_owned_cells, mpi_comm) == 0)
      ...
      {
        std::unique_ptr<MPI_Comm, void (*)(MPI_Comm *)> communicator(
          new MPI_Comm, [](MPI_Comm *comm) { delete comm; });
  ...
  get_ML_parameter_list(
    dealii::TrilinosWrappers::PreconditionAMG::AdditionalData const &ml_data,
    int const                                                        dimension)
  {
    Teuchos::ParameterList parameter_list;

    // Slightly modified from
    // deal::PreconditionAMG::AdditionalData::set_parameters().
...
#  endif
  ...
  public:
      ...
      , ml_data(ml_data)
    {
      ...
      pde_operator.init_system_matrix(
        system_matrix,
    ...
    void
    apply_krylov_solver_with_amg_preconditioner(
      VectorType                      &dst,
    ...
    update() override
    {
      // Clear content of matrix since calculate_system_matrix() adds the
      // result.
      system_matrix *= 0.0;
      ...
      if (not some_constant_mode_set)
        {
          bool const some_constant_mode_values_set =
            dealii::Utilities::MPI::logical_or(constant_modes_values.size() > 0,
                                               mpi_comm);
          AssertThrow(
            some_constant_mode_values_set > 0,
            dealii::ExcMessage(
  ...
  private:
    ...
    typedef dealii::PETScWrappers::PreconditionBoomerAMG::AdditionalData
      BoomerData;
  ...
  public:
    ...
    PreconditionerBoomerAMG(Operator const &op,
      ...
      : subcommunicator(create_subcommunicator(
          op.get_matrix_free().get_dof_handler(op.get_dof_index())))
      , pde_operator(op)
  ...
  private:
    ...
    calculate_preconditioner()
    {
      // calculate_matrix in case the current MPI rank participates in the PETSc
      // communicator
      if (system_matrix.m() > 0)
        {
          pde_operator.calculate_system_matrix(system_matrix);
  ...
  /**
   * Implementation of AMG preconditioner unifying PreconditionerML and
   * PreconditionerBoomerAMG.
   */
  template <typename Operator, typename Number>
  ...
  public:
      ...
      if (data.amg_type == AMGType::BoomerAMG)
        {
          ...
          preconditioner_boomer =
            std::make_shared<PreconditionerBoomerAMG<Operator, Number>>(
              pde_operator, initialize, data.boomer_data);
#else
          AssertThrow(
            false, dealii::ExcMessage("deal.II is not compiled with PETSc!"));
#endif
        }
...
#ifdef DEAL_II_WITH_TRILINOS
          preconditioner_ml =
            std::make_shared<PreconditionerML<Operator>>(pde_operator,
                                                         initialize,
                                                         data.ml_data);
#else
          AssertThrow(false,
      ...
      SolverData const                &solver_data) const
    {
      if (data.amg_type == AMGType::BoomerAMG)
        {
#ifdef DEAL_II_WITH_PETSC
          std::shared_ptr<PreconditionerBoomerAMG<Operator, Number>>
            preconditioner = std::dynamic_pointer_cast<
              PreconditionerBoomerAMG<Operator, Number>>(preconditioner_boomer);

          preconditioner->apply_krylov_solver_with_amg_preconditioner(
            dst, src, solver_type, solver_data);
#else
          AssertThrow(
            false, dealii::ExcMessage("deal.II is not compiled with PETSc!"));
#endif
          ...
          apply_function_in_double_precision(
            ...
            [&](dealii::LinearAlgebra::distributed::Vector<double> &dst_double,
                dealii::LinearAlgebra::distributed::Vector<double> const
                  &src_double) {
              preconditioner->apply_krylov_solver_with_amg_preconditioner(
                dst_double, src_double, solver_type, solver_data);
            });
#else
          AssertThrow(false,
                      dealii::ExcMessage(
