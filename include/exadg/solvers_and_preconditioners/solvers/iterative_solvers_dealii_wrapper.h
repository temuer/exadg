...
namespace ExaDG
{
  namespace Krylov
  {
    ...
    public:
      ...
      mutable double
        n10; // number of iterations needed to reduce the residual by 1e10
    ...
    public:
      ...
      get_timings() const override
      {
        if (solver_data.use_preconditioner)
          {
            this->timer_tree->insert({"SolverCG"},
    ...
    void
    output_eigenvalues(std::vector<Number> const &eigenvalues,
    ...
    public:
      ...
      solve(VectorType &dst, VectorType const &rhs) const override
      {
        ...
        typename dealii::SolverGMRES<VectorType>::AdditionalData
          additional_data;
        ...
        if (solver_data.use_preconditioner == false)
          ...
          {
            solver.solve(this->underlying_operator,
                         dst,
                         rhs,
                         this->preconditioner);
          }
      ...
      get_timings() const override
      {
        if (solver_data.use_preconditioner)
          {
            this->timer_tree->insert({"SolverGMRES"},
                                     preconditioner.get_timings());
          }
    ...
    public:
      ...
      solve(VectorType &dst, VectorType const &rhs) const override
      {
        ...
        typename dealii::SolverFGMRES<VectorType>::AdditionalData
          additional_data;
        additional_data.max_basis_size = solver_data.max_n_tmp_vectors;
        // FGMRES always uses right preconditioning

        dealii::SolverFGMRES<VectorType> solver(solver_control,
                                                additional_data);
      ...
      get_timings() const override
      {
        if (solver_data.use_preconditioner)
          {
            this->timer_tree->insert({"SolverFGMRES"},
                                     preconditioner.get_timings());
          }
