  ...
  namespace Elementwise
  {
              ...
              unsigned int                     MAX_ITER)
    {
      for (unsigned int v = 0; v < dealii::VectorizedArray<Number>::size(); ++v)
        {
          if ((norm_r_abs[v] < ABS_TOL or norm_r_rel[v] < REL_TOL or
               k >= MAX_ITER))
            is_converged[v] = 1.0;
          else
            is_converged[v] = -1.0;
    ...
    void
    scale(value_type             *dst,
          value_type_scalar const scalar,
    ...
    template <typename value_type>
    value_type
    inner_product(value_type const  *vector1,
     ...
     */
    template <typename value_type, typename Matrix, typename Preconditioner>
    SolverCG<value_type, Matrix, Preconditioner>::SolverCG(
    ...
    template <typename value_type, typename Matrix, typename Preconditioner>
    void
    SolverCG<value_type, Matrix, Preconditioner>::solve(
      Matrix const         *matrix,
      value_type           *solution,
      value_type const     *rhs,
      ...
      while (true)
        {
          ...
          // check convergence
          if (all_smaller(norm_r_abs, ABS_TOL) or
              all_smaller(norm_r_rel, REL_TOL) or (n_iter > MAX_ITER))
            {
              break;
            }
    ...
    public:
      ...
      void
      solve(Matrix const         *A,
            value_type           *x,
            value_type const     *b,
            Preconditioner const *P) final;
    ...
    private:
      ...
      template <typename Number>
      void perform_givens_rotation_and_calculate_residual(
     ...
     */
    template <typename value_type, typename Matrix, typename Preconditioner>
    SolverGMRES<value_type, Matrix, Preconditioner>::SolverGMRES(
      unsigned int const unknowns,
      SolverData const  &solver_data)
      : M(unknowns)
      , ABS_TOL(solver_data.abs_tol)
    ...
    template <typename Number>
    void
    SolverGMRES<value_type, Matrix, Preconditioner>::print(Number, std::string)
    {}
    ...
    template <typename Number>
    void
    SolverGMRES<value_type, Matrix, Preconditioner>::print(
      dealii::VectorizedArray<Number> y,
      std::string                     name)
    {
    ...
    template <typename Number>
    void
    SolverGMRES<value_type, Matrix, Preconditioner>::
      perform_givens_rotation_and_calculate_residual(Number)
    {
    ...
    void
    SolverGMRES<value_type, Matrix, Preconditioner>::
      perform_givens_rotation_and_calculate_residual(
        dealii::VectorizedArray<Number>)
    {
      dealii::VectorizedArray<Number> H_i_k = dealii::VectorizedArray<Number>();
      dealii::VectorizedArray<Number> H_ip1_k =
        dealii::VectorizedArray<Number>();
      ...
      dealii::VectorizedArray<Number> res_k_store =
        dealii::VectorizedArray<Number>();
      res_k_store                           = res[k];
      dealii::VectorizedArray<Number> res_k = dealii::VectorizedArray<Number>();
      dealii::VectorizedArray<Number> res_kp1 =
        dealii::VectorizedArray<Number>();
      ...
      for (unsigned int v = 0; v < dealii::VectorizedArray<Number>::size(); ++v)
        {
          if (convergence_status[v] > 0.0)
            ...
            {
              // Givens rotations for residual-vector
              beta[v]    = std::sqrt(H[k][k][v] * H[k][k][v] +
                                  H[k][k + 1][v] * H[k][k + 1][v]);
    ...
    template <typename value_type, typename Matrix, typename Preconditioner>
    void
    SolverGMRES<value_type, Matrix, Preconditioner>::solve(
      Matrix const         *A,
      value_type           *x,
      value_type const     *b,
      Preconditioner const *P)
    {
      ...
      while (not converged(convergence_status,
                           norm_r_abs,
                           ABS_TOL,
                           norm_r_rel,
                           REL_TOL,
      ...
      //
      //    //temp2 = A*x
      //    dealii::AlignedVector<value_type> temp2 =
      //    dealii::AlignedVector<value_type>(M); A->vmult(temp2.begin(),x);
    ...
    template <typename value_type, typename Matrix, typename Preconditioner>
    void
    SolverGMRES<value_type, Matrix, Preconditioner>::do_solve(
      Matrix const         *A,
      value_type           *x,
      value_type const     *b,
      Preconditioner const *P)
    {
      ...
      dealii::AlignedVector<value_type> delta =
        dealii::AlignedVector<value_type>(M);
       ...
       *
       *  dealii::VectorizedArray: assume that the solver converged after 2
       * iterations for some components of the dealii::VectorizedArray but after
       * 4 iterations for the other components _                    _   _     _
       * _         _
