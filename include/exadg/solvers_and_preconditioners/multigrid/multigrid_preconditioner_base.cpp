...
namespace ExaDG
{
    ...
    bool const         is_dg)
  {
    ...
    if (not(data.involves_h_transfer()) or
        (grid->triangulation->n_global_levels() == 1))
      {
    ...
    else // involves_h_transfer == true and n_global_levels() > 1
      {
        ...
        if (grid->coarse_triangulations.size() > 0)
          {
            for (unsigned int h = 0; h < grid->coarse_triangulations.size() + 1;
                 h++)
              {
                h_levels.push_back(h);
                dealii_tria_levels.push_back(
                  dealii::numbers::invalid_unsigned_int);
              }
          }
        else
          {
            for (unsigned int h = 0; h < grid->triangulation->n_global_levels();
                 h++)
              {
                h_levels.push_back(h);
    ...
    if (mg_type == MultigridType::hMG)
    ...
    else // p-MG is involved with high- and low-order elements
      {
        ...
        if (is_dg)
          {
            if (mg_type == MultigridType::cpMG or
                mg_type == MultigridType::hcpMG or
                mg_type == MultigridType::chpMG or
                mg_type == MultigridType::cphMG)
              {
        ...
        if (is_dg)
          {
            if (mg_type == MultigridType::pcMG or
                mg_type == MultigridType::hpcMG or
                mg_type == MultigridType::phcMG or
    ...
    if (mg_type == MultigridType::hMG)
      {
        for (unsigned int h = 0; h < h_levels.size(); h++)
          level_info.push_back(
            {h_levels[h], dealii_tria_levels[h], p_levels.front()});
      }
    else if (mg_type == MultigridType::cMG)
      {
        level_info.push_back(
          {h_levels.back(), dealii_tria_levels.back(), p_levels.front()});
        level_info.push_back(
          {h_levels.back(), dealii_tria_levels.back(), p_levels.back()});
      }
      ...
      {
        level_info.push_back(
          {h_levels.front(), dealii_tria_levels.front(), p_levels.front()});

        for (unsigned int h = 0; h < h_levels.size(); h++)
          level_info.push_back(
            {h_levels[h], dealii_tria_levels[h], p_levels.back()});
      }
             ...
             mg_type == MultigridType::cpMG)
      {
        for (unsigned int p = 0; p < p_levels.size(); p++)
          level_info.push_back(
            {h_levels.front(), dealii_tria_levels.front(), p_levels[p]});
      }
    else if (mg_type == MultigridType::phMG or
             mg_type == MultigridType::cphMG or mg_type == MultigridType::pchMG)
      {
    ...
    else if (mg_type == MultigridType::hpMG or
             mg_type == MultigridType::hcpMG or mg_type == MultigridType::hpcMG)
      {
    ...
    else if (mg_type == MultigridType::phcMG)
      {
        ...
        for (; it != p_levels.end(); ++it)
          level_info.push_back(
            {h_levels.back(), dealii_tria_levels.back(), *it});
      }
    else if (mg_type == MultigridType::chpMG)
      {
        ...
        for (unsigned int h = 0; h < h_levels.size(); h++)
          level_info.push_back({h_levels[h],
                                dealii_tria_levels[h],
                                p_levels[p_levels.size() - 2]});
      ...
      {
        AssertThrow(
          false, dealii::ExcMessage("This multigrid type is not implemented!"));
      }
    ...
    for (unsigned int l = 1; l < get_number_of_levels(); l++)
      {
        ...
        AssertThrow(
          (fine.h_level() != coarse.h_level()) xor
            (fine.degree() != coarse.degree()) xor
    ...
    AssertThrow(h_levels.size() == dealii_tria_levels.size(),
                dealii::ExcMessage(
                  "h_levels and dealii_tria_levels have different size."));
  }
  ...
  MultigridPreconditionerBase<dim, Number, MultigridNumber>::
    initialize_mapping()
  {
    multigrid_mappings->initialize_coarse_mappings(
      *grid, this->get_number_of_h_levels());
  }
  ...
  {
    return multigrid_mappings->get_mapping(h_level,
                                           this->get_number_of_h_levels());
  }
  ...
  MultigridPreconditionerBase<dim, Number, MultigridNumber>::
    do_initialize_dof_handler_and_constraints(
      ...
      dealii::MGLevelObject<
        ...
        &constraints)
  {
    dealii::MGLevelObject<std::shared_ptr<dealii::MGConstrainedDoFs>>
      constrained_dofs;
    ...
    if (grid->coarse_triangulations.size() > 0 or
        not(is_hypercube_mesh_without_hanging_nodes))
      {
        ...
        for_all_levels([&](unsigned int const l) {
          ...
          std::shared_ptr<dealii::FiniteElement<dim>> fe =
            create_finite_element<dim>(get_element_type(*grid->triangulation),
                                       level.is_dg(),
                                       n_components,
                                       level.degree());
          ...
          if (level.h_level() ==
              level_info.back().h_level()) // fine-level triangulation
            {
          ...
          auto affine_constraints_own =
            new dealii::AffineConstraints<MultigridNumber>();
          ...
          // hanging nodes (needs to be done before imposing periodicity
          // constraints and boundary conditions)
          dealii::DoFTools::make_hanging_node_constraints(
            *dof_handler, *affine_constraints_own);
      ...
      {
        AssertThrow(
          is_hypercube_mesh_without_hanging_nodes,
          dealii::ExcMessage(
        ...
        std::map<MGDoFHandlerIdentifier,
                 std::shared_ptr<dealii::DoFHandler<dim> const>>
          map_dofhandlers;
        std::map<MGDoFHandlerIdentifier,
                 std::shared_ptr<dealii::MGConstrainedDoFs>>
        ...
        for (auto level : p_levels)
          {
            ...
            std::shared_ptr<dealii::FiniteElement<dim>> fe =
              create_finite_element<dim>(ElementType::Hypercube,
                                         level.is_dg,
                                         n_components,
                                         level.degree);
            ...
            // create dof handler
            auto dof_handler =
              new dealii::DoFHandler<dim>(*grid->triangulation);
                ...
                for (auto it : dirichlet_bc)
                  {
                    ...
                    constrained_dofs->make_zero_boundary_constraints(
                      *dof_handler, dirichlet_boundary, mask);
                  }
              }

            // put in temporal storage
            map_dofhandlers[level] =
              std::shared_ptr<dealii::DoFHandler<dim> const>(dof_handler);
            map_constrained_dofs[level] =
              std::shared_ptr<dealii::MGConstrainedDoFs>(constrained_dofs);
          }

        // populate dof-handler and constrained dofs of a certain p-levels to
        // all multigrid levels with the same FE / DoFHandler
    ...
    initialize_matrix_free_objects()
  {
    ...
    for_all_levels([&](unsigned int const level) {
      matrix_free_data_objects[level] =
        std::make_shared<MatrixFreeData<dim, MultigridNumber>>();
      fill_matrix_free_data(*matrix_free_data_objects[level],
                            level,
                            level_info[level].dealii_tria_level());

      matrix_free_objects[level] =
        std::make_shared<dealii::MatrixFree<dim, MultigridNumber>>();

      matrix_free_objects[level]->reinit(
        get_mapping(level_info[level].h_level()),
  ...
  {
    for_all_levels([&](unsigned int const level) {
      matrix_free_objects[level]->update_mapping(
        get_mapping(level_info[level].h_level()));
    });
  }
    ...
    initialize_smoothers(bool const initialize_preconditioner)
  {
    ...
    for_all_smoothing_levels([&](unsigned int const level) {
      this->initialize_smoother(*this->operators[level],
                                level,
  ...
  {
    // do nothing in base class (has to be implemented by derived classes if
    // necessary)
  }
  ...
  template <int dim, typename Number, typename MultigridNumber>
  void
  MultigridPreconditionerBase<dim, Number, MultigridNumber>::vmult(
    VectorType       &dst,
  ...
  template <int dim, typename Number, typename MultigridNumber>
  unsigned int
  MultigridPreconditionerBase<dim, Number, MultigridNumber>::solve(
    VectorType       &dst,
    VectorType const &src) const
  {
    AssertThrow(
  ...
  void
  MultigridPreconditionerBase<dim, Number, MultigridNumber>::
    apply_smoother_on_fine_level(VectorTypeMG       &dst,
  ...
  void
  MultigridPreconditionerBase<dim, Number, MultigridNumber>::
    initialize_smoother(Operator    &mg_operator,
                        unsigned int level,
                        bool const   initialize_preconditioner)
  {
    AssertThrow(
      level > 0 and level < this->get_number_of_levels(),
      dealii::ExcMessage(
        ...
        case MultigridSmoother::Chebyshev:
          {
            ...
            std::shared_ptr<Chebyshev> smoother =
              std::dynamic_pointer_cast<Chebyshev>(smoothers[level]);
            smoother->setup(mg_operator,
                            initialize_preconditioner,
                            smoother_data);
            break;
          }
        case MultigridSmoother::GMRES:
          {
            ...
            std::shared_ptr<GMRES> smoother =
              std::dynamic_pointer_cast<GMRES>(smoothers[level]);
            smoother->setup(mg_operator,
                            initialize_preconditioner,
                            smoother_data);
            break;
        ...
        case MultigridSmoother::CG:
          {
            ...
            std::shared_ptr<CG> smoother =
              std::dynamic_pointer_cast<CG>(smoothers[level]);
            smoother->setup(mg_operator,
                            initialize_preconditioner,
                            smoother_data);
            break;
        ...
        case MultigridSmoother::Jacobi:
          {
            ...
            smoother_data.number_of_smoothing_steps =
              data.smoother_data.iterations;
            smoother_data.damping_factor = data.smoother_data.relaxation_factor;

            std::shared_ptr<Jacobi> smoother =
              std::dynamic_pointer_cast<Jacobi>(smoothers[level]);
            smoother->setup(mg_operator,
                            initialize_preconditioner,
                            smoother_data);
            break;
        ...
        default:
          {
            AssertThrow(false,
                        dealii::ExcMessage(
                          "Specified MultigridSmoother not implemented!"));
          }
      }
  }
  ...
  MultigridPreconditionerBase<dim, Number, MultigridNumber>::update_smoothers()
  {
    for_all_smoothing_levels(
      [&](unsigned int const level) { smoothers[level]->update(); });
  }
    ...
    switch (data.coarse_problem.solver)
      {
        case MultigridCoarseGridSolver::Chebyshev:
          {
            coarse_grid_solver = std::make_shared<MGCoarseChebyshev<Operator>>(
              coarse_operator,
              initialize_preconditioners,
        ...
        case MultigridCoarseGridSolver::GMRES:
          {
            ...
            coarse_grid_solver = std::make_shared<MGCoarseKrylov<Operator>>(
              coarse_operator,
              initialize_preconditioners,
              additional_data,
              mpi_comm);
        ...
        case MultigridCoarseGridSolver::AMG:
          {
            if (data.coarse_problem.amg_data.amg_type == AMGType::ML or
              ...
              {
                coarse_grid_solver = std::make_shared<MGCoarseAMG<Operator>>(
                  coarse_operator,
                  initialize_preconditioners,
        ...
        default:
          {
            AssertThrow(false,
                        dealii::ExcMessage(
                          "Unknown coarse-grid solver specified."));
          }
      }
  }
  ...
  {
    transfers =
      std::make_shared<MultigridTransfer<dim, MultigridNumber, VectorTypeMG>>();
  ...
  {
    multigrid_algorithm =
      std::make_shared<MultigridAlgorithm<VectorTypeMG, Operator, Smoother>>(
