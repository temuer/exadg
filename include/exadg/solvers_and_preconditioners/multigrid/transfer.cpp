...
namespace ExaDG
{
    ...
    std::vector<MGLevelInfo> const &global_levels)
  {
    ...
    mg_transfer =
      std::make_unique<dealii::MGTransferGlobalCoarsening<dim, VectorType>>(
        transfers, [&](auto const l, auto &vec) {
          mg_matrixfree[l]->initialize_dof_vector(vec);
        });
  }

  template <int dim, typename Number, typename VectorType>
  void
  MultigridTransfer<dim, Number, VectorType>::interpolate(
  ...
  template <int dim, typename Number, typename VectorType>
  void
  MultigridTransfer<dim, Number, VectorType>::restrict_and_add(
    unsigned int const level,
    VectorType        &dst,
    VectorType const  &src) const
  {
  ...
  template <int dim, typename Number, typename VectorType>
  void
  MultigridTransfer<dim, Number, VectorType>::prolongate_and_add(
    unsigned int const level,
    VectorType        &dst,
    VectorType const  &src) const
  {
