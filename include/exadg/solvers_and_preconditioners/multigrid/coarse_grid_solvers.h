...
{
  /**
   * Base class for multigrid coarse-grid solvers in order to define update()
   * function in addition to the interface of dealii::MGCoarseGridBase.
   */
  template <typename Operator>
  class CoarseGridSolverBase
    : public dealii::MGCoarseGridBase<dealii::LinearAlgebra::distributed::
                                        Vector<typename Operator::value_type>>
  {
  public:
  ...
  public:
    ...
    struct AdditionalData
    {
      ...
      // in case of singular operators (with constant vectors forming the
      // nullspace) the rhs vector has to be projected onto the space of vectors
      // with zero mean prior to solving the coarse grid problem
    ...
    MGCoarseKrylov(Operator const       &pde_operator_in,
      ...
      , mpi_comm(comm)
    {
      if (additional_data.preconditioner ==
          MultigridCoarseGridPreconditioner::PointJacobi)
        {
          preconditioner =
            std::make_shared<JacobiPreconditioner<Operator>>(pde_operator,
                                                             initialize);

          std::shared_ptr<JacobiPreconditioner<Operator>> jacobi =
            std::dynamic_pointer_cast<JacobiPreconditioner<Operator>>(
              preconditioner);
          AssertDimension(jacobi->get_size_of_diagonal(), pde_operator.m());
        }
      else if (additional_data.preconditioner ==
               MultigridCoarseGridPreconditioner::BlockJacobi)
        {
          preconditioner =
            std::make_shared<BlockJacobiPreconditioner<Operator>>(pde_operator,
                                                                  initialize);
        }
      else if (additional_data.preconditioner ==
        ...
        {
          preconditioner =
            std::make_shared<PreconditionerAMG<Operator, Number>>(
              pde_operator, initialize, additional_data.amg_data);
        }
      else
        {
          AssertThrow(
            additional_data.preconditioner ==
                MultigridCoarseGridPreconditioner::None or
              additional_data.preconditioner ==
                MultigridCoarseGridPreconditioner::PointJacobi or
              additional_data.preconditioner ==
                MultigridCoarseGridPreconditioner::BlockJacobi or
              additional_data.preconditioner ==
                MultigridCoarseGridPreconditioner::AMG,
            dealii::ExcMessage(
               ...
               VectorType const &src) const final
    {
      ...
      if (additional_data.preconditioner ==
          MultigridCoarseGridPreconditioner::AMG)
        {
          std::shared_ptr<PreconditionerAMG<Operator, Number>>
            preconditioner_amg =
              std::dynamic_pointer_cast<PreconditionerAMG<Operator, Number>>(
                preconditioner);
          ...
          preconditioner_amg->apply_krylov_solver_with_amg_preconditioner(
            dst, r, additional_data.solver_type, additional_data.solver_data);
        }
      else
        {
          ...
          if (additional_data.solver_type == MultigridCoarseGridSolver::CG)
            {
              ...
              solver.reset(new Krylov::SolverCG<Operator,
                                                PreconditionerBase<Number>,
                                                VectorType>(pde_operator,
                                                            *preconditioner,
                                                            solver_data));
            }
                   ...
                   MultigridCoarseGridSolver::GMRES)
            {
              ...
              solver_data.max_n_tmp_vectors =
                additional_data.solver_data.max_krylov_size;
              ...
              solver.reset(new Krylov::SolverGMRES<Operator,
                                                   PreconditionerBase<Number>,
                                                   VectorType>(
  ...
  public:
    ...
    typedef dealii::PreconditionChebyshev<Operator,
                                          VectorType,
    ...
    update() final
    {
      ...
      std::pair<double, double> eigenvalues =
        compute_eigenvalues(coarse_operator,
                            diagonal_vector,
      ...
      dealii_additional_data.smoothing_range =
      ...
      // calculate/estimate the number of Chebyshev iterations needed to reach a
      // specified relative solver tolerance
  ...
  /**
   * The aim of this class is to translate PreconditionerAMG to a coarse-grid
   * solver with the function operator()().
   */
  template <typename Operator>
  ...
  public:
    ...
    {
      amg_preconditioner =
        std::make_shared<PreconditionerAMG<Operator, Number>>(op,
                                                              initialize,
                                                              data);
    }
