    ...
    namespace // anonymous namespace
    {
        ...
        if (level == 0)
          {
            ...
            for (unsigned int i = 0; i < dofs_per_face; ++i)
              {
                if (constraints.can_store_line(dofs_2[i]) and
                    constraints.can_store_line(dofs_1[i]) and
                    not(constraints.is_constrained(dofs_2[i])))
                  {
                    // constraint dof and ...
                    constraints.add_line(dofs_2[i]);
                    // specify type of constraint: equality
                    // (dof_2[i]=dof_1[j]*1.0)
        ...
        else if (face1->has_children() and face2->has_children())
          {
            ...
            for (unsigned int c = 0; c < face1->n_children(); ++c)
              {
                add_periodicity_constraints<dim>(level - 1,
                                                 target_level,
                                                 face1->child(c),
                                                 face2->child(c),
                                                 constraints);
        ...
        for (auto &it : periodic_face_pairs_level0)
          {
            ...
            typename dealii::DoFHandler<dim>::cell_iterator cell1(
              &dof_handler.get_triangulation(),
              0,
              it.cell[1]->index(),
              &dof_handler);
            typename dealii::DoFHandler<dim>::cell_iterator cell0(
              &dof_handler.get_triangulation(),
              0,
              it.cell[0]->index(),
              &dof_handler);

            // get reference to periodic faces on level and add recursively
            // their subfaces on the given level
        ...
        unsigned int const level)
      {
        ...
        if (mg_constrained_dofs.have_boundary_indices())
          affine_constraints_own.add_lines(
            mg_constrained_dofs.get_boundary_indices(level));

        // constrain zeroth DoF in continuous case (the mean value constraint
        // will be applied in the DG case). In case we have interface matrices,
        // there are Dirichlet constraints on parts of the boundary and no such
        // transformation is required.
        if (operator_is_singular and affine_constraints_own.can_store_line(0))
          {
            // if dof 0 is constrained, it must be a periodic dof, so we take
            // the value on the other side
            dealii::types::global_dof_index line_index = 0;
            while (true)
              {
                auto const *lines =
                  affine_constraints_own.get_constraint_entries(line_index);

                if (lines == 0)
                  {
                    affine_constraints_own.add_line(line_index);
                    // add the constraint back to the dealii::MGConstrainedDoFs
                    // field. This is potentially dangerous but we know what we
                    // are doing... ;-)
                    if (mg_constrained_dofs.have_boundary_indices() &&
                        level != dealii::numbers::invalid_unsigned_int)
                      {
                        if (mg_constrained_dofs.get_boundary_indices(level)
                              .size() != dof_handler.n_dofs(level))
                          const_cast<dealii::IndexSet &>(
                            mg_constrained_dofs.get_boundary_indices(level))
                  ...
                  {
                    Assert(lines->size() == 1 and
                             std::abs((*lines)[0].second - 1.) < 1e-15,
                           dealii::ExcMessage(
