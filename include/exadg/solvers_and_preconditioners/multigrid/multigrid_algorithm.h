...
{
  /*
   * Re-implementation of multigrid preconditioner (V-cycle) in order to have
   * more direct control over its individual components and avoid inner products
  ...
  public:
    MultigridAlgorithm(
      dealii::MGLevelObject<std::shared_ptr<MatrixType>> const   &matrix,
      ...
      while (not converged)
        {
          ...
          converged = (norm_r < abstol or norm_r / norm_r_0 < reltol or
                       n_iter >= max_iter);
  ...
  private:
      ...
      if (level == minlevel)
...
#if ENABLE_TIMING
          timer_tree->insert({"Multigrid", "level " + std::to_string(level)},
                             timer.wall_time());
#endif
        }
      else
        {
          ...
          if (multigrid_is_a_solver)
            {
              // One has to take into account the initial guess of the solution
              // when used as a solver and, therefore, call the function step().
              (*smoother)[level]->step(solution[level], defect[level]);
            }
          else
            {
              // We can assume that solution[level] = 0 when used as a
              // preconditioner and, therefore, call the function vmult(), which
              // makes use of this assumption in order to apply optimizations
              // (e.g., one does not need to evaluate the residual in the first
     ...
     */
    dealii::SmartPointer<
      dealii::MGLevelObject<std::shared_ptr<MatrixType>> const>
      matrix;
     ...
     */
    dealii::SmartPointer<
      dealii::MGLevelObject<std::shared_ptr<SmootherType>> const>
      smoother;
