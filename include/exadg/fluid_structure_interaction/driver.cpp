...
namespace ExaDG
{
  namespace FSI
  {
      ...
      , application(app)
    {
      ...
      partitioned_solver =
        std::make_shared<PartitionedSolver<dim, Number>>(parameters, mpi_comm);
    }
    ...
    Driver<dim, Number>::setup()
    {
      ...
      // setup structure
      {
        ...
        timer_tree.insert({"FSI", "Setup", "Structure"},
    ...
    Driver<dim, Number>::setup_interface_coupling()
    {
      // structure to ALE
      {
        ...
        pcout << std::endl
              << "Setup interface coupling structure -> ALE ..." << std::endl;
        ...
        auto const marked_vertices_structure =
          get_marked_vertices_via_boundary_ids(tria, boundary_ids);

        if (application->fluid->get_parameters().mesh_movement_type ==
            IncNS::MeshMovementType::Poisson)
          {
            structure_to_ale =
              std::make_shared<InterfaceCoupling<1, dim, Number>>();

            std::shared_ptr<Poisson::DeformedMapping<dim, Number>>
              poisson_grid_motion = std::dynamic_pointer_cast<
                Poisson::DeformedMapping<dim, Number>>(fluid->ale_mapping);
            structure_to_ale->setup(poisson_grid_motion->get_pde_operator()
                 ...
                 IncNS::MeshMovementType::Elasticity)
          {
            ...
            std::shared_ptr<Structure::DeformedMapping<dim, Number>>
              elasticity_grid_motion = std::dynamic_pointer_cast<
                Structure::DeformedMapping<dim, Number>>(fluid->ale_mapping);
            structure_to_ale->setup(
              elasticity_grid_motion->get_pde_operator()
        ...
        timer_tree.insert({"FSI", "Setup", "Coupling structure -> ALE"},
                          timer_local.wall_time());
      }
      ...
      // structure to fluid
      {
        ...
        pcout << std::endl
              << "Setup interface coupling structure -> fluid ..." << std::endl;
        ...
        timer_tree.insert({"FSI", "Setup", "Coupling structure -> fluid"},
                          timer_local.wall_time());
      }
      ...
      // fluid to structure
      {
        ...
        pcout << std::endl
              << "Setup interface coupling fluid -> structure ..." << std::endl;

        auto const &tria =
          fluid->pde_operator->get_dof_handler_u().get_triangulation();
        auto const boundary_ids = application->fluid->get_boundary_descriptor()
                                    ->velocity->dirichlet_cached_bc;
        auto const marked_vertices_fluid =
          get_marked_vertices_via_boundary_ids(tria, boundary_ids);
        ...
        timer_tree.insert({"FSI", "Setup", "Coupling fluid -> structure"},
                          timer_local.wall_time());
      }
    }
    ...
    Driver<dim, Number>::set_start_time() const
    {
      ...
      structure->time_integrator->reset_time(
        fluid->time_integrator->get_time());
    }
    ...
    template <int dim, typename Number>
    void
    Driver<dim, Number>::coupling_structure_to_ale(
      VectorType const &displacement_structure) const
    {
      dealii::Timer sub_timer;
      sub_timer.restart();

      structure_to_ale->update_data(displacement_structure);
      timer_tree.insert({"FSI", "Coupling structure -> ALE"},
                        sub_timer.wall_time());
    }

    template <int dim, typename Number>
    void
    Driver<dim, Number>::coupling_structure_to_fluid(
      unsigned int iteration) const
    {
      ...
      timer_tree.insert({"FSI", "Coupling structure -> fluid"},
                        sub_timer.wall_time());
    }

    template <int dim, typename Number>
    void
    Driver<dim, Number>::coupling_fluid_to_structure(
      bool const end_of_time_step) const
    {
      dealii::Timer sub_timer;
      sub_timer.restart();
      ...
      if (end_of_time_step)
        {
          fluid->pde_operator->interpolate_stress_bc(
            stress_fluid,
      ...
      timer_tree.insert({"FSI", "Coupling fluid -> structure"},
                        sub_timer.wall_time());
    }

    template <int dim, typename Number>
    void
    Driver<dim, Number>::apply_dirichlet_neumann_scheme(
      VectorType       &d_tilde,
      VectorType const &d,
      unsigned int      iteration) const
    {
    ...
    Driver<dim, Number>::solve() const
    {
      ...
      // The fluid domain is the master that dictates when the time loop is
      // finished
      ...
      double const total_time) const
    {
      ...
      pcout << "Performance results for fluid-structure interaction solver:"
            << std::endl;
      ...
      timer_tree.insert({"FSI"},
                        fluid->time_integrator->get_timings(),
                        "Fluid");
      timer_tree.insert({"FSI"}, fluid->get_timings_ale());
      timer_tree.insert({"FSI"},
                        structure->time_integrator->get_timings(),
                        "Structure");
      ...
      dealii::types::global_dof_index DoFs =
        ...
        structure->pde_operator->get_number_of_dofs();
      ...
      if (application->fluid->get_parameters().mesh_movement_type ==
          IncNS::MeshMovementType::Poisson)
        {
          std::shared_ptr<Poisson::DeformedMapping<dim, Number>>
            poisson_ale_mapping =
              std::dynamic_pointer_cast<Poisson::DeformedMapping<dim, Number>>(
                fluid->ale_mapping);
        ...
        {
          std::shared_ptr<Structure::DeformedMapping<dim, Number>>
            structure_ale_mapping = std::dynamic_pointer_cast<
              Structure::DeformedMapping<dim, Number>>(fluid->ale_mapping);

          DoFs +=
            structure_ale_mapping->get_pde_operator()->get_number_of_dofs();
        }
      else
        {
          AssertThrow(false, dealii::ExcMessage("not implemented."));
      ...
      unsigned int N_time_steps =
        fluid->time_integrator->get_number_of_time_steps();

      print_throughput_unsteady(
        pcout, DoFs, total_time_avg, N_time_steps, N_mpi_processes);
