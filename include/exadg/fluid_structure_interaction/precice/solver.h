...
namespace ExaDG
{
  ...
  create_input_file(std::string const &input_file)
  {
    ...
    FSI::get_application<Dim, Number>(input_file, MPI_COMM_WORLD)
      ->add_parameters(prm);
      ...
      bool const         is_test)
  {
    ...
    // We have different drivers for Fluid and Solid since the data exchange and
    // underlying configuration are different. However, both solver share a
    // common base class. One could also compile different executables for both
    // solvers or try to create a common Driver class for both parts.
    if (precice_param.physics == "Structure")
      {
        driver = std::make_shared<FSI::preCICE::DriverSolid<dim, Number>>(
          input_file, mpi_comm, application, is_test);
      }
    else if (precice_param.physics == "Fluid")
      {
        driver = std::make_shared<FSI::preCICE::DriverFluid<dim, Number>>(
          input_file, mpi_comm, application, is_test);
      }
    else
      {
...
// The preCICE coupled executable is called solver_precice and we compile the
// Solid and the Fluid Driver into the same executable. Configuration can be
// done via individual parameter files.
int
