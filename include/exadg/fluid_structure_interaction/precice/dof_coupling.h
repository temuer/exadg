...
namespace ExaDG
{
  namespace preCICE
  {
    ...
    private:
      ...
      std::vector<std::array<dealii::types::global_dof_index, data_dim>>
        global_indices;
    ...
    DoFCoupling<dim, data_dim, VectorizedArrayType>::define_coupling_mesh()
    {
      ...
      auto const get_component_dofs = [&](int const component) {
        ...
        Assert(indices.n_elements() * data_dim ==
                 this->matrix_free.get_dof_handler(mf_dof_index)
                   .n_boundary_dofs(std::set<dealii::types::boundary_id>{
                     this->dealii_boundary_surface_id}),
      ...
      // Compute the coordinates of the indices (only one component required
      // here) We select the zeroth component
      ...
      for (std::size_t i = 0; i < global_indices.size(); ++i)
        {
          ...
          if constexpr (data_dim > 1)
            {
              this->precice->writeVectorData(write_data_id,
                                             coupling_nodes_ids[i],
                                             write_data.data());
            }
          else
            {
              this->precice->writeScalarData(write_data_id,
                                             coupling_nodes_ids[i],
                                             write_data[0]);
            }
#else
