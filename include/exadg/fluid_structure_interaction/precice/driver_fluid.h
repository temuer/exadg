    ...
    namespace preCICE
    {
      ...
      public:
        ...
        setup_fluid_and_ale()
        {
          ...
          this->timer_tree.insert({"FSI", "Setup", "Fluid"},
        ...
        {
          this->precice =
            std::make_shared<ExaDG::preCICE::Adapter<dim, dim, VectorType>>(
              this->precice_parameters, this->mpi_comm);
          ...
          // structure to ALE
          {
            ...
            else if (this->application->fluid->get_parameters()
                       .mesh_movement_type ==
                     IncNS::MeshMovementType::Elasticity)
              {
                std::shared_ptr<Structure::DeformedMapping<dim, Number>>
                  structure_ale_mapping = std::dynamic_pointer_cast<
                    Structure::DeformedMapping<dim, Number>>(
                    fluid->ale_mapping);

                this->precice->add_read_surface(
                  structure_ale_mapping->get_matrix_free(),
                  structure_ale_mapping->get_pde_operator()
        ...
        setup() override
        {
          ...
          this->pcout << std::endl
                      << "Setting up fluid-structure interaction solver:"
                      << std::endl;
        ...
        solve() const final
        {
          Assert(this->application->fluid->get_parameters()
          ...
          while (this->precice->is_coupling_ongoing())
            {
              // pre-solve
              fluid->time_integrator->advance_one_timestep_pre_solve(
                is_new_time_window);
...

              {
                ...
                // TODO: Add synchronization for the time-step size here. For
                // now, we only allow a constant time-step size
                dealii::Timer precice_timer;
                this->precice->advance(
                  fluid->time_integrator->get_time_step_size());
                is_new_time_window = this->precice->is_time_window_complete();
                this->timer_tree.insert({"FSI", "preCICE"},
                                        precice_timer.wall_time());
              }
        ...
        print_performance_results(double const total_time) const override
        {
          ...
          this->pcout
            << "Performance results for fluid-structure interaction solver:"
            << std::endl;
          ...
          if (this->application->fluid->get_parameters().mesh_movement_type ==
              IncNS::MeshMovementType::Poisson)
            {
              ...
              DoFs +=
                poisson_ale_mapping->get_pde_operator()->get_number_of_dofs();
            }
          else if (this->application->fluid->get_parameters()
                     .mesh_movement_type == IncNS::MeshMovementType::Elasticity)
            {
              std::shared_ptr<Structure::DeformedMapping<dim, Number>>
                elasticity_ale_mapping = std::dynamic_pointer_cast<
                  Structure::DeformedMapping<dim, Number>>(fluid->ale_mapping);

              DoFs += elasticity_ale_mapping->get_pde_operator()
                        ->get_number_of_dofs();
            }
          else
            {
          ...
          unsigned int const N_mpi_processes =
            dealii::Utilities::MPI::n_mpi_processes(this->mpi_comm);
          ...
          print_throughput_unsteady(
            this->pcout, DoFs, total_time_avg, N_time_steps, N_mpi_processes);
