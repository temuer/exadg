...
namespace ExaDG
{
  namespace preCICE
  {
    ...
    public:
       ...
       *             corresponding MatrixFree object which should be used for
       * data reading
       ...
       *
       * @param[in]  dealii_to_precice Data, which should be given to preCICE
       * and exchanged with other participants. Wether this data is required
       * already in the beginning depends on your individual configuration and
       * preCICE determines it automatically. In many cases, this data will just
       * represent your initial condition.
       */
      void
      ...
      void
      read_block_data(std::string const &mesh_name,
    ...
    private:
      ...
      std::map<
        std::string,
        std::shared_ptr<CouplingBase<dim, data_dim, VectorizedArrayType>>>
        writer;

      // We restrict the reader to be of type ExaDGCoupling for the moment, as
      // all other choices don't make sense
      std::map<
        std::string,
        std::shared_ptr<ExaDGCoupling<dim, data_dim, VectorizedArrayType>>>
        reader;
      ...
      if (found_reader != reader.end())
      ...
      else
        {
          Assert(write_data_type == WriteDataType::values_on_q_points or
                   write_data_type ==
                     WriteDataType::normal_gradients_on_q_points,
      ...
      VectorType const &dealii_to_precice)
    {
      ...
      // Maybe, read block-wise and work with an AlignedVector since the read
      // data (forces) is multiple times required during the Newton iteration
      ...
      double const       computed_timestep_length)
    {
      ...
      if (precice->isWriteDataRequired(computed_timestep_length))
        writer.at(write_mesh_name)
          ->write_data(dealii_to_precice, write_data_name);
#else
      ...
      double const computed_timestep_length)
    {
      ...
      // preCICE
      // TODO: The function returns the available time-step size which is
      // required for time-step sync
