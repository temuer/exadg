...
namespace ExaDG
{
  namespace preCICE
  {
     ...
     * data_dim is equivalent to n_components, indicating the type of your data
     * in the preCICE sense (Vector vs Scalar)
     */
    ...
    public:
      ...
      using FEFaceIntegrator =
        typename CouplingBase<dim, data_dim, VectorizedArrayType>::
          FEFaceIntegrator;
      using value_type =
    ...
    private:
      ...
      write_data_factory(
        ...
        std::function<value_type(FEFaceIntegrator &, unsigned int)> const
          &get_write_value);
    ...
    QuadCoupling<dim, data_dim, VectorizedArrayType>::define_coupling_mesh()
    {
      ...
      coupling_nodes_ids.reserve(this->matrix_free.n_boundary_face_batches() /
                                 2);

      // Set up data structures
      FEFaceIntegrator phi(this->matrix_free,
                           true,
                           mf_dof_index,
                           mf_quad_index);
      ...
      for (unsigned int face = this->matrix_free.n_inner_face_batches();
           face < this->matrix_free.n_boundary_face_batches() +
                    this->matrix_free.n_inner_face_batches();
           ++face)
        {
          ...
          phi.reinit(face);
          int const active_faces =
            this->matrix_free.n_active_entries_per_face_batch(face);
          ...
          for (unsigned int q = 0; q < phi.n_q_points; ++q)
            {
              ...
              // active_faces), but it allows us to use a static loop as well as
              // a static array for the indices
              for (int d = 0; d < dim; ++d)
      ...
      for (unsigned int face = this->matrix_free.n_inner_face_batches();
           ...
           ++face)
        {
          ...
          for (unsigned int q = 0; q < phi.n_q_points; ++q, ++index)
            {
              Assert(index != coupling_nodes_ids.end(),
                     dealii::ExcInternalError());
...
#ifdef EXADG_WITH_PRECICE
                  this->precice->writeBlockVectorData(
                    write_data_id,
                    active_faces,
                    index->data(),
