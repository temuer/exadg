  ...
  namespace preCICE
  {
    ...
    public:
      ...
      using FEFaceIntegrator =
        FaceIntegrator<dim, data_dim, double, VectorizedArrayType>;
    ...
    protected:
      ...
      /// The dealii::MatrixFree object (preCICE can only handle double
      /// precision)
    ...
    template <int dim, int data_dim, typename VectorizedArrayType>
    void
    CouplingBase<dim, data_dim, VectorizedArrayType>::add_read_data(
      std::string const &read_data_name)
    {
    ...
    template <int dim, int data_dim, typename VectorizedArrayType>
    void
    CouplingBase<dim, data_dim, VectorizedArrayType>::read_block_data(
    ...
    template <int dim, int data_dim, typename VectorizedArrayType>
    void
    CouplingBase<dim, data_dim, VectorizedArrayType>::print_info(
      bool const         reader,
      unsigned int const local_size) const
    {
      ...
      pcout << "--     Data " << (reader ? "reading" : "writing") << ":\n"
            ...
            << dealii::Utilities::MPI::sum(
                 local_size, matrix_free.get_dof_handler().get_communicator())
            << "\n"
