...
namespace ExaDG
{
  namespace FSI
  {
    ...
    public:
      ...
      void
      solve(std::function<
            void(VectorType &, VectorType const &, unsigned int)> const
      ...
      void
      get_structure_velocity(VectorType        &velocity_structure,
    ...
    private:
      ...
      std::vector<std::shared_ptr<std::vector<VectorType>>> D_history,
        R_history, Z_history;
      ...
      /*
       * The first number counts the number of time steps, the second number the
       * total number (accumulated over all time steps) of iterations of the
       * partitioned FSI scheme.
       */
      std::pair<unsigned int, unsigned long long> partitioned_iterations;
    };

    template <int dim, typename Number>
    PartitionedSolver<dim, Number>::PartitionedSolver(
    ...
    template <int dim, typename Number>
    void
    PartitionedSolver<dim, Number>::setup(
      std::shared_ptr<SolverFluid<dim, Number>>     fluid_,
      std::shared_ptr<SolverStructure<dim, Number>> structure_)
    {
      fluid     = fluid_;
    ...
    template <int dim, typename Number>
    bool
    PartitionedSolver<dim, Number>::check_convergence(
      VectorType const &residual) const
    {
      double const residual_norm = residual.l2_norm();
      double const ref_norm_abs =
        std::sqrt(structure->pde_operator->get_number_of_dofs());
      double const ref_norm_rel =
        structure->time_integrator->get_velocity_np().l2_norm() *
        structure->time_integrator->get_time_step_size();

      bool const converged =
        (residual_norm < parameters.abs_tol * ref_norm_abs) or
    ...
    template <int dim, typename Number>
    void
    PartitionedSolver<dim, Number>::print_solver_info_header(
      unsigned int const iteration) const
    {
      if (fluid->time_integrator->print_solver_info())
        {
          pcout
            ...
            << "======================================================================"
            << std::endl
            << " Partitioned FSI: iteration counter = " << std::left
            << std::setw(8) << iteration << std::endl
            << "======================================================================"
            << std::endl;
        }
    }
    ...
    template <int dim, typename Number>
    void
    PartitionedSolver<dim, Number>::print_solver_info_converged(
      unsigned int const iteration) const
    {
      if (fluid->time_integrator->print_solver_info())
        {
    ...
    template <int dim, typename Number>
    void
    PartitionedSolver<dim, Number>::print_iterations(
      dealii::ConditionalOStream const &pcout) const
    {
      ...
      iterations_avg[0] = (double)partitioned_iterations.second /
                          std::max(1.0, (double)partitioned_iterations.first);
    ...
    template <int dim, typename Number>
    void
    PartitionedSolver<dim, Number>::get_structure_velocity(
      ...
      if (iteration == 0)
        {
          if (parameters.initial_guess_coupling_scheme ==
            ...
            {
              structure->time_integrator->extrapolate_velocity_to_np(
                velocity_structure);
            }
    ...
    template <int dim, typename Number>
    void
    PartitionedSolver<dim, Number>::get_structure_displacement(
      ...
      if (iteration == 0)
        {
          if (this->parameters.initial_guess_coupling_scheme ==
              InitialGuessCouplingScheme::SolutionExtrapolatedToEndOfTimeStep)
            {
              structure->time_integrator->extrapolate_displacement_to_np(
            ...
            {
              displacement_structure =
                structure->time_integrator->get_displacement_n();
            }
          else
            {
              AssertThrow(
        ...
        {
          displacement_structure =
            structure->time_integrator->get_displacement_np();
        }
    }
              ...
              if (not(converged))
                {
                  ...
                  // compute b vector
                  inv_jacobian_times_residual(
                    b, D_history, R_history, Z_history, r);
