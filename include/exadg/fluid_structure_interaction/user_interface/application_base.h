  ...
  namespace StructureFSI
  {
    ...
    public:
            ...
            std::shared_ptr<MultigridMappings<dim, Number>> &multigrid_mappings)
      {
        ...
        // boundary conditions
        boundary_descriptor =
          std::make_shared<Structure::BoundaryDescriptor<dim>>();
  ...
  namespace FluidFSI
  {
    ...
    public:
            ...
            std::shared_ptr<MultigridMappings<dim, Number>> &multigrid_mappings)
      {
        ...
        // Some FSI specific Asserts
        AssertThrow(
          param.problem_type == IncNS::ProblemType::Unsteady,
          dealii::ExcMessage(
            "Invalid parameter in context of fluid-structure interaction."));
        AssertThrow(
        ...
        if (param.mesh_movement_type == IncNS::MeshMovementType::Poisson)
          {
            ...
            ale_poisson_field_functions =
              std::make_shared<Poisson::FieldFunctions<dim>>();
            set_field_functions_ale_poisson();
          }
        else if (param.mesh_movement_type ==
                 IncNS::MeshMovementType::Elasticity)
          {
            ...
            ale_elasticity_material_descriptor =
              std::make_shared<Structure::MaterialDescriptor>();
            set_material_descriptor_ale_elasticity();

            // field functions
            ale_elasticity_field_functions =
              std::make_shared<Structure::FieldFunctions<dim>>();
    ...
    protected:
      ...
      std::shared_ptr<Structure::FieldFunctions<dim>>
        ale_elasticity_field_functions;
      std::shared_ptr<Structure::BoundaryDescriptor<dim>>
        ale_elasticity_boundary_descriptor;
      std::shared_ptr<Structure::MaterialDescriptor>
        ale_elasticity_material_descriptor;
