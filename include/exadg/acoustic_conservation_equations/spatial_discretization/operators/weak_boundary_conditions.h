...
namespace ExaDG
{
  namespace Acoustics
  {
    ...
    inline DEAL_II_ALWAYS_INLINE //
      ...
      calculate_exterior_value_pressure(
    ...
    inline DEAL_II_ALWAYS_INLINE //
      ...
      calculate_exterior_value_velocity(
        unsigned int const                      q,
        ...
        Number const                            time)
    {
      if (boundary_type == BoundaryType::VelocityDirichlet)
      ...
      else if (boundary_type == BoundaryType::Admittance)
        {
          ...
          auto const rho_up_normal =
            (Number{2.0} * Y * pm / speed_of_sound) * n - rho_um_normal;

          // tangential share of velocity stays the same (rho_up_tangential =
          // rho_um_tangential)
    ...
    /**
     * Class to access values of pressure at boundaries similar to
     * FaceIntegrators.
     */
    template <int dim, typename Number>
    class BoundaryFaceIntegratorP
      : public BoundaryFaceIntegratorBase<BoundaryDescriptor<dim>, Number>
    {
      using FaceIntegratorP = FaceIntegrator<dim, 1, Number>;

    public:
      BoundaryFaceIntegratorP(
        FaceIntegratorP const         &integrator_m_in,
    ...
    public:
      BoundaryFaceIntegratorU(
        FaceIntegratorU const         &integrator_velocity_m_in,
