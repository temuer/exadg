  ...
  namespace Acoustics
  {
    namespace Operators
    {
      ...
      public:
        ...
        /*
         * Volume flux for the momentum equation, i.e., the term occurring in
         * the volume integral for weak formulation (performing
         * integration-by-parts)
         */
        inline DEAL_II_ALWAYS_INLINE //
          vector
          get_volume_flux_weak_momentum(CellIntegratorU const &velocity,
        ...
        /*
         * Volume flux for the momentum equation, i.e., the term occurring in
         * the volume integral for strong formulation (integration-by-parts
         * performed twice)
         */
        inline DEAL_II_ALWAYS_INLINE //
          scalar
          get_volume_flux_strong_momentum(CellIntegratorU const &velocity,
                                          unsigned int const     q) const
        {
        ...
        /*
         * Volume flux for the mass equation, i.e., the term occurring in the
         * volume integral for weak formulation (performing
         * integration-by-parts)
         */
        inline DEAL_II_ALWAYS_INLINE //
          scalar
          get_volume_flux_weak_mass(CellIntegratorP const &pressure,
                                    unsigned int const     q) const
        {
          // minus sign due to integration by parts
        ...
        /*
         * Volume flux for the mass equation, i.e., the term occurring in the
         * volume integral for strong formulation (integration-by-parts
         * performed twice)
         */
        inline DEAL_II_ALWAYS_INLINE //
          vector
          get_volume_flux_strong_mass(CellIntegratorP const &pressure,
                                      unsigned int const     q) const
        {
    ...
    public:
        ...
        if (data_in.formulation == Formulation::Weak)
          {
            ...
            integrator_flags_p.cell_integrate =
            ...
            integrator_flags_u.cell_integrate =
              dealii::EvaluationFlags::gradients;
      ...
      void
      evaluate_add(BlockVectorType       &dst,
                   BlockVectorType const &src,
                   double const           time) const
      {
        do_evaluate(dst, src, time, true);
    ...
    private:
      ...
      void
      do_cell_integral(CellIntegratorP &pressure,
                       CellIntegratorU &velocity) const
      {
        if (data.formulation == Formulation::Weak)
            ...
            for (unsigned int q : pressure.quadrature_point_indices())
              {
                vector const flux_momentum =
                  kernel.get_volume_flux_weak_momentum(velocity, q);
                scalar const flux_mass =
                  kernel.get_volume_flux_weak_mass(pressure, q);
        ...
        else if (data.formulation == Formulation::Strong)
          {
            for (unsigned int q : pressure.quadrature_point_indices())
              {
                scalar const flux_momentum =
                  kernel.get_volume_flux_strong_momentum(velocity, q);
                vector const flux_mass =
                  kernel.get_volume_flux_strong_mass(pressure, q);
      ...
      template <bool weight_neighbor, // = false for cell centric loops and
                                      // boundary loops
        ...
        for (unsigned int q : pressure_m.quadrature_point_indices())
          {
            ...
            vector const flux_momentum =
              kernel.calculate_lax_friedrichs_flux_momentum(
                rho_um, rho_up, gamma, pm, pp, n);

            scalar const flux_mass = kernel.calculate_lax_friedrichs_flux_mass(
              pm, pp, tau, rho_um, rho_up, n);
      ...
      {
        FaceIntegratorP pressure_m(matrix_free_in,
                                   true,
                                   data.dof_index_pressure,
                                   data.quad_index);
        FaceIntegratorP pressure_p(matrix_free_in,
                                   false,
                                   data.dof_index_pressure,
                                   data.quad_index);

        FaceIntegratorU velocity_m(matrix_free_in,
                                   true,
                                   data.dof_index_velocity,
                                   data.quad_index);
        FaceIntegratorU velocity_p(matrix_free_in,
                                   false,
                                   data.dof_index_velocity,
                                   data.quad_index);
        ...
        for (unsigned int face = face_range.first; face < face_range.second;
             face++)
          {
            ...
            do_face_integral<true>(pressure_m,
                                   pressure_p,
                                   velocity_m,
                                   velocity_p);
                         ...
                         Range const &face_range) const
      {
        ...
        for (unsigned int face = face_range.first; face < face_range.second;
             face++)
          {
            ...
            do_face_integral<false>(pressure_m,
                                    pressure_p,
                                    velocity_m,
                                    velocity_p);
