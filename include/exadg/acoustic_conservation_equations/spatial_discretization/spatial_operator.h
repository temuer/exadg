...
namespace ExaDG
{
  namespace Acoustics
  {
    ...
    public:
       ...
       */
      void
      prescribe_initial_conditions(BlockVectorType &dst,
      ...
      /**
       * This function applies the inverse mass matrix and scales the pressure
       * by c^2. This is because the PDE we are solving for reads as
       *
       * 1/c^2 dp/dt + div  u = f
       *       du/dt + grad p = 0
       */
      void
      apply_scaled_inverse_mass_operator(BlockVectorType       &dst,
    ...
    private:
      ...
      // Quadrature that works for both, pressure and velocity, i.e.
      // n_q=(max(k_p,k_u)+1)^dim. This quadrature is needed for the acoustic
      // operator which iterates over pressure and velocity quadrature points in
      // the same loop (for performance reasons).
