  ...
  namespace Acoustics
  {
      ...
      MatrixFreeData<dim, Number> &matrix_free_data) const
    {
      ...
      matrix_free_data.append_mapping_flags(
        Operators::Kernel<dim, Number>::get_mapping_flags());
      ...
      std::shared_ptr<dealii::Quadrature<dim>> quadrature_p_u =
        create_quadrature<dim>(param.grid.element_type,
                               std::max(param.degree_p, param.degree_u) + 1);
      matrix_free_data.insert_quadrature(*quadrature_p_u,
                                         field + quad_index_p_u);
    }
    ...
    template <int dim, typename Number>
    void
    SpatialOperator<dim, Number>::initialize_dof_vector(
      BlockVectorType &dst) const
    {
      dst.reinit(2);

      matrix_free->initialize_dof_vector(dst.block(block_index_pressure),
                                         get_dof_index_pressure());
      matrix_free->initialize_dof_vector(dst.block(block_index_velocity),
    ...
    template <int dim, typename Number>
    void
    SpatialOperator<dim, Number>::initialize_dof_vector_pressure(
    ...
    template <int dim, typename Number>
    void
    SpatialOperator<dim, Number>::prescribe_initial_conditions(
      BlockVectorType &dst,
      ...
      if (param.aero_acoustic_source_term)
        {
          AssertThrow(aero_acoustic_source_term,
                      dealii::ExcMessage(
                        "Aero-acoustic source term not valid."));
