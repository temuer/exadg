...
namespace ExaDG
{
  ...
  void
  PointwiseOutputGeneratorBase<dim, Number>::do_evaluate(
    std::function<void()> const &write_solution,
    double const                 time,
    bool const                   unsteady)
  {
    AssertThrow(unsteady,
                dealii::ExcMessage("Only implemented for the unsteady case."));
  ...
  template <int dim, typename Number>
  PointwiseOutputGeneratorBase<dim, Number>::PointwiseOutputGeneratorBase(
    MPI_Comm const &comm)
    : mpi_comm(comm)
    , n_out_samples(dealii::numbers::invalid_unsigned_int)
    ...
    PointwiseOutputDataBase<dim> const &pointwise_output_data_in)
  {
    if (pointwise_output_data_in.time_control_data.is_active and
        pointwise_output_data_in.evaluation_points.size() > 0)
      {
        ...
        AssertThrow(
          (get_unsteady_evaluation_type(
             pointwise_output_data.time_control_data) ==
        ...
        componentwise_result.reinit(
          pointwise_output_data.evaluation_points.size());

        // number of samples to write into file
        n_out_samples =
          1 + static_cast<unsigned int>(std::ceil(
                (pointwise_output_data.time_control_data.end_time -
        ...
        (void)mapping_in;
        AssertThrow(false,
                    dealii::ExcMessage("deal.II is not compiled with HDF5!"));
#endif
      }
  }
  ...
  template <int dim, typename Number>
  void
  PointwiseOutputGeneratorBase<dim, Number>::add_quantity(
    std::string const &name,
    unsigned int const n_components)
  {
    AssertThrow(n_components > 0,
                dealii::ExcMessage("n_components has to be > 0."));

    auto const &[it, success] =
      name_to_components.try_emplace(name, n_components);
    AssertThrow(success,
                dealii::ExcMessage("Name already given to quantity dataset."));
    ...
    for (unsigned int comp = 0; comp < n_components; ++comp)
      {
        group.template create_dataset<Number>(
          ...
          std::vector<hsize_t>{pointwise_output_data.evaluation_points.size(),
                               1 * n_out_samples});
      }
#else
    AssertThrow(false,
  ...
  {
    remote_evaluator =
      std::make_shared<dealii::Utilities::MPI::RemotePointEvaluation<dim>>(
        1e-6, false, 0);
    reinit_remote_evaluator();
  }
  ...
  PointwiseOutputGeneratorBase<dim, Number>::create_hdf5_file()
  {
    ...
    hdf5_file = std::make_unique<dealii::HDF5::File>(
      pointwise_output_data.directory + pointwise_output_data.filename + ".h5",
  ...
  template <int dim, typename Number>
  void
  PointwiseOutputGeneratorBase<dim, Number>::write_evaluation_points(
    std::string const &name)
  {
#ifdef DEAL_II_WITH_HDF5
    ...
    if (dealii::Utilities::MPI::this_mpi_process(mpi_comm) == 0)
      {
        ...
        dataset.write_hyperslab(evaluation_points,
                                hyperslab_offset,
  ...
  PointwiseOutputGeneratorBase<dim, Number>::add_evaluation_points_dataset(
    ...
    std::string const   &name)
  {
    std::vector<hsize_t> hdf5_point_dims{
      pointwise_output_data.evaluation_points.size(), dim};
  ...
  template <int dim, typename Number>
  void
  PointwiseOutputGeneratorBase<dim, Number>::add_time_dataset(
    dealii::HDF5::Group &group,
    std::string const   &name)
  {
    group.template create_dataset<Number>(
      name, std::vector<hsize_t>{1, 1 * n_out_samples});
  }
#endif
