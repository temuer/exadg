...
namespace ExaDG
{
  ...
  void
  TimeControlDataStatistics::print(dealii::ConditionalOStream &pcout,
  ...
  TimeControlStatistics::TimeControlStatistics()
    : final_output_written(false)
  {}

  void
  TimeControlStatistics::setup(
    TimeControlDataStatistics const &time_control_data_statistics_in)
  {
  ...
  bool
  TimeControlStatistics::write_preliminary_results(
    double const           time,
    types::time_step const time_step_number) const
  {
    if (Utilities::is_valid_timestep(
            ...
            .write_preliminary_results_every_nth_time_step))
      {
        ...
        if (time < time_control_data_statistics.time_control_data.start_time -
                     time_control.get_epsilon() or
        ...
        // check whether one of the criteria for writing results is met
                 ...
                 (time_step_number - 1) %
                   ...
                   0)
          {
            // make sure that we do no longer write results once the end time
            // has been reached
