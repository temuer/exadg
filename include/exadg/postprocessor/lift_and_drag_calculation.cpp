...
namespace ExaDG
{
            ...
            for (unsigned int q = 0; q < integrator_velocity.n_q_points; ++q)
              {
                ...
                dealii::Tensor<1, dim, dealii::VectorizedArray<Number>> tau =
                  pressure * normal -
                  viscosity *
                    (velocity_gradient + transpose(velocity_gradient)) * normal;

                integrator_velocity.submit_value(tau, q);
              }

            dealii::Tensor<1, dim, dealii::VectorizedArray<Number>>
              Force_local = integrator_velocity.integrate_value();
  ...
  void
  LiftAndDragData::print(dealii::ConditionalOStream &pcout,
  ...
  template <int dim, typename Number>
  void
  LiftAndDragCalculator<dim, Number>::setup(
