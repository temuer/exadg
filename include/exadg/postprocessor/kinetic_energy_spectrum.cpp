...
namespace ExaDG
{
  ...
  /**
   * Class ordering the calls of the deal.spectrum-components in a meaningful
   * order such that the user can simply integrate in his/her own application.
   *
   * Purpose is twofold:
   *      (1) create files which can be processed by the DEAL.SPECTRUM
   * standalone application (2) post process on the fly
   */
         ...
         Tria                           &tria)
    {
          ...
          if (cell->is_locally_owned())
            {
              ...
              for (dealii::types::global_dof_index i = 0; i < dim; i++)
                c[i] = (c[i] + dealii::numbers::PI) /
                       (2 * dealii::numbers::PI / n_cells_1D);
      ...
      for (auto const &I : local_cells)
        for (dealii::types::global_dof_index i = 0;
             i < dealii::Utilities::pow(points_dst, dim);
             i++)
          for (dealii::types::global_dof_index d = 0; d < dim; d++)
            {
              dealii::types::global_dof_index index =
                (I / (n_cells_1D * n_cells_1D) * points_dst +
                 i / (points_dst * points_dst)) *
              ...
              indices_has.push_back(
                d * dealii::Utilities::pow(points_dst * n_cells_1D, dim) +
                index);
            }
            ...
            for (dealii::types::global_dof_index j = 0; j < N; j++)
              for (dealii::types::global_dof_index i = 0; i < Nx; i++, c++)
                if (i < N)
                  ...
                  indices_want.push_back(
                    dealii::numbers::invalid_dof_index); // x-padding

          for (; c < static_cast<dealii::types::global_dof_index>(fftw.bsize);
               c++)
            indices_want.push_back(
              dealii::numbers::invalid_dof_index); // z-padding
        }

      nonconti =
        std::make_shared<dealii::Utilities::MPI::NoncontiguousPartitioner>(
          indices_has, indices_want, comm);
    }
      ...
      if (write)
        {
          ...
          AssertThrow(file_name != "",
                      dealii::ExcMessage("No file name has been provided!"));
          s.time = time;
      ...
      if (inplace)
        {
          ...
          dealii::types::global_dof_index const size =
            dealii::Utilities::pow(static_cast<dealii::types::global_dof_index>(
  ...
  private:
    template <int dim>
    std::size_t
    norm_point_to_lex(dealii::Point<dim> const &c,
                      unsigned int const       &n_cells_1D)
    {
      // convert normalized point [0, 1] to lex
      if (dim == 2)
        return static_cast<std::size_t>(std::floor(c[0]) +
                                        n_cells_1D * std::floor(c[1]));
      else if (dim == 3)
        return static_cast<std::size_t>(
          std::floor(c[0]) + n_cells_1D * std::floor(c[1]) +
...
namespace ExaDG
{
  ...
  void
  apply_taylor_green_symmetry(
    dealii::DoFHandler<dim> const                            &dof_handler_symm,
    dealii::DoFHandler<dim> const                            &dof_handler,
    double const                                              n_cells_1d,
    double const                                              delta,
    dealii::LinearAlgebra::distributed::Vector<Number> const &vector_symm,
    dealii::LinearAlgebra::distributed::Vector<Number>       &vector)
  {
    ...
    dealii::IndexSet range_has_lex(
      dof_handler_symm.n_dofs()); // has in symm system
    dealii::IndexSet range_want_lex(
      dof_handler_symm.n_dofs()); // want in full system
    ...
    std::map<unsigned int, std::vector<unsigned int>>
      recv_map_proc_to_lex_offset;
    auto const [owning_ranks_of_ghosts, send_map_proc_to_lex] =
      dealii::Utilities::MPI::compute_index_owner_and_requesters(range_has_lex,
                                                                 range_want_lex,
                                                                 comm);
    ...
    std::vector<double> data_buffer(range_want_lex.n_elements() *
                                    fe.n_dofs_per_cell());
    {
      ...
      std::map<unsigned int, std::vector<double>> recv_buffer;
      {
        ...
        std::vector<MPI_Request> recv_requests(
          recv_map_proc_to_lex_offset.size());
        ...
        for (auto const &recv_offset : recv_map_proc_to_lex_offset)
          {
            recv_buffer[recv_offset.first].resize(recv_offset.second.size() *
                                                  fe.n_dofs_per_cell());
        ...
        for (auto const &send_index_set : send_map_proc_to_lex)
          {
            ...
            send_buffer.resize(send_index_set.second.n_elements() *
                               fe.n_dofs_per_cell());
        ...
        for (auto const &recv_offset : recv_map_proc_to_lex_offset)
          {
            ...
            for (auto const &offset : recv_offset.second)
              for (unsigned int i = 0; i < fe.n_dofs_per_cell(); i++)
                data_buffer[offset * fe.n_dofs_per_cell() + i] =
                  buffer[counter++];
          }
      }
    }
              ...
              for (unsigned int i = 0; i < n_dofs_per_component; i++)
                {
                  ...
                  unsigned int const shift =
                    dim == 2 ? (p[0] + p[1] * (1 + fe.degree)) :
                               (p[0] + p[1] * (1 + fe.degree) +
                                p[2] * (1 + fe.degree) * (1 + fe.degree));

                  for (unsigned int d = 0; d < dim; d++)
                    vector[dof_indices[i + d * n_dofs_per_component]] =
                      send_buffer_ptr[shift + d * n_dofs_per_component] *
                      (c[d] < 0.0 ? -1.0 : +1.0);
                }
            }
          send_buffer_ptr += fe.n_dofs_per_cell();
        ...
        if (data.exploit_symmetry)
          {
            ...
            fe_full = std::make_shared<dealii::FESystem<dim>>(
              dealii::FE_DGQ<dim>(data.degree), dim);
            dof_handler_full =
              std::make_shared<dealii::DoFHandler<dim>>(*tria_full);
            ...
            deal_spectrum_wrapper->init(
              dim, cells, data.degree + 1, evaluation_points, *tria_full);
          }
        else
          {
            ...
            MPI_Allreduce(
              MPI_IN_PLACE, &cells, 1, MPI_INTEGER, MPI_SUM, mpi_comm);
            cells = static_cast<int>(std::round(std::pow(cells, 1.0 / dim)));
            deal_spectrum_wrapper->init(dim,
                                        cells,
                                        data.degree + 1,
                                        evaluation_points,
  ...
  template <int dim, typename Number>
  void
  KineticEnergySpectrumCalculator<dim, Number>::evaluate(
        ...
        if (data.exploit_symmetry)
          {
            unsigned int n_cells_1d =
              data.n_cells_1d_coarse_grid *
              dealii::Utilities::pow(2, data.refine_level);
  ...
  template <int dim, typename Number>
  void
  KineticEnergySpectrumCalculator<dim, Number>::do_evaluate(
    VectorType const &velocity,
    double const      time)
  {
    ...
    std::string const file_name =
      data.filename + "_" +
      dealii::Utilities::int_to_string(time_control.get_counter(), 4);
    ...
    if (data.do_fftw)
      {
        ...
        if (dealii::Utilities::MPI::this_mpi_process(mpi_comm) == 0)
          {
            std::cout << std::endl
                      << "Write kinetic energy spectrum at time t = " << time
                      << ":" << std::endl;
            ...
            int     len        = deal_spectrum_wrapper->get_results(
              kappa, E, C /*unused*/, e_physical, e_spectral);
            ...
            f << std::endl
              << "Calculate kinetic energy spectrum at time t = " << time << ":"
              << std::endl
              << std::scientific << std::setprecision(precision)
              << std::setw(precision + 8) << std::endl
              << "  Energy physical space e_phy = " << e_physical << std::endl
              << "  Energy spectral space e_spe = " << e_spectral << std::endl
              << "  Difference  |e_phy - e_spe| = "
              << std::abs(e_physical - e_spectral) << std::endl
              << std::endl
            ...
            for (int i = 0; i < len; i++)
              {
                f << std::scientific << std::setprecision(0)
                  << std::setw(2 + static_cast<unsigned int>(std::ceil(
                                     std::max(3.0, log(len) / log(10)))))
                  << i << std::scientific << std::setprecision(precision)
                  << std::setw(precision + 8) << kappa[i] << "   " << E[i]
                  << std::endl;
              }
          }
      }
