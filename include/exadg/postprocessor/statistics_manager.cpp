...
namespace ExaDG
{
  ...
  template <int dim, typename Number>
  void
  StatisticsManager<dim, Number>::setup(
    std::function<double(double const &)> const &grid_transform,
    ...
    if (data_in.time_control_data_statistics.time_control_data.is_active)
      {
        ...
        // find the number of refinements in the mesh, first the number of
        // coarse cells in y-direction and then the number of refinements.
        unsigned int n_cells_y_dir = 1;

        typename dealii::Triangulation<dim>::cell_iterator cell =
          dof_handler.get_triangulation().begin(0);
        while (cell != dof_handler.get_triangulation().end(0) and
               !cell->at_boundary(2))
          {
            ++cell;
          }
        ...
        AssertThrow(n_points_y_per_cell >= 2,
                    dealii::ExcMessage(
                      "Number of points in y-direction per cell is invalid."));

        n_cells_y_dir *= dealii::Utilities::pow(
          2, dof_handler.get_triangulation().n_global_levels() - 1);

        unsigned int const n_points_y_glob =
          n_cells_y_dir * (n_points_y_per_cell - 1) + 1;
        ...
        // calculate y-coordinates in physical space where we want to perform
        // the sampling (averaging)
        ...
        if (data.cells_are_stretched == true)
          {
            for (unsigned int cell = 0; cell < n_cells_y_dir; cell++)
              {
                // determine lower and upper y-coordinates of current cell in
                // ref space [0,1]
                double pointlower = 1. / (double)n_cells_y_dir * (double)cell;
                double pointupper =
                  1. / (double)n_cells_y_dir * (double)(cell + 1);

                // loop over all y-coordinates inside the current cell
                for (unsigned int plane = 0; plane < n_points_y_per_cell - 1;
                     plane++)
                  {
                    // reference space: use a linear distribution inside each
                    // cell [0,1]
                    double coord_ref =
                      pointlower + (pointupper - pointlower) /
                                     (n_points_y_per_cell - 1) * plane;
            ...
            std::cout << std::endl
                      << "Intermediate vector with y-coordinates:" << std::endl;
            ...
            // However, when calculating the statistics we use the polynomial
            // mapping of degree 'fe_degree' which leads to slightly different
            // values as compared to the exact mapping.
            // -> overwrite values in y_glob with values resulting from
            // polynomial mapping
            ...
            y_processor.resize(n_points_y_glob,
                               std::numeric_limits<double>::lowest());

            // vector of dealii::FEValues for all x-z-planes of a cell
            std::vector<std::shared_ptr<dealii::FEValues<dim, dim>>> fe_values(
              n_points_y_per_cell);
            ...
            for (unsigned int i = 0; i < n_points_y_per_cell; ++i)
              {
                ...
                fe_values[i].reset(new dealii::FEValues<dim>(
                  mapping,
                ...
                if (cell->is_locally_owned())
                  {
                    ...
                    for (unsigned int i = 0; i < n_points_y_per_cell; ++i)
                      {
                        fe_values[i]->reinit(typename dealii::Triangulation<
                                             dim>::active_cell_iterator(cell));

                        // Transform cell index 'i' to global index 'idx' of
                        // y_glob-vector

                        // find index within the y-values: first do a binary
                        // search to find the next larger value of y in the
                        // list...
                        double const y = fe_values[i]->quadrature_point(0)[1];

                        // identify index for first point located on the
                        // boundary of the cell because for this point the
                        // mapping can not cause any trouble. For interior
                        // points, the deviations introduced may the mapping can
                        // be so strong that the identification of the index is
                        // no longer unique.
                        if (i == 0)
                          {
                            idx = std::distance(y_glob.begin(),
                                                std::lower_bound(y_glob.begin(),
                                                                 y_glob.end(),
                                                                 y));

                            // make sure that the index does not exceed the
                            // array bounds in case of round-off errors
                            if (idx == y_glob.size())
                              idx--;

                            // reduce index by 1 in case that the previous point
                            // is closer to y than the next point
                            if (idx > 0 and std::abs(y_glob[idx - 1] - y) <
                                              std::abs(y_glob[idx] - y))
                              idx--;
                          }
                        else // simply increment index for subsequent points of
                             // a cell
                          {
                            ++idx;
                          }
            ...
            // print final vector
            std::cout << std::endl
                      << "Final vector with y-coordinates:" << std::endl;
            for (unsigned int i = 0; i < y_glob.size(); ++i)
              std::cout << "y_glob[" << i << "] = " << y_glob[i] << std::endl;
            ...
            for (unsigned int cell = 0; cell < n_cells_y_dir; cell++)
              {
                ...
                for (unsigned int plane = 0; plane < n_points_y_per_cell - 1;
                     plane++)
                  {
                    ...
                    double coord = ylower + (yupper - ylower) /
                                              (n_points_y_per_cell - 1) * plane;
        ...
        AssertThrow(y_glob.size() == n_points_y_glob,
                                           ...
                                           bool const        unsteady)
  {
    AssertThrow(unsteady,
                dealii::ExcMessage(
                  "Only implemented for unsteady simulation."));
  ...
  template <int dim, typename Number>
  void
  StatisticsManager<dim, Number>::evaluate_statistics(
   ...
   *   - and subsequently averaging the x-z-plane-averaged quantities over time
   * samples
   *
   *  Therefore, we have to compute the following quantities: <u>, <u²>, <u*v>,
   * since ...
   *
   *   - <u'²> = <(u-<u>)²> = <u² - 2*u<u> + <u>²> = <u²> - 2*<u>² + <u>² = <u²>
   * - <u>²
   *   - <u'v'> = <(u-<u>)*(v-<v>)> = <u*v> - <u*<v>> - <<u>*v> + <u><v> = <u*v>
   * - <u><v> = <u*v> since <v> = 0
   */
  template <int dim, typename Number>
  void
  StatisticsManager<dim, Number>::do_evaluate(
    std::vector<VectorType const *> const &velocity)
  {
    ...
    for (unsigned int i = 0; i < n_points_y_per_cell; ++i)
      {
        ...
        fe_values[i].reset(
          new dealii::FEValues<dim>(mapping,
                                    dof_handler.get_fe().base_element(0),
                                    dealii::Quadrature<dim>(points, weights),
    ...
    unsigned int const scalar_dofs_per_cell =
      dof_handler.get_fe().base_element(0).dofs_per_cell;
    std::vector<dealii::Tensor<1, dim>> velocity_vector(scalar_dofs_per_cell);
    std::vector<dealii::types::global_dof_index> dof_indices(
      dof_handler.get_fe().dofs_per_cell);

    // loop over all cells and perform averaging/integration for all locally
    // owned cells
    for (auto const &cell : dof_handler.active_cell_iterators())
      {
        if (cell->is_locally_owned())
          {
            ...
            // vector-valued FE where all components are explicitly listed in
            // the dealii::DoFHandler
                ...
                for (unsigned int j = 0; j < dof_indices.size(); ++j)
                  {
                    ...
                    if (comp.first < dim)
                      velocity_vector[comp.second][comp.first] =
                        (*velocity[0])(dof_indices[j]);
                  }
              }
            else // scalar FE where we have several vectors referring to the
                 // same dealii::DoFHandler
              {
                AssertDimension(dof_handler.get_fe().element_multiplicity(0),
                                1);
            ...
            for (unsigned int i = 0; i < n_points_y_per_cell; ++i)
              {
                ...
                for (unsigned int q = 0; q < fe_values[i]->n_quadrature_points;
                     ++q)
                  {
                    ...
                    for (unsigned int j = 0; j < velocity_vector.size(); ++j)
                      velocity +=
                        fe_values[i]->shape_value(j, q) * velocity_vector[j];
                    ...
                    if (dim == 3)
                      {
                        ...
                        reduced_jacobian[0][0] =
                          fe_values[i]->jacobian(q)[0][0];
                        reduced_jacobian[0][1] =
                          fe_values[i]->jacobian(q)[0][2];
                        reduced_jacobian[1][0] =
                          fe_values[i]->jacobian(q)[2][0];
                        reduced_jacobian[1][1] =
                          fe_values[i]->jacobian(q)[2][2];
                    ...
                    double area_ele =
                      det * fe_values[i]->get_quadrature().weight(q);
                    area += area_ele;
                ...
                // Tranform cell index 'i' to global index 'idx' of
                // y_glob-vector

                // find index within the y-values: first do a binary search to
                // find the next larger value of y in the list...
                double const y = fe_values[i]->quadrature_point(0)[1];
                // std::lower_bound: returns iterator to first element that is
                // >= y. Note that the vector y_glob has to be sorted. As a
                // result, the index might be too large.
                unsigned int idx = std::distance(
                  y_glob.begin(),
                  std::lower_bound(y_glob.begin(), y_glob.end(), y));

                // make sure that the index does not exceed the array bounds in
                // case of round-off errors
                if (idx == y_glob.size())
                  idx--;

                // reduce index by 1 in case that the previous point is closer
                // to y than the next point
                if (idx > 0 and
                    std::abs(y_glob[idx - 1] - y) < std::abs(y_glob[idx] - y))
                  idx--;

                AssertThrow(
                  std::abs(y_glob[idx] - y) < 1e-13,
                  dealii::ExcMessage(
                    "Could not locate " + std::to_string(y) +
                ...
                // Add results of cellwise integral to xxx_loc vectors since we
                // want to average/integrate over all locally owned cells.
                for (unsigned int i = 0; i < dim; i++)
  ...
  template <int dim, typename Number>
  void
  StatisticsManager<dim, Number>::do_write_output(
    ...
    double const      density)
  {
    if (dealii::Utilities::MPI::this_mpi_process(mpi_comm) == 0)
      {
        // tau_w = mu * d<u>/dy = mu * (<u>(y2)-<u>(y1))/(y2-y1), where mu = rho
        // * nu
        double tau_w =
          dynamic_viscosity *
          ((vel_glob[0].at(1) - vel_glob[0].at(0)) / (double)number_of_samples /
           (y_glob.at(1) - y_glob.at(0)));

        // Re_tau = u_tau * delta / nu = sqrt(tau_w/rho) * delta / (mu/rho),
        // where delta = 1
