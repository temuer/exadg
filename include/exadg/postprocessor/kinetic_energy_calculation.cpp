...
namespace ExaDG
{
  ...
  template <int dim, typename Number>
  void
  KineticEnergyCalculator<dim, Number>::setup(
  ...
  template <int dim, typename Number>
  void
  KineticEnergyCalculator<dim, Number>::calculate_basic(
  ...
  template <int dim, typename Number>
  Number
  KineticEnergyCalculator<dim, Number>::integrate(
    ...
    Number                                &max_vorticity)
  {
    ...
    matrix_free.cell_loop(&KineticEnergyCalculator<dim, Number>::cell_loop,
                          this,
                          dst,
                          velocity);
  ...
  {
    CellIntegrator<dim, dim, Number> fe_eval(matrix_free,
                                             dof_index,
                                             quad_index);
        ...
        for (unsigned int q = 0; q < fe_eval.n_q_points; ++q)
          {
            ...
            dealii::Tensor<1, number_vorticity_components, scalar> omega =
              fe_eval.get_curl(q);

            scalar norm_omega = omega * omega;

            enstrophy_vec += fe_eval.JxW(q) *
                             dealii::make_vectorized_array<Number>(0.5) *
                             norm_omega;

            max_vorticity_vec =
              std::max(max_vorticity_vec, std::sqrt(norm_omega));
          }
