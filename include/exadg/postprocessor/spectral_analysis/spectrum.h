...
namespace dealspectrum
{
  ...
  public:
    ...
    init()
    {
      ...
      alloc_local =
        fftw_mpi_local_size(dim, n, comm, &local_elements, &local_start);
      local_end = local_start + local_elements;

      // determine how many rows each process has
      int *global_elements = new int[size];
      MPI_Allgather(
        &local_elements, 1, MPI_INTEGER, global_elements, 1, MPI_INTEGER, comm);
      ...
      // initialize input array with zero (not needed: only useful for IO ->
      // hard zero)
    ...
    execute()
    {
      // perform FFT for u
      fftw_plan pu =
        fftw_mpi_plan_dft_r2c(dim, n, u_real, u_comp, comm, FFTW_ESTIMATE);
      fftw_execute(pu);
      fftw_destroy_plan(pu);

      // ... for v
      fftw_plan pv =
        fftw_mpi_plan_dft_r2c(dim, n, v_real, v_comp, comm, FFTW_ESTIMATE);
      fftw_execute(pv);
      ...
      if (dim == 3)
        {
          fftw_plan pw =
            fftw_mpi_plan_dft_r2c(dim, n, w_real, w_comp, comm, FFTW_ESTIMATE);
          fftw_execute(pw);
          ...
          for (int j = local_start; j < local_end; j++)
            {
              for (int i = 0; i < N; i++)
                {
                  e_spectral +=
                    u_comp2(MIN(i, N - i), j)[0] *
                      u_comp2(MIN(i, N - i), j)[0] +
                    u_comp2(MIN(i, N - i), j)[1] *
                      u_comp2(MIN(i, N - i), j)[1] +
                    v_comp2(MIN(i, N - i), j)[0] *
                      v_comp2(MIN(i, N - i), j)[0] +
              ...
              for (int j = 0; j < N; j++)
                {
                  for (int i = 0; i < N; i++)
                    {
                      e_spectral += u_comp3(MIN(i, N - i), j, k_)[0] *
                                      u_comp3(MIN(i, N - i), j, k_)[0] +
                                    u_comp3(MIN(i, N - i), j, k_)[1] *
                                      u_comp3(MIN(i, N - i), j, k_)[1] +
                                    v_comp3(MIN(i, N - i), j, k_)[0] *
                                      v_comp3(MIN(i, N - i), j, k_)[0] +
                                    v_comp3(MIN(i, N - i), j, k_)[1] *
                                      v_comp3(MIN(i, N - i), j, k_)[1] +
                                    w_comp3(MIN(i, N - i), j, k_)[0] *
                                      w_comp3(MIN(i, N - i), j, k_)[0] +
                                    w_comp3(MIN(i, N - i), j, k_)[1] *
                                      w_comp3(MIN(i, N - i), j, k_)[1];
                    }
                }
            }
              ...
              for (int i = 0; i < N; i++)
                {
                  ...
                  double r =
                    sqrt(pow(MIN(i, N - i), 2.0) + pow(MIN(j, N - j), 2.0));
                  // ... use for binning
    ...
    serialize(char const *filename)
    {
      ...
      int delta_all =
        2 * (N / 2 + 1) * dealii::Utilities::pow(N, dim - 1) * sizeof(double);

      //
      int        dofs = (end - start) * delta;
      MPI_Offset disp =
        8 * sizeof(int) + start * delta * sizeof(double); // bytes
