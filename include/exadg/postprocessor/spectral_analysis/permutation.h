...
namespace dealspectrum
{
   ...
   *      - elements are in ascending order according array__list for each
   * process
   *
   * @param a     compare a ...
   * @param b     ... to b
  ...
  cmp(void const *a, void const *b)
  {
    ...
    return array_proc[ia] == array_proc[ib] ?
             // same process
             array_list[ia] < array_list[ib] ? -1 :
                                               array_list[ia] > array_list[ib] :
             // different processes
   ...
   *
   * This class expects an input array as described in the interpolation class
   * and produces an array of the format which comply the requirements described
   * in the FFTW wrapper class.
   */
  class Permutator
  {
    MPI_Comm const comm;
  ...
  public:
    ...
    init(MAPPING &MAP, FFTW &FFT)
    {
      ...
      int want_length =
        (end_ - start_) * dealii::Utilities::pow(n * points, dim - 1);
              ...
              for (unsigned long int I = 0; I < points; I++, counter++)
                {
                  ...
                  unsigned long int temp =
                    ((dim == 3 ?
                        ((MAP.lbf(i * dim + 2) * points + K) * points * n) :
                        0) +
                  ...
                  int proc =
                    FFT.indices_proc_rows(temp / pow(n * points, dim - 1));
      ...
      std::partial_sum(send_offset.begin(),
                       send_offset.end(),
                       send_offset.begin());
        ...
        for (int jj = 0; jj < pow(n * points, dim - 1); jj++, counter++)
          {
            // ... determine dof
            unsigned long int temp =
              ii * dealii::Utilities::pow(n * points, dim - 1) + jj;
            want[counter] = temp;
            // ... determine owning process
            int t               = dim == 3 ? (temp % pn) / points +
                                 ((temp % (pn * pn)) / pn) / points * n +
      ...
      std::partial_sum(recv_offset.begin(),
                       recv_offset.end(),
                       recv_offset.begin());
        ...
        for (int i = 0; i < send_procs.size(); i++)
          {
            ...
            for (int j = 0;
                 j < (send_offset[i + 1] - send_offset[i] - 1) * 5 + 1;
                 j++)
              fprintf(f, " ");
          }
        fprintf(f, "\n");
        ...
        for (int i = 0; i < recv_procs.size(); i++)
          {
            ...
            for (int j = 0;
                 j < (recv_offset[i + 1] - recv_offset[i] - 1) * 5 + 1;
                 j++)
              fprintf(f, " ");
          }
        fprintf(f, "\n");
...
#  endif
     ...
     *      - write data into buffers for each process and send away data right
     * away
    ...
    ipermute(double *source, double *target)
    {
      ...
      for (int s = send_offset[0], t = recv_offset[0]; s < send_offset[1];
           s++, t++)
        for (int d = 0; d < dim; d++)
