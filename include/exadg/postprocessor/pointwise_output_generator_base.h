...
namespace ExaDG
{
  ...
  protected:
    void
    do_evaluate(std::function<void()> const &write_solution,
                double const                 time,
                bool const                   unsteady);
      ...
      unsigned int const first_component)
    {
      ...
      for (unsigned int comp = 0; comp < components; ++comp)
        {
          extract_component_from_tensors(componentwise_result,
                                         values,
                                         first_component + comp);
          write_component((components == 1) ? name :
    ...
    [[nodiscard]] std::vector<
      typename dealii::FEPointEvaluation<n_components, dim, dim, Number>::
        value_type>
  ...
  private:
...
#ifdef DEAL_II_WITH_HDF5
    void
    add_evaluation_points_dataset(dealii::HDF5::Group &group,
                                  std::string const   &name);
    ...
    void
    write_component(std::string const                &name,
                    ComponentwiseContainerType const &componentwise_result)
    {
#ifdef DEAL_II_WITH_HDF5
      ...
      std::vector<hsize_t> hyperslab_dim    = {
        pointwise_output_data.evaluation_points.size(), 1};

      if (dealii::Utilities::MPI::this_mpi_process(mpi_comm) == 0)
        dataset.write_hyperslab(componentwise_result,
                                hyperslab_offset,
                                hyperslab_dim);
      else
    ...
    std::shared_ptr<dealii::Utilities::MPI::RemotePointEvaluation<dim>>
         remote_evaluator;
