...
namespace ExaDG
{
  ...
  get_unsteady_evaluation_type(TimeControlData const &data)
  {
    ...
    bool const timestep =
      Utilities::is_valid_timestep(data.trigger_every_time_steps);

    // We do not allow users to specify both a time interval and a number of
    // time steps.
  ...
  void
  TimeControlData::print(dealii::ConditionalOStream &pcout,
    ...
    if (unsteady or is_active)
      {
        ...
        if (get_unsteady_evaluation_type(*this) ==
            TimeControlData::UnsteadyEvalType::Interval)
          print_parameter(pcout,
                          "TimeControl triggers every interval",
                          trigger_interval);
        if (get_unsteady_evaluation_type(*this) ==
            TimeControlData::UnsteadyEvalType::Timestep)
          print_parameter(pcout,
                          "TimeControl triggers every timestep",
                          trigger_every_time_steps);
      }
  }
  ...
  bool
  TimeControl::needs_evaluation(double const           time,
                                types::time_step const time_step_number) const
  {
    ...
    // unsteady evaluation
    AssertThrow(
      get_unsteady_evaluation_type(time_control_data) !=
    ...
    bool evaluate = false;
    if (get_unsteady_evaluation_type(time_control_data) ==
        TimeControlData::UnsteadyEvalType::Timestep)
      {
        if ((time_step_number - 1) %
            ...
            0)
          {
            ...
            ++counter;
             ...
             TimeControlData::UnsteadyEvalType::Interval)
      {
        // In case of a restart, time might be significantly larger than
        // start_time in the first time step of the restarted simulation. Hence,
        // we have to reconstruct the counter at the end of the previous
        // simulation, in order to obtain the correct behavior at the beginning
        // of the restarted simulation.
        if (reset_counter)
          {
            counter += static_cast<unsigned int>(
              (time - time_control_data.start_time + EPSILON) /
              time_control_data.trigger_interval);
            reset_counter = false;
          }

        if ((time > (time_control_data.start_time +
                     counter * time_control_data.trigger_interval - EPSILON)))
          {
            evaluate = true;
            ++counter;
          }
    ...
    else
      {
        AssertThrow(
          false,
          dealii::ExcMessage(
            "Not implemented for given TimeControlData::UnsteadyEvalType"));
      }
  ...
  {
    // the counter is incremented during needs_evaluation(), thus counter is
    // ahead by 1
    AssertThrow(counter > 0,
