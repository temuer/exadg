...
namespace ExaDG
{
                     ...
                     MPI_Comm const                   &mpi_comm)
  {
    ...
    data_out_surface.write_vtu_with_pvtu_record(
      folder, file + "_surface", counter, mpi_comm, 4);
  }
               ...
               MPI_Comm const                        &mpi_comm)
  {
    ...
    dealii::Particles::ParticleHandler<dim, dim> particle_handler(triangulation,
                                                                  mapping);
    ...
    particle_output.write_vtu_with_pvtu_record(folder,
                                               filename,
                                               counter,
                                               mpi_comm);
  }
    ...
    MPI_Comm const                        &mpi_comm)
  {
    ...
    auto const               boundary_points =
      bounding_box
        .create_extended(1e-3 *
                         std::pow(bounding_box.volume(), 1.0 / ((double)dim)))
        .get_boundary_points();
    ...
    write_points(particle_dummy_tria,
