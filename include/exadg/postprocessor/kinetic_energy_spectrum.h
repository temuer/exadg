...
namespace ExaDG
{
      ...
      if (time_control_data.is_active)
        {
          ...
          print_parameter(pcout,
                          "Evaluation points per cell",
                          evaluation_points_per_cell);
  ...
  /**
   * This class evaluates the kinetic energy spectrum on a periodic box. The
   * mesh must be composed of hypercube (quad, hex) elements. Exploiting the
   * symmetries of the Taylor-Green problem is implemented as a special case, in
   * order to be able to solve the PDE problem only on the symmetric box instead
   * of the periodic box ("full" domain).
   *
   */
