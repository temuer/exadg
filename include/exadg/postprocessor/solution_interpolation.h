...
namespace ExaDG
{
    ...
    dealii::Point<dim> const &point_in_ref_coord)
  {
    ...
    Assert(dealii::GeometryInfo<dim>::distance_to_unit_cell(
             point_in_ref_coord) < 1e-10,
    ...
    std::vector<dealii::Vector<Number>> solution_vector(
      1, dealii::Vector<Number>(fe.n_components()));
  ...
  {
    typedef std::pair<typename dealii::DoFHandler<dim>::active_cell_iterator,
                      dealii::Point<dim>>
      Pair;

    std::vector<Pair> adjacent_cells =
      dealii::GridTools::find_all_active_cells_around_point(mapping,
                                                            dof_handler,
                                                            point,
                                                            tolerance);

    // processor local variables: initialize with zeros since we add values to
    // these variables
    ...
    // parallel computations: add results of all processors and calculate mean
    // value
    ...
    double const              tolerance = 1.e-10)
  {
    ...
    for (unsigned int d = 0; d < dim; ++d)
      solution_value[d] =
        dealii::Utilities::MPI::sum(solution_value[d], mpi_comm);
    solution_value /= (double)counter;
  }

  /*
   * For a given vector of adjacent cells and points in reference coordinates,
   * determine and return the dof_indices and shape_values to be used later for
  ...
  get_dof_indices_and_shape_values(
    std::vector<
      std::pair<typename dealii::Triangulation<dim>::active_cell_iterator,
    ...
    dealii::LinearAlgebra::distributed::Vector<Number> const &solution)
  {
    ...
    for (auto cell_tria : adjacent_cells)
      {
        // transform Triangulation::active_cell_iterator into
        // dealii::DoFHandler::active_cell_iterator, see constructor of
        // DoFCellAccessor
        ...
        if (cell_and_point.first->is_locally_owned())
          {
            const dealii::Quadrature<dim> quadrature(
              dealii::GeometryInfo<dim>::project_to_unit_cell(
                cell_and_point.second));
            ...
            std::vector<dealii::types::global_dof_index> dof_indices_global(
              fe.dofs_per_cell);
            for (unsigned int i = 0; i < fe.dofs_per_cell; ++i)
              dof_indices_global[i] =
                solution.get_partitioner()->global_to_local(dof_indices[i]);

            std::vector<Number> fe_shape_values(fe.dofs_per_cell);
            for (unsigned int i = 0; i < fe.dofs_per_cell; ++i)
              fe_shape_values[i] = fe_values.shape_value(i, 0);

            dof_indices_and_shape_values.emplace_back(dof_indices_global,
                                                      fe_shape_values);
          }
      }
  ...
  /*
   * Interpolate solution from dof vector by using precomputed shape function
   * values.
   */
