...
namespace ExaDG
{
    ...
    std::map<dealii::types::boundary_id, Number> &mean_scalar)
  {
    ...
    dealii::Utilities::MPI::sum(
      dealii::ArrayView<double const>(&(*mean_scalar_vector.begin()),
                                      mean_scalar_vector.size()),
      mpi_comm,
      dealii::ArrayView<double>(&(*mean_scalar_vector.begin()),
                                mean_scalar_vector.size()));

    dealii::Utilities::MPI::sum(
      dealii::ArrayView<double const>(&(*area_vector.begin()),
                                      area_vector.size()),
      mpi_comm,
    ...
    for (unsigned int counter = 0; counter < mean_scalar.size(); ++counter)
      {
        (iterator++)->second =
          mean_scalar_vector[counter] / area_vector[counter];
      }
