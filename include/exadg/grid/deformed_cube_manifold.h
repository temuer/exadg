...
namespace ExaDG
{
   ...
   *
   *  dealii::GridGenerator::subdivided_hyper_cube(tria, n_cells_1d, left,
   * right);
   */
  template <int dim>
                               ...
                               unsigned int const          frequency)
  {
    ...
    // the vertices need to be placed correctly according to the manifold
    // description such that mesh refinements and high-order mappings are done
    // correctly (which invoke pull-back and push-forward operations)
