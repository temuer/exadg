...
namespace ExaDG
{
        ...
        if (cell->is_locally_owned())
          {
            ...
            for (unsigned int i = 0; i < vertices.size(); ++i)
              for (unsigned int j = i + 1; j < vertices.size(); ++j)
                min_cell_diameter = std::min(min_cell_diameter,
  ...
  /**
   * This function calculates a characteristic element length given the volume
   * of the element, the number of space dimensions, and the element type.
   */
                                          ...
                                          ElementType const &element_type)
  {
    if (element_type == ElementType::Hypercube)
      {
        // calculate the length h of a dim-dimensional cube that has the volume
        // "element_volume".
        return std::exp(std::log(element_volume) / (double)dim);
      }
    else if (element_type == ElementType::Simplex)
      {
        // calculate the length h of an equilateral triangle/tetrahedron that
        // has the volume "element_volume" (V = h^dim / factor).
        double factor = 1.0;
        ...
        if (dim == 2)
          ...
          AssertThrow(false,
                      dealii::ExcMessage("Only dim = 2, 3 implemented."));
  ...
  /**
   * This function calculates a characteristic resolution limit for high-order
   * Lagrange polynomials given a mesh size h. This one-dimensional resolution
   * limit is calculated as the grid size divided by the number of nodes per
   * coordinate direction. Hence, the result depends on the function space (H^1
   * vs. L^2).
   */
  template <typename Number>
  inline Number
