...
namespace ExaDG
{
  ...
  public:
    ...
    value(dealii::Point<dim> const &x,
          unsigned int const        coordinate_direction = 0) const override
    {
  ...
  private:
    double
    compute_displacement_share(
      dealii::Point<dim> const &x,
      ...
      switch (data.shape)
        {
          ...
          case MeshMovementShape::Sin:
            if (coordinate_direction == 0)
              ...
              solution = std::sin(2.0 * pi * (x(0) - left) *
                                  data.spatial_number_of_oscillations / width) *
                         data.amplitude *
                         std::sin(2.0 * pi * (x(1) - left) *
                                  data.spatial_number_of_oscillations / width);
      ...
      switch (data.temporal)
        {
          ...
          case MeshMovementAdvanceInTime::SinSquared:
            solution =
              std::pow(std::sin(2.0 * pi * this->get_time() / time_period), 2);
            break;
  ...
  public:
    ...
    double
    value(dealii::Point<dim> const &x_in,
          unsigned int const        coordinate_direction = 0) const override
    {
      ...
      switch (data.shape)
        {
          ...
          case MeshMovementShape::Sin:
            if (coordinate_direction == 0)
              solution =
                std::sin(2.0 * pi * (x(1) - (height / 2.0)) / height) *
                data.amplitude *
                (dim == 3 ? std::sin(2 * pi * (x(2.0) - (depth / 2)) / depth) :
                            1.0);
            else if (coordinate_direction == 1)
              solution =
                std::sin(2.0 * pi * (x(0) - (length / 2.0)) / length) *
                data.amplitude *
                (dim == 3 ? std::sin(2 * pi * (x(2) - (depth / 2.0)) / depth) :
                            1.0);
            else if (coordinate_direction == 2)
              solution = std::sin(2.0 * pi * (x(1) - (height / 2.0)) / height) *
                         data.amplitude *
                         std::sin(2.0 * pi * (x(0) - (length / 2.0)) / length);
            break;
