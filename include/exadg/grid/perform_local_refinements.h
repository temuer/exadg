...
{
  /**
   * This function performs local refinements on a given triangulation, where
   * the number of local refinements is described by a vector containing the
   * number of local refinements for each material id, i.e. vector[material_id]
   * = n_local_refinements.
   *
   * TODO: change the design of this function from a vector towards a map from
   * material IDs to the number of local refinements.
   */
  template <int dim>
  void
    ...
    while (*max_element(refine_local.begin(), refine_local.end()) > 0)
      {
        ...
        for (size_t material_id = 0; material_id < refine_local.size();
             material_id++)
          {
