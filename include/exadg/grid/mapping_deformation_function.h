...
{
  /**
   * Class for mesh deformations that can be described analytically via a
  ...
  public:
    ...
    /**
     * Updates the grid coordinates using a dealii::Function<dim> object
  ...
  private:
    /**
     * Initializes the dealii::MappingQCache object by providing a
     * dealii::Function<dim> that describes the displacement of the grid
     * compared to an undeformed reference configuration described by the
     * mapping_undeformed member of the base class.
     */
    void
                  ...
                  std::shared_ptr<dealii::Function<dim>> displacement_function)
    {
      ...
      dealii::FEValues<dim>   fe_values(
        *this->mapping_undeformed,
        dummy_fe,
        dealii::QGaussLobatto<dim>(this->mapping_q_cache->get_degree() + 1),
