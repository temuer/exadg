...
{
  /**
   * A mapping class based on dealii::MappingQCache equipped with practical
   * interfaces that can be used to initialize the mapping.
   *
   * The two functions fill_grid_coordinates_vector() and
   * initialize_mapping_q_cache() should become member functions of
   * dealii::MappingQCache in dealii, rendering this class superfluous in ExaDG.
   */
  ...
  public:
    ...
    /**
     * Extract the grid coordinates of the current mesh configuration described
     * by the dealii::MappingQCache object and fill a dof-vector given a
      ...
      dealii::DoFHandler<dim> const &dof_handler) const
    {
      ...
      fill_grid_coordinates_vector(*mapping_q_cache,
                                   grid_coordinates,
                                   dof_handler);
    }
      ...
      dealii::DoFHandler<dim> const &dof_handler) const
    {
      ...
      AssertThrow(get_element_type(dof_handler.get_triangulation()) ==
      ...
      // Set up dealii::FEValues with FE_Nothing and the Gauss-Lobatto
      // quadrature to reduce setup cost, as we only use the geometry
      // information (this means we need to call fe_values.reinit(cell) with
      // Triangulation::cell_iterator rather than
    ...
    /**
     * Initializes the dealii::MappingQCache object by providing a mapping that
     * describes an undeformed reference configuration and a displacement
     * dof-vector (with a corresponding dealii::DoFHandler object) that
     * describes the displacement of the mesh compared to that reference
     * configuration. There are two special cases:
     *
     * If the mapping pointer is invalid, this implies that the reference
     * coordinates are interpreted as zero, i.e., the displacement vector
     * describes the absolute coordinates of the grid points.
     *
     * If the displacement_vector is empty or uninitialized, this implies that
     * no displacements will be added to the grid coordinates of the reference
     * configuration described by mapping.
     */
    void
      ...
      dealii::DoFHandler<dim> const              &dof_handler)
    {
      ...
      if (dof_handler.n_dofs() > 0 and
          displacement_vector.size() == dof_handler.n_dofs())
        {
          ...
          displacement_vector_ghosted.reinit(dof_handler.locally_owned_dofs(),
                                             ...
                                             dof_handler.get_communicator());
          displacement_vector_ghosted.copy_locally_owned_data_from(
            displacement_vector);
      ...
      if (mapping.get() != 0)
        {
          fe_values = std::make_shared<dealii::FEValues<dim>>(
            *mapping,
            fe_nothing,
      ...
      // take the grid coordinates described by mapping and add deformation
      // described by displacement vector
      mapping_q_cache->initialize(
        ...
        [&](const typename dealii::Triangulation<dim>::cell_iterator &cell_tria)
          ...
          std::vector<dealii::Point<dim>> grid_coordinates(
          ...
          if (mapping.get() != 0)
            {
              ...
              for (unsigned int i = 0; i < scalar_dofs_per_cell; ++i)
                {
                  grid_coordinates[i] = fe_values->quadrature_point(
              ...
              for (unsigned int i = 0; i < dof_indices.size(); ++i)
                {
                  ...
                  if (fe.dofs_per_vertex > 0) // dealii::FE_Q
                    ...
                    {
                      grid_coordinates
                        [this->lexicographic_to_hierarchic_numbering[id.second]]
                        [id.first] +=
                        displacement_vector_ghosted(dof_indices[i]);
                    }
                }
  ...
  namespace MappingTools
  {
     ...
     *
     * This function only takes the grid coordinates described by the fine
     * mapping without adding displacements in order to initialize the coarse
     * mappings for all multigrid h-levels.
     *
     * Prior to calling this function, the vector of coarse_mappings must have
     * the correct size according to the number of h-multigrid levels (excluding
     * the finest level). The first entry corresponds to the coarsest
     * triangulation, the last element to the level below the fine
     * triangulation.
     */
    template <int dim, typename Number>
    void
    initialize_coarse_mappings_from_mapping_dof_vector(
      std::vector<std::shared_ptr<MappingDoFVector<dim, Number>>>
                        &coarse_mappings,
      ...
      dealii::Triangulation<dim> const                           &triangulation)
    {
      ...
      std::shared_ptr<MappingDoFVector<dim, Number>>
        mapping_dof_vector_all_levels =
          std::make_shared<MappingDoFVector<dim, Number>>(
            degree_coarse_mappings);

      // fill a dof vector with grid coordinates of the fine level using
      // degree_coarse_mappings
      typedef dealii::LinearAlgebra::distributed::Vector<Number> VectorType;
      VectorType grid_coordinates_fine_level;

      {
        mapping_dof_vector_all_levels->fill_grid_coordinates_vector(
          *fine_mapping->get_mapping(),
          grid_coordinates_fine_level,
          dof_handler);
      }

      // project the solution onto all coarse levels of the triangulation using
      // degree_coarse_mappings
      ...
      for (unsigned int level = 0; level < n_levels; level++)
        {
          ...
          grid_coordinates_all_levels_ghosted[level]
            .copy_locally_owned_data_from(grid_coordinates_all_levels[level]);

          grid_coordinates_all_levels_ghosted[level].update_ghost_values();
        }

      // Call the initialize() function of dealii::MappingQCache, which
      // initializes the mapping for all levels according to
      // grid_coordinates_all_levels_ghosted.
      mapping_dof_vector_all_levels->get_mapping_q_cache()->initialize(
        ...
        [&](const typename dealii::Triangulation<dim>::cell_iterator &cell_tria)
          ...
          unsigned int const scalar_dofs_per_cell =
            dealii::Utilities::pow(fe.degree + 1, dim);
              ...
              for (unsigned int i = 0; i < dof_indices.size(); ++i)
                {
                  ...
                  if (fe.dofs_per_vertex > 0) // dealii::FE_Q
                  ...
                  else // dealii::FE_DGQ
                    {
                      grid_coordinates
                        [mapping_dof_vector_all_levels
                           ->lexicographic_to_hierarchic_numbering[id.second]]
                        [id.first] = grid_coordinates_all_levels_ghosted[level](
                          dof_indices[i]);
                    }
                }
            }
      ...
      // Finally, let all coarse grid mappings point to the same
      // MappingDoFVector object. Using the same Mapping object for all
      // multigrid h-levels is some form of legacy code. The class
      // dealii::MatrixFree can internally extract the coarse-level mapping
      // information (provided through the fine-level Mapping object).
      for (unsigned int h_level = 0; h_level < coarse_mappings.size();
           ++h_level)
        {
     ...
     *
     * Use this function to initialize the coarse mappings for use in multigrid
     * in case the multigrid algorithm uses a separate triangulation object for
     * each multigrid h-level.
     *
     * The second argument describes the mapping of the fine triangulation.
     *
     * This function only takes the grid coordinates described by the fine
    ...
    initialize_coarse_mappings_from_mapping_dof_vector(
      ...
      unsigned int const degree_coarse_mappings,
      std::shared_ptr<MappingDoFVector<dim, Number> const> const &fine_mapping,
      std::shared_ptr<dealii::Triangulation<dim> const> const
        &fine_triangulation,
        ...
        &coarse_triangulations)
    {
      ...
      // setup dof-handlers and constraints for all levels using
      // degree_coarse_mappings
      dealii::FESystem<dim> fe(dealii::FE_Q<dim>(degree_coarse_mappings), dim);
      ...
      std::vector<dealii::AffineConstraints<Number>> constraints_all_levels(
        n_h_levels);
      for (unsigned int h_level = 0; h_level < n_h_levels; ++h_level)
        {
          if (h_level == n_h_levels - 1)
            ...
            dof_handlers_all_levels[h_level].reinit(
              *coarse_triangulations[h_level]);
...

      {
        ...
        auto const &dof_handler_fine_level =
          dof_handlers_all_levels[n_h_levels - 1];
        mapping_dof_vector_fine_level->fill_grid_coordinates_vector(
          *fine_mapping->get_mapping(),
          grid_coordinates_fine_level,
      ...
      dealii::MGLevelObject<dealii::MGTwoLevelTransfer<dim, VectorType>>
        transfers(0, n_h_levels - 1);
      for (unsigned int h_level = 1; h_level < n_h_levels; ++h_level)
        {
          transfers[h_level].reinit_geometric_transfer(
      ...
      // a function that initializes the dof-vector for a given level and
      // dof_handler
      const std::function<void(unsigned int const, VectorType &)>
        initialize_dof_vector =
          [&](unsigned int const h_level, VectorType &vector) {
            dealii::IndexSet locally_relevant_dofs;
            dealii::DoFTools::extract_locally_relevant_dofs(
              dof_handlers_all_levels[h_level], locally_relevant_dofs);
            vector.reinit(dof_handlers_all_levels[h_level].locally_owned_dofs(),
                          locally_relevant_dofs,
                          dof_handlers_all_levels[h_level].get_communicator());
          };

      dealii::MGTransferGlobalCoarsening<dim, VectorType>
      ...
      dealii::MGLevelObject<VectorType> grid_coordinates_all_levels(0,
                                                                    n_h_levels -
                                                                      1);
      mg_transfer_global_coarsening.interpolate_to_mg(
        dof_handler_dummy,
        grid_coordinates_all_levels,
        grid_coordinates_fine_level);

      // initialize mapping for all coarse h-levels using the dof-vectors with
      // grid coordinates
      AssertThrow(
        coarse_mappings.size() == n_h_levels - 1,
        dealii::ExcMessage(
      ...
      for (unsigned int h_level = 0; h_level < coarse_mappings.size();
           ++h_level)
        {
          ...
          // grid_coordinates_all_levels describes absolute coordinates -> use
          // an uninitialized mapping in order to interpret the grid coordinates
          // vector as absolute coordinates and not as displacements.
    ...
    /**
     * A function to initalize mappings for coarse multigrid h-levels in case
     * the mapping on the fine level is of type MappingDoFVector.
     *
     * This function unifies the two functions above depending on whether the
     * multigrid algorithm uses separate triangulation objects for the coarse
     * triangulations or just a single triangulation object for all levels. In
     * the latter case, the last argument of this function remains unused.
     *
      ...
      if (fine_mapping.get())
        {
          if (coarse_triangulations.size() > 0)
            {
              MappingTools::initialize_coarse_mappings_from_mapping_dof_vector<
                dim,
                Number>(coarse_mappings,
                        degree_coarse_mappings,
                        fine_mapping,
                        fine_triangulation,
                        coarse_triangulations);
