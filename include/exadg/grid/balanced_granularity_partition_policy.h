...
namespace ExaDG
{
  ...
  public:
    ...
    partition(
      dealii::Triangulation<dim, spacedim> const &tria_coarse_in) const override
    {
      dealii::types::global_cell_index const n_cells =
      ...
      unsigned int const grain_size_limit = std::min<unsigned int>(
        200, 8 * n_cells / n_mpi_processes_per_level.back() + 1);
