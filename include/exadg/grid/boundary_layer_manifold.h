...
{
  /**
   * A "boundary-layer" manifold that corresponds to a triangulation generated
   * via
   *
   *   dealii::GridGenerator::hyper_rectangle(tria,
   *                                          dealii::Point<dim>(-dimensions
   * / 2.0), dealii::Point<dim>(dimensions / 2.0));
   *
   * where dealii::Tensor<1, dim> dimensions; describes the physical dimensions
   * in the dim coordinate directions.
   */
  template <int dim>
  ...
  public:
    ...
    clone() const final
    {
      return std::make_unique<BoundaryLayerManifold<dim>>(dimensions,
                                                          grid_stretch_factor);
    }
    ...
    grid_transform_y(double const &eta) const
    {
      ...
      if (grid_stretch_factor >= 0)
        y = dimensions[1] / 2.0 *
            std::tanh(grid_stretch_factor * (2. * eta - 1.)) /
    ...
    inverse_grid_transform_y(double const &y) const
    {
      ...
      if (grid_stretch_factor >= 0)
        eta = (std::atanh(y * std::tanh(grid_stretch_factor) * 2.0 /
                          dimensions[1]) /
                 grid_stretch_factor +
               1.0) /
              2.0;
