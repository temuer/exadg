...
namespace ExaDG
{
  ...
  {
    template <int dim>
    using PeriodicFacePairs = std::vector<dealii::GridTools::PeriodicFacePair<
      ...
      if (element_type == ElementType::Hypercube)
      ...
      else if (element_type == ElementType::Simplex)
        {
          mapping = std::make_shared<dealii::MappingFE<dim>>(
            dealii::FE_SimplexP<dim>(mapping_degree));
        }
      else
        {
          AssertThrow(false,
                      dealii::ExcMessage("Invalid parameter element_type."));
        }
    }

    /**
     * This function creates mapping and multigrid_mappings (while the mapping
     * for coarse multigrid h-levels is created only if involves_h_multigrid is
     * true and if mapping_degree_fine is unequal mapping_degree_coarse).
     * Internally, the above function is called, creating a dealii::Mapping
     * depending on the element type and the mapping_degree.
     */
    template <int dim, typename Number>
    void
    create_mapping_with_multigrid(
      ...
      bool const                                       involves_h_multigrid)
    {
      ...
      if (involves_h_multigrid and
          (mapping_degree_coarse != mapping_degree_fine))
        {
          create_mapping(coarse_mapping, element_type, mapping_degree_coarse);
        }
      multigrid_mappings =
        std::make_shared<MultigridMappings<dim, Number>>(mapping,
                                                         coarse_mapping);
    }

    /**
     * This function can be seen as some form of "copy constructor" for periodic
     * face pairs, transforming the template argument of
     * dealii::GridTools::PeriodicFacePair from Triangulation::cell_iterator to
      ...
      for (auto it : periodic_faces)
        {
          ...
          face_pair_dof_hander.cell[0] =
            it.cell[0]->as_dof_handler_iterator(dof_handler);
          face_pair_dof_hander.cell[1] =
    ...
    /**
     * This function creates a triangulation based on a lambda function and
     * refinement parameters for global and local mesh refinements. This
     * function is used to create the fine triangulation on the one hand and the
     * coarse triangulations required for multigrid (if needed) on the other
     * hand.
     *
     * This function expects that the argument tria has already been
     * constructed.
     */
      ...
      std::vector<unsigned int> const &vector_local_refinements)
    {
      ...
      // the mesh used for a simulation should not depend on the preconditioner
      // used to solve the problem (like whether multigrid is used or not).
      // Hence, we always set this parameter for ElementType::Hypercube. It is
      // not posible to set this parameter for ElementType::Simplex due to the
      ...
      if (data.triangulation_type == TriangulationType::Serial)
        {
          ...
          triangulation =
            std::make_shared<dealii::Triangulation<dim>>(mesh_smoothing);
      ...
      else if (data.triangulation_type == TriangulationType::Distributed)
        {
          ...
          triangulation =
            std::make_shared<dealii::parallel::distributed::Triangulation<dim>>(
              mpi_comm, mesh_smoothing, distributed_settings);
        ...
        {
          auto const serial_grid_generator =
          ...
          auto const serial_grid_partitioner =
            [&](dealii::Triangulation<dim, dim> &tria_serial,
                MPI_Comm const                   comm,
          ...
          triangulation = std::make_shared<
            dealii::parallel::fullydistributed::Triangulation<dim>>(mpi_comm);
        ...
        {
          AssertThrow(
            false, dealii::ExcMessage("Invalid parameter triangulation_type."));
        }
    }

    /**
     * Given a fine_triangulation, this function creates all the coarse
     * triangulations required for multigrid implementations that expect a
     * vector of triangulations.
     *
     * The vector coarse_triangulations only includes the levels coarser than
     * the fine triangulation, where the first entry of the vector corresponds
     * to the coarsest level.
     *
     * We have to distinguish between traingulation types, since for the case of
     * a fully distributed triangulation, one cannot coarse triangulations
     * automatically. Instead, the lambda function
     * `lambda_create_coarse_triangulation()` is used together with the
     * `vector_local_refinemetns`.
     */
    template <int dim>
    inline void
    create_coarse_triangulations_automatically_from_fine_triangulation(
      ...
      std::vector<std::shared_ptr<dealii::Triangulation<dim> const>>
                                          &coarse_triangulations_const,
      std::vector<PeriodicFacePairs<dim>> &coarse_periodic_face_pairs,
      GridData const                      &data)
    {
      // In case of a serial or distributed triangulation, deal.II can
      // automatically generate the coarse triangulations.
      ...
      if (data.triangulation_type == TriangulationType::Serial)
        {
          ...
          coarse_triangulations_const =
            dealii::MGTransferGlobalCoarseningTools::
              create_geometric_coarsening_sequence(fine_triangulation);
        }
      else if (data.triangulation_type == TriangulationType::Distributed)
        {
          AssertThrow(
      ...
      // deal.II adds the fine triangulation as the last entry of the vector.
      // According to our convention in ExaDG, we only include the
      // triangulations coarser than the fine level. Hence, we remove the last
      // entry of the vector.
      coarse_triangulations_const.pop_back();

      coarse_periodic_face_pairs.resize(coarse_triangulations_const.size());
      for (unsigned int level = 0; level < coarse_periodic_face_pairs.size();
           ++level)
        {
    ...
    create_coarse_triangulations_for_fully_distributed_triangulation(
      ...
      std::vector<unsigned int> const vector_local_refinements)
    {
      // In case of a fully distributed triangulation, deal.II cannot
      // automatically generate the coarse triangulations. Create the coarse
      // traingulations using the lambda function and the vector of local
      // refinements.
      AssertThrow(data.triangulation_type ==
                    TriangulationType::FullyDistributed,
                  dealii::ExcMessage("Invalid parameter triangulation_type."));

      if (fine_triangulation.n_global_levels() >= 2)
        {
          // Resize the empty coarse_triangulations and
          // coarse_periodic_face_pairs vectors.
          ...
          if (data.n_refine_global >= 1)
            {
              unsigned int const n_refine_global_start =
                (unsigned int)(data.n_refine_global - 1);
              for (int refine_global = n_refine_global_start;
                   refine_global >= 0;
                   --refine_global)
                {
          ...
          if (refine_local.size() > 0)
            {
              while (*std::max_element(refine_local.begin(),
                                       refine_local.end()) != 0)
                {
                  for (size_t material_id = 0;
                       material_id < refine_local.size();
                       material_id++)
                    {
    ...
    {
      // In case of a serial or distributed triangulation, deal.II can
      // automatically generate the coarse triangulations, otherwise, the coarse
      // triangulations have to be explicitily created using the provided lambda
      // function and the vector of local refinements.
    ...
    /**
     * This utility function initializes a Grid object by creating a
     * triangulation and filling the periodic_face_pairs. According to the
     * settings in GridData, the corresponding constructor of
     * dealii::Triangulation (or derived classes) is called. The actual
     * functionality "creating" the triangulation needs to be provided via
     * lambda_create_triangulation.
     */
    template <int dim>
    inline void
    create_triangulation(
      ...
      std::vector<unsigned int> const vector_local_refinements)
    {
      GridUtilities::create_triangulation(
        grid.triangulation,
    ...
    /**
     * This function creates both the fine triangulation and, if needed, the
     * coarse triangulations required for certain geometric coarsening sequences
     * in multigrid. In addition to the functions above, this function exists in
     * order to provide a simple interface for applications.
     */
    template <int dim>
    inline void
      ...
      std::vector<unsigned int> const vector_local_refinements)
    {
      if (involves_h_multigrid)
        {
          // Make sure that we create coarse triangulations in case of simplex
          // meshes.
          if (data.element_type == ElementType::Simplex)
            {
              AssertThrow(
                data.create_coarse_triangulations == true,
                dealii::ExcMessage(
          ...
          // Make sure that we create coarse triangulations in case of meshes
          // with hanging nodes.
          ...
          if (data.create_coarse_triangulations)
            {
              GridUtilities::create_coarse_triangulations(
                *grid.triangulation,
                grid.periodic_face_pairs,
        ...
        {
          // If no h-multigrid is involved, simply re-direct to the other
          // function that creates the fine triangulation only.
    ...
    /**
     * Function to create the coarse_triangulations and
     * coarse_periodic_face_pairs given the fine_triangulation and
     * fine_periodic_face_pairs.
     */
    template <int dim>
    inline void
    ...
    /**
     * This function reads an external triangulation. The function takes
     * GridData as an argument and stores the external triangulation in "tria".
     */
    template <int dim>
    inline void
    read_external_triangulation(dealii::Triangulation<dim, dim> &tria,
                                GridData const                  &data)
    {
      AssertThrow(
        not data.file_name.empty(),
        dealii::ExcMessage(
      ...
      std::string extension =
        data.file_name.substr(data.file_name.find_last_of('.') + 1);
      ...
      AssertThrow(
        get_element_type(tria) == data.element_type,
        dealii::ExcMessage(
