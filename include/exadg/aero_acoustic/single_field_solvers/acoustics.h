...
namespace ExaDG
{
  namespace AeroAcoustic
  {
    ...
    public:
      void
      setup(std::shared_ptr<AcousticsAeroAcoustic::ApplicationBase<dim, Number>>
                           application,
            ...
            bool const     is_test)
      {
        ...
        pde_operator =
          std::make_shared<Acoustics::SpatialOperator<dim, Number>>(
            grid,
            mapping,
        ...
        time_integrator =
          std::make_shared<Acoustics::TimeIntAdamsBashforthMoulton<Number>>(
            pde_operator,
            application->get_parameters(),
            postprocessor,
            mpi_comm,
      ...
      {
        // in case the macro time step is smaller than the acoustic timestep,
        // simply run with macro time step, otherwise, ensure that
        // n_sub_time_steps * sub_dt = macro_dt
        double const dt = time_integrator->get_time_step_size();
        double const sub_dt =
          (macro_dt < dt) ? macro_dt :
                            adjust_time_step_to_hit_end_time(0.0, macro_dt, dt);
      ...
      {
        return get_number_of_sub_time_steps() /
               get_number_of_macro_time_steps();
      }
      ...
      std::shared_ptr<Acoustics::TimeIntAdamsBashforthMoulton<Number>>
        time_integrator;
    ...
    private:
      std::pair<unsigned int /* n_macro_dt */,
                unsigned long long /* n_sub_dt */>
        sub_time_steps;
    };
