...
namespace ExaDG
{
  namespace AeroAcoustic
  {
    ...
    enum class AcousticSourceTermComputation
    {
      Undefined,
      FromFluid,
      // In case the aero-acoustic source term is known analytically do not
      // compute a CFD, but interpolate the surce term to the CFD grid and use
      // the given coupling strategy to transfer the source term. If this
      // coupling strategy is set IncNS::TemporalDiscretization has to be
      // InterpolateAnalyticalSolution. The analytical source term is
      // interpolated as is, i.e. at the point of interpolation there is no
      // check if source_term_with_convection is true. It is the resposibility
      // of the user to make sure the source term includes convection or not.
    ...
    public:
      Parameters()
        ...
        , fluid_to_acoustic_coupling_strategy(
            FluidToAcousticCouplingStrategy::Undefined)
        , acoustic_source_term_computation(
            AcousticSourceTermComputation::Undefined)
      {}

      void
      check() const
      {
        AssertThrow(density >= 0.0,
                    dealii::ExcMessage("Density has to be set."));

        AssertThrow(fluid_to_acoustic_coupling_strategy !=
                      FluidToAcousticCouplingStrategy::Undefined,
                    dealii::ExcMessage("Coupling strategy has to be set."));

        AssertThrow(acoustic_source_term_computation !=
                      AcousticSourceTermComputation::Undefined,
                    dealii::ExcMessage(
            ...
            std::string const                &name) const
      {
        ...
        print_parameter(pcout,
                        "Source term has convective part",
                        source_term_with_convection);
        print_parameter(pcout, "Blend in source term", blend_in_source_term);
        print_parameter(pcout,
                        "Fluid to acoustic coupling",
                        fluid_to_acoustic_coupling_strategy);
        print_parameter(pcout,
                        "Acoustic source term compuation",
                        acoustic_source_term_computation);
      }

      void
      add_parameters(dealii::ParameterHandler &prm,
                     std::string const        &subsection_name)
      {
