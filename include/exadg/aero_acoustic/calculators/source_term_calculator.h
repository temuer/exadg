...
namespace ExaDG
{
  namespace AeroAcoustic
  {
    ...
    /**
     * A class that knows how to compute the aeroacoustic source term on the
     * fluid mesh. evaluate_integrate() computes and integrates the source term
     * on the fluid mesh.
     *
     * The aeroacoustic source term f is definded as:
     * f = - rho * (dp/dt + u * grad(p)).
     * The scaling factor rho has to be used since the pressure of the
     * incompressible module is a kinematic pressure. Using
     * consider_convection=false f = -rho * (dp/dt).
     */
    ...
    public:
                         ...
                         double const           evaluation_time)
      {
        ...
        matrix_free->cell_loop(
          &This::compute_source_term, this, dst, analytical_source_term, true);
      }
                         ...
                         double const      evaluation_time)
      {
        ...
        matrix_free->cell_loop(&This::compute_source_term,
                               this,
                               dst,
    ...
    private:
      ...
      {
        CellIntegratorScalar dpdt(matrix_free_in,
                                  data.dof_index_pressure,
                                  data.quad_index);
        CellIntegratorScalar p(matrix_free_in,
                               data.dof_index_pressure,
                               data.quad_index);
        CellIntegratorVector u(matrix_free_in,
        ...
        for (unsigned int cell = cell_range.first; cell < cell_range.second;
             ++cell)
          {
            ...
            if (data.consider_convection)
              {
                p.reinit(cell);
                p.gather_evaluate(*pressure_cfd,
                                  dealii::EvaluationFlags::gradients);
                u.reinit(cell);
                u.gather_evaluate(*velocity_cfd,
                                  dealii::EvaluationFlags::values);

                for (unsigned int q = 0; q < dpdt.n_q_points; ++q)
                  {
                    scalar flux = -rho * dpdt.get_value(q) +
                                  u.get_value(q) * p.get_gradient(q);
        ...
        for (unsigned int cell = cell_range.first; cell < cell_range.second;
             ++cell)
          {
            ...
            for (unsigned int q = 0; q < dpdt.n_q_points; ++q)
              {
                scalar flux =
                  -rho * FunctionEvaluator<0, dim, Number>::value(
                           analytical_source_term, dpdt.quadrature_point(q));
      ...
      {
        // In case we blend in the source term, we check if the scaling is space
        // dependent. Only in that case we have to evaluate the function in
        // every equadrature point. Otherwise the scaling is purely temporal and
        // constant during this function.
        if (data.blend_in)
          {
        ...
        bool const space_dependent_scaling =
          data.blend_in_function != nullptr ?
            data.blend_in_function->varies_in_space(time) :
            false;
        Number const pure_temporal_scaling_factor =
          (not space_dependent_scaling) ?
            data.blend_in_function->compute_time_factor(time) :
            1.0;
