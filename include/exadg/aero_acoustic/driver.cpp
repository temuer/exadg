  ...
  namespace AeroAcoustic
  {
    ...
    Driver<dim, Number>::setup()
    {
      ...
      // setup acoustic solver
      {
        ...
        timer_tree.insert({"AeroAcoustic", "Setup", "Acoustic"},
      ...
      // setup fluid solver
      {
        ...
        timer_tree.insert({"AeroAcoustic", "Setup", "Fluid"},
                          timer_local.wall_time());
      }
    ...
    Driver<dim, Number>::setup_volume_coupling()
    {
      // fluid to structure
      {
        ...
        pcout << std::endl
              << "Setup volume coupling fluid -> acoustic ..." << std::endl;
        ...
        timer_tree.insert({"AeroAcoustic",
                           "Setup",
                           "Coupling fluid -> acoustic"},
                          timer_local.wall_time());
      }
    }
    ...
    {
      AssertThrow(
        fluid->time_integrator->get_time() - 1e-12 <
          acoustic->time_integrator->get_time(),
        dealii::ExcMessage(
    ...
    Driver<dim, Number>::couple_fluid_to_acoustic()
    {
      ...
      timer_tree.insert({"AeroAcoustic", "Coupling fluid -> acoustic"},
                        sub_timer.wall_time());
    }
    ...
    {
      std::pair<bool, dealii::Timer> timer =
        std::make_pair(false, dealii::Timer());

      set_start_time();

      AssertThrow(std::abs(application->fluid->get_parameters().end_time -
                           application->acoustic->get_parameters().end_time) <
                    1.0e-12,
                  dealii::ExcMessage(
                    "Acoustic and fluid simulation need the same end time."));
      ...
      while (not fluid->time_integrator->finished())
        {
          ...
          // To check if acoustics starts during the following sub-stepping
          // sweep we can not simply check acoustic->time_integrator->started().
          // Instead we compute this information as follows:
          bool const acoustic_starts_during_present_timestep =
            fluid->time_integrator->get_next_time() +
              fluid->time_integrator->get_time_step_size() >
            application->acoustic->get_parameters().start_time;

          // The acoustic simulation uses explicit time-stepping while the fluid
          // solver uses implicit time-stepping. Therefore, we advance the
          // acoustic solver to t^(n+1) first and directly use the result in the
          // fluid solver.
          if (acoustic_starts_during_present_timestep)
            couple_fluid_to_acoustic();
          acoustic->advance_multiple_timesteps(
            fluid->time_integrator->get_time_step_size());

          // We can not simply check acoustic->time_integrator->started() since
          // acoustic might start during a sub-stepping sweep. To check if the
          // acoustic might be started during the next sub-stepping BEFORE
          // performing the current fluid time step we have to check if dt_{n+2}
          // is larger than the acoustic start time. We need this information
          // BEFORE the fluid time-step since we have to know if we have to
          // compute dp/dt.
          bool const acoustic_might_start_during_next_timestep =
            fluid->time_integrator->get_next_time() +
              fluid->max_next_time_step_size() >
            application->acoustic->get_parameters().start_time;
      ...
      double const total_time) const
    {
      ...
      pcout << std::endl
            << "Average number of sub-time steps Acoustic:" << std::endl;
      pcout << "Adams-Bashforth-Moulton    "
            << acoustic->get_average_number_of_sub_time_steps() << std::endl;
      ...
      timer_tree.insert({"AeroAcoustic"},
                        ...
                        "Fluid");
      timer_tree.insert({"AeroAcoustic"},
                        acoustic->time_integrator->get_timings(),
                        "Acoustic");
      ...
      dealii::Utilities::MPI::MinMaxAvg time_solvers_side_by_side_data =
        dealii::Utilities::MPI::min_max_avg(time_solvers_side_by_side,
                                            mpi_comm);
      double const time_solvers_side_by_side_avg =
        time_solvers_side_by_side_data.avg;


      dealii::types::global_dof_index const DoFs_f =
        fluid->pde_operator->get_number_of_dofs();
      dealii::types::global_dof_index const DoFs_a =
        acoustic->pde_operator->get_number_of_dofs();
      double const sub_dt_per_macro_dt =
        acoustic->get_average_number_of_sub_time_steps();
      ...
      print_throughput_unsteady(pcout,
                                (double)DoFs_f / sub_dt_per_macro_dt +
                                  (double)DoFs_a,
                                time_solvers_side_by_side_avg,
