...
namespace ExaDG
{
  ...
  create_input_file(std::string const &input_file)
  {
    ...
    auto app =
      AeroAcoustic::get_application<Dim, Number>(input_file, MPI_COMM_WORLD);
      ...
      bool const         is_test)
  {
    ...
    auto application =
      AeroAcoustic::get_application<dim, Number>(input_file, mpi_comm);

    auto driver =
      std::make_shared<AeroAcoustic::Driver<dim, Number>>(mpi_comm,
                                                          application,
                                                          is_test);
