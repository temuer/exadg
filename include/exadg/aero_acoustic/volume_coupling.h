  ...
  namespace AeroAcoustic
  {
    ...
    public:
            ...
            std::shared_ptr<FieldFunctions<dim>>         field_functions_in)
      {
        ...
        acoustic_solver_in->pde_operator->initialize_dof_vector_pressure(
          source_term_acoustic);
        fluid_solver_in->pde_operator->initialize_vector_pressure(
          source_term_fluid);
        ...
        if (parameters.fluid_to_acoustic_coupling_strategy ==
          ...
          {
            non_nested_grid_transfer.reinit(
              fluid_solver_in->pde_operator->get_dof_handler_p(),
        ...
        data.dof_index_pressure =
          fluid_solver_in->pde_operator->get_dof_index_pressure();
        data.dof_index_velocity =
          fluid_solver_in->pde_operator->get_dof_index_velocity();
        data.quad_index =
          fluid_solver_in->pde_operator->get_quad_index_pressure();
        ...
        source_term_calculator.setup(
          fluid_solver_in->pde_operator->get_matrix_free(), data);
      }
      ...
      fluid_to_acoustic()
      {
        if (parameters.fluid_to_acoustic_coupling_strategy ==
            FluidToAcousticCouplingStrategy::ConservativeInterpolation)
          {
            ...
            non_nested_grid_transfer.restrict_and_add(source_term_acoustic,
                                                      source_term_fluid);
          }
        else
          {
        ...
        acoustic_solver->pde_operator->set_aero_acoustic_source_term(
          source_term_acoustic);
      }
    ...
    private:
      ...
      // Transfer operator
      dealii::MGTwoLevelTransferNonNested<dim, VectorType>
        non_nested_grid_transfer;
