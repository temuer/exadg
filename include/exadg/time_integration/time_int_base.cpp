...
namespace ExaDG
{
  ...
  {
    return not(time < (end_time - eps) and
  ...
  TimeIntBase::reset_time(double const &current_time)
  {
    ...
    // (which is needed when coupling different solvers, different domains,
    // etc.).
    if (current_time <= start_time + eps)
      ...
      AssertThrow(
        false,
        dealii::ExcMessage(
          "The variable time may not be overwritten via public access."));
  }
  ...
  TimeIntBase::prepare_coarsening_and_refinement()
  {
    AssertThrow(false,
                dealii::ExcMessage(
                  "Overwrite in derived class to enable adaptivity."));
  }
  ...
  TimeIntBase::write_restart() const
  {
    ...
    if (restart_data.do_restart(wall_time,
                                time - start_time,
                                time_step_number,
                                time_step_number == 2))
      {
        pcout << std::endl
              ...
              << std::endl
              << " Writing restart file at time t = " << this->get_time() << ":"
              << std::endl;

        std::string const filename =
          restart_filename(restart_data.filename, mpi_comm);
  ...
  TimeIntBase::read_restart()
  {
    ...
    AssertThrow(in,
                dealii::ExcMessage("File " + filename + " does not exist."));
  ...
  TimeIntBase::output_solver_info_header() const
  {
    pcout << std::endl
          << print_horizontal_line() << std::endl
          << std::endl
          << " Time step number = " << std::left << std::setw(8)
          << time_step_number << "t = " << std::scientific
  ...
  /*
   *  This function estimates the remaining wall time based on the overall time
   * interval to be simulated and the measured wall time already needed to
   * simulate from the start time until the current time.
   */
  void
    ...
    if (not(this->is_test))
      {
        if (time > start_time)
          {
            double const remaining_time = global_timer.wall_time() *
                                          (end_time - time) /
                                          (time - start_time);
            ...
            int const seconds =
              int((remaining_time - hours * 3600.0 - minutes * 60.0));

            pcout << std::endl
                  << "Estimated time until completion is " << hours << " h "
                  << minutes << " min " << seconds << " s." << std::endl;
          }
      }
  }
