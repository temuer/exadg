...
namespace ExaDG
{
  TimeIntMultistepBase::TimeIntMultistepBase(
  ...
  {
    this->pcout << std::endl
                << "Setup Multistep time integrator ..." << std::endl
                << std::flush;
    ...
    if (do_restart)
      {
        if (order > 1 and start_with_low_order == true)
          {
            this->pcout
              << "WARNING A higher-order time integration scheme is used, "
              << std::endl
        ...
        // The solution vectors and the current time, the time step size, etc.
        // have to be read from restart files.
        read_restart();
      }
    else
      {
        ...
        // The time step size has to be computed before the solution can be
        // initialized at times t - dt[1], t - dt[1] - dt[2], etc.
        time_steps[0] = calculate_time_step_size();

        // initialize time_steps array as a preparation for
        // initialize_former_multistep_dof_vectors()
        for (unsigned int i = 1; i < order; ++i)
          time_steps[i] = time_steps[0];

        // Finally, prescribe initial conditions at former instants of time.
        // This is only necessary if the time integrator starts with a
        // high-order scheme in the first time step.
  ...
  TimeIntMultistepBase::get_time_step_size(int const i /* dt[i] */) const
  {
     ...
     *  time_steps-vec:   dt[2]     dt[1]    dt[0]
     */
    AssertThrow(i >= 0 and i <= int(order) - 1,
                dealii::ExcMessage("Invalid access."));

    AssertThrow(time_steps[i] > 0.0,
                dealii::ExcMessage("Invalid or uninitialized time step size."));
  ...
  {
    // constant time step sizes: allow setting of time step size only in the
    // first time step or after a restart (where time_step_number is also 1),
    // e.g., to continue will smaller time step sizes
    if (adaptive_time_stepping == false)
      {
        AssertThrow(
          time_step_number == 1,
          dealii::ExcMessage(
            "For time integration with constant time step sizes this "
  ...
  TimeIntMultistepBase::do_read_restart(std::ifstream &in)
  {
    ...
    // In order to change the CFL number (or the time step calculation criterion
    // in general), start_with_low_order = true has to be used. Otherwise, the
    // old solutions would not fit the time step increments.
  ...
  TimeIntMultistepBase::read_restart_preamble(BoostInputArchiveType &ia)
  {
    ...
    AssertThrow(n_old_ranks == n_ranks,
                dealii::ExcMessage("Tried to restart with " +
                                   dealii::Utilities::to_string(n_ranks) +
                                   " processes, "
                                   "but restart was written on " +
                                   dealii::Utilities::to_string(n_old_ranks) +
                                   " processes."));

    // 2. time
    ia &time;

    // Note that start_time has to be set to the new start_time (since
    // param.start_time might still be the original start time).
    ...
    AssertThrow(old_order == order,
                dealii::ExcMessage("Order of time integrator may not change."));
