...
namespace ExaDG
{
  AMTimeIntegratorConstants::AMTimeIntegratorConstants(
    ...
    , alpha(order - 1)
  {
    AssertThrow(order >= 1 and order <= 4,
                dealii::ExcMessage(
                  "Specified order of Adams-Moulton scheme not implemented."));
  ...
  {
    AssertThrow(gamma0 > 0.0,
                dealii::ExcMessage(
                  "Constant gamma0 has not been initialized."));
  ...
  void
  AMTimeIntegratorConstants::set_constant_time_step(
  ...
  void
  AMTimeIntegratorConstants::set_adaptive_time_step(
    unsigned int const         current_order,
    ...
    switch (current_order)
      {
        ...
        case 3:
          {
            gamma0 = (1.0 / 3.0 * time_steps[0] + 1.0 / 2.0 * time_steps[1]) /
                     (time_steps[0] + time_steps[1]);
            alpha[0] = 1.0 - (1.0 / 3.0 * time_steps[0] * time_steps[0] +
                              1.0 / 2.0 * time_steps[0] *
                                (time_steps[1] - time_steps[0])) /
                               (time_steps[0] * time_steps[1]);
            alpha[1] = (time_steps[0] * time_steps[0]) /
                       (-6.0 * time_steps[1] * (time_steps[0] + time_steps[1]));
            break;
          }
        case 4:
          {
            ...
            alpha[1] =
              (1.0 / 4.0 * time_steps[0] * time_steps[0] * time_steps[0] +
               1.0 / 3.0 * time_steps[0] * time_steps[0] *
                 (time_steps[1] + time_steps[2] - time_steps[0]) -
               1.0 / 2.0 * time_steps[0] * time_steps[0] *
                 (time_steps[1] + time_steps[2])) /
