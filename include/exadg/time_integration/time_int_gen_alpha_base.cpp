...
namespace ExaDG
{
  ...
  TimeIntGenAlphaBase<Number>::TimeIntGenAlphaBase(
  ...
  template <typename Number>
  void
  TimeIntGenAlphaBase<Number>::set_current_time_step_size(
  ...
  {
    return ((1.0 - alpha_m) /
            ((1.0 - alpha_f) * beta * std::pow(time_step, 2.0)));
  }
    ...
    VectorType const &acceleration_n) const
  {
    ...
    double const factor_acc =
      -(1.0 - alpha_f) * (gamma - 2.0 * beta) / (2.0 * beta) * time_step;
  ...
  template <typename Number>
  void
  TimeIntGenAlphaBase<Number>::update_displacement(
    VectorType       &displacement_np,
  ...
  template <typename Number>
  void
  TimeIntGenAlphaBase<Number>::update_velocity(
    ...
    VectorType const &acceleration_n) const
  {
    ...
    velocity_np.add(-(gamma - 2.0 * beta) * time_step / (2.0 * beta),
                    acceleration_n);
  }

  template <typename Number>
  void
  TimeIntGenAlphaBase<Number>::update_acceleration(
    VectorType       &acceleration_np,
    VectorType const &displacement_np,
    VectorType const &displacement_n,
    VectorType const &velocity_n,
    VectorType const &acceleration_n) const
  {
    acceleration_np.equ(1.0 / (beta * std::pow(time_step, 2.0)),
                        displacement_np);
    acceleration_np.add(-1.0 / (beta * std::pow(time_step, 2.0)),
                        displacement_n);
