...
namespace ExaDG
{
  ...
  {
    std::string const rank = dealii::Utilities::int_to_string(
  ...
  {
    // backup: rename current restart file into restart.old in case something
    // fails while writing
    ...
    if ((bool)ifile) // rename only if file already exists
      {
        ...
        AssertThrow(error == 0,
                    dealii::ExcMessage("Can not rename file: " + from + " -> " +
                                       to));
      }
  }
  ...
  print_vector_l2_norm(VectorType const &vector)
  {
    ...
    if (dealii::Utilities::MPI::this_mpi_process(mpi_comm) == 0)
      {
        std::cout << "    vector global l2 norm: " << std::scientific
        ...
        for (unsigned int i = 0; i < vector.locally_owned_size(); ++i)
          {
            archive &vector.local_element(i);
                                    ...
                                    dealii::LinearAlgebra::distributed::
                                      BlockVector<Number>>::value)
      {
        for (unsigned int i = 0; i < vector.n_blocks(); ++i)
          {
            for (unsigned int j = 0; j < vector.block(i).locally_owned_size();
                 ++j)
              {
