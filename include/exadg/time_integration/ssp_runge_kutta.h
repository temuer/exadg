...
namespace ExaDG
{
   ...
   *
   *    Kubatko et al., Optimal Strong-Stability-Preserving Runge-Kutta Time
   * Discretizations for Discontinuous Galerkin Methods, J Sci Comput (2014)
   * 60:313-344.
   *
   *    The Runge-Kutta scheme is implemented in Shu-Osher form instead of the
   * Butcher form.
   *
   */
                                              ...
                                              double const time_step)
  {
    ...
    if (u_vec.empty() or not(u_vec[0].partitioners_are_globally_compatible(
    ...
    for (unsigned int s = 1; s <= stages; ++s)
      {
        this->underlying_operator->evaluate(F_vec[s - 1],
                                            u_vec[s - 1],
                                            time + c[s - 1] * time_step);
