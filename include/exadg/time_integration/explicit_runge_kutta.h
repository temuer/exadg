...
namespace ExaDG
{
  ...
  public:
    // Constructor
    ExplicitRungeKuttaTimeIntegrator(
                   ...
                   double const time_step) final
    {
      if (order == 1) // explicit Euler method
      ...
      else if (order == 2) // Runge-Kutta method of order 2
        {
          ...
          this->underlying_operator->evaluate(dst,
                                              vec_rhs,
                                              time + time_step / 2.);
          dst.sadd(time_step, 1., src);
        }
      else if (order == 3) // Heun's method of order 3
        {
          ...
          this->underlying_operator->evaluate(vec_temp,
                                              vec_rhs,
                                              time + time_step / 3.);
   ...
   *                                                                                      *
   *      X = C: linear-stability-error compromise * X = S: maximum linear
   *stability                                                 * X = F: FSAL -
   *first same as last                                                * X = M:
   *minimum truncation error                                                 *
   *      X = N: maximum nonlinear stability * X = P: minimum phase error *
   *                                                                                      *
   *  For applications where the time step is selected according to stability *
   *  considerations (time step size close to CFL limit, temporal error small
   *enough),    * schemes of type C or type S should be used. *
   *                                                                                      *
   ****************************************************************************************/

  /*
   *  Low storage Runge-Kutta method of order 3 with 4 stages and 2 registers
   * according to Kennedy et al. (2000), where this method is denoted as
   * RK3(2)4[2R+]C, see Table 1 on page 189 for the coefficients.
   */
  template <typename Operator, typename VectorType>
  ...
  public:
    ...
    solve_timestep(VectorType  &vec_np,
                   ...
                   double const time_step) final
    {
      if (not vec_tmp1.partitioners_are_globally_compatible(
            *vec_n.get_partitioner()))
        {
      ...
      // stage 1
      this->underlying_operator->evaluate(vec_tmp1,
                                          vec_n /* u_1 */,
                                          time + c1 * time_step);
  ...
  /*
   *  Low storage Runge-Kutta method of order 4 with 5 stages and 2 registers
   * according to Kennedy et al. (2000), where this method is denoted as
   * RK4(3)5[2R+]C, see Table 1 on page 189 for the coefficients.
   */
  template <typename Operator, typename VectorType>
  ...
  /*
   *  Low storage Runge-Kutta method of order 4 with 5 stages and 3 registers
   * according to Kennedy et al. (2000), where this method is denoted as
   * RK4(3)5[3R+]C, see Table 2 on page 190 for the coefficients.
   */
  template <typename Operator, typename VectorType>
  ...
  public:
                   ...
                   double const time_step) final
    {
      ...
      // stage 1
      this->underlying_operator->evaluate(vec_np /* F_1 */,
                                          vec_n /* u_1 */,
                                          time + c1 * time_step);
      ...
      // stage 2
      this->underlying_operator->evaluate(vec_tmp2 /* F_2 */,
                                          vec_n /* u_2 */,
                                          time + c2 * time_step);
      ...
      // stage 3
      this->underlying_operator->evaluate(vec_n /* F_3 */,
                                          vec_tmp1 /* u_3 */,
                                          time + c3 * time_step);
  ...
  /*
   *  Low storage Runge-Kutta method of order 5 with 9 stages and 2 registers
   * according to Kennedy et al. (2000), where this method is denoted as
   * RK5(4)9[2R+]S, see Table 1 on page 189 for the coefficients.
   */
  template <typename Operator, typename VectorType>
  ...
  /*
   *  Explicit Runge-Kutta of Toulorge & Desmet (2011) in low-storage format (2N
   * scheme) of order q with additional stages s>q in order to optimize the
   * stability region of the time integration scheme and to optimize costs =
   * stages / CFL_crit:
   *
   *  We consider two methods of order 3 and 4 with 7 and 8 stages,
   * respectively. The Runge-Kutta coefficients for these schemes can be found
   * in Toulorge & Desmet in Tables A.15 for the RKC73 scheme and Table A.21 for
   * the RKC84 scheme. In our nomenclature, these time integration schemes are
   * deonted as ExplRK3Stage7Reg2 and ExplRK4Stage8Reg2, respectively.
   */
  template <typename Operator, typename VectorType>
  ...
  public:
    ...
    solve_timestep(VectorType  &vec_np,
                   ...
                   double const time_step) final
    {
      if (not vec_tmp.partitioners_are_globally_compatible(
            *vec_n.get_partitioner()))
        {
      ...
      for (unsigned int s = 0; s < stages; s++)
        {
          this->underlying_operator->evaluate(vec_np,
                                              vec_n,
                                              time + c[s] * time_step);
