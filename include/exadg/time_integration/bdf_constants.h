...
{
  /**
   * Class that manages Backward Differentiation Formula time integrator
   * constants.
   */
  class BDFTimeIntegratorConstants : public TimeIntegratorConstantsBase
  {
  public:
    BDFTimeIntegratorConstants(unsigned int const order,
  ...
  private:
     ...
     *
     *  du/dt = (gamma_0 u^{n+1} - alpha_0 u^{n} - alpha_1 u^{n-1} ... -
     * alpha_{J-1} u^{n-J+1})/dt
     */
    double gamma0;
   ...
   *
   *  derivative = du/dt = (gamma_0 u^{n+1} - alpha_0 u^{n} - alpha_1 u^{n-1}
   * ... - alpha_{J-1} u^{n-J+1})/dt
   */
    ...
    double const                          &time_step_size)
  {
    ...
    for (unsigned int i = 0; i < previous_solutions.size(); ++i)
      derivative.add(-bdf.get_alpha(i) / time_step_size,
  ...
  compute_bdf_time_derivative(VectorType                    &derivative,
                              ...
                              double const                     &time_step_size)
  {
    std::vector<VectorType const *> previous_solutions_ptrs(
    ...
    std::vector<double> const             &times_np)
  {
    ...
    std::vector<VectorType const *> previous_solutions(
      previous_solutions_np.begin() + 1, previous_solutions_np.end());
    ...
    compute_bdf_time_derivative(derivative,
                                *previous_solutions_np[0],
                                previous_solutions,
                                bdf,
                                time_steps[0]);
  }
