...
namespace ExaDG
{
  namespace Categorization
  {
     ...
     *   1) cells which have the same boundary IDs for all faces are put into
     * the same category 2) cell based loops are enabled (incl.
     * dealii::FEEvaluationBase::read_cell_data() for all neighboring cells)
     */
      ...
      unsigned int const level = dealii::numbers::invalid_unsigned_int)
    {
      ...
      if (is_mg)
        data.cell_vectorization_category.resize(
...

      {
        ...
        int          offset = 1;
        for (unsigned int i = 0; i < n_faces_per_cell;
             i++, offset = offset * bids)
          factors[i] = offset;
      }
