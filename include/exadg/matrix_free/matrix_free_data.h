...
namespace ExaDG
{
  ...
  public:
    ...
    /**
     * Append MatrixFreeData by the needs of (another) pde_operator provided as
    ...
    void
    insert_dof_handler(dealii::DoFHandler<dim> const *dof_handler,
    ...
    void
    insert_constraint(dealii::AffineConstraints<Number> const *constraint,
                      std::string const                       &name)
    {
    ...
    void
    insert_quadrature(dealii::Quadrature<dim_quad> const &quadrature,
                      std::string const                  &name)
    {
      insert_element(quadrature_vec,
                     quad_index_map,
  ...
  private:
      ...
      if (it == map.end())
      ...
      else
        {
          AssertThrow(it == map.end(),
                      dealii::ExcMessage("Element already exists. Aborting."));
        }
    ...
    unsigned int
    get_index(std::map<std::string, unsigned int> const &map,
              std::string const                         &name) const
    {
      ...
      if (it != map.end())
        ...
        {
          AssertThrow(it != map.end(),
                      dealii::ExcMessage("Could not find element. Aborting."));
        }

      return index;
    }

    // maps between names and indices for dealii::DoFHandler, Constraint,
    // dealii::Quadrature
    ...
    // collection of data structures required for initialization and update of
    // matrix_free
