...
{
  /*
   * This function calculates the time step such that the spatial and temporal
   ...
   *
   * where h is a mesh size defined globally for the whole mesh, e.g. the global
  ...
  {
    double const time_step = pow(global_min_cell_diameter, 2.0) /
                             pow(fe_degree, exponent_fe_degree) / diffusivity;

    return time_step;
  }

  /*
   * This function calculates the maximum velocity for a given velocity field
   * (which is known analytically). The maximum value is defined as the maximum
  ...
  /*
   * Calculate time step size according to local CFL criterion where the
   * velocity field is a prescribed analytical function. The computed time step
   * size corresponds to CFL = 1.0.
   *
    ...
    for (unsigned int cell = 0; cell < data.n_cell_batches(); ++cell)
      {
        dealii::VectorizedArray<value_type> delta_t_cell =
          dealii::make_vectorized_array<value_type>(
        ...
        for (unsigned int q = 0; q < fe_eval.n_q_points; ++q)
          {
            dealii::Point<dim, dealii::VectorizedArray<value_type>> q_point =
              fe_eval.quadrature_point(q);

            dealii::Tensor<1, dim, dealii::VectorizedArray<value_type>> u_x =
              FunctionEvaluator<1, dim, value_type>::value(*velocity,
                                                           q_point,
                                                           time);
            ...
            if (cfl_condition_type == CFLConditionType::VelocityNorm)
            ...
            else if (cfl_condition_type == CFLConditionType::VelocityComponents)
              {
                for (unsigned int d = 0; d < dim; ++d)
                  delta_t_cell =
                    std::min(delta_t_cell, cfl_p / (std::abs(ut_xi[d])));
              }
            else
              {
        ...
        for (unsigned int v = 0;
             v < dealii::VectorizedArray<value_type>::size();
             ++v)
          {
  ...
  /*
   * Calculate time step size according to local CFL criterion where the
   * velocity field is a numerical solution field. The computed time step size
   * corresponds to CFL = 1.0.
   */
  template <int dim, typename value_type>
  inline double
  calculate_time_step_cfl_local(
    ...
    MPI_Comm const        &mpi_comm)
  {
    ...
    // Cut time step size after, e.g., 4 digits of accuracy in order to make
    // sure that there is no drift in the time step size depending on the number
    // of processors when using adaptive time stepping. This effect can occur
    // since the velocity field and the time step size are coupled (there is
    // some form of feedback loop in case of adaptive time stepping, i.e., a
    // minor change in the time step size due to round-off errors implies that
    // the velocity field is evaluated at a slightly different time in the next
    // time step and so on). This way, it can be ensured that the sequence of
    // time step sizes is exactly reproducible with the results being
    // independent of the number of processors, which is important for code
    // verification in a parallel setting.
  ...
  /*
   * this function computes the actual CFL number in each cell given a global
   * time step size (that holds for all cells)
   */

  template <int dim, typename value_type>
  void
  calculate_cfl(
    dealii::LinearAlgebra::distributed::Vector<value_type>       &cfl,
