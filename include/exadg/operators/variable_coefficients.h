...
{
  /**
   * This class serves as a manager of several table objects, which hold
   * quadrature-point-level coefficient information for the different
   * matrix-free loop types and access methods.
   *
   * The class provides data structures for cell loops, separate face loops, and
   * cell-based face loops. It is responsible solely for the storage of
   * coefficients. No generic way of computing the coefficients is provided
   * here. This task is left to the owner of a `VariableCoefficients` object.
   *
  ...
  public:
     ...
     * i.e. resizes the desired tables to the required number of cells, faces,
     ...
     * face access. This is an additional and optional way of accessing the face
     * coefficients and
     ...
     */
    coefficient_type
    get_coefficient_face_cell_based(unsigned int const cell_based_face,
                                    unsigned int const q) const
    {
  ...
  private:
    /**
     * Reinitializes the tables in use. This resizes the tables and initializes
    ...
    reinit(dealii::MatrixFree<dim, Number> const &matrix_free,
           unsigned int const                     quad_index)
    {
      coefficients_cell.reinit(matrix_free.n_cell_batches(),
      ...
      if (store_face_data)
        {
          ...
          coefficients_face_neighbor.reinit(matrix_free.n_inner_face_batches(),
                                            matrix_free.get_n_q_points_face(
                                              quad_index));
          ...
          if (store_cell_based_face_data)
            {
              ...
              coefficients_face_cell_based.reinit(
                matrix_free.n_cell_batches() * n_faces_per_cell,
                matrix_free.get_n_q_points_face(quad_index));
            }
        }
    }
     ...
     *
     * This is an additional/optional data structure which can be used when
     * cell-based face access is required alongside the separate face access.
     */
