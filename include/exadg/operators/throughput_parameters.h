...
{
  inline double
  measure_operator_evaluation_time(
    std::function<void(void)> const &evaluate_operator,
        ...
        for (unsigned int i_outer = 0; i_outer < n_repetitions_outer; ++i_outer)
          {
            ...
            wall_time =
              std::min(wall_time,
                       wall_time_inner.avg / (double)n_repetitions_inner);
          }

        global_time =
          dealii::Utilities::MPI::min_max_avg(global_timer.wall_time(),
                                              mpi_comm);
      }
  ...
  struct ThroughputParameters
  {
    ...
    {
      print_throughput(wall_times,
                       ExaDG::Utilities::enum_to_string(operator_type),
                       mpi_comm);
    }

    // type of PDE operator
    EnumOperatorType operator_type =
      Utilities::default_constructor<EnumOperatorType>();

    // number of repetitions used to determine the average/minimum wall time
    // required to compute the matrix-vector product
    unsigned int n_repetitions_inner =
      100; // take the average of inner repetitions
    unsigned int n_repetitions_outer =
      1; // take the minimum of outer repetitions

    // variable used to store the wall times for different polynomial degrees
    // and problem sizes
    mutable std::vector<
