...
namespace ExaDG
{
  ...
  private:
    ...
    // use a template parameter of -1 to select the precompiled version of this
    // operator
    typedef dealii::MatrixFreeOperators::
      CellwiseInverseMassMatrix<dim, -1, n_components, Number>
  ...
  public:
               ...
               InverseMassOperatorData const inverse_mass_operator_data)
    {
      ...
      // The inverse mass operator is only available for discontinuous Galerkin
      // discretizations
      ...
      if (data.implementation_type == InverseMassType::MatrixfreeOperator)
        {
          // Currently, the inverse mass realized as matrix-free operator
          // evaluation is only available in deal.II for tensor-product
          // elements.
          AssertThrow(
          ...
          // Currently, the inverse mass realized as matrix-free operator
          // evaluation is only available in deal.II if n_q_points_1d =
          // n_nodes_1d.
          AssertThrow(
            this->matrix_free->get_shape_info(0, quad_index)
                .data[0]
                .n_q_points_1d == fe.degree + 1,
            dealii::ExcMessage(
              "The matrix-free inverse mass operator is currently only available if n_q_points_1d = n_nodes_1d."));
        }
      // We create a block-Jacobi preconditioner with MassOperator as underlying
      // operator in case the inverse mass can not be realized as a matrix-free
      // operator.
      else if (data.implementation_type ==
                 InverseMassType::ElementwiseKrylovSolver or
               data.implementation_type == InverseMassType::BlockMatrices)
        {
          ...
          if (data.implementation_type ==
              InverseMassType::ElementwiseKrylovSolver)
            {
              mass_operator_data
                .implement_block_diagonal_preconditioner_matrix_free = true;
              mass_operator_data.solver_block_diagonal =
                Elementwise::Solver::CG;
              if (inverse_mass_operator_data.parameters.preconditioner ==
                  PreconditionerMass::None)
                {
                  mass_operator_data.preconditioner_block_diagonal =
                    Elementwise::Preconditioner::None;
                }
          ...
          block_jacobi_preconditioner = std::make_shared<
            BlockJacobiPreconditioner<MassOperator<dim, n_components, Number>>>(
    ...
    /**
     * Updates the inverse mass operator. This function recomputes the
     * diagonal/block-diagonal in case the geometry has changed (e.g. the mesh
     * has been deformed).
     */
    void
    update()
    {
      if (data.implementation_type == InverseMassType::MatrixfreeOperator)
        {
          // no updates needed as long as the MatrixFree object is up-to-date
          // (which is not the responsibility of the present class).
        }
      else if (data.implementation_type ==
                 InverseMassType::ElementwiseKrylovSolver or
               data.implementation_type == InverseMassType::BlockMatrices)
        {
          // the mass operator does not need to be updated as long as the
          // MatrixFree object is up-to-date (which is not the responsibility of
          // the present class).

          // update the matrix-based components of the block-Jacobi
          // preconditioner
      ...
      if (data.implementation_type == InverseMassType::MatrixfreeOperator)
        {
          ...
          matrix_free->cell_loop(
            ...
            dst,
            src,
            /*operation before cell operation*/ {}, /*operation after cell
                                                       operation*/
            [&](unsigned int const start_range, unsigned int const end_range) {
  ...
  private:
    ...
    // This variable is only relevant if the inverse mass can not be realized as
    // a matrix-free operator. Since this class allows only L2-conforming spaces
    // (discontinuous Galerkin method), the mass matrix is block-diagonal and a
    // block-Jacobi preconditioner inverts the mass operator exactly (up to
    // solver tolerances). The implementation of the block-Jacobi preconditioner
    // can be matrix-based or matrix-free, depending on the parameters
    // specified.
    std::shared_ptr<
      BlockJacobiPreconditioner<MassOperator<dim, n_components, Number>>>
      block_jacobi_preconditioner;

    // In case we realize the inverse mass as block-Jacobi preconditioner, we
    // need a MassOperator as underlying operator for the block-Jacobi
    // preconditioner.
   ...
   *
   * This class applies the inverse mass operator by solving the mass system as
   * a global linear system of equations for all degrees of freedom. It is used
   * in case the mass operator is not block-diagonal and can not be inverted
   * element-wise (e.g. H(div)-conforming space).
   */
  template <int dim, int n_components, typename Number>
  ...
  public:
               ...
               InverseMassOperatorDataHdiv const inverse_mass_operator_data)
    {
      ...
      solver_data.max_iter =
        inverse_mass_operator_data.parameters.solver_data.max_iter;
      ...
      if (inverse_mass_operator_data.parameters.preconditioner ==
               ...
               PreconditionerMass::PointJacobi)
        {
          preconditioner = std::make_shared<
            JacobiPreconditioner<MassOperator<dim, n_components, Number>>>(
            mass_operator, true /* initialize_preconditioner */);
      ...
      solver = std::make_shared<
        Krylov::SolverCG<MassOperator<dim, n_components, Number>,
    ...
    /**
     * This function applies the inverse mass operator. Note that this function
     * allows identical dst, src vector, i.e. the function can be called like
     * apply(dst, dst).
     */
    unsigned int
    apply(VectorType &dst, VectorType const &src) const
    {
      ...
      // Note that the inverse mass operator might be called like
      // inverse_mass.apply(dst, dst), i.e. with identical destination and
      // source vectors. In this case, we need to make sure that the result is
      // still correct.
      if (&dst == &src)
        {
          temp = src;
  ...
  private:
    // Solver/preconditioner for mass system solving a global linear system of
    // equations for all degrees of freedom.
