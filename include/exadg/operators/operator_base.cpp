...
namespace ExaDG
{
    ...
    OperatorBaseData const                  &data)
  {
    ...
    // check if DG or CG
    // An approximation can have degrees of freedom on vertices, edges, quads
    // and hexes. A vertex degree of freedom means that the degree of freedom is
    // the same on all cells that are adjacent to this vertex. A face degree of
    // freedom means that the two cells adjacent to that face see the same
    // degree of freedom. A DG element does not share any degrees of freedom
    // over a vertex but has all of them in the last item, i.e., quads in 2D and
    // hexes in 3D, and thus necessarily has dofs_per_vertex=0
    is_dg = (this->matrix_free->get_dof_handler(this->data.dof_index)
               .get_fe()
               .dofs_per_vertex == 0);
    ...
    // The default value is is_mg = false and this variable is set to true in
    // case the operator is applied in multigrid algorithm. By convention, the
    // default argument dealii::numbers::invalid_unsigned_int corresponds to the
    // default value is_mg = false
    ...
    n_mpi_processes =
      dealii::Utilities::MPI::n_mpi_processes(dof_handler.get_communicator());
  }
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::vmult(VectorType       &dst,
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::vmult_add(
    VectorType       &dst,
    VectorType const &src) const
  {
    if (this->data.use_matrix_based_vmult)
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::vmult_interface_down(
    VectorType       &dst,
    VectorType const &src) const
  {
    vmult(dst, src);
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::vmult_add_interface_up(
    VectorType       &dst,
    VectorType const &src) const
  {
    vmult_add(dst, src);
  ...
  template <int dim, typename Number, int n_components>
  Number
  OperatorBase<dim, Number, n_components>::el(unsigned int const,
                                              unsigned int const) const
  {
    AssertThrow(
      false, dealii::ExcMessage("Matrix-free does not allow for entry access"));
    return Number();
  }
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::initialize_dof_vector(
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::set_constrained_dofs_to_zero(
    VectorType &vector) const
  {
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::calculate_inverse_diagonal(
    VectorType &diagonal) const
  {
    ...
    if (false)
      {
        verify_calculation_of_diagonal(*this,
                                       diagonal,
                                       matrix_free
                                         ->get_dof_handler(this->data.dof_index)
                                         .get_communicator());
      }
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::apply(VectorType       &dst,
                                                 VectorType const &src) const
  {
    if (is_dg)
      {
    ...
    else
      {
        ...
        // MatrixFree via AffineConstraints with the standard "dof_index".
        matrix_free->cell_loop(&This::cell_loop, this, dst, src, true);

        // Constrained degree of freedom are not removed from the system of
        // equations. Instead, we set the diagonal entries of the matrix to 1
        // for these constrained degrees of freedom. This means that we simply
        // copy the constrained values to the dst vector.
        for (unsigned int const constrained_index :
             matrix_free->get_constrained_dofs(this->data.dof_index))
          {
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::apply_add(
    VectorType       &dst,
    VectorType const &src) const
  {
    if (is_dg)
    ...
    else
      {
        ...
        // Note that MatrixFree will not touch constrained degrees of freedom in
        // the dst-vector.
        matrix_free->cell_loop(&This::cell_loop, this, dst, src);
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::apply_matrix_based(
    VectorType       &dst,
    VectorType const &src) const
  {
    if (this->data.sparse_matrix_type == SparseMatrixType::Trilinos)
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::apply_matrix_based_add(
    VectorType       &dst,
    VectorType const &src) const
  {
    VectorType tmp;
  ...
  OperatorBase<dim, Number, n_components>::rhs_add(VectorType &rhs) const
  {
    if (is_dg)
        ...
        if (evaluate_face_integrals())
          {
            ...
            // multiply by -1.0 since the boundary face integrals have to be
    ...
    else
      {
        ...
        // Set constrained degrees of freedom according to inhomogeneous
        // Dirichlet boundary conditions.
        //  The rest of the vector remains unchanged.
        set_inhomogeneous_boundary_values(src_tmp);

        // Since src_tmp = 0 apart from inhomogeneous boundary data, the
        // function evaluate_add() only computes the inhomogeneous part of the
        // operator.
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::evaluate(VectorType       &dst,
                                                    VectorType const &src) const
  {
    dst = 0;
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::evaluate_add(
    VectorType       &dst,
    VectorType const &src) const
  {
    if (is_dg)
      ...
      {
        AssertThrow(
          data.dof_index_inhomogeneous != dealii::numbers::invalid_unsigned_int,
          dealii::ExcMessage("dof_index_inhomogeneous is uninitialized."));

        // Perform matrix-vector product using a src-vector which contains
        // inhomogeneous Dirichlet values to obtain the hom+inhom action of the
        // operator.
        if (evaluate_face_integrals())
          {
          ...
          {
            matrix_free->cell_loop(&This::cell_loop_full_operator,
                                   this,
                                   dst,
                                   src);
          }
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::calculate_diagonal(
    VectorType &diagonal) const
  {
    if (diagonal.size() == 0)
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::add_diagonal(
    VectorType &diagonal) const
  {
    // compute diagonal
    if (is_dg and evaluate_face_integrals())
      {
        if (data.use_cell_based_loops)
          {
            matrix_free->cell_loop(&This::cell_based_loop_diagonal,
                                   this,
                                   diagonal,
                                   diagonal);
          }
        ...
        dealii::MatrixFreeTools::compute_diagonal<
          ...
          *matrix_free,
          ...
          [&](auto &integrator) -> void {
            ...
            this->reinit_cell_derived(integrator,
  ...
  {
    AssertThrow(is_dg,
                dealii::ExcMessage("Block Jacobi only implemented for DG!"));

    if (evaluate_face_integrals())
      {
        if (data.use_cell_based_loops)
          {
            matrix_free->cell_loop(&This::cell_based_loop_block_diagonal,
                                   this,
                                   matrices,
                                   matrices);
          }
      ...
      {
        matrix_free->cell_loop(&This::cell_loop_block_diagonal,
                               this,
                               matrices,
                               matrices);
      }
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::apply_inverse_block_diagonal(
    VectorType       &dst,
    VectorType const &src) const
  {
    // matrix-free
        ...
        if (evaluate_face_integrals())
          {
            AssertThrow(
              ...
              dealii::ExcMessage(
                "Cell based face loops have to be activated for matrix-free "
            ...
            AssertThrow(
              ...
              dealii::ExcMessage(
                "Can't do cell based loop over faces for simplices."));
          }

        // Solve elementwise block Jacobi problems iteratively using an
        // elementwise solver vectorized over several elements.
        elementwise_solver->solve(dst, src);
      }
    else // matrix-based
      {
        // Simply apply inverse of block matrices (using the LU factorization
  ...
  OperatorBase<dim, Number, n_components>::
    apply_inverse_block_diagonal_matrix_based(VectorType       &dst,
                                              VectorType const &src) const
  {
    AssertThrow(is_dg,
                dealii::ExcMessage("Block Jacobi only implemented for DG!"));
    ...
    matrix_free->cell_loop(
      &This::cell_loop_apply_inverse_block_diagonal_matrix_based,
      this,
      dst,
      src);
  }
      ...
      bool const initialize) const
  {
    ...
    if (data.preconditioner_block_diagonal == Elementwise::Preconditioner::None)
      {
        ...
        IntegratorCell integrator = IntegratorCell(*this->matrix_free,
                                                   this->data.dof_index,
    ...
    else if (data.preconditioner_block_diagonal ==
             Elementwise::Preconditioner::PointJacobi)
      {
        typedef Elementwise::
          JacobiPreconditioner<dim, n_components, Number, This>
            POINT_JACOBI;

        elementwise_preconditioner =
          std::make_shared<POINT_JACOBI>(get_matrix_free(),
                                         get_dof_index(),
                                         get_quad_index(),
                                         *this,
                                         initialize);
      ...
      {
        typedef Elementwise::
          InverseMassPreconditioner<dim, n_components, Number>
            INVERSE_MASS;

        elementwise_preconditioner =
          std::make_shared<INVERSE_MASS>(get_matrix_free(),
                                         get_dof_index(),
                                         get_quad_index());
      }
    else
    ...
    elementwise_solver = std::make_shared<ELEMENTWISE_SOLVER>(
      ...
      *std::dynamic_pointer_cast<ELEMENTWISE_PRECONDITIONER>(
        elementwise_preconditioner),
  ...
  {
    // allocate memory
    auto dofs = matrix_free->get_shape_info(this->data.dof_index)
    ...
    if (is_dg and evaluate_face_integrals())
      {
        IntegratorFace integrator_m = IntegratorFace(*this->matrix_free,
                                                     ...
                                                     this->data.dof_index,
                                                     this->data.quad_index);
        IntegratorFace integrator_p = IntegratorFace(*this->matrix_free,
                                                     false,
                                                     this->data.dof_index,
                                                     this->data.quad_index);

        // face integrals
        unsigned int const n_faces =
          dealii::ReferenceCells::template get_hypercube<dim>().n_faces();
        for (unsigned int face = 0; face < n_faces; ++face)
          {
            ...
            this->reinit_face_cell_based(
              integrator_m, integrator_p, cell, face, bid);

            for (unsigned int i = 0; i < integrator_m.dofs_per_cell; ++i)
              integrator_m.begin_dof_values()[i] = src[i];

            // no need to read dof values for integrator_p (already initialized
            // with 0)
            ...
            if (bid ==
              ...
              {
                this->do_face_int_integral_cell_based(integrator_m,
                                                      integrator_p);
              }
            else // boundary face
              {
                this->do_boundary_integral(integrator_m,
    ...
    update_block_diagonal_preconditioner() const
  {
    ...
    if (data.implement_block_diagonal_preconditioner_matrix_free)
      {
        // For the matrix-free variant we have to update the elementwise
        // preconditioner.
    ...
    MPI_Comm const                 &mpi_comm) const
  {
    ...
    dealii::IndexSet const &owned_dofs =
      is_mg ? dof_handler.locally_owned_mg_dofs(this->level) :
    ...
    bool const my_rank_is_part_of_subcommunicator =
      sum_of_locally_owned_dofs == owned_dofs.size();
    AssertThrow(
      my_rank_is_part_of_subcommunicator or owned_dofs.n_elements() == 0,
      dealii::ExcMessage(
    ...
    if (is_dg and is_mg)
      dealii::MGTools::make_flux_sparsity_pattern(dof_handler,
                                                  dsp,
                                                  this->level);
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::reinit_cell(
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::reinit_cell_derived(
    IntegratorCell    &integrator,
    unsigned int const cell) const
  {
    (void)integrator;
    ...
    // override this function in derived classes if additional initialization is
    // necessary
  }

  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::reinit_face(
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::reinit_face_derived(
    IntegratorFace    &integrator_m,
    IntegratorFace    &integrator_p,
    unsigned int const face) const
  {
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::reinit_boundary_face(
    IntegratorFace    &integrator_m,
    unsigned int const face) const
  {
    integrator_m.reinit(face);
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::reinit_boundary_face_derived(
    IntegratorFace    &integrator_m,
    unsigned int const face) const
  {
    (void)integrator_m;
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::do_cell_integral(
    IntegratorCell &integrator) const
  {
    ...
    AssertThrow(
      false,
      dealii::ExcMessage(
        "OperatorBase::do_cell_integral() has not been implemented!"));
  }

  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::do_face_integral(
    IntegratorFace &integrator_m,
    ...
    dealii::types::boundary_id const &boundary_id) const
  {
    ...
    AssertThrow(
      false,
      dealii::ExcMessage(
        "OperatorBase::do_boundary_integral() has not been implemented!"));
  }
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::do_face_int_integral(
    IntegratorFace &integrator_m,
    IntegratorFace &integrator_p) const
  {
    (void)integrator_m;
    ...
    AssertThrow(
      false,
      dealii::ExcMessage(
        "OperatorBase::do_face_int_integral() has not been implemented!"));
  }

  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::do_face_ext_integral(
    IntegratorFace &integrator_m,
    IntegratorFace &integrator_p) const
  {
    (void)integrator_m;
    ...
    AssertThrow(
      false,
      dealii::ExcMessage(
        "OperatorBase::do_face_ext_integral() has not been implemented!"));
  }
    ...
    dealii::types::boundary_id const boundary_id) const
  {
    ...
    reinit_face_cell_based_derived(
      integrator_m, integrator_p, cell, face, boundary_id);
  }
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::create_standard_basis(
    unsigned int    j,
    IntegratorCell &integrator) const
  {
    ...
    Range const                           &range) const
  {
    AssertThrow(not is_dg,
                dealii::ExcMessage(
                  "This function should not be called for is_dg = true."));

    IntegratorCell integrator_inhom =
      IntegratorCell(matrix_free,
                     this->data.dof_index_inhomogeneous,
                     this->data.quad_index);
    ...
    for (auto cell = range.first; cell < range.second; ++cell)
      {
        ...
        integrator_inhom.integrate(integrator_flags.cell_integrate,
    ...
    Range const                           &range) const
  {
    IntegratorFace integrator_m = IntegratorFace(matrix_free,
                                                 ...
                                                 this->data.dof_index,
                                                 this->data.quad_index);
    IntegratorFace integrator_p = IntegratorFace(matrix_free,
                                                 false,
                                                 this->data.dof_index,
                                                 this->data.quad_index);
        ...
        for (unsigned int face = range.first; face < range.second; face++)
          {
            ...
            integrator_m.integrate_scatter(integrator_flags.face_integrate,
                                           dst);
          }
      }
    ...
    Range const                           &range) const
  {
    AssertThrow(is_dg,
                dealii::ExcMessage(
                  "OperatorBase::boundary_face_loop_full_operator() "
                ...
                for (unsigned int j = 0; j < dofs_per_cell; ++j)
                  {
                    ...
                    if (bid ==
                        dealii::numbers::internal_face_boundary_id) // internal
                                                                    // face
                      {
                        this->do_face_int_integral_cell_based(integrator_m,
                                                              integrator_p);
                    ...
                    // note: += for accumulation of all contributions of this
                    // (macro) cell
                    //          including: cell-, face-, boundary-stiffness
                    //          matrix
                    local_diag[j] += integrator_m.begin_dof_values()[j];
                  }
              }
    ...
    for (auto cell = range.first; cell < range.second; ++cell)
      {
        ...
        std::fill_n(matrices,
                    ...
                    FullMatrix_(dofs_per_cell, dofs_per_cell));
        ...
        for (unsigned int v = 0; v < n_filled_lanes; v++)
          {
            ...
            if (not is_dg)
              {
                ...
                for (unsigned int j = 0; j < dof_indices.size(); j++)
                  dof_indices[j] =
                    temp[matrix_free.get_shape_info(this->data.dof_index)
            ...
            constraint_double.distribute_local_to_global(matrices[v],
                                                         dof_indices,
                                                         dst);
          }
      }
    ...
    Range const                           &range) const
  {
    ...
    // create two local matrix: first one tested by test functions on element m
    // and ...
    ...
    for (auto face = range.first; face < range.second; ++face)
      {
        ...
        for (unsigned int j = 0; j < dofs_per_cell; ++j)
          {
            // write standard basis into dof values of first
            // dealii::FEFaceEvaluation and clear dof values of second
            // dealii::FEFaceEvaluation
        ...
        for (unsigned int v = 0; v < n_filled_lanes; v++)
          {
            auto const cell_number_m =
              matrix_free.get_face_info(face).cells_interior[v];
            auto const cell_number_p =
              matrix_free.get_face_info(face).cells_exterior[v];

            auto cell_m = matrix_free.get_cell_iterator(cell_number_m /
                                                          vectorization_length,
                                                        cell_number_m %
                                                          vectorization_length);
            auto cell_p = matrix_free.get_cell_iterator(cell_number_p /
                                                          vectorization_length,
                                                        cell_number_p %
                                                          vectorization_length);

            // get position in global matrix
            std::vector<dealii::types::global_dof_index> dof_indices_m(
              dofs_per_cell);
            std::vector<dealii::types::global_dof_index> dof_indices_p(
              dofs_per_cell);
            if (is_mg)
              {
  ...
  void
  OperatorBase<dim, Number, n_components>::
    boundary_face_loop_calculate_system_matrix(
      dealii::MatrixFree<dim, Number> const &matrix_free,
      SparseMatrix                          &dst,
      SparseMatrix const                    &src,
      Range const                           &range) const
        ...
        for (unsigned int v = 0; v < n_filled_lanes; v++)
          {
            ...
            auto cell_v =
              matrix_free.get_cell_iterator(cell_number / vectorization_length,
  ...
  template <int dim, typename Number, int n_components>
  void
  OperatorBase<dim, Number, n_components>::set_constrained_dofs_to_one(
    VectorType &vector) const
  {
  ...
  {
    return (integrator_flags.face_evaluate !=
            dealii::EvaluationFlags::nothing) or
           (integrator_flags.face_integrate !=
  ...
  {
#ifdef DEAL_II_WITH_TRILINOS
    internal_compute_factorized_additive_schwarz_matrices<
    ...
    if (is_dg)
    ...
    else
      {
        ...
        unsigned int const dofs_per_cell =
          matrix_free->get_dofs_per_cell(this->data.dof_index);

        unsigned int const n_cells =
          matrix_free->n_cell_batches() * vectorization_length;

        // assemble a temporary sparse matrix to cut out the blocks
        SparseMatrix                   tmp_matrix;
        dealii::DynamicSparsityPattern dsp;
        internal_init_system_matrix(tmp_matrix,
                                    dsp,
                                    dof_handler.get_communicator());
            ...
            for (unsigned int v = 0; v < n_filled_lanes; ++v)
              {
                ...
                auto &dof_indices =
                  dof_indices_all_cells[cell * vectorization_length + v];
                if (is_mg)
                  cell_v->get_mg_dof_indices(dof_indices);
                else
                  cell_v->get_dof_indices(dof_indices);
            ...
            for (unsigned int v = 0; v < n_filled_lanes; ++v)
              {
                ...
                auto const &lex_to_hier =
                  matrix_free->get_shape_info(this->data.dof_index)
                ...
                for (unsigned int i = 0; i < dofs_per_cell; i++)
                  for (unsigned int j = 0; j < dofs_per_cell; j++)
                    lapack_matrix.set(
                      i,
                      j,
                      overlapped_cell_matrix[lex_to_hier[i]][lex_to_hier[j]]);
    ...
    if (is_dg)
    ...
    else
      {
        ...
        matrix_free->template cell_loop<VectorType, VectorType>(
          [&](auto const &matrix_free,
              auto       &dst,
              auto const &src,
              auto const &cell_range) {
            auto const dofs_per_cell =
              matrix_free.get_dofs_per_cell(this->data.dof_index);

            IntegratorCell integrator = IntegratorCell(matrix_free,
                                                       this->data.dof_index,
                                                       this->data.quad_index);

            dealii::Vector<Number> local_vector(dofs_per_cell);
            dealii::AlignedVector<dealii::VectorizedArray<Number>>
              local_weights_vector(dofs_per_cell);
            for (unsigned int cell = cell_range.first; cell < cell_range.second;
                 ++cell)
              {
                ...
                for (unsigned int v = 0; v < n_filled_lanes; ++v)
                  {
                    // apply symmetric weighting, first before applying the
                    // inverse
                    for (unsigned int i = 0; i < dofs_per_cell; ++i)
                      local_vector[i] = integrator.begin_dof_values()[i][v] *
                                        local_weights_vector[i][v];

                    matrices[cell * vectorization_length + v].solve(
                      local_vector);

                    // and after applying the inverse
                    for (unsigned int i = 0; i < dofs_per_cell; ++i)
                      integrator.begin_dof_values()[i][v] =
                        local_vector[i] * local_weights_vector[i][v];
                  }
