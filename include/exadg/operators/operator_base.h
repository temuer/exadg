...
namespace ExaDG
{
  struct OperatorBaseData
  {
    ...
    // In addition to the "standard" dof_index above, we need a separate dof
    // index to evaluate inhomogeneous boundary data correctly. This
    // "inhomogeneous" dof index corresponds to an AffineConstraints object that
    // only applies periodicity and hanging node constraints.
    unsigned int dof_index_inhomogeneous;

    unsigned int quad_index;

    // this parameter can be used to use sparse matrices for the vmult()
    // operation. The default case is to use a matrix-free implementation, i.e.
    // use_matrix_based_vmult = false.
  ...
  public:
    ...
    /*
     * Returns whether the operator is singular, e.g., the Laplace operator with
     * pure Neumann boundary conditions is singular.
     */
    bool
    ...
    /*
     * For time-dependent problems, the function set_time() needs to be called
     * prior to the present function.
     */
    virtual void
    ...
    /*
     * Initialize block diagonal preconditioner: initialize everything related
     * to block diagonal preconditioner.
     */
    void
    initialize_block_diagonal_preconditioner(bool const initialize) const;

    /*
     * Update block diagonal preconditioner: Recompute block matrices in case of
    ...
    /**
     * This function applies the inverse block diagonal to a src vector and
     * stores the result in the dst vector. This function may be called with
     * identical dst, src vectors.
     */
    void
    ...
    /*
     * Provide near null space basis vectors used e.g. in AMG setup. ExaDG
     * assumes a scalar Laplace operator as default, filling `constant_modes`,
     * which can be overwritten in derived classes if necessary.
     * `constant_modes_values` may alternatively be used to provide non-trivial
     * modes.
     */
    virtual void
    get_constant_modes(
    ...
    /*
     * Evaluate the homogeneous part of an operator. The homogeneous operator is
     * the operator that is obtained for homogeneous boundary conditions. This
     * operation is typically applied in linear iterative solvers (as well as
     * multigrid preconditioners and smoothers). Operations of this type are
     * called apply_...() and vmult_...() as required by deal.II interfaces.
     */
    void
    ...
    /*
     * evaluate inhomogeneous parts of operator related to inhomogeneous
     * boundary face integrals. Operations of this type are called rhs_...()
     * since these functions are called to calculate the vector forming the
     * right-hand side vector of linear systems of equations. Functions of type
     * rhs only make sense for linear operators (but they have e.g. no meaning
     * for linearized operators of nonlinear problems). For this reason, these
     * functions are currently defined 'virtual' to provide the opportunity to
     * override and assert these functions in derived classes.
     *
     * For continuous Galerkin discretizations, this function calls internally
     * the member function set_inhomogeneous_boundary_values(). Hence, prior to
     * calling this function, one needs to call set_time() for a correct
     * evaluation in case of time-dependent problems.
     *
     * This function sets the dst vector to zero, and afterwards calls
     * rhs_add().
     */
    virtual void
    ...
    /*
     * Evaluate the operator including homogeneous and inhomogeneous
     * contributions. The typical use case would be explicit time integration
     * where a splitting into homogeneous and inhomogeneous contributions is not
     * required. Functions of type evaluate only make sense for linear operators
     * (but they have e.g. no meaning for linearized operators of nonlinear
     * problems). For this reason, these functions are currently defined
     * 'virtual' to provide the opportunity to override and assert these
     * functions in derived classes.
     *
     * Unlike the function rhs(), this function does not internally call the
     * function set_inhomogeneous_boundary_values() prior to evaluation. Hence,
     * one needs to explicitly call the function
     * set_inhomogeneous_boundary_values() in case of continuous Galerkin
     * discretizations with inhomogeneous Dirichlet boundary conditions before
    ...
    // This function has to initialize everything related to the block diagonal
    // preconditioner when using the matrix-free variant with elementwise
    ...
    // updates block diagonal preconditioner when using the matrix-free variant
    // with elementwise iterative solvers and matrix-free operator evaluation.
    void
  ...
  protected:
    ...
    /*
     * These methods have to be overwritten by derived classes because these
     * functions are operator-specific and define how the operator looks like.
     */

    // standard integration procedure with separate loops for cell and face
    // integrals
    virtual void
    ...
    // The computation of the diagonal and block-diagonal requires face
    // integrals of type interior (int) and exterior (ext)
    virtual void
    ...
    // This function is currently only needed due to limitations of deal.II
    // which do currently not allow to access neighboring data in case of
    // cell-based face loops. Once this functionality is available, this
    // function should be removed again. Since only special operators need to
    // evaluate neighboring data, this function simply redirects to
    // do_face_int_integral() if this function is not overwritten by a derived
    // class (such as convective terms that require an additional evaluation of
    // velocity fields for example).
    virtual void
  ...
  protected:
    ...
    /*
     * Block Jacobi preconditioner/smoother: matrix-free version with
    ...
    mutable std::shared_ptr<ELEMENTWISE_PRECONDITIONER>
                                                elementwise_preconditioner;
  ...
  private:
     ...
     * Helper functions:
     *
     * The diagonal, block-diagonal, as well as the system matrix (assembled
     * into a sparse matrix) are computed columnwise. This means that column i
     * of the block-matrix is computed by evaluating the operator for a unit
     * vector which takes a value of 1 in row i and is 0 for all other entries.
     */
    void
    ...
    /*
     * This function calculates cell integrals for the full (= homogeneous +
     * inhomogeneous) part, where inhomogeneous part may occur for continuous
     * Galerkin discretizations due to inhomogeneous Dirichlet BCs. A
     * prerequisite to call this function is to set inhomogeneous Dirichlet
     * degrees of freedom appropriately in the DoF-vector src. In case the
     * DoF-vector src is zero apart from the inhomogeneous Dirichlet degrees of
     * freedom, this function calculates only the inhomogeneous part of the
     * operator, because the homogeneous operator is zero in this case. For DG
    ...
    /*
     * This function loops over all interior faces and calculates face
     * integrals.
     */
    void
    ...
    /*
     * The following functions loop over all boundary faces and calculate
     * boundary face integrals. Depending on the operator type, we distinguish
     * between boundary face integrals of type homogeneous, inhomogeneous, and
     * full.
     */

    // homogeneous operator
    void
    boundary_face_loop_hom_operator(
    ...
    void
    boundary_face_loop_inhom_operator(
      dealii::MatrixFree<dim, Number> const &matrix_free,
      VectorType                            &dst,
      VectorType const                      &src,
      Range const                           &range) const;
    ...
    // full operator
    void
    boundary_face_loop_full_operator(
      dealii::MatrixFree<dim, Number> const &matrix_free,
      VectorType                            &dst,
      VectorType const                      &src,
      Range const                           &range) const;
    ...
    /*
     * inhomogeneous operator: For the inhomogeneous operator, we only have to
     * calculate boundary face integrals. The matrix-free implementation,
     * however, does not offer interfaces for boundary face integrals only.
     * Hence we have to provide empty functions for cell and interior face
     * integrals.
     */
    void
    ...
    void
    boundary_face_loop_block_diagonal(
      dealii::MatrixFree<dim, Number> const &matrix_free,
      std::vector<LAPACKMatrix>             &matrices,
      std::vector<LAPACKMatrix> const       &src,
      Range const                           &range) const;
    ...
    void
    cell_based_loop_block_diagonal(
      dealii::MatrixFree<dim, Number> const &matrix_free,
      std::vector<LAPACKMatrix>             &matrices,
      std::vector<LAPACKMatrix> const       &src,
      Range const                           &range) const;
    ...
    template <typename SparseMatrix>
    void
    cell_loop_calculate_system_matrix(
      dealii::MatrixFree<dim, Number> const &matrix_free,
      SparseMatrix                          &dst,
    ...
    template <typename SparseMatrix>
    void
    face_loop_calculate_system_matrix(
      dealii::MatrixFree<dim, Number> const &matrix_free,
      SparseMatrix                          &dst,
      SparseMatrix const                    &src,
      Range const                           &range) const;
    ...
    /*
     * This function sets entries of the DoF-vector corresponding to constraint
     * DoFs to one.
     */
    void
    ...
    /*
     * Do we have to evaluate (boundary) face integrals for this operator? For
     * example, operators such as the mass operator only involve cell integrals.
     */
    bool
    ...
    /*
     * Multigrid level: 0 <= level <= max_level. If the operator is not used as
     * a multigrid level operator, this variable takes a value of
