...
namespace ExaDG
{
  ...
  enum class RunType
  {
    RefineHAndP, // run simulation for a specified range of mesh refinements and
                 // polynomial degrees
    FixedProblemSize,     // increase polynomial degree and keep problem size
                          // approximately constant
    IncreasingProblemSize // run at fixed polynomial degree
  };

  /*
   * Determines mesh refinement level l and number of subdivisions in 1d of
   * hyper_cube mesh for a given number of minimal and maximal number of
   * unknowns and a given RunType.
   */
  inline void
  fill_resolutions_vector(
    std::vector<std::tuple<unsigned int /*k*/,
                           unsigned int /*l*/,
                           unsigned int /*subdivisions hypercube*/>>
                                         &resolutions,
    ...
    ElementType const                    &element_type)
  {
    ...
    // in case of simplex elements, we create a mesh of hypercube elements that
    // gets later subdivided into simplex elements using
    // dealii::GridTools::subdivided_hyper_cube_with_simplices()
    if (element_type == ElementType::Simplex)
      {
        ...
        // From now on, we think of a hyerpcube mesh that gets later subdivided
        // into simplex elements with n_cells_per_cube simplices per hypercube
        // cell
    ...
    dealii::types::global_dof_index const n_cells_max =
      n_dofs_max / dofs_per_cube;

    // From the maximum number of cells, we derive a maximum refinement level
    // for a uniformly refined mesh with one coarse-grid cells
    unsigned int const refine_level_max =
      int(std::log((double)n_cells_max) /
          std::log((double)dealii::Utilities::pow(2ULL, dim))) +
      1;

    // we start with the coarsest possible mesh and then increase the refine
    // level by 1 until we hit the maximum refinement level
    unsigned int refine_level = 0;

    // This loop is for a uniformly refined hypercube mesh. Inside the
    // loop, we test whether additional combinations of coarse-grid cells and
    // mesh refinement levels are suitable in terms of n_cells_min/n_cells_max.
    // The reason behind is that we want to have more data points than just data
    // points differing by a factor of 2^dim in the number of cells.
    while (refine_level <= refine_level_max)
      {
        // To obtain additional data points, we test coarse grids with {3,4}^dim
        // elements in 2D and {3,4,5}^dim elements in 3D. The cases with 3 and 5
        // subdivisions per coordinate direction are only relevant for
        // refine_level >= 2.
    ...
    if (run_type == RunType::FixedProblemSize)
      {
        AssertThrow(
          resolutions.size() > resolutions_initial_size,
          dealii::ExcMessage(
  ...
  struct HypercubeResolutionParameters
  {
    ...
    HypercubeResolutionParameters(std::string const &input_file,
                                  unsigned int const dim)
      : dim(dim)
    {
    ...
    verify_parameters()
    {
      if (run_type == RunType::RefineHAndP)
        {
          AssertThrow(degree_max >= degree_min,
                      dealii::ExcMessage("Invalid parameters."));
          AssertThrow(refine_space_max >= refine_space_min,
                      dealii::ExcMessage("Invalid parameters."));
        }
      else if (run_type == RunType::FixedProblemSize)
        {
          AssertThrow(degree_max >= degree_min,
                      dealii::ExcMessage("Invalid parameters."));
          AssertThrow(n_dofs_max >= n_dofs_min,
                      dealii::ExcMessage("Invalid parameters."));
        }
    ...
    fill_resolution_vector(
      std::function<double(unsigned int, unsigned int, ElementType)> const
      ...
      if (run_type == RunType::RefineHAndP)
        {
          ...
          for (unsigned int degree = degree_min; degree <= degree_max; ++degree)
            {
              // h-refinement
              for (unsigned int refine_space = refine_space_min;
                   refine_space <= refine_space_max;
                   ++refine_space)
                {
                  resolutions.push_back(
                    std::tuple<unsigned int, unsigned int, unsigned int>(
                      degree, refine_space, n_cells_1d));
                }
            }
        }
      else if (run_type == RunType::FixedProblemSize or
               run_type == RunType::IncreasingProblemSize)
        {
          for (unsigned int degree = degree_min; degree <= degree_max; ++degree)
            {
              double dofs_per_element =
                get_dofs_per_element(dim, degree, element_type);
    ...
    dealii::types::global_dof_index n_dofs_min =
      1e4; // minimal number of unknowns
    dealii::types::global_dof_index n_dofs_max =
      3e4; // maximal number of unknowns

    // a vector storing tuples of the form (degree k, refine level l,
    // n_subdivisions_1d)
