...
namespace ExaDG
{
  struct AdaptiveMeshRefinementData
  {
    ...
    print(dealii::ConditionalOStream const &pcout) const
    {
      ...
      print_parameter(pcout,
                      "Triggered every n time steps",
                      trigger_every_n_time_steps);
      print_parameter(pcout,
      ...
      print_parameter(pcout,
                      "Preserve boundary cells",
                      preserve_boundary_cells);
      print_parameter(pcout,
                      "Fraction of cells to be refined",
                      fraction_of_cells_to_be_refined);
      print_parameter(pcout,
                      "Fraction of cells to be coarsened",
                      fraction_of_cells_to_be_coarsened);
    }
  ...
  /**
   * Check if adaptive mesh refinement should be triggered depending on the
   * time_step_number.
   */
  inline bool
  trigger_coarsening_and_refinement_now(
    unsigned int const trigger_every_n_time_steps,
    unsigned int const time_step_number)
  {
    return ((time_step_number == 0) or
            not(time_step_number % trigger_every_n_time_steps));
  }

  /**
   * Limit the coarsening and refinement by adapting the flags set in the
   * triangulation. This considers the maximum and minimum refinement levels
   * provided and might remove refinement flags set on the boundary if
   * requested.
   */
  template <int dim>
  void
    ...
    dealii::Triangulation<dim> const &tria)
  {
    ...
    any_flag_set =
      dealii::Utilities::MPI::logical_or(any_flag_set, tria.get_communicator());
  ...
  void
  mark_cells_kelly_error_estimator(
