...
namespace ExaDG
{
  ...
  template <int dim, typename Number>
  void
  DivergenceCalculator<dim, Number>::compute_divergence(
    VectorType       &dst,
  ...
  {
    CellIntegratorVector integrator_vector(matrix_free,
                                           dof_index_vector,
                                           quad_index,
                                           0);
    CellIntegratorScalar integrator_scalar(matrix_free,
                                           dof_index_scalar,
                                           quad_index,
                                           0);
  ...
  template <int dim, typename Number>
  void
  ShearRateCalculator<dim, Number>::compute_shear_rate(
    VectorType       &dst,
    VectorType const &src) const
  {
    dst = 0;
  ...
  template <int dim, typename Number>
  void
  ShearRateCalculator<dim, Number>::cell_loop(
    dealii::MatrixFree<dim, Number> const &matrix_free,
    VectorType                            &dst,
    VectorType const                      &src,
    Range const                           &cell_range) const
  {
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
      {
        integrator_vector.reinit(cell);
        integrator_vector.gather_evaluate(src,
        ...
        for (unsigned int q = 0; q < integrator_scalar.n_q_points; q++)
          {
            ...
            scalar shear_rate =
              std::sqrt(2.0 * scalar_product(sym_grad_u, sym_grad_u));

            integrator_scalar.submit_value(shear_rate, q);
          }

        integrator_scalar.integrate_scatter(dealii::EvaluationFlags::values,
                                            dst);
      }
  }
  ...
  template <int dim, typename Number>
  void
  VorticityCalculator<dim, Number>::initialize(
    dealii::MatrixFree<dim, Number> const &matrix_free_in,
  ...
  template <int dim, typename Number>
  void
  VorticityCalculator<dim, Number>::compute_vorticity(
    VectorType       &dst,
    VectorType const &src) const
  {
    matrix_free->cell_loop(&This::cell_loop, this, dst, src);
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
      {
        ...
        for (unsigned int q = 0; q < integrator.n_q_points; ++q)
          {
            dealii::Tensor<1,
                           number_vorticity_components,
                           dealii::VectorizedArray<Number>>
              omega = integrator.get_curl(q);
  ...
  template <int dim, typename Number>
  void
  MagnitudeCalculator<dim, Number>::initialize(
    dealii::MatrixFree<dim, Number> const &matrix_free_in,
    unsigned int const                     dof_index_u_in,
    unsigned int const                     dof_index_u_scalar_in,
    unsigned int const                     quad_index_in)
  ...
  template <int dim, typename Number>
  void
  MagnitudeCalculator<dim, Number>::compute(VectorType       &dst,
                                            VectorType const &src) const
  {
    dst = 0;
  ...
  template <int dim, typename Number>
  void
  MagnitudeCalculator<dim, Number>::cell_loop(
    dealii::MatrixFree<dim, Number> const &matrix_free,
    VectorType                            &dst,
    VectorType const                      &src,
    Range const                           &cell_range) const
  {
    ...
    IntegratorScalar integrator_scalar(matrix_free,
                                       dof_index_u_scalar,
                                       quad_index);
  ...
  template <int dim, typename Number>
  void
  QCriterionCalculator<dim, Number>::compute(VectorType       &dst,
                                             VectorType const &src) const
  {
    dst = 0;
  ...
  template <int dim, typename Number>
  void
  QCriterionCalculator<dim, Number>::cell_loop(
    dealii::MatrixFree<dim, Number> const &matrix_free,
    VectorType                            &dst,
    VectorType const                      &src,
    Range const                           &cell_range) const
        ...
        for (unsigned int q = 0; q < integrator_scalar.n_q_points; q++)
          {
            ...
            // part of S (Kolar, V.: Compressibility Effect in Vortex
            // Identification, AIAA, 2009. https://doi.org/10.2514/1.40131)
