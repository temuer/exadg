...
namespace ExaDG
{
  namespace IP // IP = interior penalty
  {
      ...
      unsigned int const                     dof_index = 0)
    {
      ...
      auto const reference_cells = matrix_free.get_dof_handler(dof_index)
                                     .get_triangulation()
                                     .get_reference_cells();
      ...
      auto const quadrature =
        reference_cells[0].template get_gauss_type_quadrature<dim>(degree + 1);
      ...
      auto const face_quadrature =
        reference_cells[0]
          ...
          .template get_gauss_type_quadrature<dim - 1>(degree + 1);
              ...
              for (unsigned int const f : cell->face_indices())
                {
                  ...
                  Number const factor = (cell->at_boundary(f) and
     ...
     * for quadrilateral/hexahedral or for triangular/tetrahedral elements for a
     * given polynomial degree of the shape functions, a specified penalty
     * factor (scaling factor) and the dimension of the problem.
     */
                       ...
                       Number const       factor = 1.0)
    {
      if (element_type == ElementType::Simplex)
        {
          // use penalty factor for simplex elements according to K. Shahbazi,
          // An explicit expression for the penalty parameter of the interior
          // penalty method, Journal of Computational Physics 205, 401–407,
          // 2005.
          return factor * (degree + 1.0) * (degree + dim) / dim;
        }
      else if (element_type == ElementType::Hypercube)
        {
          // use penalty factor for hypercube elements according to K.
          // Hillewaert, Development of the discontinuous Galerkin method for
          // high-resolution, large scale CFD and acoustics in industrial
          // geometries, PhD thesis, Univ. de Louvain, 2013.
