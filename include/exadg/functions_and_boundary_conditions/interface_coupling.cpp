...
namespace ExaDG
{
  ...
  InterfaceCoupling<rank, dim, Number>::InterfaceCoupling()
    : dof_handler_src(nullptr)
  {}
    ...
    double const                   tolerance_)
  {
    AssertThrow(interface_data_dst_.get(),
                dealii::ExcMessage(
                  "Received uninitialized variable. Aborting."));

    if (marked_vertices_src_.size() > 0)
      {
        AssertThrow(
          marked_vertices_src_.size() ==
    ...
    for (auto quad_index : interface_data_dst->get_quad_indices())
      {
        ...
        auto const *points =
          &interface_data_dst->get_array_q_points(quad_index);

        map_evaluator[quad_index]->reinit(*points,
        ...
        if (not map_evaluator[quad_index]->all_points_found())
          {
            ...
            n_points_not_found =
              dealii::Utilities::MPI::sum(n_points_not_found,
                                          dof_handler_src->get_communicator());

            std::string const file_name =
              "interface_coupling_quad_index_" +
              dealii::Utilities::to_string(quad_index);
            ...
            AssertThrow(map_evaluator[quad_index]->all_points_found(),
                        dealii::ExcMessage(std::string(
                          "Setup of InterfaceCoupling was not successful: " +
  ...
  template <int rank, int dim, typename Number>
  void
  InterfaceCoupling<rank, dim, Number>::update_data(
    VectorType const &dof_vector_src)
  {
    ...
    for (auto quadrature : interface_data_dst->get_quad_indices())
      {
        auto const result = dealii::VectorTools::point_values<n_components>(
          *map_evaluator[quadrature],
          *dof_handler_src,
          dof_vector_src,
          dealii::VectorTools::EvaluationFlags::avg);

        auto &array_solution =
          interface_data_dst->get_array_solution(quadrature);
