...
{
  /**
   * A data structure storing quadrature point information for each quadrature
   * point on boundary faces of a given set of boundary IDs. The type of data
   * stored for each q-point is dealii::Tensor<rank, dim, number_type>.
   */
  ...
  private:
    ...
    using Id =
      std::tuple<unsigned int /*face*/, unsigned int /*q*/, unsigned int /*v*/>;
      ...
      for (auto q_index : quad_indices)
        {
          ...
          ArrayQuadraturePoints &array_q_points_dst =
            map_q_points.find(q_index)->second;
          ArraySolutionValues &array_solution_dst =
            map_solution.find(q_index)->second;

          // create map "ID = {face, q, v} <-> vector_index" and fill array of
          // quadrature points
          for (unsigned int face = matrix_free_.n_inner_face_batches();
               face < matrix_free_.n_inner_face_batches() +
                        matrix_free_.n_boundary_face_batches();
               ++face)
            {
              // only consider relevant boundary IDs
              if (set_bids_.find(matrix_free_.get_boundary_id(face)) !=
                  set_bids_.end())
                {
                  FaceIntegrator<dim, n_components, Number> integrator(
                    matrix_free_, true, dof_index_, q_index);
