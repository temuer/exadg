...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    MultigridPreconditioner<dim, Number>::MultigridPreconditioner(
      MPI_Comm const &comm)
      : Base(comm)
      ...
      Map_DBC_ComponentMask const &dirichlet_bc_component_mask)
    {
      ...
      // When solving the reaction-convection-diffusion problem, it might be
      // possible that one wants to apply the multigrid preconditioner only to
      // the reaction-diffusion operator (which is symmetric, Chebyshev
      // smoother, etc.) instead of the non-symmetric
      // reaction-convection-diffusion operator. Accordingly, we have to reset
      // which operators should be "active" for the multigrid preconditioner,
      // independently of the actual equation type that is solved.
      ...
      else if (this->mg_operator_type ==
               MultigridOperatorType::ReactionConvectionDiffusion)
        {
          AssertThrow(data.convective_problem == true,
    ...
    MultigridPreconditioner<dim, Number>::update()
    {
      ...
      if (data.unsteady_problem)
        {
          this->for_all_levels([&](unsigned int const level) {
            // The operator also depends on the time. This is due to the fact
            // that the linearized convective term does not only depend on the
            // linearized velocity field but also on Dirichlet boundary data
            // which itself depends on the current time.
            this->get_operator(level)->set_time(pde_operator->get_time());
            // In case of adaptive time stepping, the scaling factor of the time
            // derivative term changes over time.
      ...
      if (mg_operator_type ==
          MultigridOperatorType::ReactionConvectionDiffusion)
        {
          ...
          // convert Number --> MultigridNumber, e.g., double --> float, but
          // only if necessary
          ...
          if (std::is_same<MultigridNumber, Number>::value)
            {
              vector_multigrid_type_ptr =
                reinterpret_cast<VectorTypeMG const *>(&vector_linearization);
            }
          else
            {
          ...
          this->transfer_from_fine_to_coarse_levels(
            [&](unsigned int const fine_level,
                unsigned int const coarse_level) {
              auto const &vector_fine_level =
                this->get_operator(fine_level)->get_velocity();
              auto vector_coarse_level =
                this->get_operator(coarse_level)->get_velocity();
              this->transfers->interpolate(fine_level,
                                           vector_coarse_level,
                                           vector_fine_level);
              this->get_operator(coarse_level)
                ->set_velocity_copy(vector_coarse_level);
            });
        }

      // In case the operators have been updated, we also need to update the
      // smoothers and the coarse grid solver. This is generic functionality
      // implemented in the base class.
      if (mesh_is_moving or data.unsteady_problem or
          (mg_operator_type ==
           MultigridOperatorType::ReactionConvectionDiffusion) or
          this->update_needed)
        {
      ...
      unsigned int const level)
    {
      ...
      // The operator also depends on the time. This is due to the fact that the
      // linearized convective term does not only depend on the linearized
      // velocity field but also on Dirichlet boundary data which itself depends
      // on the current time.
      pde_operator_level->set_time(pde_operator->get_time());
      // make sure that scaling factor of time derivative term has been set
      // before the smoothers are initialized
    ...
    template <int dim, typename Number>
    std::shared_ptr<MomentumOperator<
      dim,
      typename MultigridPreconditionerBase<dim, Number>::MultigridNumber>>
