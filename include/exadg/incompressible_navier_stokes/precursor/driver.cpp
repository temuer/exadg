    ...
    namespace Precursor
    {
      ...
      Driver<dim, Number>::set_start_time() const
      {
        AssertThrow(application->precursor_is_active(),
                    dealii::ExcMessage(
                      "One should only arrive here if precursor is active."));

        // Setup time integrator and get time step size
        double const start_time =
          std::min(application->precursor->get_parameters().start_time,
      ...
      Driver<dim, Number>::synchronize_time_step_size() const
      {
        ...
        if (use_adaptive_time_stepping == true)
          {
            if (solver_precursor.time_integrator->get_time() >
              ...
              {
                time_step_size_pre =
                  solver_precursor.time_integrator->get_time_step_size();
              }
            ...
            if (solver_main.time_integrator->get_time() >
              ...
              {
                time_step_size =
                  solver_main.time_integrator->get_time_step_size();
              }
          }
        else
          {
        ...
        if (use_adaptive_time_stepping == false)
          {
            // assume that the precursor domain is the first to start and the
            // last to end
            time_step_size = adjust_time_step_to_hit_end_time(
              application->precursor->get_parameters().start_time,
              application->precursor->get_parameters().end_time,
              time_step_size);

            pcout << std::endl
                  << "Combined time step size for both domains: "
                  << time_step_size << std::endl;
          }

        // set the time step size
        solver_precursor.time_integrator->set_current_time_step_size(
          time_step_size);
      ...
      {
        AssertThrow(application->precursor->get_parameters().ale_formulation ==
                      false,
                    dealii::ExcMessage("not implemented."));
        AssertThrow(application->main->get_parameters().ale_formulation ==
                      false,
                    dealii::ExcMessage("not implemented."));

        AssertThrow(
          application->precursor->get_parameters()
        ...
        AssertThrow(
          application->precursor->get_parameters().adaptive_time_stepping ==
            application->main->get_parameters().adaptive_time_stepping,
          dealii::ExcMessage(
            "Type of time step calculation has to be the same for both domains."));

        AssertThrow(application->precursor->get_parameters().solver_type ==
                        SolverType::Unsteady and
                      application->main->get_parameters().solver_type ==
                        SolverType::Unsteady,
                    dealii::ExcMessage(
                      "This is an unsteady solver. Check parameters."));

        // For the two-domain solver the parameter start_with_low_order has to
        // be true. This is due to the fact that the setup function of the time
        // integrator initializes the solution at previous time instants t_0 -
        // dt, t_0 - 2*dt, ... in case of start_with_low_order == false.
        // However, the combined time step size is not known at this point since
        // the two domains have to first communicate with each other in order to
        // find the minimum time step size. Hence, the easiest way to avoid
        // these kind of inconsistencies is to preclude the case
        // start_with_low_order == false.
        AssertThrow(
          application->precursor->get_parameters().start_with_low_order ==
              true and
            application->main->get_parameters().start_with_low_order == true,
          dealii::ExcMessage(
            "start_with_low_order has to be true for two-domain solver."));
      }
      ...
      Driver<dim, Number>::setup()
      {
        ...
        pcout << std::endl
              << "Setting up incompressible Navier-Stokes solver:" << std::endl;

        // main domain
        AssertThrow(application->main.get(),
                    dealii::ExcMessage("Domain main is uninitialized."));
        solver_main.setup(
          application->main, {"Main"}, "main", mpi_comm, is_test);
        ...
        if (application->precursor_is_active())
          {
            ...
            solver_precursor.setup(application->precursor,
                                   {"Precursor"},
                                   "precursor",
                                   mpi_comm,
                                   is_test);

            // make some additional checks (i.e. enforce constraints between
            // main and precursor parameters)
            consistency_checks();
          }
            ...
            do
              {
                ...
                // Note that the coupling of both solvers via the inflow
                // boundary conditions is performed in the postprocessing step
                // of the solver for the precursor domain, overwriting the data
                // global structures which are subsequently used by the solver
                // for the actual domain to evaluate the boundary conditions.

                // advance one time step for actual domain
                solver_main.time_integrator->advance_one_timestep();

                // Both domains have already calculated the new, adaptive time
                // step size individually in function advance_one_timestep().
                // Here, we have to synchronize the time step size for both
                // domains.
                if (use_adaptive_time_stepping == true)
                  {
                    synchronize_time_step_size();
        ...
        double const total_time) const
      {
        ...
        // Wall times
        pcout << std::endl
              << "Wall times for incompressible Navier-Stokes solver:"
              << std::endl;
