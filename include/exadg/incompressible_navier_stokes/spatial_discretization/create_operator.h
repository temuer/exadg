...
namespace ExaDG
{
  ...
  {
    /**
     * Creates spatial discretization operator depending on type of solution
     * strategy.
     */
      ...
      MPI_Comm const                                       &mpi_comm)
    {
      ...
      if (parameters.temporal_discretization ==
        ...
        {
          pde_operator =
            std::make_shared<OperatorCoupled<dim, Number>>(grid,
                                                           mapping,
                                                           multigrid_mappings,
                                                           boundary_descriptor,
                                                           field_functions,
               ...
               TemporalDiscretization::BDFDualSplittingScheme)
        {
          pde_operator = std::make_shared<OperatorDualSplitting<dim, Number>>(
            ...
            mpi_comm);
               ...
               // we can not instantiate the base class and instantiate an
               // arbitrary deriving class for InterpolateAnalyticalSolution.
               parameters.temporal_discretization ==
                 TemporalDiscretization::InterpolateAnalyticalSolution)
        {
          pde_operator =
            std::make_shared<OperatorPressureCorrection<dim, Number>>(
              grid,
              mapping,
              multigrid_mappings,
              boundary_descriptor,
