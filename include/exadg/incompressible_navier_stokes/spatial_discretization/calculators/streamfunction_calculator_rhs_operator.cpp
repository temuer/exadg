...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    StreamfunctionCalculatorRHSOperator<dim, Number>::
    ...
    template <int dim, typename Number>
    void
    StreamfunctionCalculatorRHSOperator<dim, Number>::apply(
      VectorType       &dst,
      ...
      for (unsigned int cell = cell_range.first; cell < cell_range.second;
           ++cell)
        {
          ...
          for (unsigned int q = 0; q < integrator_scalar.n_q_points; q++)
            {
              // we exploit that the (scalar) vorticity is stored in the first
              // component of the vector in case of 2D problems
              integrator_scalar.submit_value(integrator_vector.get_value(q)[0],
                                             q);
            }
