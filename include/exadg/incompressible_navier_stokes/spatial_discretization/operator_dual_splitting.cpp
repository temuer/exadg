...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    template <int dim, typename Number>
    void
    OperatorDualSplitting<dim, Number>::apply_velocity_divergence_term(
      VectorType       &dst,
    ...
    template <int dim, typename Number>
    void
    OperatorDualSplitting<dim, Number>::rhs_ppe_div_term_body_forces_add(
      VectorType   &dst,
    ...
    template <int dim, typename Number>
    void
    OperatorDualSplitting<dim, Number>::
        ...
        Range const &face_range) const
    {
      ...
      FaceIntegratorP integrator(matrix_free,
                                 true,
                                 dof_index_pressure,
                                 quad_index_pressure);
      ...
      for (unsigned int face = face_range.first; face < face_range.second;
           face++)
        {
          ...
          BoundaryTypeU boundary_type =
            this->boundary_descriptor->velocity->get_boundary_type(
          ...
          for (unsigned int q = 0; q < integrator.n_q_points; ++q)
            {
              if (boundary_type == BoundaryTypeU::Dirichlet or
                  boundary_type == BoundaryTypeU::DirichletCached)
                {
                  ...
                  vector rhs = FunctionEvaluator<1, dim, Number>::value(
                    *(this->field_functions->right_hand_side),
                    q_points,
                    this->evaluation_time);

                  scalar flux_times_normal =
                    rhs * integrator.get_normal_vector(q);
                  // minus sign is introduced here which allows to call a
                  // function of type ...add() and avoids a scaling of the
                  // resulting vector by the factor -1.0
                  integrator.submit_value(-flux_times_normal, q);
                }
              else if (boundary_type == BoundaryTypeU::Neumann or
                       boundary_type == BoundaryTypeU::Symmetry)
                {
                  // Do nothing on Neumann and symmetry boundaries.
                  // Remark: On symmetry boundaries it follows from g_u * n = 0
                  // that also g_{u_hat} * n = 0. Hence, a symmetry boundary for
                  // u is also a symmetry boundary for u_hat. Hence, there are
                  // no inhomogeneous contributions on symmetry boundaries.
        ...
        Range const                           &face_range) const
    {
      ...
      unsigned int const quad_index =
        this->get_quad_index_velocity_overintegration();
      ...
      FaceIntegratorU grid_velocity(matrix_free,
                                    true,
                                    dof_index_velocity,
                                    quad_index);
          ...
          if (this->param.ale_formulation)
            {
              ...
              grid_velocity.gather_evaluate(
                this->convective_kernel->get_grid_velocity(),
                dealii::EvaluationFlags::values);
            }
    ...
    void
    OperatorDualSplitting<dim, Number>::
      rhs_ppe_nbc_numerical_time_derivative_add(
        VectorType       &dst,
        ...
        Range const                           &face_range) const
    {
      ...
      FaceIntegratorU integrator_velocity(data,
                                          true,
                                          dof_index_velocity,
                                          quad_index_velocity);
      FaceIntegratorP integrator_pressure(data,
                                          true,
                                          dof_index_pressure,
                                          quad_index_velocity);
      ...
      for (unsigned int face = face_range.first; face < face_range.second;
           face++)
        {
          ...
          integrator_velocity.gather_evaluate(acceleration,
                                              dealii::EvaluationFlags::values);
    ...
    template <int dim, typename Number>
    void
    OperatorDualSplitting<dim, Number>::rhs_ppe_nbc_body_force_term_add(
      VectorType   &dst,
      double const &time) const
    {
      this->evaluation_time = time;
        ...
        Range const &face_range) const
    {
      ...
      FaceIntegratorP integrator(data,
                                 true,
                                 dof_index_pressure,
                                 quad_index_pressure);
    ...
    template <int dim, typename Number>
    void
    OperatorDualSplitting<dim, Number>::rhs_ppe_nbc_convective_add(
      VectorType       &dst,
      VectorType const &src) const
    {
      this->get_matrix_free().loop(
        ...
        Range const                           &face_range) const
    {
      ...
      for (unsigned int face = face_range.first; face < face_range.second;
           face++)
        {
          ...
          BoundaryTypeP boundary_type =
            this->boundary_descriptor->pressure->get_boundary_type(
              matrix_free.get_boundary_id(face));
              ...
              if (boundary_type == BoundaryTypeP::Neumann)
              ...
              else if (boundary_type == BoundaryTypeP::Dirichlet)
                {
                  pressure.submit_value(
                    dealii::make_vectorized_array<Number>(0.0), q);
                }
              else
                {
                  AssertThrow(
    ...
    template <int dim, typename Number>
    void
    OperatorDualSplitting<dim, Number>::rhs_ppe_nbc_viscous_add(
      VectorType       &dst,
      VectorType const &src) const
    {
      this->get_matrix_free().loop(
              ...
              if (boundary_type == BoundaryTypeP::Neumann)
                {
                  ...
                  vector curl_omega =
                    CurlCompute<dim, FaceIntegratorU>::compute(omega, q);
    ...
    template <int dim, typename Number>
    void
    OperatorDualSplitting<dim, Number>::rhs_ppe_laplace_add(
      VectorType   &dst,
    ...
    template <int dim, typename Number>
    unsigned int
    OperatorDualSplitting<dim, Number>::solve_pressure(
      VectorType       &dst,
      VectorType const &src,
              ...
              for (unsigned int q = 0; q < integrator.n_q_points; ++q)
                {
                  ...
                  unsigned int const index =
                    matrix_free.get_shape_info(dof_index, quad_index)
                  ...
                  if (boundary_type == BoundaryTypeU::Dirichlet)
                    {
                      auto bc =
                        this->boundary_descriptor->velocity->dirichlet_bc
                          .find(boundary_id)
                          ->second;
                      auto q_points = integrator.quadrature_point(q);

                      g = FunctionEvaluator<1, dim, Number>::value(
                        *bc, q_points, this->evaluation_time);
                    }
                  else if (boundary_type == BoundaryTypeU::DirichletCached)
                    {
                      auto bc = this->boundary_descriptor->velocity
                                  ->get_dirichlet_cached_data();
    ...
    template <int dim, typename Number>
    void
    OperatorDualSplitting<dim, Number>::apply_helmholtz_operator(
      VectorType       &dst,
      VectorType const &src) const
    {
