...
namespace ExaDG
{
  namespace IncNS
  {
      ...
      if (this->param.solver_coupled == SolverCoupled::GMRES)
        {
          ...
          solver_data.solver_tolerance_abs =
            this->param.solver_data_coupled.abs_tol;
          solver_data.solver_tolerance_rel =
            this->param.solver_data_coupled.rel_tol;
          solver_data.max_n_tmp_vectors =
            this->param.solver_data_coupled.max_krylov_size;
          ...
          linear_solver = std::make_shared<
            Krylov::SolverGMRES<LinearOperatorCoupled<dim, Number>,
                                Preconditioner,
                                BlockVectorType>>(linear_operator,
      ...
      else if (this->param.solver_coupled == SolverCoupled::FGMRES)
        {
          ...
          linear_solver = std::make_shared<
            Krylov::SolverFGMRES<LinearOperatorCoupled<dim, Number>,
                                 Preconditioner,
                                 BlockVectorType>>(linear_operator,
                                                   block_preconditioner,
                                                   solver_data);
      ...
      else
        {
          AssertThrow(
            false,
            dealii::ExcMessage(
              "Specified solver for linearized problem is not implemented."));
        }
      ...
      if (this->param.nonlinear_problem_has_to_be_solved())
        {
          ...
          newton_solver = std::make_shared<
            Newton::Solver<BlockVectorType,
    ...
    template <int dim, typename Number>
    void
    OperatorCoupled<dim, Number>::update_divergence_penalty_operator(
    ...
    template <int dim, typename Number>
    void
    OperatorCoupled<dim, Number>::update_continuity_penalty_operator(
      VectorType const &velocity)
    {
    ...
    template <int dim, typename Number>
    void
    OperatorCoupled<dim, Number>::set_scaling_factor_continuity(
    ...
    template <int dim, typename Number>
    unsigned int
    OperatorCoupled<dim, Number>::solve_linear_problem(
      BlockVectorType       &dst,
      ...
      double const          &scaling_factor_mass)
    {
      ...
      // We do not need to set the time here, because time affects the operator
      // only in the form of boundary conditions. The result of such boundary
      // condition evaluations is handed over to this function via the vector
      // src.
      this->momentum_operator.set_scaling_factor_mass_operator(
        scaling_factor_mass);
      ...
      if (this->param.convective_problem() and
        ...
        {
          this->momentum_operator.set_solution_linearization(
            transport_velocity);
        }
    ...
    template <int dim, typename Number>
    void
    OperatorCoupled<dim, Number>::rhs_linear_problem(
      BlockVectorType  &dst,
      VectorType const &transport_velocity,
      double const     &time) const
    {
      ...
      // multiply by -1.0 since we use a formulation with symmetric saddle point
      // matrix with respect to pressure gradient term and velocity divergence
      // term scale by scaling_factor_continuity
      dst.block(1) *= -scaling_factor_continuity;
    }

    template <int dim, typename Number>
    void
    OperatorCoupled<dim, Number>::apply_linearized_problem(
      BlockVectorType       &dst,
    ...
    OperatorCoupled<dim, Number>::solve_nonlinear_problem(
      BlockVectorType  &dst,
    ...
    template <int dim, typename Number>
    void
    OperatorCoupled<dim, Number>::evaluate_nonlinear_residual(
      BlockVectorType       &dst,
      BlockVectorType const &src,
      VectorType const      *rhs_vector,
      double const          &time,
      double const          &scaling_factor_mass) const
    {
      ...
      if (this->unsteady_problem_has_to_be_solved())
        this->mass_operator.apply_scale(dst.block(0),
                                        scaling_factor_mass,
                                        src.block(0));
      else
        dst.block(0) = 0.0;
      ...
      if (this->param.implicit_nonlinear_convective_problem())
        {
          this->convective_operator.evaluate_nonlinear_operator_add(
            dst.block(0), src.block(0), time);
        }
      ...
      if (this->param.apply_penalty_terms_in_postprocessing_step == false)
        {
          if (this->param.use_divergence_penalty == true)
            this->div_penalty_operator.apply_add(dst.block(0), src.block(0));
          if (this->param.use_continuity_penalty == true)
            this->conti_penalty_operator.evaluate_add(dst.block(0),
                                                      src.block(0),
                                                      time);
        }
      ...
      // constant right-hand side vector (body force, sum_alphai_ui and explicit
      // convective terms)
    ...
    template <int dim, typename Number>
    void
    OperatorCoupled<dim, Number>::evaluate_nonlinear_residual_steady(
      BlockVectorType       &dst,
      BlockVectorType const &src,
      double const          &time) const
    {
    ...
    OperatorCoupled<dim, Number>::initialize_preconditioner_velocity_block()
    {
      ...
      if (type == MomentumPreconditioner::PointJacobi)
        {
          preconditioner_momentum = std::make_shared<
            JacobiPreconditioner<MomentumOperator<dim, Number>>>(
            this->momentum_operator, false);
        }
      else if (type == MomentumPreconditioner::BlockJacobi)
        {
          preconditioner_momentum = std::make_shared<
            BlockJacobiPreconditioner<MomentumOperator<dim, Number>>>(
            this->momentum_operator, false);
        }
      else if (type == MomentumPreconditioner::InverseMassMatrix)
        {
          ...
          inverse_mass_operator_data.parameters =
            this->param.inverse_mass_preconditioner;

          preconditioner_momentum =
            std::make_shared<InverseMassPreconditioner<dim, dim, Number>>(
              this->get_matrix_free(), inverse_mass_operator_data);
        }
        ...
        {
          AssertThrow(type == MomentumPreconditioner::None,
    ...
    OperatorCoupled<dim, Number>::setup_multigrid_preconditioner_momentum()
    {
      ...
      for (auto iter :
           this->momentum_operator.get_data().bc->dirichlet_cached_bc)
        {
          typedef std::pair<dealii::types::boundary_id,
                            std::shared_ptr<dealii::Function<dim>>>
            pair;
      ...
      mg_preconditioner->initialize(
        this->param.multigrid_data_velocity_block,
        this->grid,
    ...
    OperatorCoupled<dim, Number>::setup_iterative_solver_momentum()
    {
      AssertThrow(preconditioner_momentum.get() != 0,
                  dealii::ExcMessage(
                    "preconditioner_momentum is uninitialized"));
      ...
      gmres_data.solver_tolerance_abs =
        this->param.solver_data_velocity_block.abs_tol;
      gmres_data.solver_tolerance_rel =
        this->param.solver_data_velocity_block.rel_tol;
      gmres_data.max_n_tmp_vectors =
        this->param.solver_data_velocity_block.max_krylov_size;

      solver_velocity_block =
        std::make_shared<Krylov::SolverFGMRES<MomentumOperator<dim, Number>,
      ...
      if (type == SchurComplementPreconditioner::InverseMassMatrix)
        {
          inverse_mass_preconditioner_schur_complement =
            std::make_shared<InverseMassPreconditioner<dim, 1, Number>>(
              this->get_matrix_free(), inverse_mass_operator_data);
        }
        ...
        {
          AssertThrow(
            this->unsteady_problem_has_to_be_solved() == true,
            dealii::ExcMessage(
          ...
          // inverse mass operator to also include the part of the
          // preconditioner that is beneficial when using large time steps and
          // large viscosities.
          inverse_mass_preconditioner_schur_complement =
            std::make_shared<InverseMassPreconditioner<dim, 1, Number>>(
              this->get_matrix_free(), inverse_mass_operator_data);
               ...
               SchurComplementPreconditioner::PressureConvectionDiffusion)
        {
          ...
          // I. multigrid for negative Laplace operator (classical or compatible
          // discretization)
          setup_multigrid_preconditioner_schur_complement();
        ...
        {
          AssertThrow(type == SchurComplementPreconditioner::None,
                      dealii::ExcNotImplemented());
        }
    }
    ...
    void
    OperatorCoupled<dim,
                    Number>::setup_multigrid_preconditioner_schur_complement()
    {
      ...
      laplace_operator_data.operator_is_singular =
        this->is_pressure_level_undefined();
      ...
      multigrid_preconditioner_schur_complement =
        std::make_shared<MultigridPoisson>(this->mpi_comm);

      std::shared_ptr<MultigridPoisson> mg_preconditioner =
        std::dynamic_pointer_cast<MultigridPoisson>(
          multigrid_preconditioner_schur_complement);
    ...
    OperatorCoupled<dim, Number>::setup_iterative_solver_schur_complement()
    {
      ...
      solver_data.solver_tolerance_abs =
        this->param.solver_data_pressure_block.abs_tol;
      solver_data.solver_tolerance_rel =
        this->param.solver_data_pressure_block.rel_tol;
      solver_data.use_preconditioner = true;
      ...
      laplace_operator =
        std::make_shared<Poisson::LaplaceOperator<dim, Number, 1>>();
      laplace_operator->initialize(this->get_matrix_free(),
                                   this->get_constraint_p(),
                                   laplace_operator_data);

      solver_pressure_block = std::make_shared<
        Krylov::SolverCG<Poisson::LaplaceOperator<dim, Number, 1>,
                         PreconditionerBase<Number>,
                         VectorType>>(
        *laplace_operator,
        *multigrid_preconditioner_schur_complement,
    ...
    OperatorCoupled<dim, Number>::setup_pressure_convection_diffusion_operator()
    {
      ...
      boundary_descriptor =
        std::make_shared<ConvDiff::BoundaryDescriptor<dim>>();

      // For the pressure convection-diffusion operator the homogeneous
      // operators are applied, so there is no need to specify functions for
      // boundary conditions since they will never be used.
           ...
           ++it)
        {
          boundary_descriptor->dirichlet_bc.insert(
            std::pair<dealii::types::boundary_id,
                      std::shared_ptr<dealii::Function<dim>>>(it->first,
                                                              dummy));
        }
      ...
      for (typename std::set<dealii::types::boundary_id>::const_iterator it =
           ...
           ++it)
        {
          boundary_descriptor->neumann_bc.insert(
            std::pair<dealii::types::boundary_id,
                      std::shared_ptr<dealii::Function<dim>>>(*it, dummy));
        }

      // convective operator:
      // use numerical velocity field with dof index of velocity field and local
      // Lax-Friedrichs flux to mimic the upwind-like discretization of the
      // linearized convective term in the Navier-Stokes equations.
      ConvDiff::Operators::ConvectiveKernelData<dim> convective_kernel_data;
      convective_kernel_data.velocity_type =
        ConvDiff::TypeVelocityField::DoFVector;
      convective_kernel_data.dof_index_velocity =
      ...
      // diffusive operator:
      // take interior penalty factor of diffusivity of viscous operator, but
      // use polynomial degree of pressure shape functions.
      ConvDiff::Operators::DiffusiveKernelData diffusive_kernel_data;
      diffusive_kernel_data.IP_factor = this->param.IP_factor_viscous;
      // Note: the diffusive operator is initialized with constant viscosity. In
      // case of spatially (and temporally) varying viscosities the diffusive
      // operator has to be extended so that it can deal with variable
      // coefficients (and should be updated in case of time dependent problems
      ...
      operator_data.unsteady_problem =
        this->unsteady_problem_has_to_be_solved();
      operator_data.convective_problem =
      ...
      pressure_conv_diff_operator =
        std::make_shared<ConvDiff::CombinedOperator<dim, Number>>();
 ...
 */
      ...
      BlockVectorType const &src) const
    {
      ...
      if (type == PreconditionerCoupled::BlockDiagonal)
      ...
      else if (type == PreconditionerCoupled::BlockTriangularFactorization)
        {
          ...
          apply_preconditioner_velocity_block(vec_tmp_velocity_2,
                                              vec_tmp_velocity);
      ...
      VectorType const &src) const
    {
      ...
      if (type == MomentumPreconditioner::None)
        ...
        {
          // use the inverse mass operator as an approximation to the momentum
          // block
          preconditioner_momentum->vmult(dst, src);
          dst *=
            1. / this->momentum_operator.get_scaling_factor_mass_operator();
        }
      else if (type == MomentumPreconditioner::Multigrid)
        {
      ...
      */
              ...
              // already been updated in the function
              ...
              if (print_iterations)
                {
                  dealii::ConditionalOStream pcout(
                    ...
                    dealii::Utilities::MPI::this_mpi_process(this->mpi_comm) ==
                      0);
                  pcout << "Number of iterations for inner solver = "
                        << iterations << std::endl;
                }
            }
        }
      ...
      VectorType const &src) const
    {
      ...
      if (type == SchurComplementPreconditioner::None)
      ...
      else if (type == SchurComplementPreconditioner::CahouetChabard)
        {
          ...
          // II. M_p^{-1}, apply inverse pressure mass operator to src-vector
          // and store the result in a temporary vector
          // TODO consider variable viscosity here
          inverse_mass_preconditioner_schur_complement->vmult(tmp_scp_pressure,
                                                              src);
               ...
               SchurComplementPreconditioner::PressureConvectionDiffusion)
        {
          ...
          if (this->param.non_explicit_convective_problem())
            {
              pressure_conv_diff_operator->set_velocity_ptr(
                this->convective_kernel->get_velocity());
            }
      ...
      // scaling_factor_continuity: Since the Schur complement includes both the
      // velocity divergence and the pressure gradient operators as factors, we
      // have to scale by 1/(scaling_factor*scaling_factor) when applying (an
      // approximation of) the inverse Schur complement.
    ...
    template <int dim, typename Number>
    void
    OperatorCoupled<dim, Number>::apply_inverse_negative_laplace_operator(
      VectorType       &dst,
      VectorType const &src) const
    {
      if (this->param.iterative_solve_of_pressure_block == false)
        {
          // perform one multigrid V-cycle in order to approximately invert the
          // negative Laplace operator (classical or compatible)
          multigrid_preconditioner_schur_complement->vmult(dst, src);
        }
      else // iterative_solve_of_pressure_block == true
        {
          // solve a linear system of equations related to the negative Laplace
          // operator to given (relative) tolerance using an iterative method
