...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    public:
      ...
      void
      update(VectorType const &rhs_vector,
             double const     &time,
      ...
      evaluate_residual(BlockVectorType &dst, BlockVectorType const &src) const
      {
        pde_operator->evaluate_nonlinear_residual(
          dst, src, rhs_vector, time, scaling_factor_mass);
      }
    ...
    public:
      ...
      void
      set_solution_linearization(
        BlockVectorType const &solution_linearization) const
      {
    ...
    public:
      /*
       *  Update divergence penalty operator by recalculating the penalty
       * parameter which depends on the current velocity field
       */
      void
      update_divergence_penalty_operator(VectorType const &velocity);

      /*
       *  Update continuity penalty operator by recalculating the penalty
       * parameter which depends on the current velocity field
       */
      void
      ...
      /*
       *  This function sets the variable scaling_factor_continuity, and also
       * the related scaling factor for the pressure gradient operator.
       */
      void
      set_scaling_factor_continuity(double const scaling_factor);

      /*
       * Stokes equations or convective term treated explicitly: solve linear
       * system of equations
       */

      /*
       * This function solves the linear Stokes problem (Stokes equations or
       * Navier-Stokes equations with an explicit treatment of the convective
       * term). The parameter scaling_factor_mass has to be specified for
       * unsteady problem and can be omitted for steady problems.
       */
      unsigned int
      ...
      /*
       * Convective term treated implicitly: solve non-linear system of
       * equations
       */
      ...
      /*
       * This function evaluates the nonlinear residual of the steady
       * Navier-Stokes equations. This function has to be implemented separately
       * (for example, the convective term will be evaluated in case of the
       * Navier-Stokes equations and the time-derivative term is never
       * evaluated).
       */
      void
      ...
      /*
       * This function calculates the matrix-vector product for the linear(ized)
       * problem.
       */
      void
      ...
      /*
       * This function calculates the right-hand side of the steady Stokes
       * problem, or unsteady Stokes problem, or unsteady Navier-Stokes problem
       * with explicit treatment of the convective term. The parameter 'time'
       * has to be specified for unsteady problems and can be omitted for steady
       * problems.
       */
      void
      ...
      void
      apply_block_preconditioner(BlockVectorType       &dst,
                                 BlockVectorType const &src) const;
    ...
    private:
      ...
      void
      apply_preconditioner_velocity_block(VectorType       &dst,
                                          VectorType const &src) const;

      void
      apply_preconditioner_pressure_block(VectorType       &dst,
                                          VectorType const &src) const;

      void
      apply_inverse_negative_laplace_operator(VectorType       &dst,
                                              VectorType const &src) const;
      ...
      // temporary vector needed to evaluate both the nonlinear residual and the
      // linearized operator
