...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    template <int dim, typename Number>
    void
    OperatorPressureCorrection<dim, Number>::update_after_grid_motion(
    ...
    template <int dim, typename Number>
    void
    OperatorPressureCorrection<dim,
                               Number>::setup_inverse_mass_operator_pressure()
    {
      // inverse mass operator pressure (needed for pressure update in case of
      // rotational formulation)
      InverseMassOperatorData inverse_mass_operator_data_pressure;
      inverse_mass_operator_data_pressure.dof_index =
        this->get_dof_index_pressure();
      inverse_mass_operator_data_pressure.quad_index =
        this->get_quad_index_pressure();
      inverse_mass_operator_data_pressure.parameters =
        this->param.inverse_mass_operator;

      inverse_mass_pressure.initialize(this->get_matrix_free(),
                                       inverse_mass_operator_data_pressure);
    }
      ...
      double const     &time) const
    {
      ...
      if (this->param.implicit_nonlinear_convective_problem())
        {
          this->convective_operator.evaluate_nonlinear_operator_add(dst_u,
                                                                    src_u,
                                                                    time);
        }
    ...
    template <int dim, typename Number>
    void
    OperatorPressureCorrection<dim, Number>::apply_momentum_operator(
      VectorType       &dst,
    ...
    void
    OperatorPressureCorrection<dim, Number>::
      apply_inverse_pressure_mass_operator(VectorType       &dst,
    ...
    template <int dim, typename Number>
    unsigned int
    OperatorPressureCorrection<dim, Number>::solve_pressure(
      VectorType       &dst,
      VectorType const &src,
    ...
    template <int dim, typename Number>
    void
    OperatorPressureCorrection<dim, Number>::rhs_ppe_laplace_add(
      VectorType   &dst,
      double const &time) const
    {
    ...
    void
    OperatorPressureCorrection<dim, Number>::
      rhs_ppe_laplace_add_dirichlet_bc_from_dof_vector(
        VectorType       &dst,
        VectorType const &src) const
    {
        ...
        Range const &face_range) const
    {
      ...
      unsigned int const quad_index =
        this->get_quad_index_pressure_nodal_points();
              ...
              for (unsigned int q = 0; q < integrator.n_q_points; ++q)
                {
                  ...
                  auto bc = this->boundary_descriptor->pressure->dirichlet_bc
                              .find(boundary_id)
                              ->second;
                  auto q_points = integrator.quadrature_point(q);

                  scalar g = FunctionEvaluator<0, dim, Number>::value(
