...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    protected:
      ...
      typedef typename Poisson::MultigridPreconditioner<dim, Number, 1>
        MultigridPoisson;
    ...
    public:
       ...
       * single-field solvers. Another typical use case is the use of an ALE
       * formulation.
       */
      void
    ...
    protected:
      /*
       * This function initializes operators, preconditioners, and solvers
       * related to the solution of (non-)linear systems of equation required
       * for implicit formulations. It has to be extended by derived classes if
       * necessary.
       */
      virtual void
    ...
    public:
      ...
      /*
       * Prescribe initial conditions using a specified analytical/initial
       * solution function.
       */
      void
      ...
      // fills a DoF-vector (velocity) with values of traction on
      ...
      /*
       * Returns characteristic element length for high-order elements / shape
       * functions
       */
      double
      ...
      /*
       * For certain setups and types of boundary conditions, the pressure field
       * is only defined up to an additive constant which originates from the
       * fact that only the derivative of the pressure appears in the
       * incompressible Navier-Stokes equations. Examples of such a scenario are
       * problems where the velocity is prescribed on the whole boundary or
       * periodic boundaries.
       */

      // This function can be used to query whether the pressure level is
      // undefined.
      bool
      is_pressure_level_undefined() const;

      // This function adjust the pressure level, where different options are
      // available to fix the pressure level. The method selected by this
      // function depends on the specified parameter.
      void
      adjust_pressure_level_if_undefined(VectorType   &pressure,
                                         double const &time) const;
      ...
      /*
       * Computation of derived quantities which is needed for postprocessing
       * but some of them are also needed, e.g., for special splitting-type time
       * integration schemes.
       */
       ...
       */
      void
      update_projection_operator(VectorType const &velocity,
                                 double const      time_step_size) const;
       ...
       */
      double
      calculate_dissipation_convective_term(VectorType const &velocity,
                                            double const      time) const;
      ...
      /*
       *  Calls constraint_u.distribute(u) and updates the constrained DoFs of
       * the velocity field
       */
      void
    ...
    protected:
      ...
      /*
       * dealii::Mapping (In case of moving meshes (ALE), this is the dynamic
       * mapping describing the deformed configuration.)
       */
      std::shared_ptr<dealii::Mapping<dim> const> mapping;
      ...
      /*
       * In case of projection type incompressible Navier-Stokes solvers this
       * variable is needed to solve the pressure Poisson equation. However,
       * this variable is also needed in case of a coupled solution approach. In
       * that case, it is used for the block preconditioner (or more precisely
       * for the Schur-complement preconditioner and the preconditioner used to
       * approximately invert the Laplace operator).
       *
       * While the functions specified in BoundaryDescriptorLaplace are relevant
       * for projection-type solvers (pressure Poisson equation has to be
       * solved), the function specified in BoundaryDescriptorLaplace are
       * irrelevant for a coupled solution approach (since the pressure Poisson
       * operator is only needed for preconditioning, and hence, only the
       * homogeneous part of the operator has to be evaluated so that the
       * boundary conditions are never applied).
       *
       */
      ...
      /*
       * Element variable used to store the current physical time. This variable
       * is needed for the evaluation of certain integrals or weak forms.
       */
    ...
    private:
      ...
      dealii::AffineConstraints<Number> constraint_u, constraint_p,
        constraint_u_scalar;
      ...
      std::string const quad_index_u_overintegration =
      ...
      // If we want to be able to update the mapping, we need a pointer to a
      // non-const MatrixFree object. In case this object is created, we let the
      // above object called matrix_free point to matrix_free_own_storage. This
      // variable is needed for ALE formulations.
    ...
    protected:
      ...
      std::shared_ptr<Operators::ConvectiveKernel<dim, Number>>
                                                             convective_kernel;
      std::shared_ptr<Operators::ViscousKernel<dim, Number>> viscous_kernel;

      std::shared_ptr<Operators::DivergencePenaltyKernel<dim, Number>>
        div_penalty_kernel;
      std::shared_ptr<Operators::ContinuityPenaltyKernel<dim, Number>>
      ...
      // Elementwise solver/preconditioner used in case that only the divergence
      // penalty term is used and the system of equations is block-diagonal.
      typedef Elementwise::OperatorBase<dim, Number, ProjOperator>
        ELEMENTWISE_PROJ_OPERATOR;
      std::shared_ptr<ELEMENTWISE_PROJ_OPERATOR>
      ...
      std::shared_ptr<ELEMENTWISE_PRECONDITIONER>
        elementwise_preconditioner_projection;

      // global solver/preconditioner to be used if the continuity penalty term
      // is applied.
    ...
    private:
      ...
      void
      local_interpolate_stress_bc_boundary_face(
        dealii::MatrixFree<dim, Number> const &matrix_free,
        VectorType                            &dst,
        VectorType const                      &src,
        Range const                           &face_range) const;

      // Interpolation of stress requires velocity and pressure, but the
      // MatrixFree interface only provides one argument, so we store pointers
      // to have access to both velocity and pressure.
      ...
      mutable GeneralizedNewtonianModel<dim, Number>
