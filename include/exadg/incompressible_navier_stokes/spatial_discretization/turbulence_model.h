...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    public:
      ...
      /**
       * Function for *setting* the viscosity taking the viscosity stored in the
       * viscous_kernel's data as a basis.
       */
      void
      set_viscosity(VectorType const &velocity) const final;

      /**
       * Function for *adding to* the viscosity taking the currently stored
       * viscosity as a basis.
       */
      void
    ...
    private:
      ...
      /**
       *  This function adds the turbulent eddy-viscosity to the laminar
       * viscosity by using one of the implemented models.
       */
      void
      ...
      /**
       *  Vreman model (2004): Note that we only consider the isotropic variant
       * of the Vreman model:
       *
       ...
       *
       *  Note that if ||grad(u)||^{2} = 0, nu_SGS is consistently defined as
       * zero.
       *
       */
      void
      ...
      /**
       *  WALE (wall-adapting local eddy-viscosity) model (Nicoud & Ducros
       * 1999):
       *
       *    nu_SGS = (C * filter_width)^{2} * D ,
       *
