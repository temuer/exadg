  ...
  namespace IncNS
  {
    ...
    SpatialOperatorBase<dim, Number>::initialize_dof_handler_and_constraints()
    {
      ...
      if (param.spatial_discretization == SpatialDiscretization::L2)
        {
          fe_u = create_finite_element<dim>(param.grid.element_type,
                                            true,
                                            dim,
                                            param.degree_u);
        }
      else if (param.spatial_discretization == SpatialDiscretization::HDIV)
        {
          ...
          // The constructor of FE_RaviartThomas takes the degree in tangential
          // direction as an argument.
          fe_u = std::make_shared<dealii::FE_RaviartThomasNodal<dim>>(
            param.degree_u - 1);
        }
      else
        {
      ...
      if (param.spatial_discretization == SpatialDiscretization::HDIV)
        {
          // Periodic boundaries
          // We need to make sure the normal dofs are shared between cells on
          // the periodic boundaries, since these are continuous for HDIV.
          ...
          // Symmetry boundaries
          // Constraints the normal components of the velocity, where "0" as
          // second argument indicates the first component in the dof_handler.
          ...
          if (not(boundary_descriptor->velocity->dirichlet_bc.empty()))
            {
              ...
              // We would like to do something similar to above. Probably would
              // work with the same function i.e
              // dealii::VectorTools::project_boundary_values_div_conforming().
              // Otherwise one might want to look into:
      ...
      if (param.spatial_discretization == SpatialDiscretization::L2)
      ...
      else if (param.spatial_discretization == SpatialDiscretization::HDIV)
        {
          print_parameter(pcout,
                          "degree of 1D polynomials (normal)",
                          param.degree_u);
          print_parameter(pcout,
                          "degree of 1D polynomials (tangential)",
                          (param.degree_u - 1));
        }
      else
        {
      ...
      print_parameter(pcout,
                      ...
                      fe_u->n_dofs_per_cell());
      print_parameter(pcout, "number of dofs (total)", dof_handler_u.n_dofs());
      if (param.spatial_discretization == SpatialDiscretization::HDIV)
        {
          pcout
            << "NOTE. Continuity constraints in case of periodic boundary conditions"
            << "are not taken into account regarding the number of total DoFs."
            << std::endl;
        }
    ...
    {
      boundary_descriptor_laplace =
        std::make_shared<Poisson::BoundaryDescriptor<0, dim>>();

      // Dirichlet BCs for pressure
      boundary_descriptor_laplace->dirichlet_bc =
        boundary_descriptor->pressure->dirichlet_bc;

      // Neumann BCs for pressure: These boundary conditions are empty.
      // However, when using projection methods with the solution of a pressure
      // Poisson equation, the interface of the Laplace operator requires to set
      // functions on Neumann boundaries, which we simply fill by ZeroFunction.
      // In case that a projection method prescribes inhomogeneous Neumann
      // boundary conditions for the pressure (e.g. dual splitting projection
      // scheme), this is done by separate routines.
      for (typename std::set<dealii::types::boundary_id>::const_iterator it =
           ...
           ++it)
        {
          ...
          boundary_descriptor_laplace->neumann_bc.insert(
            std::pair<dealii::types::boundary_id,
                      std::shared_ptr<dealii::Function<dim>>>(*it,
                                                              zero_function));
        }
    }
    ...
    { // Erroneously, the boundary descriptor might contain too many boundary
      // IDs which
      // do not even exist in the triangulation. Here, we make sure that each
      // entry of the boundary descriptor has indeed a counterpart in the
      // triangulation.
      std::vector<dealii::types::boundary_id> boundary_ids =
        grid->triangulation->get_boundary_ids();
      for (auto it = boundary_descriptor->pressure->dirichlet_bc.begin();
           ...
           ++it)
        {
          bool const triangulation_has_boundary_id =
            std::find(boundary_ids.begin(), boundary_ids.end(), it->first) !=
            boundary_ids.end();

          AssertThrow(
            triangulation_has_boundary_id,
            dealii::ExcMessage(
              "The boundary descriptor for the pressure contains boundary IDs "
              "that are not part of the triangulation."));
        }

      pressure_level_is_undefined =
        boundary_descriptor->pressure->dirichlet_bc.empty();

      if (is_pressure_level_undefined())
        {
          if (param.adjust_pressure_level ==
              AdjustPressureLevel::ApplyAnalyticalSolutionInPoint)
            {
              dof_index_first_point = 0;
              ...
              if (dealii::Utilities::MPI::this_mpi_process(mpi_comm) == 0)
                {
                  ...
                  AssertThrow(processor_has_active_cells == true,
                              dealii::ExcMessage(
                                "No active cells on Processor with ID=0"));
              ...
              dof_index_first_point =
                dealii::Utilities::MPI::sum(dof_index_first_point, mpi_comm);
              for (unsigned int d = 0; d < dim; ++d)
                {
                  first_point[d] =
                    dealii::Utilities::MPI::sum(first_point[d], mpi_comm);
                }
            }
        }
    }
      ...
      MatrixFreeData<dim, Number> &matrix_free_data) const
    {
      ...
      matrix_free_data.insert_dof_handler(&dof_handler_u_scalar,
                                          field + dof_index_u_scalar);
      ...
      matrix_free_data.insert_constraint(&constraint_u_scalar,
                                         field + dof_index_u_scalar);
      ...
      std::shared_ptr<dealii::Quadrature<dim>> quadrature_p =
        create_quadrature<dim>(param.grid.element_type,
      ...
      if (param.grid.element_type == ElementType::Hypercube)
        {
          ...
          matrix_free_data.insert_quadrature(
            dealii::QGaussLobatto<1>(param.get_degree_p(param.degree_u) + 1),
      ...
      if (not(boundary_descriptor->velocity->dirichlet_cached_bc.empty()))
        {
          ...
          interface_data_dirichlet_cached =
            std::make_shared<ContainerInterfaceData<1, dim, double>>();
          interface_data_dirichlet_cached->setup(
            *matrix_free,
            get_dof_index_velocity(),
            quad_indices,
            boundary_descriptor->velocity->dirichlet_cached_bc);

          boundary_descriptor->velocity->set_dirichlet_cached_data(
            interface_data_dirichlet_cached);
        }
    }
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::initialize_operators(
      std::string const &dof_index_temperature)
    {
      ...
      inverse_mass_data_hdiv.parameters =
        this->param.inverse_mass_operator_hdiv;

      inverse_mass_hdiv.initialize(*matrix_free,
                                   constraint_u,
                                   inverse_mass_data_hdiv);

      // inverse mass operator
      if (param.spatial_discretization == SpatialDiscretization::L2)
        {
          InverseMassOperatorData inverse_mass_operator_data_velocity;
          inverse_mass_operator_data_velocity.dof_index =
            get_dof_index_velocity();
          inverse_mass_operator_data_velocity.quad_index =
            get_quad_index_velocity_standard();
          inverse_mass_operator_data_velocity.parameters =
            param.inverse_mass_operator;
      ...
      inverse_mass_operator_data_velocity_scalar.dof_index =
        get_dof_index_velocity_scalar();
      inverse_mass_operator_data_velocity_scalar.quad_index =
        get_quad_index_velocity_standard();
      inverse_mass_operator_data_velocity_scalar.parameters =
        param.inverse_mass_operator;
      inverse_mass_velocity_scalar.initialize(
        *matrix_free, inverse_mass_operator_data_velocity_scalar);

      // body force operator
      RHSOperatorData<dim> rhs_data;
      rhs_data.dof_index = get_dof_index_velocity();
      if (param.boussinesq_term)
        rhs_data.dof_index_scalar =
          matrix_free_data->get_dof_index(dof_index_temperature);
      ...
      rhs_data.kernel_data.boussinesq_dynamic_part_only =
        param.boussinesq_dynamic_part_only;
      rhs_data.kernel_data.thermal_expansion_coefficient =
        param.thermal_expansion_coefficient;
      rhs_data.kernel_data.reference_temperature = param.reference_temperature;
      rhs_data.kernel_data.gravitational_force =
        field_functions->gravitational_force;
      ...
      gradient_operator_data.integration_by_parts =
        param.gradp_integrated_by_parts;
      ...
      divergence_operator_data.integration_by_parts =
        param.divu_integrated_by_parts;
      ...
      convective_kernel_data.temporal_treatment =
        param.treatment_of_convective_term;
      convective_kernel_data.upwind_factor = param.upwind_factor;
      convective_kernel_data.use_outflow_bc =
        param.use_outflow_bc_convective_term;
      convective_kernel_data.type_dirichlet_bc =
        param.type_dirichlet_bc_convective;
      convective_kernel_data.ale = param.ale_formulation;
      convective_kernel =
        std::make_shared<Operators::ConvectiveKernel<dim, Number>>();
      ...
      convective_operator_data.quad_index =
        this->get_quad_index_velocity_linearized();
      convective_operator_data.use_cell_based_loops =
        param.use_cell_based_face_loops;
      convective_operator_data.quad_index_nonlinear =
        get_quad_index_velocity_overintegration();
      ...
      viscous_kernel_data.formulation_viscous_term =
        param.formulation_viscous_term;
      viscous_kernel_data.penalty_term_div_formulation =
      ...
      viscous_kernel_data.variable_normal_vector =
        param.neumann_with_variable_normal_vector;
      viscous_kernel =
        std::make_shared<Operators::ViscousKernel<dim, Number>>();
      ...
      if (param.generalized_newtonian_model_data.is_active)
        {
          generalized_newtonian_model.initialize(
            *matrix_free,
            viscous_kernel,
      ...
      viscous_operator_data.use_cell_based_loops =
        param.use_cell_based_face_loops;
      ...
      if (param.use_divergence_penalty)
        {
          ...
          div_penalty_data.type_penalty_parameter =
            param.type_penalty_parameter;
          ...
          div_penalty_kernel =
            std::make_shared<Operators::DivergencePenaltyKernel<dim, Number>>();
          ...
          div_penalty_operator.initialize(*matrix_free,
                                          operator_data,
                                          div_penalty_kernel);
        }
      ...
      if (param.use_continuity_penalty)
        {
          ...
          conti_penalty_kernel =
            std::make_shared<Operators::ContinuityPenaltyKernel<dim, Number>>();
          ...
          operator_data.dof_index  = get_dof_index_velocity();
          operator_data.quad_index = get_quad_index_velocity_standard();
          operator_data.use_boundary_data =
            param.continuity_penalty_use_boundary_data;
          operator_data.bc = this->boundary_descriptor->velocity;

          conti_penalty_operator.initialize(*matrix_free,
                                            operator_data,
                                            conti_penalty_kernel);
        }
      ...
      if (param.use_divergence_penalty or param.use_continuity_penalty)
        {
          if (param.temporal_discretization ==
                TemporalDiscretization::BDFDualSplittingScheme or
              param.temporal_discretization ==
                TemporalDiscretization::BDFPressureCorrection or
              (param.temporal_discretization ==
                 TemporalDiscretization::BDFCoupledSolution and
               param.apply_penalty_terms_in_postprocessing_step == true))
            {
              ...
              data.solver_block_diagonal = Elementwise::Solver::CG;
              data.preconditioner_block_diagonal =
                param.preconditioner_block_diagonal_projection;
              data.solver_data_block_diagonal =
                param.solver_data_block_diagonal_projection;

              projection_operator = std::make_shared<ProjOperator>();

              projection_operator->initialize(*matrix_free,
                                              constraint_dummy,
                                              data,
                                              div_penalty_kernel,
                                              conti_penalty_kernel);
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim,
                        Number>::initialize_calculators_for_derived_quantities()
    {
    ...
    SpatialOperatorBase<dim, Number>::setup()
    {
      ...
      // Subsequently, call the other setup function with
      // MatrixFree/MatrixFreeData objects as arguments.
    ...
    template <int dim, typename Number>
    unsigned int
    SpatialOperatorBase<dim, Number>::get_quad_index_velocity_overintegration()
      const
    {
      return matrix_free_data->get_quad_index(field +
                                              quad_index_u_overintegration);
    }

    template <int dim, typename Number>
    unsigned int
    SpatialOperatorBase<dim, Number>::get_quad_index_velocity_nodal_points()
      const
    {
      return matrix_free_data->get_quad_index(field +
                                              quad_index_u_nodal_points);
    }

    template <int dim, typename Number>
    unsigned int
    SpatialOperatorBase<dim, Number>::get_quad_index_pressure_nodal_points()
      const
    {
      return matrix_free_data->get_quad_index(field +
                                              quad_index_p_nodal_points);
    }
    ...
    {
      if (param.quad_rule_linearization ==
          QuadratureRuleLinearization::Standard)
        {
          return get_quad_index_velocity_standard();
        }
      else if (param.quad_rule_linearization ==
    ...
    SpatialOperatorBase<dim, Number>::get_viscosity_boundary_face(
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::set_velocity_ptr(
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::initialize_vector_velocity(
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::initialize_vector_velocity_scalar(
      VectorType &src) const
    {
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::initialize_vector_pressure(
      VectorType &src) const
    {
      ...
      BlockVectorType &src) const
    {
      ...
      matrix_free->initialize_dof_vector(src.block(0),
                                         get_dof_index_velocity());
      matrix_free->initialize_dof_vector(src.block(1),
      ...
      double const                                  time) const
    {
      ...
      Utilities::interpolate(
        *get_mapping(), dof_handler_u, *f_velocity, velocity, time);
      Utilities::interpolate(
        *get_mapping(), dof_handler_p, *f_pressure, pressure, time);
    }

    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::prescribe_initial_conditions(
      VectorType  &velocity,
      VectorType  &pressure,
      double const time) const
    {
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::interpolate_analytical_solution(
      VectorType  &velocity,
      VectorType  &pressure,
      double const time) const
    {
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::interpolate_stress_bc(
      VectorType       &stress,
    ...
    {
      return calculate_minimum_vertex_distance(
        dof_handler_u.get_triangulation(), *get_mapping(), mpi_comm);
    }
      ...
      unsigned int const order_time_integrator) const
    {
      ...
      return ExaDG::calculate_time_step_max_efficiency(h_min,
                                                       param.degree_u,
    ...
    template <int dim, typename Number>
    double
    SpatialOperatorBase<dim, Number>::calculate_time_step_cfl(
      VectorType const &velocity) const
    {
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::calculate_cfl_from_time_step(
      VectorType       &cfl,
      VectorType const &velocity,
    ...
    SpatialOperatorBase<dim, Number>::get_characteristic_element_length() const
    {
      ...
      return calculate_high_order_element_length(h_min,
                                                 param.degree_u,
                                                 true /* is_dg */);
    }
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::apply_mass_operator(
      VectorType       &dst,
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::apply_mass_operator_add(
      VectorType       &dst,
      VectorType const &src) const
    {
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::adjust_pressure_level_if_undefined(
      VectorType   &pressure,
      double const &time) const
    {
      if (is_pressure_level_undefined())
        {
          // If an analytical solution is available: shift pressure so that the
          // numerical pressure solution coincides with the analytical pressure
          // solution in an arbitrary point. Note that the parameter 'time' is
          // only needed for unsteady problems.
          if (this->param.adjust_pressure_level ==
              AdjustPressureLevel::ApplyAnalyticalSolutionInPoint)
            {
              field_functions->analytical_solution_pressure->set_time(time);
              double const exact =
                field_functions->analytical_solution_pressure->value(
                  first_point);

              double current = 0.;
              if (pressure.locally_owned_elements().is_element(
                    dof_index_first_point))
          ...
          else if (this->param.adjust_pressure_level ==
                   AdjustPressureLevel::ApplyZeroMeanValue)
            {
              dealii::VectorTools::subtract_mean_value(pressure);
            }
          // If an analytical solution is available: shift pressure so that the
          // numerical pressure solution has a mean value identical to the
          // "exact pressure solution" obtained by interpolation of analytical
          // solution. Note that the parameter 'time' is only needed for
          // unsteady problems.
          else if (this->param.adjust_pressure_level ==
                   AdjustPressureLevel::ApplyAnalyticalMeanValue)
            {
              // one cannot use Number as template here since Number might be
              // float while analytical_solution_pressure is of type
              // dealii::Function<dim,double>
              typedef dealii::LinearAlgebra::distributed::Vector<double>
                VectorTypeDouble;
          ...
          else
            {
              AssertThrow(
                false,
                dealii::ExcMessage(
                  "Specified method to adjust pressure level is not implemented."));
            }
        }
    }
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::set_temperature(
      VectorType const &temperature)
    {
      AssertThrow(param.boussinesq_term,
                  dealii::ExcMessage("Invalid parameters detected."));
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::compute_vorticity(
      VectorType       &dst,
      VectorType const &src) const
    {
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::compute_divergence(
      VectorType       &dst,
      VectorType const &src) const
    {
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::compute_shear_rate(
      VectorType       &dst,
      VectorType const &src) const
    {
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::compute_velocity_magnitude(
      VectorType       &dst,
      VectorType const &src) const
    {
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::compute_vorticity_magnitude(
      VectorType       &dst,
      VectorType const &src) const
    {
      magnitude_calculator.compute(dst, src);
    ...
    /*
     *  Streamfunction psi (2D only): defined as u1 = d(psi)/dx2, u2 = -
     * d(psi)/dx1
     *
     ...
     *      - laplace(psi) = omega
     *
     *  with homogeneous Dirichlet BC's (assumption: whole boundary ==
     * streamline)
     */
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::compute_streamfunction(
      VectorType       &dst,
      VectorType const &src) const
    {
      AssertThrow(
        dim == 2,
        dealii::ExcMessage(
          "Calculation of streamfunction can only be used for dim==2."));
      ...
      std::shared_ptr<Poisson::BoundaryDescriptor<0, dim>>
        boundary_descriptor_streamfunction;
      boundary_descriptor_streamfunction =
        std::make_shared<Poisson::BoundaryDescriptor<0, dim>>();

      // fill boundary descriptor: Assumption: only Dirichlet BC's
      boundary_descriptor_streamfunction->dirichlet_bc =
        boundary_descriptor->velocity->dirichlet_bc;

      AssertThrow(boundary_descriptor->velocity->neumann_bc.empty() == true,
                  dealii::ExcMessage(
                    "Assumption is not fulfilled. Streamfunction calculator is "
      ...
      typedef std::map<dealii::types::boundary_id, dealii::ComponentMask>
      ...
      Krylov::SolverCG<Laplace, PreconditionerBase<Number>, VectorType>
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::compute_q_criterion(
      VectorType       &dst,
      VectorType const &src) const
    {
    ...
    template <int dim, typename Number>
    unsigned int
    SpatialOperatorBase<dim, Number>::apply_inverse_mass_operator(
      VectorType       &dst,
      VectorType const &src) const
    {
      if (param.spatial_discretization == SpatialDiscretization::L2)
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::evaluate_add_body_force_term(
      VectorType  &dst,
      double const time) const
    {
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::evaluate_convective_term(
      VectorType       &dst,
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::evaluate_pressure_gradient_term(
      VectorType       &dst,
      VectorType const &src,
      double const      time) const
    {
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::evaluate_velocity_divergence_term(
      VectorType       &dst,
      VectorType const &src,
      double const      time) const
    {
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::update_viscosity(
      VectorType const &velocity) const
    {
      AssertThrow(
        param.viscous_problem() and param.viscosity_is_variable(),
        dealii::ExcMessage(
      ...
      // note that the apparent viscosity is used to compute the turbulent
      // viscosity, such that the *sequence of calls matters*, i.e., we can only
      // compute the turbulent viscosity once the laminar viscosity has been
      // computed
      if (param.turbulence_model_data.is_active)
    ...
    template <int dim, typename Number>
    double
    SpatialOperatorBase<dim, Number>::calculate_dissipation_convective_term(
      VectorType const &velocity,
      double const      time) const
    {
      if (param.convective_problem())
      ...
      bool const update_matrix_free)
    {
      if (update_matrix_free)
        {
          // Since matrix_free points to matrix_free_own_storage, we also update
          // the actual/main MatrixFree object called matrix_free.
          matrix_free_own_storage->update_mapping(*get_mapping());
        }

      if (param.turbulence_model_data.is_active)
        {
          // the mesh (and hence the filter width) changes in case of an ALE
          // formulation
      ...
      if (this->param.viscous_problem())
        {
          // update SIPG penalty parameter of viscous operator which depends on
          // the deformation of elements
          viscous_kernel->calculate_penalty_parameter(*matrix_free,
                                                      get_dof_index_velocity());
        }

      // The inverse mass operator might contain matrix-based components, in
      // which cases it needs to be updated after the grid has been deformed.
      inverse_mass_velocity.update();
      inverse_mass_velocity_scalar.update();

      // note that the update of div-div and continuity penalty terms is done
      // separately
    }

    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::set_grid_velocity(
      VectorType const &u_grid_in)
    {
    ...
    SpatialOperatorBase<dim, Number>::setup_projection_solver()
    {
      ...
      if (param.use_divergence_penalty == true and
          param.use_continuity_penalty == false)
        {
          ...
          if (param.preconditioner_projection == PreconditionerProjection::None)
            {
              typedef Elementwise::PreconditionerIdentity<
                dealii::VectorizedArray<Number>>
                IDENTITY;
          ...
          else if (param.preconditioner_projection ==
            ...
            {
              typedef Elementwise::
                JacobiPreconditioner<dim, dim, Number, ProjOperator>
                  JACOBI;
          ...
          else
            {
              AssertThrow(
                false,
                dealii::ExcMessage(
                  "The specified preconditioner is not implemented."));
            }
          ...
          if (param.solver_projection == SolverProjection::CG)
            {
              ...
              projection_solver_data.solver_data.abs_tol =
                param.solver_data_projection.abs_tol;
              projection_solver_data.solver_data.rel_tol =
                param.solver_data_projection.rel_tol;

              typedef Elementwise::PreconditionerBase<
                dealii::VectorizedArray<Number>>
                PROJ_PRECONDITIONER;
              ...
              projection_solver = std::make_shared<PROJ_SOLVER>(
                *std::dynamic_pointer_cast<ELEMENTWISE_PROJ_OPERATOR>(
                  elementwise_projection_operator),
                *std::dynamic_pointer_cast<PROJ_PRECONDITIONER>(
                  elementwise_preconditioner_projection),
          ...
          else
            {
              AssertThrow(param.solver_projection == SolverProjection::CG,
                          dealii::ExcMessage(
                            "Specified projection solver not implemented."));
            }
        }
      // continuity penalty term with/without divergence penalty term ->
      // globally coupled problem
      else if (param.use_continuity_penalty == true)
        {
          ...
          if (param.preconditioner_projection == PreconditionerProjection::None)
                   ...
                   PreconditionerProjection::InverseMassMatrix)
            {
              ...
              inverse_mass_operator_data.quad_index =
                get_quad_index_velocity_standard();
              inverse_mass_operator_data.parameters =
                param.inverse_mass_preconditioner;

              preconditioner_projection =
                std::make_shared<InverseMassPreconditioner<dim, dim, Number>>(
                  *matrix_free, inverse_mass_operator_data);
            }
          else if (param.preconditioner_projection ==
                   PreconditionerProjection::PointJacobi)
            {
              // Note that at this point (when initializing the Jacobi
              // preconditioner and calculating the diagonal) the penalty
              // parameter of the projection operator has not been calculated
              // and the time step size has not been set. Hence,
              // 'update_preconditioner = true' should be used for the Jacobi
              // preconditioner in order to use to correct diagonal for
              // preconditioning.
              preconditioner_projection =
                std::make_shared<JacobiPreconditioner<ProjOperator>>(
                  *projection_operator, false);
            }
          else if (param.preconditioner_projection ==
                   PreconditionerProjection::BlockJacobi)
            {
              // Note that at this point (when initializing the Jacobi
              // preconditioner) the penalty parameter of the projection
              // operator has not been calculated and the time step size has not
              // been set. Hence, 'update_preconditioner = true' should be used
              // for the Jacobi preconditioner in order to use to correct
              // diagonal blocks for preconditioning.
              preconditioner_projection =
                std::make_shared<BlockJacobiPreconditioner<ProjOperator>>(
                  *projection_operator, false);
            }
          else if (param.preconditioner_projection ==
                   PreconditionerProjection::Multigrid)
            {
              typedef MultigridPreconditionerProjection<dim, Number> Multigrid;

              preconditioner_projection =
                std::make_shared<Multigrid>(this->mpi_comm);
              ...
              // We also need to add DirichletCached boundary conditions. From
              // the perspective of multigrid, there is no difference between
              // standard and cached Dirichlet BCs. Since multigrid does not
              // need information about inhomogeneous boundary data, we simply
              // fill the map with dealii::Functions::ZeroFunction for
              // DirichletCached BCs.
              for (auto iter : this->projection_operator->get_data()
                                 .bc->dirichlet_cached_bc)
              ...
              mg_preconditioner->initialize(
                this->param.multigrid_data_projection,
                grid,
                multigrid_mappings,
                dof_handler.get_fe(),
          ...
          if (param.solver_projection == SolverProjection::CG)
            {
              ...
              if (param.preconditioner_projection !=
                  PreconditionerProjection::None)
                {
                  solver_data.use_preconditioner = true;
                }
              ...
              // setup solver
              projection_solver =
                std::make_shared<Krylov::SolverCG<ProjOperator,
          ...
          else if (param.solver_projection == SolverProjection::FGMRES)
            {
              ...
              solver_data.solver_tolerance_rel =
                param.solver_data_projection.rel_tol;
              solver_data.max_n_tmp_vectors =
                param.solver_data_projection.max_krylov_size;
              ...
              projection_solver = std::make_shared<
                Krylov::SolverFGMRES<ProjOperator,
                                     PreconditionerBase<Number>,
                                     VectorType>>(*projection_operator,
                                                  *preconditioner_projection,
                                                  solver_data);
            }
          else
        ...
        {
          AssertThrow(
            param.use_divergence_penalty == false and
              param.use_continuity_penalty == false,
            dealii::ExcMessage(
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::update_projection_operator(
      VectorType const &velocity,
      double const      time_step_size) const
    {
      AssertThrow(projection_operator.get() != 0,
                  dealii::ExcMessage(
                    "Projection operator is not initialized."));

      // Update projection operator, i.e., the penalty parameters that depend on
      // the velocity field and the time step size
      projection_operator->update(velocity, time_step_size);
    }

    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::rhs_add_projection_operator(
      VectorType  &dst,
      double const time) const
    {
    ...
    template <int dim, typename Number>
    unsigned int
    SpatialOperatorBase<dim, Number>::solve_projection(
      VectorType       &dst,
      VectorType const &src,
      ...
      Range const &face_range) const
    {
      ...
      for (unsigned int face = face_range.first; face < face_range.second;
           face++)
        {
          ...
          // a Dirichlet boundary for the fluid is a stress boundary for the
          // structure
          if (boundary_type == BoundaryTypeU::DirichletCached)
            {
              ...
              integrator_p.gather_evaluate(*pressure_ptr,
                                           dealii::EvaluationFlags::values);

              for (unsigned int q = 0; q < integrator_u.n_q_points; ++q)
                {
                  unsigned int const local_face_number =
                    matrix_free.get_face_info(face).interior_face_no;

                  unsigned int const index =
                    matrix_free.get_shape_info(dof_index_u, quad_index)
                      .face_to_cell_index_nodal[local_face_number][q];

                  // compute traction acting on structure with normal vector in
                  // opposite direction as compared to the fluid domain
                  ...
                  // incompressible flow solver is formulated in terms of
                  // kinematic viscosity and kinematic pressure
                  // -> multiply by density to get true traction in N/m^2.
                  vector traction =
                    param.density *
                    (viscosity * (grad_u + transpose(grad_u)) * normal -
                     p * normal);
    ...
    template <int dim, typename Number>
    void
    SpatialOperatorBase<dim, Number>::distribute_constraint_u(
