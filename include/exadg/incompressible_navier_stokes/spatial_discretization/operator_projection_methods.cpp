...
namespace ExaDG
{
  namespace IncNS
  {
      ...
      bool const update_matrix_free)
    {
      ...
      // update SIPG penalty parameter of Laplace operator which depends on the
    ...
    OperatorProjectionMethods<dim, Number>::initialize_laplace_operator()
    {
      ...
      /*
       * In case no Dirichlet boundary conditions as prescribed for the
       * pressure, the pressure Poisson equation is singular (i.e. vectors
       * describing a constant pressure state form the nullspace of the discrete
       * pressure Poisson operator). To solve the pressure Poisson equation in
       * that case, a Krylov subspace projection is applied during the solution
       * of the linear system of equations.
       *
       * Strictly speaking, this projection is only needed if the linear system
       * of equations
       *
       ...
       *  so that
       *
       *  p0^T * A * p = (A^T * p0)^T * p = 0 != p0 * rhs (since A is
       * symmetric).
       */
      if (this->param.temporal_discretization ==
          TemporalDiscretization::BDFDualSplittingScheme)
        {
          // the Krylov subspace projection is needed for the dual splitting
          // scheme since the linear system of equations is not consistent for
          // this splitting method (due to the boundary conditions).
        ...
        {
          // One can show that the linear system of equations of the pressure
          // Poisson equation is consistent in case of the pressure-correction
          // scheme if the velocity Dirichlet BC is consistent. So there should
          // be no need to solve a transformed linear system of equations.
          ...
          // specific parameters. Hence, for reasons of robustness we also solve
          // a transformed linear system of equations in case of the
      ...
      laplace_operator_data.use_cell_based_loops =
      ...
      laplace_operator_data.kernel_data.IP_factor =
        this->param.IP_factor_pressure;
    ...
    template <int dim, typename Number>
    void
    OperatorProjectionMethods<dim,
                              Number>::setup_preconditioner_pressure_poisson()
    {
      // setup preconditioner
      if (this->param.preconditioner_pressure_poisson ==
          PreconditionerPressurePoisson::None)
        {
          // do nothing, preconditioner will not be used
        }
      else if (this->param.preconditioner_pressure_poisson ==
               PreconditionerPressurePoisson::PointJacobi)
        {
          preconditioner_pressure_poisson = std::make_shared<
            JacobiPreconditioner<Poisson::LaplaceOperator<dim, Number, 1>>>(
            laplace_operator, true);
        }
      else if (this->param.preconditioner_pressure_poisson ==
               PreconditionerPressurePoisson::BlockJacobi)
        {
          preconditioner_pressure_poisson =
            std::make_shared<BlockJacobiPreconditioner<
               ...
               PreconditionerPressurePoisson::Multigrid)
        {
          ...
          std::shared_ptr<Multigrid> mg_preconditioner =
            std::dynamic_pointer_cast<Multigrid>(
              preconditioner_pressure_poisson);
    ...
    OperatorProjectionMethods<dim, Number>::setup_solver_pressure_poisson()
    {
      if (this->param.solver_pressure_poisson == SolverPressurePoisson::CG)
        {
          ...
          solver_data.max_iter =
            this->param.solver_data_pressure_poisson.max_iter;
          ...
          // setup solver
          pressure_poisson_solver = std::make_shared<
      ...
      else if (this->param.solver_pressure_poisson ==
               SolverPressurePoisson::FGMRES)
        {
          ...
          solver_data.max_n_tmp_vectors =
            this->param.solver_data_pressure_poisson.max_krylov_size;
          // use default value of update_preconditioner (=false)
          ...
          pressure_poisson_solver = std::make_shared<
            Krylov::SolverFGMRES<Poisson::LaplaceOperator<dim, Number, 1>,
                                 PreconditionerBase<Number>,
                                 VectorType>>(laplace_operator,
                                              *preconditioner_pressure_poisson,
                                              solver_data);
    ...
    {
      if (this->param.preconditioner_momentum ==
        ...
        {
          AssertThrow(this->param.preconditioner_momentum ==
                        MomentumPreconditioner::None,
      ...
      if (this->param.solver_momentum == SolverMomentum::CG)
        {
          ...
          momentum_linear_solver =
            std::make_shared<Krylov::SolverCG<MomentumOperator<dim, Number>,
                                              PreconditionerBase<Number>,
                                              VectorType>>(
      ...
      else if (this->param.solver_momentum == SolverMomentum::GMRES)
        {
          ...
          solver_data.solver_tolerance_rel =
            this->param.solver_data_momentum.rel_tol;
          solver_data.max_n_tmp_vectors =
            this->param.solver_data_momentum.max_krylov_size;
          ...
          // setup solver
          momentum_linear_solver =
            std::make_shared<Krylov::SolverGMRES<MomentumOperator<dim, Number>,
                                                 PreconditionerBase<Number>,
                                                 VectorType>>(
              this->momentum_operator,
              *momentum_preconditioner,
      ...
      else
        {
          AssertThrow(
            false,
            dealii::ExcMessage(
              "Specified solver for momentum equation is not implemented."));
        }


      // Navier-Stokes equations with an implicit treatment of the convective
      // term
      if (this->param.nonlinear_problem_has_to_be_solved())
        {
          ...
          // setup Newton solver
          momentum_newton_solver = std::make_shared<
            Newton::Solver<VectorType,
    ...
    template <int dim, typename Number>
    void
    OperatorProjectionMethods<dim, Number>::rhs_add_viscous_term(
      VectorType  &dst,
    ...
    template <int dim, typename Number>
    void
    OperatorProjectionMethods<dim, Number>::do_rhs_ppe_laplace_add(
      VectorType   &dst,
      double const &time) const
    {
    ...
    template <int dim, typename Number>
    unsigned int
    OperatorProjectionMethods<dim, Number>::do_solve_pressure(
      VectorType       &dst,
      ...
      bool const        update_preconditioner) const
    {
      ...
      //
      //  CheckMultigrid<dim, Number, Poisson::LaplaceOperator<dim, Number>,
      //  MultigridPoisson, MultigridNumber>
      ...
      this->pressure_poisson_solver->update_preconditioner(
    ...
    template <int dim, typename Number>
    void
    OperatorProjectionMethods<dim, Number>::apply_laplace_operator(
      VectorType       &dst,
      ...
      double const     &scaling_factor_mass)
    {
      ...
      this->momentum_linear_solver->update_preconditioner(
        update_preconditioner);

      auto linear_iterations =
        this->momentum_linear_solver->solve(solution, rhs);
      ...
      double const     &scaling_factor_mass)
    {
      ...
      std::tuple<unsigned int, unsigned int> iter =
        this->momentum_newton_solver->solve(dst, update);
      ...
      double const     &scaling_factor_mass) const
    {
      ...
      if (this->param.implicit_nonlinear_convective_problem())
        {
          this->convective_operator.evaluate_nonlinear_operator_add(dst,
                                                                    src,
                                                                    time);
        }
    ...
    template <int dim, typename Number>
    void
    OperatorProjectionMethods<dim, Number>::apply_projection_operator(
      VectorType       &dst,
      VectorType const &src) const
    {
      AssertThrow(this->projection_operator.get() != 0,
                  dealii::ExcMessage(
                    "Projection operator is not initialized correctly."));
