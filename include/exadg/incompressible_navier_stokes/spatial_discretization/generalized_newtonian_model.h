...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    public:
      ...
      initialize(dealii::MatrixFree<dim, Number> const &matrix_free_in,
                 std::shared_ptr<Operators::ViscousKernel<dim, Number>>
                   viscous_kernel_in,
                 GeneralizedNewtonianModelData const
       ...
       * viscous kernel's data as a basis.
       */
      void
    ...
    private:
       ...
       */
      void
      add_generalized_newtonian_viscosity(
        scalar       &viscosity,
       ...
       * with a factor [k + (l * y)^a]^[(n - 1) / a], which we denote as
       * `viscosity_factor`.
       */
      scalar
      ...
      void
      generalized_carreau_yasuda_model(scalar       &viscosity_factor,
