...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    template <int dim, typename Number>
    void
    TurbulenceModel<dim, Number>::set_viscosity(
      VectorType const &velocity) const
    {
      this->viscous_kernel->set_constant_coefficient(
        this->viscous_kernel->get_data().viscosity);
    ...
    template <int dim, typename Number>
    void
    TurbulenceModel<dim, Number>::add_viscosity(
      VectorType const &velocity) const
    {
      VectorType dummy;
      ...
      Range const      &cell_range) const
    {
      ...
      for (unsigned int cell = cell_range.first; cell < cell_range.second;
           ++cell)
        {
          ...
          scalar filter_width =
            integrator.read_cell_data(this->filter_width_vector);
          ...
          for (unsigned int q = 0; q < integrator.n_q_points; ++q)
            {
              ...
              scalar viscosity =
                this->viscous_kernel->get_viscosity_cell(cell, q);
      ...
      Range const      &face_range) const
    {
      ...
      for (unsigned int face = face_range.first; face < face_range.second;
           face++)
        {
          ...
          scalar filter_width =
            integrator_m.read_cell_data(this->filter_width_vector);
          scalar filter_width_neighbor =
            integrator_p.read_cell_data(this->filter_width_vector);

          // loop over all quadrature points
          for (unsigned int q = 0; q < integrator_m.n_q_points; ++q)
            {
              // calculate velocity gradient for both elements adjacent to the
              // current face
              ...
              scalar viscosity =
                this->viscous_kernel->get_coefficient_face(face, q);
              scalar viscosity_neighbor =
                this->viscous_kernel->get_coefficient_face_neighbor(face, q);
              ...
              this->viscous_kernel->set_coefficient_face_neighbor(
                face, q, viscosity_neighbor);
            }
        }
    }
    ...
    template <int dim, typename Number>
    void
    TurbulenceModel<dim, Number>::calculate_filter_width(
      dealii::Mapping<dim> const &mapping)
    {
      unsigned int n_cells = this->matrix_free->n_cell_batches() +
                             this->matrix_free->n_ghost_cell_batches();
      ...
      std::shared_ptr<dealii::Quadrature<dim>> quadrature =
        create_quadrature<dim>(element_type,
                               this->viscous_kernel->get_degree() + 1);
      ...
      for (unsigned int i = 0; i < n_cells; ++i)
        {
          for (unsigned int v = 0;
               v < this->matrix_free->n_active_entries_per_cell_batch(i);
               ++v)
            {
              typename dealii::DoFHandler<dim>::cell_iterator cell =
                this->matrix_free->get_cell_iterator(i,
                                                     v,
                                                     this->dof_index_velocity);
    ...
    template <int dim, typename Number>
    void
    TurbulenceModel<dim, Number>::add_turbulent_viscosity(
      scalar       &viscosity,
    ...
    template <int dim, typename Number>
    void
    TurbulenceModel<dim, Number>::smagorinsky_model(
      scalar const &filter_width,
      tensor const &velocity_gradient,
      double const &C,
    ...
    TurbulenceModel<dim, Number>::vreman_model(scalar const &filter_width,
                                               ...
                                               scalar       &viscosity) const
    {
      scalar velocity_gradient_norm_square =
      ...
      scalar B_gamma =
        +tensor[0][0] * tensor[1][1] - tensor[0][1] * tensor[0][1] +
      ...
      for (unsigned int i = 0; i < dealii::VectorizedArray<Number>::size(); i++)
        {
          // If the norm of the velocity gradient tensor is zero, the
          // subgrid-scale viscosity is defined as zero, so we do nothing in
          // that case. Make sure that B_gamma[i] is larger than zero since we
          // calculate the square root of B_gamma[i].
          if (velocity_gradient_norm_square[i] > tolerance and
              B_gamma[i] > tolerance)
            {
              viscosity[i] +=
                factor[i] * factor[i] *
                std::exp(0.5 * std::log(B_gamma[i] /
                                        velocity_gradient_norm_square[i]));
            }
        }
    }
    ...
    TurbulenceModel<dim, Number>::wale_model(scalar const &filter_width,
                                             ...
                                             scalar       &viscosity) const
    {
      tensor S = dealii::make_vectorized_array<Number>(0.5) *
                 (velocity_gradient + transpose(velocity_gradient));
      ...
      tensor S_d = dealii::make_vectorized_array<Number>(0.5) *
                     (square_gradient + transpose(square_gradient)) -
                   isotropic_tensor;
          ...
          if (S_d_norm_square[i] > tolerance)
            {
              D[i] = std::pow(S_d_norm_square[i], 1.5) /
                     (std::pow(S_norm_square[i], 2.5) +
                      std::pow(S_d_norm_square[i], 1.25));
            }
        }
                                              ...
                                              scalar       &viscosity) const
    {
       ...
       *  (see appendix in Nicoud et al. (2011)). This approach is more
       * efficient than calculating eigenvalues or singular values using LAPACK
       * routines.
       */
      scalar D = dealii::make_vectorized_array<Number>(0.0);
      ...
      for (unsigned int n = 0; n < dealii::VectorizedArray<Number>::size(); n++)
        {
          // if trace(G) = 0, all eigenvalues (and all singular values) have to
          // be zero and hence G is also zero. Set D[n]=0 in that case.
          if (invariant1[n] > 1.0e-12)
            {
              Number alpha1 =
                invariant1[n] * invariant1[n] / 9.0 - invariant2[n] / 3.0;
              Number alpha2 =
                invariant1[n] * invariant1[n] * invariant1[n] / 27.0 -
                invariant1[n] * invariant2[n] / 6.0 + invariant3[n] / 2.0;

              AssertThrow(
                alpha1 >= std::numeric_limits<
                            double>::denorm_min() /*smallest positive value*/,
                dealii::ExcMessage("alpha1 has to be larger than zero."));

              Number factor = alpha2 / std::pow(alpha1, 1.5);

              AssertThrow(std::abs(factor) <=
                            1.0 + 1.0e-12, /* we found that a larger tolerance
                                              (1e-8,1e-6,1e-4) might be
                                              necessary in some cases */
                          dealii::ExcMessage(
                            "Cannot compute arccos(value) if abs(value)>1.0."));
              ...
              sv[0] =
                invariant1[n] / 3.0 + 2 * std::sqrt(alpha1) * std::cos(alpha3);
              sv[1] = invariant1[n] / 3.0 -
                      2 * std::sqrt(alpha1) *
                        std::cos(dealii::numbers::PI / 3.0 + alpha3);
              sv[2] = invariant1[n] / 3.0 -
                      2 * std::sqrt(alpha1) *
                        std::cos(dealii::numbers::PI / 3.0 - alpha3);

              // Calculate the square root only if the value is larger than
              // zero. Otherwise set sv to zero (this is reasonable since
              // negative values will only occur due to numerical errors).
      ...
      //    scalar D_copy = D; // save a copy in order to verify the correctness
      //    of the computation for(unsigned int n = 0; n <
      //    dealii::VectorizedArray<Number>::size(); n++)
      //    {
      ...
      //      }
      //
      //      // This sorts the list in ascending order, beginning with the
      //      smallest eigenvalue. ev_list.sort();
      //
      ...
      //      for(it = ev_list.rbegin(), k=0; it != ev_list.rend() and k<dim;
      //      ++it, ++k)
      //      {
      //        ev[k] = std::sqrt(*it);
      //      }
      ...
      //    for(unsigned int n = 0; n < dealii::VectorizedArray<Number>::size();
      //    n++)
      //    {
      //      AssertThrow(std::abs(D[n]-D_copy[n])<1.e-5,dealii::ExcMessage("Calculation
      //      of singular values is incorrect."));
      //    }
       ...
       */
      //    scalar D_copy2 = D; // save a copy in order to verify the
      //    correctness of the computation D =
      ...
      //    n++)
      //    {
      //      AssertThrow(std::abs(D[n]-D_copy2[n])<1.e-5,dealii::ExcMessage("Calculation
      //      of singular values is incorrect."));
      //    }
