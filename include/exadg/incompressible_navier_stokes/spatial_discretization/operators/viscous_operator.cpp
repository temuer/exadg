...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    template <int dim, typename Number>
    void
    ViscousOperator<dim, Number>::reinit_face_derived(
    ...
    template <int dim, typename Number>
    void
    ViscousOperator<dim, Number>::reinit_boundary_face_derived(
      IntegratorFace    &integrator_m,
      unsigned int const face) const
    {
      (void)face;
    ...
    template <int dim, typename Number>
    void
    ViscousOperator<dim, Number>::do_cell_integral(
      IntegratorCell &integrator) const
    {
      for (unsigned int q = 0; q < integrator.n_q_points; ++q)
        {
          scalar viscosity =
            kernel->get_viscosity_cell(integrator.get_current_cell_index(), q);
          integrator.submit_gradient(
            kernel->get_volume_flux(integrator.get_gradient(q), viscosity), q);
        }
    }

    template <int dim, typename Number>
    void
    ViscousOperator<dim, Number>::do_face_integral(
      IntegratorFace &integrator_m,
      ...
      for (unsigned int q = 0; q < integrator_m.n_q_points; ++q)
        {
          ...
          scalar average_viscosity = kernel->get_viscosity_interior_face(
            integrator_m.get_current_cell_index(), q);
          tensor gradient_flux = kernel->calculate_gradient_flux(
            value_m, value_p, normal, average_viscosity);

          vector normal_gradient_m =
            kernel->calculate_normal_gradient(q, integrator_m);
          vector normal_gradient_p =
            kernel->calculate_normal_gradient(q, integrator_p);

          vector value_flux = kernel->calculate_value_flux(normal_gradient_m,
                                                           normal_gradient_p,
                                                           value_m,
                                                           value_p,
                                                           normal,
    ...
    template <int dim, typename Number>
    void
    ViscousOperator<dim, Number>::do_face_int_integral(
      IntegratorFace &integrator_m,
      IntegratorFace &integrator_p) const
    {
      (void)integrator_p;
    ...
    template <int dim, typename Number>
    void
    ViscousOperator<dim, Number>::do_face_ext_integral(
      IntegratorFace &integrator_m,
      IntegratorFace &integrator_p) const
    {
      (void)integrator_m;
      ...
      for (unsigned int q = 0; q < integrator_p.n_q_points; ++q)
        {
          ...
          vector value_flux = kernel->calculate_value_flux(normal_gradient_p,
                                                           normal_gradient_m,
                                                           value_p,
                                                           value_m,
                                                           normal_p,
      ...
      for (unsigned int q = 0; q < integrator.n_q_points; ++q)
        {
          ...
          scalar viscosity = kernel->get_viscosity_boundary_face(
            integrator.get_current_cell_index(), q);
          tensor gradient_flux = kernel->calculate_gradient_flux(value_m,
                                                                 value_p,
                                                                 normal,
                                                                 viscosity);

          vector normal_gradient_m =
            kernel->calculate_interior_normal_gradient(q,
                                                       integrator,
                                                       operator_type);
