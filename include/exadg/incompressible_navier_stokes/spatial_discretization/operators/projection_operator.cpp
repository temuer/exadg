  ...
  namespace IncNS
  {
      ...
      Operators::ContinuityPenaltyKernelData const &conti_kernel_data)
    {
      ...
      if (operator_data.use_divergence_penalty)
        this->integrator_flags =
          this->integrator_flags | div_kernel->get_integrator_flags();

      // continuity penalty
      if (operator_data.use_continuity_penalty)
        this->integrator_flags =
          this->integrator_flags | conti_kernel->get_integrator_flags();
    }
    ...
    ProjectionOperator<dim, Number>::get_velocity() const
    {
      AssertThrow(velocity != nullptr,
                  dealii::ExcMessage(
                    "Velocity ptr is not initialized in ProjectionOperator."));
    ...
    template <int dim, typename Number>
    void
    ProjectionOperator<dim, Number>::update(VectorType const &velocity,
                                            double const     &dt)
    {
    ...
    template <int dim, typename Number>
    void
    ProjectionOperator<dim, Number>::reinit_cell_derived(
    ...
    template <int dim, typename Number>
    void
    ProjectionOperator<dim, Number>::reinit_face_derived(
    ...
    template <int dim, typename Number>
    void
    ProjectionOperator<dim, Number>::reinit_boundary_face_derived(
      IntegratorFace    &integrator_m,
      unsigned int const face) const
    {
      (void)face;
      ...
      dealii::types::boundary_id const boundary_id) const
    {
      ...
      if (operator_data.use_continuity_penalty)
        conti_kernel->reinit_face_cell_based(boundary_id,
                                             integrator_m,
                                             integrator_p);
    }
    ...
    template <int dim, typename Number>
    void
    ProjectionOperator<dim, Number>::do_cell_integral(
      ...
      for (unsigned int q = 0; q < integrator.n_q_points; ++q)
        {
          ...
          if (operator_data.use_divergence_penalty)
            integrator.submit_divergence(
              time_step_size * div_kernel->get_volume_flux(integrator, q), q);
        }
    }

    template <int dim, typename Number>
    void
    ProjectionOperator<dim, Number>::do_face_integral(
      IntegratorFace &integrator_m,
      ...
      for (unsigned int q = 0; q < integrator_m.n_q_points; ++q)
        {
          ...
          vector flux =
            time_step_size * conti_kernel->calculate_flux(u_m, u_p, normal_m);
    ...
    template <int dim, typename Number>
    void
    ProjectionOperator<dim, Number>::do_face_int_integral(
      IntegratorFace &integrator_m,
      IntegratorFace &integrator_p) const
    {
      (void)integrator_p;
    ...
    template <int dim, typename Number>
    void
    ProjectionOperator<dim, Number>::do_face_ext_integral(
      IntegratorFace &integrator_m,
      IntegratorFace &integrator_p) const
    {
      (void)integrator_m;
      ...
      for (unsigned int q = 0; q < integrator_p.n_q_points; ++q)
        {
          ...
          vector flux =
            time_step_size * conti_kernel->calculate_flux(u_p, u_m, normal_p);
          ...
          for (unsigned int q = 0; q < integrator_m.n_q_points; ++q)
            {
              ...
              vector flux = time_step_size *
                            conti_kernel->calculate_flux(u_m, u_p, normal_m);
