...
namespace ExaDG
{
  namespace IncNS
  {
      ...
      MomentumOperatorData<dim> const         &data)
    {
      ...
      if (operator_data.unsteady_problem)
        this->integrator_flags =
          this->integrator_flags | this->mass_kernel->get_integrator_flags();
      if (operator_data.convective_problem)
        this->integrator_flags =
          this->integrator_flags |
          this->convective_kernel->get_integrator_flags();
      if (operator_data.viscous_problem)
        this->integrator_flags =
    ...
    template <int dim, typename Number>
    void
    MomentumOperator<dim, Number>::set_solution_linearization(
    ...
    template <int dim, typename Number>
    void
    MomentumOperator<dim, Number>::set_velocity_copy(
    ...
    template <int dim, typename Number>
    void
    MomentumOperator<dim, Number>::set_velocity_ptr(
      VectorType const &velocity) const
    {
      if (operator_data.convective_problem)
    ...
    template <int dim, typename Number>
    void
    MomentumOperator<dim, Number>::set_scaling_factor_mass_operator(
      Number const &number)
    {
    ...
    MomentumOperator<dim, Number>::rhs(VectorType &dst) const
    {
      ...
      AssertThrow(
        false,
        dealii::ExcMessage(
          "The function rhs() does not make sense for the momentum operator."));
    }
    ...
    template <int dim, typename Number>
    void
    MomentumOperator<dim, Number>::evaluate(VectorType       &dst,
    ...
    template <int dim, typename Number>
    void
    MomentumOperator<dim, Number>::evaluate_add(VectorType       &dst,
                                                VectorType const &src) const
    {
      (void)dst;
      (void)src;
    ...
    template <int dim, typename Number>
    void
    MomentumOperator<dim, Number>::reinit_cell_derived(
    ...
    template <int dim, typename Number>
    void
    MomentumOperator<dim, Number>::reinit_face_derived(
      ...
      unsigned int const face) const
    {
      ...
      if (operator_data.viscous_problem)
        viscous_kernel->reinit_face(integrator_m,
                                    integrator_p,
                                    operator_data.dof_index);
    }
    ...
    template <int dim, typename Number>
    void
    MomentumOperator<dim, Number>::reinit_boundary_face_derived(
      IntegratorFace    &integrator_m,
      unsigned int const face) const
    {
      if (operator_data.convective_problem)
      ...
      if (operator_data.viscous_problem)
        viscous_kernel->reinit_boundary_face(integrator_m,
                                             operator_data.dof_index);
    }
    ...
    template <int dim, typename Number>
    void
    MomentumOperator<dim, Number>::do_cell_integral(
      ...
      for (unsigned int q = 0; q < integrator.n_q_points; ++q)
        {
          ...
          vector value;
          if (this->integrator_flags.cell_evaluate &
          ...
          if (operator_data.unsteady_problem)
            {
              value_flux +=
                mass_kernel->get_volume_flux(scaling_factor_mass, value);
            }
          ...
          if (operator_data.convective_problem)
            {
              if (operator_data.convective_kernel_data.formulation ==
                ...
                {
                  gradient_flux +=
                    convective_kernel->get_volume_flux_divergence_formulation(
                      value, q);
                }
                ...
                {
                  value_flux +=
                    convective_kernel->get_volume_flux_convective_formulation(
                      value, gradient, q);
                }
              else
                {
          ...
          if (operator_data.viscous_problem)
            {
              scalar viscosity = viscous_kernel->get_viscosity_cell(
                integrator.get_current_cell_index(), q);
              gradient_flux +=
                viscous_kernel->get_volume_flux(gradient, viscosity);
            }

          if (this->integrator_flags.cell_integrate &
              dealii::EvaluationFlags::values)
    ...
    template <int dim, typename Number>
    void
    MomentumOperator<dim, Number>::do_face_integral(
      IntegratorFace &integrator_m,
          ...
          if (operator_data.viscous_problem)
            {
              scalar average_viscosity =
                viscous_kernel->get_viscosity_interior_face(
                  integrator_m.get_current_cell_index(), q);

              gradient_flux = viscous_kernel->calculate_gradient_flux(
                value_m, value_p, normal_m, average_viscosity);

              vector normal_gradient_m =
                viscous_kernel->calculate_normal_gradient(q, integrator_m);
              vector normal_gradient_p =
                viscous_kernel->calculate_normal_gradient(q, integrator_p);

              vector value_flux =
                viscous_kernel->calculate_value_flux(normal_gradient_m,
                                                     normal_gradient_p,
                                                     value_m,
                                                     value_p,
                                                     normal_m,
    ...
    template <int dim, typename Number>
    void
    MomentumOperator<dim, Number>::do_face_int_integral(
      IntegratorFace &integrator_m,
      IntegratorFace &integrator_p) const
    {
      (void)integrator_p;
          ...
          if (operator_data.convective_problem)
            {
              ...
              value_flux_m +=
                convective_kernel->calculate_flux_linear_operator_interior(
                  u_m, u_p, value_m, value_p, normal_m, q);
            }
    ...
    template <int dim, typename Number>
    void
    MomentumOperator<dim, Number>::do_face_int_integral_cell_based(
      IntegratorFace &integrator_m,
      IntegratorFace &integrator_p) const
    {
      (void)integrator_p;
          ...
          if (operator_data.convective_problem)
            {
              vector u_m = convective_kernel->get_velocity_m(q);
              // TODO
              // Accessing exterior data is currently not available in
              // deal.II/matrixfree. Hence, we simply use the interior value,
              // but note that the diagonal and block-diagonal are not
              // calculated exactly. Note that the present implementation also
              // does not take into account boundary conditions on boundary
              // faces.
              vector u_p = u_m;
    ...
    template <int dim, typename Number>
    void
    MomentumOperator<dim, Number>::do_face_ext_integral(
      IntegratorFace &integrator_m,
      IntegratorFace &integrator_p) const
    {
      (void)integrator_m;
          ...
          if (operator_data.viscous_problem)
            {
              scalar average_viscosity =
                viscous_kernel->get_viscosity_interior_face(
                  integrator_p.get_current_cell_index(), q);

              gradient_flux += viscous_kernel->calculate_gradient_flux(
                value_p, value_m, normal_p, average_viscosity);
              ...
              vector value_flux =
                viscous_kernel->calculate_value_flux(normal_gradient_p,
                                                     normal_gradient_m,
                                                     value_p,
                                                     value_m,
                                                     normal_p,
      ...
      dealii::types::boundary_id const &boundary_id) const
    {
      BoundaryTypeU boundary_type =
        operator_data.bc->get_boundary_type(boundary_id);

      for (unsigned int q = 0; q < integrator.n_q_points; ++q)
        {
          // use function calculate_interior_value() instead of
          // integrator.get_value() since OperatorType might be homogeneous or
          // inhomogeneous
          vector value_m =
            calculate_interior_value(q, integrator, operator_type);
          ...
          if (operator_data.convective_problem)
            {
              // value_p is calculated differently for the convective term and
              // the viscous term (e.g. for the convective term we distinguish
              // between the mirror principle vs. direct imposition of boundary
              // conditions while we always use the mirror principle for the
              // viscous term)
              value_p = calculate_exterior_value_convective(
                value_m,
                q,
                integrator,
                operator_type,
              ...
              else if (operator_data.convective_kernel_data
                         .temporal_treatment ==
                       TreatmentOfConvectiveTerm::LinearlyImplicit)
                {
                  // for a linearly implicit treatment of the convective term,
                  // we do not impose boundary conditions for the transport
                  // velocity u. Boundary conditions are only imposed for the
              ...
              value_flux_m +=
                convective_kernel->calculate_flux_linear_operator_boundary(
          ...
          if (operator_data.viscous_problem)
            {
              ...
              scalar viscosity = viscous_kernel->get_viscosity_boundary_face(
                integrator.get_current_cell_index(), q);
              gradient_flux += viscous_kernel->calculate_gradient_flux(
                value_m, value_p, normal_m, viscosity);

              vector normal_gradient_m =
                viscous_kernel->calculate_interior_normal_gradient(
                  q, integrator, operator_type);

              vector normal_gradient_p;

              normal_gradient_p = calculate_exterior_normal_gradient(
                normal_gradient_m,
                q,
                integrator,
                operator_type,
