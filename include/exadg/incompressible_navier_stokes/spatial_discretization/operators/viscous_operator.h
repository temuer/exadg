    ...
    namespace Operators
    {
      ...
      public:
               ...
               unsigned int const                     quad_index)
        {
          ...
          AssertThrow(data.viscosity >= 0.0,
                      dealii::ExcMessage("Viscosity is not set!"));

          if (data.viscosity_is_variable)
            {
              // allocate vectors for variable coefficients and initialize with
              // constant viscosity
              viscosity_coefficients.initialize(matrix_free,
                                                quad_index,
                                                true,
                                                false);
        ...
        scalar
        get_coefficient_face_neighbor(unsigned int const face,
                                      unsigned int const q)
        {
        ...
        /*
         * This functions return the viscosity for a given cell in a given
         * quadrature point
         */
        ...
        {
          scalar viscosity =
            dealii::make_vectorized_array<Number>(data.viscosity);
                                      ...
                                      unsigned int const q) const
        {
          ...
          // harmonic mean (harmonic weighting according to Schott and Rasthofer
          // et al. (2015))
          average_viscosity = 2.0 * coefficient_face *
                              coefficient_face_neighbor /
                              (coefficient_face + coefficient_face_neighbor);

          // arithmetic mean
          // average_viscosity = 0.5 * (coefficient_face +
          // coefficient_face_neighbor);

          // maximum value
          // average_viscosity = std::max(coefficient_face,
          // coefficient_face_neighbor);
        ...
        {
          if (data.formulation_viscous_term ==
              FormulationViscousTerm::DivergenceFormulation)
            {
              return viscosity * (gradient + transpose(gradient));
            }
          else if (data.formulation_viscous_term ==
            ...
            {
              AssertThrow(
                data.formulation_viscous_term ==
                    FormulationViscousTerm::DivergenceFormulation or
                  data.formulation_viscous_term ==
                    FormulationViscousTerm::LaplaceFormulation,
                dealii::ExcMessage(
        ...
        /**
         * Calculates the `gradient flux`, i.e. the numerical flux that is
         * tested with the shape function gradients.
         */
        inline DEAL_II_ALWAYS_INLINE //
          tensor
          ...
          if (data.formulation_viscous_term ==
                   ...
                   FormulationViscousTerm::DivergenceFormulation)
            {
              if (data.IP_formulation == InteriorPenaltyFormulation::NIPG)
                {
                  flux =
                    0.5 * viscosity * (jump_tensor + transpose(jump_tensor));
                }
              else if (data.IP_formulation == InteriorPenaltyFormulation::SIPG)
                {
                  flux =
        ...
        /*
         *  This function calculates the gradient in normal direction on element
         * e depending on the formulation of the viscous term.
         */
        template <typename Integrator>
        inline DEAL_II_ALWAYS_INLINE //
          vector
          calculate_normal_gradient(unsigned int const q,
                                    Integrator        &integrator) const
        {
          ...
          if (data.formulation_viscous_term ==
              FormulationViscousTerm::DivergenceFormulation)
            {
               ...
               */
              gradient = dealii::make_vectorized_array<Number>(2.0) *
        ...
        /**
         * Calculates the `value flux`, i.e. the flux that is tested with the
         * shape function values. Strictly speaking, this is not a numerical
         * flux, but the right-hand side of the L2 product notation of the
         * integral. In this case it is the inner product of the numerical flux
         * and the the normal vector of element e⁻.
         */
        inline DEAL_II_ALWAYS_INLINE //
          vector
                               ...
                               scalar const &viscosity) const
        {
          ...
          vector jump_value = value_m - value_p;
          vector average_normal_gradient =
            0.5 * (normal_gradient_m + normal_gradient_p);

          if (data.formulation_viscous_term ==
              FormulationViscousTerm::DivergenceFormulation)
            {
              if (data.penalty_term_div_formulation ==
                  PenaltyTermDivergenceFormulation::Symmetrized)
                {
                  flux = viscosity * average_normal_gradient -
                         viscosity * tau *
                           (jump_value + (jump_value * normal) * normal);
                }
              else if (data.penalty_term_div_formulation ==
                       PenaltyTermDivergenceFormulation::NotSymmetrized)
                {
                  flux = viscosity * average_normal_gradient -
                         viscosity * tau * jump_value;
                }
              else
                {
                  AssertThrow(
                    data.penalty_term_div_formulation ==
                        PenaltyTermDivergenceFormulation::Symmetrized or
                      data.penalty_term_div_formulation ==
                        PenaltyTermDivergenceFormulation::NotSymmetrized,
                    dealii::ExcMessage(
                      "Specified formulation of viscous term is not implemented."));
