  ...
  namespace IncNS
  {
    namespace Operators
    {
      ...
      public:
               ...
               bool const                             use_own_velocity_storage)
        {
          ...
          integrator_velocity =
            std::make_shared<IntegratorCell>(matrix_free,
                                             dof_index,
                                             quad_index_linearized);
          integrator_velocity_m = std::make_shared<IntegratorFace>(
            matrix_free, true, dof_index, quad_index_linearized);
          integrator_velocity_p = std::make_shared<IntegratorFace>(
            matrix_free, false, dof_index, quad_index_linearized);
          ...
          if (data.ale)
            {
              integrator_grid_velocity =
                std::make_shared<IntegratorCell>(matrix_free,
                                                 dof_index,
                                                 quad_index_linearized);
              integrator_grid_velocity_face = std::make_shared<IntegratorFace>(
                matrix_free, true, dof_index, quad_index_linearized);
            }
        ...
        /*
         * IntegratorFlags valid for both the nonlinear convective operator and
         * the linearized convective operator
         */
        IntegratorFlags
        ...
        reinit_cell(unsigned int const cell) const
        {
          ...
          if (data.formulation ==
              FormulationConvectiveTerm::DivergenceFormulation)
            {
              integrator_velocity->gather_evaluate(
                *velocity, dealii::EvaluationFlags::values);
            }
          else if (data.formulation ==
                   FormulationConvectiveTerm::ConvectiveFormulation)
            {
              ...
              if (data.ale)
                integrator_grid_velocity->gather_evaluate(
                  *grid_velocity, dealii::EvaluationFlags::values);
            }
          else
            {
              AssertThrow(false, dealii::ExcMessage("Not implemented."));
        ...
        reinit_face(unsigned int const face) const
        {
          ...
          integrator_velocity_m->gather_evaluate(
            *velocity, dealii::EvaluationFlags::values);

          integrator_velocity_p->reinit(face);
          integrator_velocity_p->gather_evaluate(
            *velocity, dealii::EvaluationFlags::values);

          if (data.ale)
            {
              integrator_grid_velocity_face->reinit(face);
              integrator_grid_velocity_face->gather_evaluate(
                *grid_velocity, dealii::EvaluationFlags::values);
            }
        }
          ...
          dealii::types::boundary_id const boundary_id) const
        {
          ...
          if (boundary_id ==
            ...
            {
              // TODO: Matrix-free implementation in deal.II does currently not
              // allow to access data of the neighboring element in case of
        ...
        /*
         * Volume flux, i.e., the term occurring in the volume integral. The
         * volume flux depends on the formulation used for the convective term,
         * and is therefore implemented separately for the different
         * formulations (divergence formulation vs. convective formulation).
         * Note that these functions are called by the linearized or linearly
         * implicit convective operator, but not by the nonlinear convective
         * operator.
         */
        inline DEAL_II_ALWAYS_INLINE //
          tensor
          get_volume_flux_divergence_formulation(vector const      &delta_u,
                                                 unsigned int const q) const
        {
          // u denotes the point of linearization for the linearized problem (of
          // the nonlinear implicit operator) or the transport velocity for the
          ...
          else if (data.temporal_treatment ==
                                                 ...
                                                 unsigned int const q) const
        {
          // u denotes the point of linearization for the linearized problem (of
          // the nonlinear implicit operator) or the transport velocity for the
          // linearly implicit problem in case no ALE formulation is used.

          // The velocity w also takes into account the grid velocity u_grid in
          // case of an ALE formulation.
        ...
        /*
         *  Calculates the flux for nonlinear operator on interior faces. This
         * function is needed for face-centric loops and the flux is therefore
         * computed on both sides of an interior face. The interior flux
         * (element m) is the first element in the tuple, the exterior flux
         * (element p, neighbor) is the second element in the tuple.
         */
        inline DEAL_II_ALWAYS_INLINE //
          std::tuple<vector, vector>
          calculate_flux_nonlinear_interior_and_neighbor(
            vector const &uM,
            vector const &uP,
            ...
            vector const &u_grid) const
        {
          ...
          if (data.formulation ==
                   ...
                   FormulationConvectiveTerm::ConvectiveFormulation)
            {
              ...
              // a second term is needed since the strong formulation is
              // implemented (integration by parts twice)
              flux_m = flux - average_u_normal * uM;
              flux_p =
                -flux + average_u_normal * uP; // opposite signs since n⁺ = - n⁻
            }
          else
            {
              AssertThrow(false, dealii::ExcMessage("Not implemented."));
        ...
        /*
         *  Calculates the flux for nonlinear operator on boundary faces. The
         * flux computation used on interior faces has to be "corrected" if a
         * special outflow boundary condition is used on Neumann boundaries that
         * is able to deal with backflow.
         */
        inline DEAL_II_ALWAYS_INLINE //
          vector
          calculate_flux_nonlinear_boundary(
            vector const        &uM,
            vector const        &uP,
            vector const        &normalM,
            vector const        &u_grid,
            BoundaryTypeU const &boundary_type) const
        {
          ...
          if (data.formulation ==
              FormulationConvectiveTerm::DivergenceFormulation)
            {
              ...
              if (boundary_type == BoundaryTypeU::Neumann and
                  data.use_outflow_bc == true)
                   ...
                   FormulationConvectiveTerm::ConvectiveFormulation)
            {
              ...
              // second term appears since the strong formulation is implemented
              // (integration by parts is performed twice)
        ...
        /*
         *  Calculates the flux for linear operator on interior faces. This
         * function is needed for face-centric loops and the flux is therefore
         * computed on both sides of an interior face. The interior flux
         * (element m) is the first element in the tuple, the exterior flux
         * (element p, neighbor) is the second element in the tuple.
        ...
        inline DEAL_II_ALWAYS_INLINE //
          std::tuple<vector, vector>
          calculate_flux_linear_operator_interior_and_neighbor(
            vector const      &uM,
            vector const      &uP,
            ...
            unsigned int const q) const
        {
          ...
          if (data.formulation ==
              FormulationConvectiveTerm::DivergenceFormulation)
            {
              if (data.temporal_treatment ==
                  TreatmentOfConvectiveTerm::Implicit)
                {
                  // linearization of nonlinear convective term

                  fluxM = calculate_lax_friedrichs_flux_linearized(
                    uM, uP, delta_uM, delta_uP, normalM);
                  fluxP = -fluxM;
                }
              else if (data.temporal_treatment ==
                       TreatmentOfConvectiveTerm::LinearlyImplicit)
                {
                  fluxM = calculate_lax_friedrichs_flux_linear_transport(
                    uM, uP, delta_uM, delta_uP, normalM);
                  fluxP = -fluxM;
                }
              else
                   ...
                   FormulationConvectiveTerm::ConvectiveFormulation)
            {
              ...
              if (data.temporal_treatment ==
                  TreatmentOfConvectiveTerm::Implicit)
                {
                  ...
                  vector flux = calculate_upwind_flux_linearized(
                    uM, uP, u_grid, delta_uM, delta_uP, normalM);

                  scalar average_u_normal = 0.5 * (uM + uP) * normalM;
                  if (data.ale)
                    average_u_normal -= u_grid * normalM;

                  scalar average_delta_u_normal =
                    0.5 * (delta_uM + delta_uP) * normalM;

                  // second term appears since the strong formulation is
                  // implemented (integration by parts is performed twice)
                  fluxM = flux - average_u_normal * delta_uM -
                          average_delta_u_normal * uM;
                  // opposite signs since n⁺ = - n⁻
                  fluxP = -flux + average_u_normal * delta_uP +
                          average_delta_u_normal * uP;
                }
              else if (data.temporal_treatment ==
                       TreatmentOfConvectiveTerm::LinearlyImplicit)
                {
                  ...
                  flux =
                    calculate_upwind_flux(delta_uM, delta_uP, average_u_normal);

                  // a second term is needed since the strong formulation is
                  // implemented (integration by parts twice)
                  fluxM = flux - average_u_normal * delta_uM;
                  fluxP = -flux + average_u_normal *
                                    delta_uP; // opposite signs since n⁺ = - n⁻
                }
              else
                {
                  AssertThrow(false, dealii::ExcMessage("not implemented"));
        ...
        /*
         *  Calculates the flux for linear operator on interior faces. Only the
         * flux on element e⁻ is computed.
         */
        inline DEAL_II_ALWAYS_INLINE //
          vector
          ...
          if (data.formulation ==
              FormulationConvectiveTerm::DivergenceFormulation)
            {
              if (data.temporal_treatment ==
                ...
                {
                  // linearization of nonlinear convective term
                  flux = calculate_lax_friedrichs_flux_linearized(
                    uM, uP, delta_uM, delta_uP, normalM);
                }
              else if (data.temporal_treatment ==
                       TreatmentOfConvectiveTerm::LinearlyImplicit)
                {
                  flux = calculate_lax_friedrichs_flux_linear_transport(
                    uM, uP, delta_uM, delta_uP, normalM);
                }
              else
                {
                   ...
                   FormulationConvectiveTerm::ConvectiveFormulation)
            {
              ...
              if (data.temporal_treatment ==
                  TreatmentOfConvectiveTerm::Implicit)
                {
                  ...
                  flux = calculate_upwind_flux_linearized(
                    uM, uP, u_grid, delta_uM, delta_uP, normalM);
        ...
        /*
         *  Calculates the flux for linear operator on boundary faces. The only
         * reason why this function has to be implemented separately is the fact
         * that the flux computation used on interior faces has to be
         * "corrected" if a special outflow boundary condition is used on
         * Neumann boundaries that is able to deal with backflow.
         */
        inline DEAL_II_ALWAYS_INLINE //
          vector
          calculate_flux_linear_operator_boundary(
            vector const        &uM,
            vector const        &uP,
            vector const        &delta_uM,
            vector const        &delta_uP,
         ...
         *  lambda = max ( max |lambda(flux_jacobian_M)| , max
         * |lambda(flux_jacobian_P)| )
        ...
        {
          return data.upwind_factor * 2.0 *
                 std::max(std::abs(uM_n), std::abs(uP_n));
        }

        /*
         *  Divergence formulation: Calculate Lax-Friedrichs flux for nonlinear
         * operator.
         */
        inline DEAL_II_ALWAYS_INLINE //
          vector
        ...
        /*
         *  Divergence formulation: Calculate Lax-Friedrichs flux for linearly
         * implicit formulation (linear transport with transport velocity w).
         */
        inline DEAL_II_ALWAYS_INLINE //
          vector
          calculate_lax_friedrichs_flux_linear_transport(
            vector const &wM,
            vector const &wP,
            vector const &uM,
            vector const &uP,
            vector const &normalM) const
        {
          ...
          // the function calculate_lambda() is for the nonlinear operator with
          // quadratic nonlinearity. In case of linear transport, lambda is
        ...
        /*
         *  Divergence formulation:  Calculate Lax-Friedrichs flux for
         * linearized operator.
         */
        inline DEAL_II_ALWAYS_INLINE //
          vector
                                                   ...
                                                   vector const &normalM) const
        {
          ...
          vector average_normal_flux =
            dealii::make_vectorized_array<Number>(0.5) *
            (uM * delta_uM_n + delta_uM * uM_n + uP * delta_uP_n +
             delta_uP * uP_n);
        ...
        /*
         * Convective formulation: Calculate upwind flux given an average normal
         * velocity. This function is used for the nonlinear operator and for
         * the linearly implicit treatment of the convective term.
         */
        inline DEAL_II_ALWAYS_INLINE //
          vector
                                ...
                                scalar const &average_normal_velocity) const
        {
          ...
          return (average_normal_velocity * average_velocity +
                  data.upwind_factor * 0.5 * std::abs(average_normal_velocity) *
                    jump_value);
        }

        /*
         *  Convective formulation: Calculate upwind flux for linearized
         * operator.
         */
        inline DEAL_II_ALWAYS_INLINE //
          vector
                                           ...
                                           vector const &normalM) const
        {
          ...
          scalar delta_average_normal_velocity =
            delta_average_velocity * normalM;
          ...
          apply_outflow_bc(vector &flux, scalar const &normal_velocity) const
        {
          ...
          // outflow: factor = 1.0 (do nothing, neutral element of
          // multiplication) inflow:  factor = 0.0 (set convective flux to zero)
      ...
      private:
        ...
        // linearization velocity for nonlinear problems or transport velocity
        // for "linearly implicit formulation" of convective term
    ...
    struct ConvectiveOperatorData : public OperatorBaseData
    {
      ConvectiveOperatorData()
        : OperatorBaseData()
        , quad_index_nonlinear(0)
      {}

      /*
       * Needs ConvectiveKernelData since it is not possible to remove all
       * kernel parameters from function do_cell_integral(), i.e. the parameter
       * FormulationConvectiveTerm is explicitly needed by ConvectiveOperator.
       */
      Operators::ConvectiveKernelData kernel_data;

      /*
       * In addition to the quad_index in OperatorBaseData (which denotes the
       * quadrature index for the linearized problem), an additional quadrature
       * index has to be specified for the convective operator evaluation in
       * case of explicit time integration or nonlinear residual evaluation).
       */
    ...
    public:
      ...
      // these functions are not implemented for the convective operator. They
      // only have to exist due to the definition of the base class.
      void
    ...
    private:
      ...
      // function should be removed again.
      void
