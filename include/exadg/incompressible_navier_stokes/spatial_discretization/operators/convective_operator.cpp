...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    void
    ConvectiveOperator<dim, Number>::set_velocity_copy(
    ...
    template <int dim, typename Number>
    void
    ConvectiveOperator<dim, Number>::set_velocity_ptr(
      VectorType const &src) const
    {
    ...
    template <int dim, typename Number>
    void
    ConvectiveOperator<dim, Number>::evaluate_nonlinear_operator(
      VectorType       &dst,
      ...
      Number const      time) const
    {
      ...
      this->matrix_free->loop(
        &This::cell_loop_nonlinear_operator,
    ...
    template <int dim, typename Number>
    void
    ConvectiveOperator<dim, Number>::evaluate_nonlinear_operator_add(
      VectorType       &dst,
      VectorType const &src,
      Number const      time) const
    {
    ...
    template <int dim, typename Number>
    void
    ConvectiveOperator<dim, Number>::evaluate(VectorType       &dst,
                                              VectorType const &src) const
    {
      (void)dst;
      (void)src;
    ...
    template <int dim, typename Number>
    void
    ConvectiveOperator<dim, Number>::evaluate_add(VectorType       &dst,
                                                  VectorType const &src) const
    {
      (void)dst;
      (void)src;
      ...
      Range const                           &cell_range) const
    {
      ...
      IntegratorCell integrator_grid_velocity(
        matrix_free,
        operator_data.dof_index,
        operator_data.quad_index_nonlinear);
      ...
      for (unsigned int cell = cell_range.first; cell < cell_range.second;
           ++cell)
        {
          ...
          // Strictly speaking, the variable integrator_flags refers to the
          // linear operator, but integrator_flags is also valid for the
          // nonlinear operator.
          ...
          if (operator_data.kernel_data.ale)
            {
              ...
              integrator_grid_velocity.gather_evaluate(
                kernel->get_grid_velocity(), dealii::EvaluationFlags::values);
            }

          do_cell_integral_nonlinear_operator(integrator,
                                              integrator_grid_velocity);

          integrator.integrate_scatter(this->integrator_flags.cell_integrate,
                                       dst);
        }
    }
      ...
      for (unsigned int face = face_range.first; face < face_range.second;
           face++)
        {
          ...
          integrator_m.gather_evaluate(src,
                                       this->integrator_flags.face_evaluate);
          ...
          do_face_integral_nonlinear_operator(integrator_m,
                                              integrator_p,
                                              integrator_grid_velocity);

          integrator_m.integrate_scatter(this->integrator_flags.face_integrate,
                                         dst);

          integrator_p.integrate_scatter(this->integrator_flags.face_integrate,
                                         dst);
        }
    }
      ...
      for (unsigned int q = 0; q < integrator.n_q_points; ++q)
        {
          ...
          if (operator_data.kernel_data.formulation ==
      ...
      for (unsigned int q = 0; q < integrator_m.n_q_points; ++q)
        {
          ...
          std::tuple<vector, vector> flux =
            kernel->calculate_flux_nonlinear_interior_and_neighbor(u_m,
                                                                   u_p,
                                                                   normal_m,
                                                                   u_grid);
      ...
      for (unsigned int q = 0; q < integrator.n_q_points; ++q)
        {
          ...
          vector u_p = calculate_exterior_value_convective(
            u_m,
            q,
            integrator,
            OperatorType::full,
          ...
          vector flux = kernel->calculate_flux_nonlinear_boundary(
            u_m, u_p, normal_m, u_grid, boundary_type);
    ...
    template <int dim, typename Number>
    void
    ConvectiveOperator<dim, Number>::reinit_cell_derived(
      IntegratorCell    &integrator,
    ...
    template <int dim, typename Number>
    void
    ConvectiveOperator<dim, Number>::reinit_face_derived(
      IntegratorFace    &integrator_m,
      IntegratorFace    &integrator_p,
    ...
    template <int dim, typename Number>
    void
    ConvectiveOperator<dim, Number>::reinit_boundary_face_derived(
      IntegratorFace    &integrator_m,
      unsigned int const face) const
    {
      (void)integrator_m;
    ...
    template <int dim, typename Number>
    void
    ConvectiveOperator<dim, Number>::do_cell_integral(
      ...
      for (unsigned int q = 0; q < integrator.n_q_points; ++q)
        {
          ...
          if (operator_data.kernel_data.formulation ==
            ...
            {
              tensor flux =
                kernel->get_volume_flux_divergence_formulation(delta_u, q);
                   ...
                   FormulationConvectiveTerm::ConvectiveFormulation)
            {
              ...
              vector flux =
                kernel->get_volume_flux_convective_formulation(delta_u,
                                                               grad_delta_u,
                                                               q);
    ...
    template <int dim, typename Number>
    void
    ConvectiveOperator<dim, Number>::do_face_integral(
      IntegratorFace &integrator_m,
      ...
      for (unsigned int q = 0; q < integrator_m.n_q_points; ++q)
        {
          ...
          std::tuple<vector, vector> flux =
            kernel->calculate_flux_linear_operator_interior_and_neighbor(
    ...
    template <int dim, typename Number>
    void
    ConvectiveOperator<dim, Number>::do_face_int_integral(
      IntegratorFace &integrator_m,
      IntegratorFace &integrator_p) const
    {
      (void)integrator_p;
      ...
      for (unsigned int q = 0; q < integrator_m.n_q_points; ++q)
        {
          ...
          vector flux = kernel->calculate_flux_linear_operator_interior(
            u_m, u_p, delta_u_m, delta_u_p, normal_m, q);
      ...
      for (unsigned int q = 0; q < integrator_m.n_q_points; ++q)
        {
          ...
          // deal.II/matrixfree. Hence, we simply use the interior value, but
          // note that the diagonal and block-diagonal are not calculated
          // exactly. Note that the present implementation also does not take
    ...
    template <int dim, typename Number>
    void
    ConvectiveOperator<dim, Number>::do_face_ext_integral(
      IntegratorFace &integrator_m,
      IntegratorFace &integrator_p) const
    {
      (void)integrator_m;
      ...
      for (unsigned int q = 0; q < integrator.n_q_points; ++q)
        {
          ...
          vector delta_u_p = calculate_exterior_value_convective(
            delta_u_m,
            q,
            integrator,
            operator_type,
          ...
          if (operator_data.kernel_data.temporal_treatment ==
            ...
            {
              // for a linearly implicit treatment of the convective term, we do
              // not impose boundary conditions for the transport velocity u.
              // Boundary conditions are only imposed for the solution variable
              // denoted here as "value_m" and "value_p".
              u_p = u_m;
            }
          else
            {
