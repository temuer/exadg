...
namespace ExaDG
{
  namespace IncNS
  {
     ...
     *   u_h : solution
     *
     *   jump(u_h) = u_h^{-} - u_h^{+} or ( (u_h^{-} - u_h^{+})*normal ) *
     * normal
     *
     ...
     *
     *                                  where h_eff = h / (k_u+1) with a
     * characteristic element length h derived from the element volume V_e
     *
     *            use both terms:       tau_conti = tau_conti_conv +
     * tau_conti_viscous
     */
    ...
    namespace Operators
    {
      struct ContinuityPenaltyKernelData
      {
        ...
        // the continuity penalty term can be applied to all velocity components
        // or to the normal component only
      ...
      public:
        ...
        calculate_penalty_parameter(VectorType const &velocity)
        {
          ...
          ElementType const element_type = get_element_type(
              ...
              for (unsigned int q = 0; q < integrator.n_q_points; ++q)
                {
                  ...
                  norm_U_mean +=
                    integrator.JxW(q) * integrator.get_value(q).norm();
                }

              norm_U_mean /= volume;

              scalar tau_convective = norm_U_mean;
              scalar h = calculate_characteristic_element_length(volume,
                                                                 dim,
                                                                 element_type);
              scalar h_eff =
                calculate_high_order_element_length(h, data.degree, true);
              scalar tau_viscous =
                dealii::make_vectorized_array<Number>(data.viscosity) / h_eff;

              if (data.type_penalty_parameter ==
                  TypePenaltyParameter::ConvectiveTerm)
                {
                  array_penalty_parameter[cell] =
                    data.penalty_factor * tau_convective;
                }
              else if (data.type_penalty_parameter ==
                       TypePenaltyParameter::ViscousTerm)
                {
                  array_penalty_parameter[cell] =
                    data.penalty_factor * tau_viscous;
                }
              else if (data.type_penalty_parameter ==
                       TypePenaltyParameter::ViscousAndConvectiveTerms)
                {
                  array_penalty_parameter[cell] =
                    data.penalty_factor * (tau_convective + tau_viscous);
                }
            }
        }
                               ...
                               IntegratorFace &integrator_p) const
        {
          if (boundary_id ==
            ...
            {
              tau =
                0.5 * (integrator_m.read_cell_data(array_penalty_parameter) +
                       integrator_p.read_cell_data(array_penalty_parameter));
            }
          else // boundary face
            {
        ...
        inline DEAL_II_ALWAYS_INLINE //
          vector
          calculate_flux(vector const &u_m,
                         vector const &u_p,
                         vector const &normal_m) const
        {
