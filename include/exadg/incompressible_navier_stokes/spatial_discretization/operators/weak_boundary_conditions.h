 ...
 */
     ...
     *
     *  OperatorType:
     *  - To evaluate the nonlinear convective term (residual evaluation), use
     * OperatorType::full.
     *  - To evaluate the linearization of the nonlinear convective term, use
     * OperatorType::homogeneous.
     *  - To evaluate the linearly implicit convective term, use
     * OperatorType::homogeneous for the "matrix-vector" product and
     * OperatorType::inhomogeneous for the "rhs" contribution.
     */
    template <int dim, typename Number>
    inline DEAL_II_ALWAYS_INLINE //
      dealii::Tensor<1, dim, dealii::VectorizedArray<Number>>
      ...
      if (boundary_type == BoundaryTypeU::Dirichlet or
          boundary_type == BoundaryTypeU::DirichletCached)
        {
          if (operator_type == OperatorType::full or
            ...
            {
              dealii::Tensor<1, dim, dealii::VectorizedArray<Number>> g =
                integrator_bc.get_value(q);

              value_p =
                -value_m + dealii::make_vectorized_array<Number>(2.0) * g;
            }
          else if (operator_type == OperatorType::homogeneous)
            {
              value_p = -value_m;
     ...
     * Pressure:
     *
     * These two function calculate the interior/exterior value on boundary
     * faces depending on the operator type, the type of the boundary face and
        ...
        bool const                                      variable_normal_vector)
    {
      ...
      if (boundary_type == BoundaryTypeU::Dirichlet or
          ...
          if (operator_type == OperatorType::full or
              operator_type == OperatorType::inhomogeneous)
            {
              ...
              if (variable_normal_vector == false)
                {
                  h = FunctionEvaluator<1, dim, Number>::value(*bc,
                                                               q_points,
                                                               time);
                }
              else
                {
                  ...
                  h              = FunctionEvaluator<1, dim, Number>::value(
                    *(std::dynamic_pointer_cast<FunctionWithNormal<dim>>(bc)),
                    q_points,
                    normals_m,
                    time);
                }
      ...
      else if (boundary_type == BoundaryTypeU::Symmetry)
        {
          ...
          normal_gradient_p = -normal_gradient_m +
                              2.0 * (normal_gradient_m * normal_m) * normal_m;
        }
      else
        {
          AssertThrow(
