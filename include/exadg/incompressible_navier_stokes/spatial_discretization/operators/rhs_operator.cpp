...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    template <int dim, typename Number>
    void
    RHSOperator<dim, Number>::initialize(
    ...
    template <int dim, typename Number>
    void
    RHSOperator<dim, Number>::evaluate(VectorType  &dst,
                                       Number const evaluation_time) const
    {
      ...
      matrix_free->cell_loop(
        &This::cell_loop, this, dst, src, true /*zero_dst_vector = true*/);
    }

    template <int dim, typename Number>
    void
    RHSOperator<dim, Number>::evaluate_add(VectorType  &dst,
                                           Number const evaluation_time) const
    {
      time = evaluation_time;
      ...
      VectorType src;
      matrix_free->cell_loop(
        &This::cell_loop, this, dst, src, false /*zero_dst_vector = false*/);
    }
    ...
    template <int dim, typename Number>
    void
    RHSOperator<dim, Number>::do_cell_integral(
      ...
      for (unsigned int q = 0; q < integrator.n_q_points; ++q)
        {
          integrator.submit_value(
            kernel.get_volume_flux(integrator, integrator_temperature, q, time),
            q);
        }
    }
    ...
    template <int dim, typename Number>
    void
    RHSOperator<dim, Number>::cell_loop(
      ...
      Range const                           &cell_range) const
    {
      ...
      IntegratorScalar integrator_temperature(matrix_free,
                                              data.dof_index_scalar,
                                              data.quad_index);
          ...
          if (data.kernel_data.boussinesq_term)
            {
              ...
              integrator_temperature.gather_evaluate(
                *temperature, dealii::EvaluationFlags::values);
            }
