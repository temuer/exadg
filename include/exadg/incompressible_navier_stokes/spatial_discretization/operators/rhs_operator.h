...
namespace ExaDG
{
  namespace IncNS
  {
    namespace Operators
    {
      ...
      public:
                          ...
                          Number const           &time) const
        {
          ...
          vector f =
            FunctionEvaluator<1, dim, Number>::value(*(data.f), q_points, time);

          if (data.boussinesq_term)
            {
              vector g = FunctionEvaluator<1, dim, Number>::value(
                *(data.gravitational_force), q_points, time);
              ...
              if (data.boussinesq_dynamic_part_only)
                ...
                f +=
                  g * (1.0 - data.thermal_expansion_coefficient * (T - T_ref));
            }
    ...
    private:
      void
      do_cell_integral(Integrator       &integrator,
