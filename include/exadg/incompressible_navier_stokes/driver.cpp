...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    Driver<dim, Number>::setup()
    {
      ...
      if (ale)
        {
          if (application->get_parameters().mesh_movement_type ==
              MeshMovementType::Function)
            {
              std::shared_ptr<dealii::Function<dim>> mesh_motion =
                application->create_mesh_movement_function();

              ale_mapping =
                std::make_shared<DeformedMappingFunction<dim, Number>>(
                  mapping,
          ...
          else if (application->get_parameters().mesh_movement_type ==
                   MeshMovementType::Poisson)
            {
              application->setup_poisson(grid);

              ale_mapping =
                std::make_shared<Poisson::DeformedMapping<dim, Number>>(
                  grid,
                  mapping,
                  multigrid_mappings,
          ...
          ale_multigrid_mappings =
            std::make_shared<MultigridMappings<dim, Number>>(
              ale_mapping,
              application->get_parameters().mapping_degree_coarse_grids);
      ...
      if (not is_throughput_study)
        {
          ...
          else if (application->get_parameters().solver_type ==
                   SolverType::Steady)
            {
              std::shared_ptr<OperatorCoupled<dim, Number>> operator_coupled =
                std::dynamic_pointer_cast<OperatorCoupled<dim, Number>>(
                  pde_operator);

              // initialize driver for steady state problem that depends on
              // pde_operator
              driver_steady =
                std::make_shared<DriverSteadyProblems<dim, Number>>(
                  operator_coupled,
                  postprocessor,
                  application->get_parameters(),
                  mpi_comm,
    ...
    Driver<dim, Number>::ale_update() const
    {
      ...
      timer_tree.insert({"Incompressible flow", "ALE", "Reinit mapping"},
                        sub_timer.wall_time());
      ...
      timer_tree.insert({"Incompressible flow",
                         "ALE",
                         "Update matrix-free / PDE operator"},
                        sub_timer.wall_time());
    ...
    Driver<dim, Number>::solve() const
    {
      ...
      else if (application->get_parameters().problem_type ==
               ProblemType::Steady)
        {
          if (application->get_parameters().solver_type == SolverType::Unsteady)
            {
      ...
      double const total_time) const
    {
      ...
      pcout << "Performance results for incompressible Navier-Stokes solver:"
            << std::endl;
      ...
      unsigned int const  n_repetitions_outer) const
    {
      ...
      AssertThrow(
        application->get_parameters().degree_p == DegreePressure::MixedOrder,
        dealii::ExcMessage(
      ...
      // check that the operator type is consistent with the solution approach
      // (coupled vs. splitting)
      if (application->get_parameters().temporal_discretization ==
          TemporalDiscretization::BDFCoupledSolution)
        {
          AssertThrow(
            operator_type == OperatorType::ConvectiveOperator or
            ...
            dealii::ExcMessage(
              "Invalid operator specified for coupled solution approach."));
        }
               ...
               TemporalDiscretization::BDFDualSplittingScheme)
        {
          AssertThrow(
            operator_type == OperatorType::ConvectiveOperator or
              ...
              operator_type == OperatorType::InverseMassOperator,
            dealii::ExcMessage(
              "Invalid operator specified for dual splitting scheme."));
        }
      else if (application->get_parameters().temporal_discretization ==
               TemporalDiscretization::BDFPressureCorrection)
        {
          AssertThrow(
            operator_type == OperatorType::ConvectiveOperator or
              ...
              operator_type == OperatorType::InverseMassOperator,
            dealii::ExcMessage(
              "Invalid operator specified for pressure-correction scheme."));
        }
      else
        {
          AssertThrow(false, dealii::ExcMessage("Not implemented."));
      ...
      if (operator_type == OperatorType::CoupledNonlinearResidual or
          operator_type == OperatorType::CoupledLinearized)
        {
          dofs = pde_operator->get_dof_handler_u().n_dofs() +
                 pde_operator->get_dof_handler_p().n_dofs();
      ...
      else if (operator_type == OperatorType::PressurePoissonOperator)
        {
          ...
          fe_degree = application->get_parameters().get_degree_p(
