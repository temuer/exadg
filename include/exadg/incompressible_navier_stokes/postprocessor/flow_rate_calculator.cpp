...
namespace ExaDG
{
  namespace IncNS
  {
      ...
      std::map<dealii::types::boundary_id, Number> &flow_rates)
    {
      ...
      for (auto iterator = flow_rates.begin(); iterator != flow_rates.end();
           ++iterator)
        {
          iterator->second = 0.0;
        }
      ...
      dealii::Utilities::MPI::sum(
        dealii::ArrayView<double const>(&(*flow_rates_vector.begin()),
                                        flow_rates_vector.size()),
        mpi_comm,
        dealii::ArrayView<double>(&(*flow_rates_vector.begin()),
                                  flow_rates_vector.size()));
