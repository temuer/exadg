...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    KineticEnergyCalculatorDetailed<dim, Number>::
    ...
    template <int dim, typename Number>
    void
    KineticEnergyCalculatorDetailed<dim, Number>::evaluate(
    ...
    KineticEnergyCalculatorDetailed<dim, Number>::calculate_detailed(
      ...
      double const      time)
    {
      Number kinetic_energy = 0.0, enstrophy = 0.0, dissipation = 0.0,
             max_vorticity = 0.0;

      Number volume = this->integrate(*this->matrix_free,
                                      velocity,
                                      kinetic_energy,
                                      enstrophy,
                                      dissipation,
      ...
      AssertThrow(navier_stokes_operator != nullptr,
                  dealii::ExcMessage("Invalid pointer."));
      Number dissipation_convective =
        navier_stokes_operator->calculate_dissipation_convective_term(velocity,
                                                                      time) /
        volume;
      Number dissipation_viscous =
        navier_stokes_operator->calculate_dissipation_viscous_term(velocity) /
        volume;
      Number dissipation_divergence =
        navier_stokes_operator->calculate_dissipation_divergence_term(
          velocity) /
        volume;
      Number dissipation_continuity =
        navier_stokes_operator->calculate_dissipation_continuity_term(
          velocity) /
        volume;
