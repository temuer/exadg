...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    PostProcessor<dim, Number>::PostProcessor(
      PostProcessorData<dim> const &postprocessor_data,
      MPI_Comm const               &comm)
      : mpi_comm(comm)
    ...
    PostProcessor<dim, Number>::setup(Operator const &pde_operator)
    {
      ...
      lift_and_drag_calculator.setup(
      ...
      div_and_mass_error_calculator.setup(
        pde_operator.get_matrix_free(),
        pde_operator.get_dof_index_velocity(),
        pde_operator.get_quad_index_velocity_standard(),
        pp_data.mass_data);

      kinetic_energy_calculator.setup(
        pde_operator,
        pde_operator.get_matrix_free(),
        pde_operator.get_dof_index_velocity(),
        pde_operator.get_quad_index_velocity_standard(),
      ...
      kinetic_energy_spectrum_calculator.setup(
        pde_operator.get_matrix_free(),
        pde_operator.get_dof_handler_u(),
      ...
      types::time_step const time_step_number)
    {
      ...
      if (output_generator.time_control.needs_evaluation(time,
                                                         time_step_number))
        {
          std::vector<dealii::SmartPointer<SolutionField<dim, Number>>>
          ...
          output_generator.evaluate(velocity,
                                    ...
                                    additional_fields_vtu,
                                    time,
                                    Utilities::is_unsteady_timestep(
                                      time_step_number));
        }
       ...
       */
      if (pointwise_output_generator.time_control.needs_evaluation(
       ...
       *  calculate error
       */
      if (error_calculator_u.time_control.needs_evaluation(time,
                                                           time_step_number))
        error_calculator_u.evaluate(
          velocity, time, Utilities::is_unsteady_timestep(time_step_number));
      if (error_calculator_p.time_control.needs_evaluation(time,
                                                           time_step_number))
        error_calculator_p.evaluate(
          pressure, time, Utilities::is_unsteady_timestep(time_step_number));

      /*
       *  calculation of lift and drag coefficients
       */
      if (lift_and_drag_calculator.time_control.needs_evaluation(
            time, time_step_number))
       ...
       */
      if (pressure_difference_calculator.time_control.needs_evaluation(
            time, time_step_number))
       ...
       */
      if (div_and_mass_error_calculator.time_control.needs_evaluation(
            time, time_step_number))
        {
          div_and_mass_error_calculator.evaluate(
            velocity, time, Utilities::is_unsteady_timestep(time_step_number));
        }
       ...
       */
      if (kinetic_energy_calculator.time_control.needs_evaluation(
            time, time_step_number))
        {
       ...
       */
      if (kinetic_energy_spectrum_calculator.time_control.needs_evaluation(
            time, time_step_number))
        {
       ...
       */
      if (line_plot_calculator.time_control.needs_evaluation(time,
                                                             time_step_number))
    ...
    {
      // vorticity
      if (pp_data.output_data.write_vorticity or
          pp_data.output_data.write_streamfunction or
          pp_data.output_data.write_vorticity_magnitude)
        {
          ...
          vorticity.recompute_solution_field = [&](VectorType       &dst,
      ...
      if (pp_data.output_data.write_vorticity_magnitude)
        {
          AssertThrow(pp_data.output_data.write_vorticity == true,
                      dealii::ExcMessage(
                        "You need to activate write_vorticity."));

          vorticity_magnitude.type = SolutionFieldType::scalar;
          vorticity_magnitude.name = "vorticity_magnitude";
          vorticity_magnitude.dof_handler =
            &navier_stokes_operator->get_dof_handler_u_scalar();
          ...
          vorticity_magnitude.recompute_solution_field =
            [&](VectorType &dst, VectorType const &src) {
      ...
      if (pp_data.output_data.write_streamfunction)
        {
          ...
          streamfunction.dof_handler =
            &navier_stokes_operator->get_dof_handler_u_scalar();
          streamfunction.initialize_vector = [&](VectorType &dst) {
            navier_stokes_operator->initialize_vector_velocity_scalar(dst);
          };
          streamfunction.recompute_solution_field = [&](VectorType       &dst,
                                                        VectorType const &src) {
      ...
      if (pp_data.output_data.write_divergence)
        {
          ...
          divergence.dof_handler =
            &navier_stokes_operator->get_dof_handler_u_scalar();
          divergence.initialize_vector = [&](VectorType &dst) {
            navier_stokes_operator->initialize_vector_velocity_scalar(dst);
          };
          divergence.recompute_solution_field = [&](VectorType       &dst,
                                                    VectorType const &src) {
      ...
      if (pp_data.output_data.write_shear_rate)
        {
          ...
          shear_rate.dof_handler =
            &navier_stokes_operator->get_dof_handler_u_scalar();
          shear_rate.initialize_vector = [&](VectorType &dst) {
            navier_stokes_operator->initialize_vector_velocity_scalar(dst);
          };
          shear_rate.recompute_solution_field = [&](VectorType       &dst,
                                                    VectorType const &src) {
      ...
      if (pp_data.output_data.write_velocity_magnitude)
        {
          ...
          velocity_magnitude.dof_handler =
            &navier_stokes_operator->get_dof_handler_u_scalar();
      ...
      if (pp_data.output_data.write_q_criterion)
        {
          ...
          q_criterion.dof_handler =
            &navier_stokes_operator->get_dof_handler_u_scalar();
          q_criterion.initialize_vector = [&](VectorType &dst) {
            navier_stokes_operator->initialize_vector_velocity_scalar(dst);
          };
          q_criterion.recompute_solution_field = [&](VectorType       &dst,
                                                     VectorType const &src) {
      ...
      if (pp_data.output_data.mean_velocity.is_active)
        {
          ...
          mean_velocity.recompute_solution_field =
            [&](VectorType &dst, VectorType const &velocity) {
              unsigned int const counter =
                time_control_mean_velocity.get_counter();
      ...
      if (pp_data.output_data.write_cfl)
        {
          ...
          cfl_vector.recompute_solution_field = [&](VectorType       &dst,
                                                    VectorType const &src) {
            // This time step size corresponds to CFL = 1.
            auto const time_step_size =
              navier_stokes_operator->calculate_time_step_cfl(src);
            // The computed cell-vector of CFL values contains relative CFL
            // numbers with a value of CFL = 1 in the most critical cell and CFL
            // < 1 in other cells.
            navier_stokes_operator->calculate_cfl_from_time_step(
              dst, src, time_step_size);
          };
