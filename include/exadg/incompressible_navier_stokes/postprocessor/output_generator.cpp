  ...
  namespace IncNS
  {
      ...
      MPI_Comm const    &mpi_comm)
    {
      ...
      if (output_data.write_aspect_ratio)
        {
          ...
          std::shared_ptr<dealii::Quadrature<dim>> quadrature =
            create_quadrature<dim>(element_type, 4);
      ...
      for (auto &additional_field : additional_fields)
        {
          if (additional_field->get_type() == SolutionFieldType::scalar)
            ...
            {
              data_out.add_data_vector(additional_field->get(),
                                       additional_field->get_name());
            }
      ...
      data_out.write_vtu_with_pvtu_record(
        folder, file, output_counter, mpi_comm, 4);
    }
    ...
    template <int dim, typename Number>
    void
    OutputGenerator<dim, Number>::setup(
      ...
      if (output_data.time_control_data.is_active)
        {
          ...
          // since boundary IDs typically do not change during the simulation,
          // we only do this once at the beginning of the simulation (i.e., in
          // the setup function).
          ...
          if (output_data.write_processor_id)
            {
              ...
              grid_out.write_mesh_per_processor_as_vtu(
                dof_handler_velocity->get_triangulation(),
                output_data.directory + output_data.filename + "_processor_id");
            }
        }
    }
