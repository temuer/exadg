...
namespace ExaDG
{
  namespace IncNS
  {
      ...
      if (data_in.time_control_data_statistics.time_control_data.is_active)
        {
          ...
          for (typename std::vector<std::shared_ptr<Line<dim>>>::iterator line =
               ...
               ++line, ++line_iterator)
            {
              ...
              // initialize global_points: use/assume equidistant points along
              // line
    ...
    template <int dim, typename Number>
    void
    LinePlotCalculatorStatistics<dim, Number>::evaluate(
    ...
    template <int dim, typename Number>
    void
    LinePlotCalculatorStatistics<dim, Number>::initialize_cell_data(
      VectorType const &velocity,
      VectorType const &pressure)
    {
      // Save data related to all adjacent cells for a given point along the
      // line.
      unsigned int line_iterator = 0;
      for (typename std::vector<std::shared_ptr<Line<dim>>>::iterator line =
             data.lines.begin();
          ...
          if (line_circ->average_circumferential == true)
            {
              ...
              // Calculate two unit vectors in the plane that is normal to the
              // normal_vector.
              unit_vector_1       = (*line)->end - (*line)->begin;
              double const norm_1 = unit_vector_1.norm();
              AssertThrow(norm_1 > 1.e-12,
                          dealii::ExcMessage(
                            "Invalid begin and end points found."));
          ...
          for (unsigned int p = 0; p < (*line)->n_points; ++p)
            {
              ...
              // If averaging in circumferential direction is used, we insert
              // additional points along the circle for points p>=1. The first
              // point p=0 lies in the center of the circle (point(p=0) ==
              // line.begin).
              if (p >= 1 and line_circ->average_circumferential == true)
                {
                  // begin with 1 since the first point has already been
                  // inserted.
                  for (unsigned int i = 1;
                       i < line_circ->n_points_circumferential;
                       ++i)
                    {
                      double cos = std::cos(
                        (double(i) / line_circ->n_points_circumferential) *
                      ...
                      for (unsigned int d = 0; d < dim; ++d)
                        {
                          new_point[d] = ((*line)->begin)[d] +
                                         cos * radius * unit_vector_1[d] +
              ...
              for (typename std::vector<dealii::Point<dim>>::iterator point_it =
                     points.begin();
                   point_it != points.end();
                   ++point_it)
                {
                  if (velocity_has_to_be_evaluated == true)
                    {
                      // find adjacent cells and store data required later for
                      // evaluating the solution.
                      auto adjacent_cells =
                        dealii::GridTools::find_all_active_cells_around_point(
                          mapping,
                      ...
                      auto dof_indices_and_shape_values =
                        get_dof_indices_and_shape_values(adjacent_cells,
                      ...
                      for (auto iter = dof_indices_and_shape_values.begin();
                           ...
                           ++iter)
                        {
                          cells_global_velocity[line_iterator][p].push_back(
                            *iter);
                        }
                    }
                  ...
                  if (pressure_has_to_be_evaluated == true)
                    {
                      ...
                      auto adjacent_cells =
                        dealii::GridTools::find_all_active_cells_around_point(
                          mapping,
                          dof_handler_pressure.get_triangulation(),
                          *point_it,
                          1.e-10);
                      ...
                      for (auto iter = dof_indices_and_shape_values.begin();
                           ...
                           ++iter)
                        {
                          cells_global_pressure[line_iterator][p].push_back(
                            *iter);
                        }
                    }
                }
    ...
    template <int dim, typename Number>
    void
    LinePlotCalculatorStatistics<dim, Number>::do_evaluate(
      VectorType const &velocity,
      VectorType const &pressure)
    {
      ...
      // Make sure that all data has been initialized before evaluating the
      // solution.
    ...
    template <int dim, typename Number>
    void
    LinePlotCalculatorStatistics<dim, Number>::do_evaluate_velocity(
      VectorType const  &velocity,
      Line<dim> const   &line,
      unsigned int const line_iterator)
    {
      ...
      std::vector<dealii::Tensor<1, dim, Number>> velocity_vector_local(
        line.n_points);
      ...
      for (unsigned int p = 0; p < line.n_points; ++p)
        {
          ...
          for (auto iter = adjacent_cells.begin(); iter != adjacent_cells.end();
               ++iter)
            {
              ...
              for (typename std::vector<
                   ...
                   ++quantity)
                {
                  if ((*quantity)->type == QuantityType::Velocity)
                    {
                      // interpolate solution using the precomputed shape values
                      // and the global dof index
                      dealii::Tensor<1, dim, Number> velocity_value =
                        Interpolator<1, dim, Number>::value(
                          dof_handler_velocity,
                          velocity,
                          iter->first,
                          iter->second);
      ...
      dealii::Utilities::MPI::sum(counter_vector_local,
                                  mpi_comm,
                                  counter_vector_local);

      // Perform MPI communication as well as averaging for all quantities of
      // the current line.
          ...
          if ((*quantity)->type == QuantityType::Velocity)
            {
              dealii::Utilities::MPI::sum(
                dealii::ArrayView<Number const>(&velocity_vector_local[0][0],
                                                dim *
              ...
              // When writing the output files, we calculate the time-averaged
              // values by dividing the global (accumulated) values by the
              // number of samples.
              for (unsigned int p = 0; p < line.n_points; ++p)
                {
                  // Take average value over all adjacent cells for a given
                  // point.
                  for (unsigned int d = 0; d < dim; ++d)
                    {
                      if (counter_vector_local[p] > 0)
                        {
                          velocity_global[line_iterator][p][d] +=
                            velocity_vector_local[p][d] /
                            counter_vector_local[p];
                        }
                    }
                }
            }
    ...
    template <int dim, typename Number>
    void
    LinePlotCalculatorStatistics<dim, Number>::do_evaluate_pressure(
      VectorType const  &pressure,
      Line<dim> const   &line,
      unsigned int const line_iterator)
    {
                  ...
                  if ((*quantity)->type == QuantityType::Pressure)
                    {
                      ...
                      Number pressure_value =
                        Interpolator<0, dim, Number>::value(
                          dof_handler_pressure,
                          pressure,
                          iter->first,
                          iter->second);
      ...
      // Perform MPI communcation as well as averaging for all quantities of the
      // current line.
      for (typename std::vector<std::shared_ptr<Quantity>>::const_iterator
             quantity = line.quantities.begin();
           quantity != line.quantities.end();
           ++quantity)
        {
          if ((*quantity)->type == QuantityType::Pressure)
            {
              dealii::Utilities::MPI::sum(pressure_vector_local,
                                          mpi_comm,
                                          pressure_vector_local);
