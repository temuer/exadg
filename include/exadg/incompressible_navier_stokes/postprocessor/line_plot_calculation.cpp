...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    template <int dim, typename Number>
    void
    LinePlotCalculator<dim, Number>::setup(
                                              ...
                                              VectorType const &pressure) const
    {
      ...
      for (typename std::vector<std::shared_ptr<Line<dim>>>::const_iterator
           ...
           ++line)
        {
          ...
          // we consider straight lines with an equidistant distribution of
          // points along the line
          for (unsigned int i = 0; i < n_points; ++i)
            points[i] = (*line)->begin + double(i) / double(n_points - 1) *
                                           ((*line)->end - (*line)->begin);
          ...
          for (std::vector<std::shared_ptr<Quantity>>::const_iterator quantity =
               ...
               ++quantity)
            {
              if ((*quantity)->type == QuantityType::Velocity)
                {
                  std::vector<dealii::Tensor<1, dim, Number>> solution_vector(
                    n_points);
              ...
              else if ((*quantity)->type == QuantityType::Pressure)
                {
                  ...
                  for (unsigned int i = 0; i < n_points; ++i)
                    {
                      Number p;
                      evaluate_scalar_quantity_in_point(p,
