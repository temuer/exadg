...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    template <int dim, typename Number>
    void
    DivergenceAndMassErrorCalculator<dim, Number>::evaluate(
          ...
          for (unsigned int q = 0; q < integrator.n_q_points; ++q)
            {
              ...
              div_vec +=
                integrator.JxW(q) * std::abs(integrator.get_divergence(q));
            }
      ...
      for (unsigned int face = face_range.first; face < face_range.second;
           ++face)
        {
          ...
          for (unsigned int q = 0; q < integrator_m.n_q_points; ++q)
            {
              diff_mass_flux_vec += integrator_m.JxW(q) *
                                    std::abs((integrator_m.get_value(q) -
    ...
    void
    DivergenceAndMassErrorCalculator<dim, Number>::
      local_compute_div_boundary_face(
    ...
    DivergenceAndMassErrorCalculator<dim, Number>::
      analyze_div_and_mass_error_unsteady(VectorType const &velocity,
                                          double const      time)
    {
      Number div_error = 1.0, div_error_reference = 1.0, mass_error = 1.0,
             mass_error_reference = 1.0;

      // calculate divergence and mass error
      do_evaluate(*matrix_free,
                  velocity,
                  div_error,
                  div_error_reference,
                  mass_error,
      ...
      if (dealii::Utilities::MPI::this_mpi_process(mpi_comm) == 0)
        {
          std::string filename =
            data.directory + data.filename + ".div_mass_error_timeseries";
          ...
          f << std::scientific << std::setprecision(7) << std::setw(15) << time
            << std::setw(15) << div_error_normalized << std::setw(15)
            << mass_error_normalized << std::endl;
        }
      ...
      if (dealii::Utilities::MPI::this_mpi_process(mpi_comm) == 0)
        {
          std::string filename =
            data.directory + data.filename + ".div_mass_error_average";
          ...
          f << "Mean error incompressibility constraint:   "
            << divergence_sample / number_of_samples << std::endl;
      ...
      analyze_div_and_mass_error_steady(VectorType const &velocity)
    {
      ...
      if (dealii::Utilities::MPI::this_mpi_process(mpi_comm) == 0)
        {
          std::string filename =
            data.directory + data.filename + ".div_mass_error";

          std::ofstream f;

          f.open(filename.c_str(), std::ios::trunc);
          f << "Divergence and mass error:" << std::endl;
          f << "Error incompressibility constraint:   " << div_error_normalized
            << std::endl;
          f << "Error mass flux over interior element faces:  "
            << mass_error_normalized << std::endl;
          f.close();
        }
    }
