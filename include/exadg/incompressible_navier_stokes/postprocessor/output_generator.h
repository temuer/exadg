...
namespace ExaDG
{
  namespace IncNS
  {
    struct OutputData : public OutputDataBase
    {
      ...
      // used for flow problems where the whole boundary is one streamline,
      // e.g., cavity-type flow problems where the velocity is tangential to the
      // boundary on one part of the boundary and 0 (no-slip) on the rest of the
      // boundary.
      ...
      //  Of course, this module can also be used for statistically unsteady
      //  problems, but in this case the mean velocity field is probably not
      //  meaningful.
