...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    InflowDataCalculator<dim, Number>::InflowDataCalculator(
      InflowData<dim> const &inflow_data_in,
      MPI_Comm const        &comm)
    ...
    template <int dim, typename Number>
    void
    InflowDataCalculator<dim, Number>::setup(
      ...
      dealii::Mapping<dim> const    &mapping_in)
    {
      ...
      array_dof_indices_and_shape_values.resize(inflow_data.n_points_y *
                                                inflow_data.n_points_z);
      ...
      dealii::LinearAlgebra::distributed::Vector<Number> const &velocity)
    {
      if (inflow_data.write_inflow_data == true)
        {
          // initial data: do this expensive step only once at the beginning of
          // the simulation
                  ...
                  for (unsigned int iz = 0; iz < inflow_data.n_points_z; ++iz)
                    {
                      ...
                      if (inflow_data.inflow_geometry ==
                          InflowGeometry::Cartesian)
                        {
                          AssertThrow(inflow_data.normal_direction == 0,
                                      dealii::ExcMessage("Not implemented."));

                          point =
                            dealii::Point<dim>(inflow_data.normal_coordinate,
                                               (*inflow_data.y_values)[iy],
                                               (*inflow_data.z_values)[iz]);
                        }
                      else if (inflow_data.inflow_geometry ==
                               InflowGeometry::Cylindrical)
                        {
                          AssertThrow(inflow_data.normal_direction == 2,
                                      dealii::ExcMessage("Not implemented."));

                          Number const x =
                            (*inflow_data.y_values)[iy] *
                            std::cos((*inflow_data.z_values)[iz]);
                          Number const y =
                            (*inflow_data.y_values)[iy] *
                            std::sin((*inflow_data.z_values)[iz]);
                          point =
                            dealii::Point<dim>(x,
                                               y,
                                               inflow_data.normal_coordinate);
                        }
                      else
                        {
                          AssertThrow(false,
          ...
          for (unsigned int iy = 0; iy < inflow_data.n_points_y; ++iy)
            {
              for (unsigned int iz = 0; iz < inflow_data.n_points_z; ++iz)
                {
                  // determine the array index, will be needed several times
                  // below
                  unsigned int array_index = iy * inflow_data.n_points_z + iz;

                  // initialize with zeros since we accumulate into these
                  // variables
                  ...
                  // loop over all adjacent, locally owned cells for the current
                  // point
                  for (auto iter = vector.begin(); iter != vector.end(); ++iter)
                    {
                      // increment counter (because this is a locally owned
                      // cell)
          ...
          dealii::Utilities::MPI::sum(
            dealii::ArrayView<double const>(&(*inflow_data.array)[0][0],
                                            dim * inflow_data.array->size()),
            mpi_comm,
            dealii::ArrayView<double>(&(*inflow_data.array)[0][0],
                                      dim * inflow_data.array->size()));
              ...
              for (unsigned int iz = 0; iz < inflow_data.n_points_z; ++iz)
                {
                  ...
                  if (array_counter[array_index] >= 1)
                    (*inflow_data.array)[array_index] /=
                      Number(array_counter[array_index]);
                }
            }
        }
    }
