...
namespace ExaDG
{
  namespace IncNS
  {
        ...
        if (calculate == true)
          {
            ...
            print_parameter(pcout,
                            "Calculate mean velocity/flow rate",
                            calculate);
    ...
    public:
      ...
      /*
       * Calculates the mean velocity through a given cross section of the
       * domain by dividing the flow rate through the cross section area. This
       * function is more general than calculate_mean_velocity_volume() and can
       * be used for domains with varying cross-section area in streamwise
       * direction.
       */
      Number
      calculate_mean_velocity_area(VectorType const &velocity,
                                   double const     &time);

      /*
       * Calculate mean velocity (only makes sense if the domain has a constant
       * cross-section area in streamwise direction.
       */
      Number
      calculate_mean_velocity_volume(VectorType const &velocity,
                                     double const     &time);

      /*
       * Calculate flow rate in m^3/s, for example for problems with
       * non-constant cross-section area. To obtain the flow rate, the length of
       * the domain in streamwise direction has to be specified.
       */
      Number
       ...
       */
      Number
      calculate_flow_rate_area(VectorType const &velocity,
    ...
    private:
      ...
      void
      local_calculate_flow_rate_volume(
        dealii::MatrixFree<dim, Number> const       &data,
        std::vector<Number>                         &dst,
