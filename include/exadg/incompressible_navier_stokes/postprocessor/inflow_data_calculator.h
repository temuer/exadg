...
#ifndef INCLUDE_EXADG_INCOMPRESSIBLE_NAVIER_STOKES_POSTPROCESSOR_INFLOW_DATA_CALCULATOR_H_
#define INCLUDE_EXADG_INCOMPRESSIBLE_NAVIER_STOKES_POSTPROCESSOR_INFLOW_DATA_CALCULATOR_H_
...
namespace ExaDG
{
  ...
  {
    /*
     * inflow data: use velocity at the outflow of one domain as inflow-BC for
     * another domain
     *
     * The outflow boundary has to be the y-z plane at a given x-coordinate. The
     * velocity is written at n_points_y in y-direction and n_points_z in
     * z-direction, which has to be specified by the user.
     */
    ...
    {
    public:
      InflowDataCalculator(InflowData<dim> const &inflow_data,
                           MPI_Comm const        &comm);

      void
      setup(dealii::DoFHandler<dim> const &dof_handler_velocity,
...
#endif /* INCLUDE_EXADG_INCOMPRESSIBLE_NAVIER_STOKES_POSTPROCESSOR_INFLOW_DATA_CALCULATOR_H_ \
        */
