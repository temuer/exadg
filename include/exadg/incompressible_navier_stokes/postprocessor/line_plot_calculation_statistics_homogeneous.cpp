...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    LinePlotCalculatorStatisticsHomogeneous<dim, Number>::
      LinePlotCalculatorStatisticsHomogeneous(
    ...
    {
      AssertThrow(get_element_type(dof_handler_velocity.get_triangulation()) ==
      ...
      LinePlotDataStatistics<dim> const &data_in)
    {
      ...
      AssertThrow(
        Utilities::is_valid_timestep(
          data_in.time_control_data_statistics
            .write_preliminary_results_every_nth_time_step),
        dealii::ExcMessage(
          "write_preliminary_results_every_nth_time_step has to be set."));
      ...
      if (data_in.time_control_data_statistics.time_control_data.is_active)
        {
          ...
          std::shared_ptr<LineHomogeneousAveraging<dim>> line_hom =
            std::dynamic_pointer_cast<LineHomogeneousAveraging<dim>>(
              data.lines[0]);
          AssertThrow(
            line_hom.get() != 0,
            dealii::ExcMessage(
              "Invalid line type, expected LineHomogeneousAveraging<dim>"));
          averaging_direction = line_hom->averaging_direction;

          AssertThrow(averaging_direction == 0 or averaging_direction == 1 or
                        averaging_direction == 2,
                      dealii::ExcMessage(
                        "Take the average either in x, y or z-direction"));
          ...
          for (typename std::vector<std::shared_ptr<Line<dim>>>::iterator line =
               ...
               ++line, ++line_iterator)
            {
              ...
              AssertThrow(
                averaging_direction == line_hom->averaging_direction,
                dealii::ExcMessage(
                  "All lines must use the same averaging direction."));
              ...
              cells_and_ref_points_velocity[line_iterator].resize(
                (*line)->n_points);
              cells_and_ref_points_pressure[line_iterator].resize(
                (*line)->n_points);
              ...
              for (unsigned int i = 0; i < (*line)->n_points; ++i)
                {
                  dealii::Point<dim> point =
                    (*line)->begin + double(i) / double((*line)->n_points - 1) *
                                       ((*line)->end - (*line)->begin);
              ...
              if (cell->is_locally_owned())
                {
                  ...
                  for (typename std::vector<
                         std::shared_ptr<Line<dim>>>::iterator line =
                         data.lines.begin();
                       line != data.lines.end();
                       ++line, ++line_iterator)
                    {
                      AssertThrow((*line)->quantities.size() > 0,
                                  dealii::ExcMessage(
                                    "No quantities specified for line."));

                      bool velocity_has_to_be_evaluated = false;
                      for (typename std::vector<
                             std::shared_ptr<Quantity>>::iterator quantity =
                      ...
                      if (velocity_has_to_be_evaluated == true)
                        {
                          // cells and reference points for all points along a
                          // line
                          for (unsigned int p = 0; p < (*line)->n_points; ++p)
                            {
                              // First, we move the line to the position of the
                              // current cell (vertex 0) in averaging direction
                              // and check whether this new point is inside the
                              // current cell
                              dealii::Point<dim> translated_point =
                                global_points[line_iterator][p];
                              translated_point[averaging_direction] =
                                cell->vertex(0)[averaging_direction];

                              // If the new point lies in the current cell, we
                              // have to take the current cell into account
                              dealii::Point<dim> const p_unit =
                                cell->real_to_unit_cell_affine_approximation(
                                  translated_point);

                              if (dealii::GeometryInfo<
                                    dim>::is_inside_unit_cell(p_unit,
                                                              tolerance))
                                {
                                  cells_and_ref_points_velocity
                                    [line_iterator][p]
                                      .push_back(
                                        std::pair<typename dealii::DoFHandler<
                                                    dim>::active_cell_iterator,
                                                  dealii::Point<dim>>(cell,
                                                                      p_unit));
                                }

                              //              dealii::Point<dim> p_unit =
                              //              Point<dim>(); try
                              //              {
                              //                p_unit =
                              ...
                              //              catch(...)
                              //              {
                              //                // A point that does not lie on
                              //                the reference cell. p_unit[0]
                              //                = 2.0;
                              //              }
          ...
          // Save all cells and corresponding points on unit cell that are
          // relevant for a given point along the line. We have to do the same
          // for the pressure because the dealii::DoFHandlers for velocity and
              ...
              if (cell->is_locally_owned())
                {
                  ...
                  for (typename std::vector<
                       ...
                       ++line, ++line_iterator)
                    {
                      for (typename std::vector<
                           ...
                           ++quantity)
                        {
                          ...
                          if ((*quantity)->type == QuantityType::Pressure)
                            {
                              // cells and reference points for all points along
                              // a line
                              for (unsigned int p = 0; p < (*line)->n_points;
                                   ++p)
                                {
                                  // First, we move the line to the position of
                                  // the current cell (vertex 0) in averaging
                                  // direction and check whether this new point
                                  // is inside the current cell
                                  dealii::Point<dim> translated_point =
                                    global_points[line_iterator][p];
                                  translated_point[averaging_direction] =
                                    cell->vertex(0)[averaging_direction];

                                  // If the new point lies in the current cell,
                                  // we have to take the current cell into
                                  // account
                                  dealii::Point<dim> const p_unit =
                                    cell
                      ...
                      // cells and reference points for reference pressure (only
                      ...
                      for (typename std::vector<
                           ...
                           ++quantity)
                        {
                          ...
                          // evaluate quantities that involve pressure
                          if ((*quantity)->type ==
                              QuantityType::PressureCoefficient)
                            {
                              std::shared_ptr<QuantityPressureCoefficient<dim>>
                                quantity_ref_pressure =
                                  std::dynamic_pointer_cast<
                                    QuantityPressureCoefficient<dim>>(
                                    *quantity);
                              ...
                              // current cell
                              dealii::Point<dim> translated_point =
                                quantity_ref_pressure->reference_point;
                              translated_point[averaging_direction] =
                                cell->vertex(0)[averaging_direction];
      ...
      VectorType const &pressure)
    {
      ...
      for (typename std::vector<std::shared_ptr<Line<dim>>>::iterator line =
           ...
           ++line, ++line_iterator)
        {
          ...
          for (typename std::vector<std::shared_ptr<Quantity>>::iterator
                 quantity = (*line)->quantities.begin();
               quantity != (*line)->quantities.end();
               ++quantity)
            {
      ...
      unsigned int const line_iterator)
    {
      ...
      for (unsigned int p = 0; p < line.n_points; ++p)
        {
          for (typename TYPE::const_iterator cell_and_ref_point =
               ...
               cell_and_ref_point !=
               cells_and_ref_points_velocity[line_iterator][p].end();
               ++cell_and_ref_point)
            {
              unsigned int const fe_degree_velocity =
                dof_handler_velocity.get_fe().degree;
              ...
              find_points_and_weights(
                p_unit, points, weights, averaging_direction, gauss_1d);
              ...
              for (unsigned int j = 0; j < dof_indices.size(); ++j)
                {
                  ...
                  if (comp.first < dim)
                    velocity_vector[comp.second][comp.first] =
                      velocity(dof_indices[j]);
                }

              // perform averaging in homogeneous direction
              for (unsigned int q = 0; q < fe_values.n_quadrature_points; ++q)
                {
                  double det = std::abs(fe_values.jacobian(
                    q)[averaging_direction][averaging_direction]);
                  ...
                  for (typename std::vector<
                         std::shared_ptr<Quantity>>::const_iterator quantity =
                         line.quantities.begin();
                       quantity != line.quantities.end();
                       ++quantity)
                    {
                      ...
                      if ((*quantity)->type == QuantityType::Velocity or
                          (*quantity)->type == QuantityType::ReynoldsStresses)
                        {
                          // evaluate velocity solution in current quadrature
                          // points
                          for (unsigned int j = 0; j < velocity_vector.size();
                               ++j)
                            velocity +=
                              fe_values.shape_value(j, q) * velocity_vector[j];
                        }
                      ...
                      if ((*quantity)->type == QuantityType::Velocity)
                          ...
                          for (unsigned int i = 0; i < dim; ++i)
                            {
                              for (unsigned int j = 0; j < dim; ++j)
                                {
                                  reynolds_local[p][i][j] +=
                                    velocity[i] * velocity[j] * JxW;
                                }
                            }
                        }
                      else if ((*quantity)->type == QuantityType::SkinFriction)
                        {
                          ...
                          for (unsigned int j = 0; j < velocity_vector.size();
                               ++j)
                            velocity_gradient +=
                              outer_product(velocity_vector[j],
                                            fe_values.shape_grad(j, q));

                          std::shared_ptr<QuantitySkinFriction<dim>>
                            quantity_skin_friction = std::dynamic_pointer_cast<
                              QuantitySkinFriction<dim>>(*quantity);

                          dealii::Tensor<1, dim, double> normal =
                            quantity_skin_friction->normal_vector;
                          dealii::Tensor<1, dim, double> tangent =
                            quantity_skin_friction->tangent_vector;

                          for (unsigned int i = 0; i < dim; ++i)
                            for (unsigned int j = 0; j < dim; ++j)
                              wall_shear_local[p] += tangent[i] *
      ...
      for (typename std::vector<std::shared_ptr<Quantity>>::const_iterator
             quantity = line.quantities.begin();
           quantity != line.quantities.end();
           ++quantity)
        {
          ...
          if ((*quantity)->type == QuantityType::Velocity)
            {
              dealii::Utilities::MPI::sum(
                dealii::ArrayView<double const>(&velocity_local[0][0],
                                                dim * velocity_local.size()),
                mpi_comm,
                dealii::ArrayView<double>(&velocity_local[0][0],
                                          dim * velocity_local.size()));
                  ...
                  for (unsigned int d = 0; d < dim; ++d)
                    {
                      velocity_global[line_iterator][p][d] +=
                        velocity_local[p][d] / length_local[p];
                    }
                }
            }
              ...
              dealii::Utilities::MPI::sum(
                dealii::ArrayView<double const>(&reynolds_local[0][0][0],
                                                dim * dim *
                                                  reynolds_local.size()),
                mpi_comm,
                dealii::ArrayView<double>(&reynolds_local[0][0][0],
                  ...
                  for (unsigned int i = 0; i < dim; ++i)
                    {
                      for (unsigned int j = 0; j < dim; ++j)
                        {
                          reynolds_global[line_iterator][p][i][j] +=
                            reynolds_local[p][i][j] / length_local[p];
                        }
                    }
                }
            }
            ...
            {
              dealii::Utilities::MPI::sum(wall_shear_local,
                                          mpi_comm,
                                          wall_shear_local);

              for (unsigned int p = 0; p < line.n_points; ++p)
                {
                  wall_shear_global[line_iterator][p] +=
                    wall_shear_local[p] / length_local[p];
                }
            }
        }
    }
          ...
          if ((*quantity)->type == QuantityType::Pressure)
            {
              ...
              for (unsigned int p = 0; p < line.n_points; ++p)
                {
                  TYPE vector_cells_and_ref_points =
                    cells_and_ref_points_pressure[line_iterator][p];
              ...
              // MPI communication
              dealii::Utilities::MPI::sum(length_local, mpi_comm, length_local);
              dealii::Utilities::MPI::sum(pressure_local,
                                          mpi_comm,
                                          pressure_local);

              // averaging in space (over homogeneous direction)
              for (unsigned int p = 0; p < line.n_points; ++p)
                pressure_global[line_iterator][p] +=
                  pressure_local[p] / length_local[p];
            }
          ...
          if ((*quantity)->type == QuantityType::PressureCoefficient)
            {
              ...
              TYPE vector_cells_and_ref_points =
                cells_and_ref_points_ref_pressure[line_iterator];
              ...
              // MPI communication
              length_local =
                dealii::Utilities::MPI::sum(length_local, mpi_comm);
              pressure_local =
                dealii::Utilities::MPI::sum(pressure_local, mpi_comm);

              // averaging in space (over homogeneous direction)
              reference_pressure_global[line_iterator] +=
                pressure_local / length_local;
            }
        }
    }
                                       ...
                                       double     &pressure_local)
    {
      ...
      for (typename TYPE::const_iterator cell_and_ref_point =
             vector_cells_and_ref_points.begin();
           cell_and_ref_point != vector_cells_and_ref_points.end();
           ++cell_and_ref_point)
        {
          unsigned int const fe_degree_pressure =
            dof_handler_pressure.get_fe().degree;
    ...
    template <int dim, typename Number>
    void
    LinePlotCalculatorStatisticsHomogeneous<dim, Number>::do_write_output()
      const
    {
      if (dealii::Utilities::MPI::this_mpi_process(mpi_comm) == 0)
        {
          ...
          // Iterator for lines
          unsigned int line_iterator = 0;
          for (typename std::vector<std::shared_ptr<Line<dim>>>::const_iterator
                 line = data.lines.begin();
               line != data.lines.end();
               ++line, ++line_iterator)
            {
              ...
              for (typename std::vector<
                   ...
                   ++quantity)
                {
                      ...
                      for (unsigned int p = 0; p < (*line)->n_points; ++p)
                        {
                          ...
                          for (unsigned int d = 0; d < dim; ++d)
                            f << std::setw(precision + 8) << std::left
                              << global_points[line_iterator][p][d];
                  ...
                  if ((*quantity)->type == QuantityType::ReynoldsStresses)
                    {
                      std::string filename =
                        filename_prefix + "_reynoldsstresses" + ".txt";
                      std::ofstream f;
                      if (clear_files)
                        {
                          f.open(filename.c_str(), std::ios::trunc);
                          ...
                          for (unsigned int j = 0; j < dim; ++j)
                            {
                              f << std::setw(precision + 8) << std::left
                                << "u_" +
                                     dealii::Utilities::int_to_string(i + 1) +
                                     "u_" +
                          ...
                          for (unsigned int i = 0; i < dim; ++i)
                            {
                              for (unsigned int j = 0; j < dim; ++j)
                                {
                                  // equation <u_i' u_j'> = <u_i*u_j> - <u_i> *
                                  // <u_j>
                                  f << std::setw(precision + 8) << std::left
                                    << reynolds_global[line_iterator][p][i][j] /
                                           number_of_samples -
                                         (velocity_global[line_iterator][p][i] /
                                          number_of_samples) *
                                           (velocity_global[line_iterator][p]
                                                           [j] /
                                            number_of_samples);
                                }
                            }
                  ...
                  if ((*quantity)->type == QuantityType::SkinFriction)
                    {
                      std::shared_ptr<QuantitySkinFriction<dim>>
                        averaging_quantity =
                          std::dynamic_pointer_cast<QuantitySkinFriction<dim>>(
                            *quantity);

                      std::string filename =
                        filename_prefix + "_wall_shear_stress" + ".txt";
                      std::ofstream f;
                      if (clear_files)
                        {
                          f.open(filename.c_str(), std::ios::trunc);
                      ...
                      for (unsigned int p = 0; p < (*line)->n_points; ++p)
                        {
                          ...
                          double const viscosity =
                            averaging_quantity->viscosity;
                          f << std::setw(precision + 8) << std::left
                            << viscosity * wall_shear_global[line_iterator][p] /
                                 number_of_samples;
