...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    template <int dim, typename Number>
    Number
    MeanVelocityCalculator<dim, Number>::calculate_mean_velocity_area(
    ...
    template <int dim, typename Number>
    Number
    MeanVelocityCalculator<dim, Number>::calculate_mean_velocity_volume(
      VectorType const &velocity,
      double const     &time)
    {
      if (data.calculate == true)
    ...
    template <int dim, typename Number>
    Number
    MeanVelocityCalculator<dim, Number>::calculate_flow_rate_volume(
      VectorType const &velocity,
      double const     &time,
      double const     &length) const
    {
      if (data.calculate == true)
        {
          Number flow_rate =
            1.0 / length * do_calculate_flow_rate_volume(velocity);
    ...
    template <int dim, typename Number>
    Number
    MeanVelocityCalculator<dim, Number>::calculate_flow_rate_area(
      VectorType const &velocity,
    ...
    template <int dim, typename Number>
    void
    MeanVelocityCalculator<dim, Number>::write_output(
      Number const      &value,
      double const      &time,
      std::string const &name) const
    {
      // write output file
      if (data.write_to_file == true and
    ...
    MeanVelocityCalculator<dim, Number>::calculate_volume() const
    {
      ...
      matrix_free.cell_loop(&This::local_calculate_volume,
                            this,
                            dst,
                            src_dummy);
    ...
    template <int dim, typename Number>
    Number
    MeanVelocityCalculator<dim, Number>::do_calculate_flow_rate_area(
          ...
          if (it != data.boundary_IDs.end())
            {
              ...
              scalar flow_rate_face =
                dealii::make_vectorized_array<Number>(0.0);

              for (unsigned int q = 0; q < integrator.n_q_points; ++q)
                {
                  flow_rate_face += integrator.JxW(q) *
    ...
    {
      std::vector<Number> dst(1, 0.0);
      matrix_free.cell_loop(&This::local_calculate_flow_rate_volume,
                            this,
                            dst,
                            velocity);
      ...
      VectorType const &velocity) const
    {
      ...
      // sum over all MPI processes
      Number flow_rate_times_length =
        dealii::Utilities::MPI::sum(dst.at(0), mpi_comm);
      ...
      std::pair<unsigned int, unsigned int> const &cell_range) const
    {
      ...
      for (unsigned int cell = cell_range.first; cell < cell_range.second;
           ++cell)
        {
          ...
          for (unsigned int q = 0; q < integrator.n_q_points; ++q)
            {
              scalar velocity_direction =
                this->data.direction * integrator.get_value(q);
