...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    TimeIntBDFDualSplitting<dim, Number>::update_time_integrator_constants()
    {
      ...
      // constants
      //    std::cout << "Coefficients extrapolation scheme pressure NBC:" <<
      //    std::endl; extra_pressure_nbc.print(this->pcout);
    }
    ...
    TimeIntBDFDualSplitting<dim, Number>::setup_derived()
    {
      ...
      // velocity_dbc vectors do not have to be initialized in case of a
      // restart, where the vectors are read from restart files.
    ...
    template <int dim, typename Number>
    void
    TimeIntBDFDualSplitting<dim, Number>::read_restart_vectors(
    ...
    TimeIntBDFDualSplitting<dim,
                            Number>::initialize_former_multistep_dof_vectors()
    {
    ...
    {
      // fill vector velocity_dbc: The first entry [0] is already needed if
      ...
      pde_operator->interpolate_velocity_dirichlet_bc(velocity_dbc[0],
                                                      this->get_time());
          ...
          for (unsigned int i = 1; i < velocity_dbc.size(); ++i)
            {
              ...
              pde_operator->interpolate_velocity_dirichlet_bc(velocity_dbc[i],
                                                              time);
            }
        }
    }
    ...
    template <int dim, typename Number>
    void
    TimeIntBDFDualSplitting<dim, Number>::set_velocity(
    ...
    template <int dim, typename Number>
    void
    TimeIntBDFDualSplitting<dim, Number>::set_pressure(
    ...
    TimeIntBDFDualSplitting<dim, Number>::convective_step()
    {
      ...
      // compute convective term and extrapolate convective term (if not Stokes
      // equations)
      if (this->param.convective_problem())
        {
          if (this->param.ale_formulation)
            {
              for (unsigned int i = 0; i < this->vec_convective_term.size();
                   ++i)
                {
                  // in a general setting, we only know the boundary conditions
                  // at time t_{n+1}
          ...
          for (unsigned int i = 0; i < this->vec_convective_term.size(); ++i)
            velocity_np.add(-this->extra.get_beta(i),
                            this->vec_convective_term[i]);
        }
      ...
      if (this->param.right_hand_side == true)
        {
          pde_operator->evaluate_add_body_force_term(velocity_np,
                                                     this->get_next_time());
        }
      ...
      for (unsigned int i = 0; i < velocity.size(); ++i)
        {
          velocity_np.add(this->bdf.get_alpha(i) / this->get_time_step_size(),
                          velocity[i]);
        }
      ...
      if (this->print_solver_info() and not(this->is_test))
        {
          ...
          else if (this->param.spatial_discretization ==
      ...
      this->timer_tree->insert({"Timeloop", "Convective step"},
                               timer.wall_time());
    }
    ...
    TimeIntBDFDualSplitting<dim, Number>::rhs_pressure(VectorType &rhs) const
    {
      ...
      // inhomogeneous parts of boundary face integrals of velocity divergence
      // operator
      if (this->param.divu_integrated_by_parts == true and
          this->param.divu_use_boundary_data == true)
        {
          ...
          for (unsigned int i = 0; i < velocity.size(); ++i)
            {
              ...
              // note that the minus sign related to this term is already taken
              // into account in the function rhs() of the divergence operator
              rhs.add(this->bdf.get_alpha(i) / this->get_time_step_size(),
                      temp);
            }
          ...
          if (this->param.convective_problem())
            {
              for (unsigned int i = 0; i < velocity.size(); ++i)
                {
                  temp = 0.0;
                  pde_operator->rhs_ppe_div_term_convective_term_add(
                    temp, velocity[i]);
          ...
          if (this->param.right_hand_side)
            pde_operator->rhs_ppe_div_term_body_forces_add(
              rhs, this->get_next_time());
        }

      /*
       *  II. calculate terms originating from inhomogeneous parts of boundary
       * face integrals of Laplace operator
       */
      ...
      if (this->param.right_hand_side)
        {
          pde_operator->rhs_ppe_nbc_body_force_term_add(rhs,
                                                        this->get_next_time());
        }

      // II.3. pressure Neumann boundary condition: temporal derivative of
      // velocity
      VectorType acceleration(velocity_dbc_np);
      compute_bdf_time_derivative(acceleration,
                                  velocity_dbc_np,
                                  velocity_dbc,
                                  this->bdf,
                                  this->get_time_step_size());
      pde_operator->rhs_ppe_nbc_numerical_time_derivative_add(rhs,
                                                              acceleration);

      // II.4. viscous term of pressure Neumann boundary condition on Gamma_D:
      //       extrapolate velocity, evaluate vorticity, and subsequently
      //       evaluate boundary face integral (this is possible since pressure
      //       Neumann BC is linear in vorticity)
          ...
          if (this->param.order_extrapolation_pressure_nbc > 0)
            {
              ...
              for (unsigned int i = 0; i < extra_pressure_nbc.get_order(); ++i)
                {
                  velocity_extra.add(this->extra_pressure_nbc.get_beta(i),
                                     velocity[i]);
                }
      ...
      // II.5. convective term of pressure Neumann boundary condition on
      // Gamma_D:
      ...
      // special case: pressure level is undefined
      // Set mean value of rhs to zero in order to obtain a consistent linear
      // system of equations. This is really necessary for the dual-splitting
      // scheme in contrast to the pressure-correction scheme and coupled
      // solution approach due to the Dirichlet BC prescribed for the
      // intermediate velocity field and the pressure Neumann BC in case of the
    ...
    TimeIntBDFDualSplitting<dim, Number>::projection_step()
    {
      ...
      if (this->param.apply_penalty_terms_in_postprocessing_step == false and
          (this->param.use_divergence_penalty == true or
      ...
      else // no penalty terms
        {
          if (this->print_solver_info() and not(this->is_test))
            {
              if (this->param.spatial_discretization ==
      ...
      this->timer_tree->insert({"Timeloop", "Pojection step"},
                               timer.wall_time());
    }
      ...
      if (this->param.viscous_problem() or
          this->param.non_explicit_convective_problem())
        {
          ...
          if (this->store_solution)
           ...
           // there is nothing to do in this step of the dual splitting scheme
        {
          // nothing to do
      ...
      VectorType const &transport_velocity) const
    {
      ...
      // compensate for explicit convective term taken into account in the first
      // sub-step of the dual-splitting scheme
      ...
      if (this->param.nonlinear_problem_has_to_be_solved())
        {
          // for a nonlinear problem, inhomogeneous contributions are taken into
          // account when evaluating the nonlinear residual
        }
      else // linear problem
        {
          // compute inhomogeneous contributions of linearly implicit convective
          // term
          if (this->param.convective_problem() and
              this->param.treatment_of_convective_term ==
                TreatmentOfConvectiveTerm::LinearlyImplicit)
    ...
    TimeIntBDFDualSplitting<dim, Number>::penalty_step()
    {
      if (this->param.use_divergence_penalty == true or
          this->param.use_continuity_penalty == true)
        {
          ...
          // right-hand side term: add inhomogeneous contributions of continuity
          // penalty operator to rhs-vector if desired
          ...
          this->timer_tree->insert({"Timeloop", "Penalty step"},
                                   timer.wall_time());
        }
    }
    ...
    TimeIntBDFDualSplitting<dim, Number>::prepare_vectors_for_next_timestep()
    {
      ...
      // We also have to care about the history of velocity Dirichlet boundary
      // conditions. Note that velocity_dbc_np has already been updated.
      ...
      if (this->param.spatial_discretization == SpatialDiscretization::HDIV)
        {
          ...
          iterations_avg.push_back(iterations_mass.second /
                                   std::max(1., (double)iterations_mass.first));
        }

      if (this->param.apply_penalty_terms_in_postprocessing_step)
        {
          names.push_back("Penalty step");
          iterations_avg.push_back(
            (double)iterations_penalty.second /
