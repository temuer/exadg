...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    TimeIntBDFPressureCorrection<dim, Number>::TimeIntBDFPressureCorrection(
      ...
      , extra_pressure_gradient(param_in.order_pressure_extrapolation,
    ...
    template <int dim, typename Number>
    void
    TimeIntBDFPressureCorrection<dim,
                                 Number>::update_time_integrator_constants()
    {
      // call function of base class to update the standard time integrator
      // constants
      Base::update_time_integrator_constants();

      // update time integrator constants for extrapolation scheme of pressure
      // gradient term

      // if start_with_low_order == true (no analytical solution available)
      // the pressure is unknown at t = t_0:
      // -> use no extrapolation (order=0, non-incremental) in first time step
      // (the pressure solution is calculated in the second sub step)
      // -> use first order extrapolation in second time step, second order
      // extrapolation in third time step, etc.

      // the "-1" indicates that the order of extrapolation of the pressure
      // gradient is one order lower than the order of the BDF time integration
      // scheme
      extra_pressure_gradient.update(this->get_time_step_number() - 1,
                                     this->adaptive_time_stepping,
                                     this->get_time_step_vector());

      // use this function to check the correctness of the time integrator
      // constants
    ...
    TimeIntBDFPressureCorrection<dim, Number>::setup_derived()
    {
      ...
      // pressure_dbc does not have to be initialized in case of a restart,
      // where the vectors are read from memory.
    ...
    TimeIntBDFPressureCorrection<dim, Number>::write_restart_vectors(
      BoostOutputArchiveType &oa) const
    {
    ...
    TimeIntBDFPressureCorrection<dim, Number>::initialize_current_solution()
    {
      ...
      pde_operator->prescribe_initial_conditions(velocity[0],
                                                 pressure[0],
                                                 this->get_time());
    }
    ...
    template <int dim, typename Number>
    void
    TimeIntBDFPressureCorrection<dim, Number>::
      initialize_former_multistep_dof_vectors()
    {
      // note that the loop begins with i=1! (we could also start with i=0 but
      ...
      for (unsigned int i = 1; i < velocity.size(); ++i)
        {
          ...
          pde_operator->prescribe_initial_conditions(
            velocity[i], pressure[i], this->get_previous_time(i));
        }
    }
    ...
    {
      // If start_with_low_order == true, no pressure extrapolation is used in
      // the first time step even for the incremental pressure-correction
      // scheme. Hence, there is no need to initialize the pressure_dbc vector
      // in this case.
          ...
          for (unsigned int i = 0; i < pressure_dbc.size(); ++i)
            {
              ...
              pde_operator->interpolate_pressure_dirichlet_bc(pressure_dbc[i],
                                                              time);
            }
        }
    }
    ...
    template <int dim, typename Number>
    typename TimeIntBDFPressureCorrection<dim, Number>::VectorType const &
    TimeIntBDFPressureCorrection<dim, Number>::get_velocity(
    ...
    template <int dim, typename Number>
    typename TimeIntBDFPressureCorrection<dim, Number>::VectorType const &
    TimeIntBDFPressureCorrection<dim, Number>::get_pressure(
      unsigned int i) const
    {
      return pressure[i];
    }
    ...
    template <int dim, typename Number>
    void
    TimeIntBDFPressureCorrection<dim, Number>::set_velocity(
    ...
    template <int dim, typename Number>
    void
    TimeIntBDFPressureCorrection<dim, Number>::set_pressure(
    ...
    void
    TimeIntBDFPressureCorrection<dim,
                                 Number>::postprocessing_stability_analysis()
    {
      AssertThrow(
        this->order == 1,
        dealii::ExcMessage(
          "Order of BDF scheme has to be 1 for this stability analysis."));

      AssertThrow(
        this->param.convective_problem() == false,
        dealii::ExcMessage(
          "Stability analysis can not be performed for nonlinear convective problems."));

      AssertThrow(
        velocity[0].l2_norm() < 1.e-15 and pressure[0].l2_norm() < 1.e-15,
        dealii::ExcMessage(
          "Solution vector has to be zero for this stability analysis."));
      ...
      for (unsigned int i = 0; i < size; ++i)
        {
          ...
          std::cout << std::scientific << std::setprecision(5)
                    << propagation_matrix.eigenvalue(i) << std::endl;
        }
    ...
    TimeIntBDFPressureCorrection<dim, Number>::momentum_step()
    {
      ...
      // in case we need to iteratively solve a linear or nonlinear system of
      // equations
      if (this->param.viscous_problem() or
          this->param.non_explicit_convective_problem())
        {
          // Extrapolate old solutions to get a good initial estimate for the
          // solver.
           ...
           */
          if (this->param.viscous_problem() and
              this->param.viscosity_is_variable() and
              ...
              if (this->print_solver_info() and not(this->is_test))
                {
                  ...
                  print_wall_time(this->pcout,
                                  timer_viscosity_update.wall_time());
                }
            }
          ...
          bool const update_preconditioner =
            ...
            ((this->time_step_number - 1) %
               this->param.update_preconditioner_momentum_every_time_steps ==
             0);

          if (this->param.nonlinear_problem_has_to_be_solved())
            {
              /*
               *  Calculate the vector that is constant when solving the
               * nonlinear momentum equation (where constant means that the
               * vector does not change from one Newton iteration to the next,
               * i.e., it does not depend on the current solution of the
               * nonlinear solver)
               */
            ...
            {
              // linearly implicit convective term: use extrapolated/stored
              // velocity as transport velocity
              ...
              /*
               *  Calculate the right-hand side of the linear system of
               * equations.
               */
              VectorType rhs(velocity_np);
              ...
              unsigned int n_iter =
                pde_operator->solve_linear_momentum_equation(
                  velocity_np,
                  rhs,
                  transport_velocity,
                  update_preconditioner,
              ...
              if (this->print_solver_info() and not(this->is_test))
                {
                  this->pcout << std::endl << "Solve momentum step:";
                  print_solver_info_linear(this->pcout,
                                           n_iter,
                                           timer.wall_time());
                }
            }
      ...
      else // no viscous term and no (linearly) implicit convective term, i.e.
           // we only need to invert the mass matrix
        {
          /*
      ...
      this->timer_tree->insert({"Timeloop", "Momentum step"},
                               timer.wall_time());
    }

    template <int dim, typename Number>
    void
    TimeIntBDFPressureCorrection<dim, Number>::rhs_momentum(
      VectorType       &rhs,
      VectorType const &transport_velocity)
    {
      ...
      /*
       *  Add extrapolation of pressure gradient term to the rhs in case of
          ...
          for (unsigned int i = 0; i < extra_pressure_gradient.get_order(); ++i)
            {
              ...
              pde_operator
                ->evaluate_pressure_gradient_term_dirichlet_bc_from_dof_vector(
                  temp, pressure[i], pressure_dbc[i]);
      ...
      if (this->param.right_hand_side == true)
        {
          pde_operator->evaluate_add_body_force_term(rhs,
                                                     this->get_next_time());
        }

      /*
       *  Convective term formulated explicitly (additive decomposition):
       *  Evaluate convective term and add extrapolation of convective term to
       * the rhs
       */
      if (this->param.convective_problem())
        {
          if (this->param.treatment_of_convective_term ==
              TreatmentOfConvectiveTerm::Explicit)
            {
              if (this->param.ale_formulation)
                {
                  for (unsigned int i = 0; i < this->vec_convective_term.size();
                       ++i)
                    {
                      // in a general setting, we only know the boundary
                      // conditions at time t_{n+1}
                      pde_operator->evaluate_convective_term(
          ...
          if (this->param.treatment_of_convective_term ==
              TreatmentOfConvectiveTerm::LinearlyImplicit)
            {
              pde_operator->rhs_add_convective_term(rhs,
                                                    transport_velocity,
                                                    this->get_next_time());
            }
        }
      ...
      /*
       *  calculate sum (alpha_i/dt * u_i) and apply mass operator to this
       * vector
       */
      VectorType sum_alphai_ui(velocity[0]);

      // calculate sum (alpha_i/dt * u_i)
      sum_alphai_ui.equ(this->bdf.get_alpha(0) / this->get_time_step_size(),
                        velocity[0]);
      for (unsigned int i = 1; i < velocity.size(); ++i)
        {
       ...
       *  If there is no nonlinearity, we solve a linear system of equations,
       * where inhomogeneous parts of boundary face integrals of the viscous
       * operator have to be shifted to the right-hand side of the equation.
       */
      if (this->param.viscous_problem() and
          not(this->param.nonlinear_problem_has_to_be_solved()))
        {
      ...
      VectorType &pressure_increment)
    {
      ...
      if (this->use_extrapolation)
        {
          ...
          if (extra_pressure_gradient.get_order() > 0)
            {
              // Subtract extrapolation of pressure since the PPE is solved for
              // the pressure increment phi = p_{n+1} - sum_i
              // (beta_pressure_extra_i * pressure_i), where p_{n+1} is
              // approximated by an extrapolation of order J (=order of BDF
              // scheme). Note that the divergence correction term in case of
              // the rotational formulation is not considered when calculating a
              // good initial guess for the solution of the PPE, which will
              // slightly increase the number of iterations compared to the
              // standard formulation of the pressure-correction scheme.
              for (unsigned int i = 0; i < extra_pressure_gradient.get_order();
                   ++i)
                {
                  pressure_increment.add(-extra_pressure_gradient.get_beta(i),
                                         pressure[i]);
                }
            }
        }
      ...
      unsigned int const n_iter =
        pde_operator->solve_pressure(pressure_increment,
                                     rhs,
                                     update_preconditioner);
      ...
      // Adjust the pressure level in order to allow a calculation of the
      // pressure error. This is necessary because otherwise the pressure
      // solution moves away from the exact solution.
      pde_operator->adjust_pressure_level_if_undefined(pressure_np,
                                                       this->get_next_time());
      ...
      this->timer_tree->insert({"Timeloop", "Pressure step"},
                               timer.wall_time());
    }
    ...
    {
      if (this->param.formulation_viscous_term ==
          FormulationViscousTerm::LaplaceFormulation)
        {
          chi = 1.0;
        }
      else if (this->param.formulation_viscous_term ==
    ...
    template <int dim, typename Number>
    void
    TimeIntBDFPressureCorrection<dim, Number>::rhs_pressure(
      VectorType &rhs) const
    {
      ...
      pde_operator->evaluate_velocity_divergence_term(temp,
                                                      velocity_np,
                                                      this->get_next_time());
      ...
      /*
       *  II. calculate terms originating from inhomogeneous parts of boundary
       * face integrals, i.e., pressure Dirichlet boundary conditions on Gamma_N
       * and pressure Neumann boundary conditions on Gamma_D (always h=0 for
      ...
      for (unsigned int i = 0; i < extra_pressure_gradient.get_order(); ++i)
        {
          ...
          temp = 0.0;
          pde_operator->rhs_ppe_laplace_add_dirichlet_bc_from_dof_vector(
            temp, pressure_dbc[i]);
      ...
      // In principle, it works (since the linear system of equations is
      // consistent) but we detected no convergence for some test cases and
      // specific parameters. Hence, for reasons of robustness we also solve a
      // transformed linear system of equations in case of the
      // pressure-correction scheme.
    ...
    TimeIntBDFPressureCorrection<dim, Number>::pressure_update(
      VectorType const &pressure_increment)
    {
      ...
      if (this->param.rotational_formulation == true)
        {
          ...
          pde_operator->evaluate_velocity_divergence_term(
            pressure_np, velocity_np, this->get_next_time());

          pde_operator->apply_inverse_pressure_mass_operator(pressure_np,
                                                             pressure_np);
      ...
      // This is done for both the incremental and the non-incremental
      // formulation, the standard and the rotational formulation.
      pressure_np.add(1.0, pressure_increment);

      // Incremental formulation only.

      // add extrapolation of pressure to the pressure-increment solution in
      // order to obtain the pressure solution at the end of the time step,
      // i.e., p^{n+1} = (pressure_increment)^{n+1} + sum_i
      // (beta_pressure_extrapolation_i * p^{n-i});
      for (unsigned int i = 0; i < extra_pressure_gradient.get_order(); ++i)
        {
      ...
      VectorType const &pressure_increment) const
    {
      ...
      /*
       *  II. calculate pressure gradient term including boundary condition
       * g_p(t_{n+1})
       */
      VectorType temp(rhs);
       ...
       */
      if (this->param.gradp_integrated_by_parts == true and
          this->param.gradp_use_boundary_data == true)
        {
          for (unsigned int i = 0; i < extra_pressure_gradient.get_order(); ++i)
            {
              // evaluate inhomogeneous parts of boundary face integrals
              // note that the function rhs_...() already includes a factor of
              // -1.0
              pde_operator
                ->rhs_pressure_gradient_term_dirichlet_bc_from_dof_vector(
                  temp, pressure_dbc[i]);

              rhs.add(-extra_pressure_gradient.get_beta(i) *
                        this->get_time_step_size() / this->bdf.get_gamma0(),
                      temp);
            }
        }
    }
      ...
      VectorType const &pressure_increment)
    {
      ...
      // apply inverse mass operator: this is the solution if no penalty terms
      // are applied and serves as a good initial guess for the case with
      // penalty terms
      pde_operator->apply_inverse_mass_operator(velocity_np, rhs);

      if (this->param.use_divergence_penalty == true or
          this->param.use_continuity_penalty == true)
        {
          // extrapolate velocity to time t_{n+1} and use this velocity field to
          // calculate the penalty parameter for the divergence and continuity
          // penalty terms
          ...
          pde_operator->update_projection_operator(velocity_extrapolated,
                                                   this->get_time_step_size());

          // add inhomogeneous contributions of continuity penalty term after
          // computing the initial guess for the linear system of equations to
          // make sure that the initial guess is as accurate as possible
          if (this->param.use_continuity_penalty and
              this->param.continuity_penalty_use_boundary_data)
            pde_operator->rhs_add_projection_operator(rhs,
                                                      this->get_next_time());
          ...
          unsigned int const n_iter =
            pde_operator->solve_projection(velocity_np,
                                           rhs,
                                           update_preconditioner);
      ...
      this->timer_tree->insert({"Timeloop", "Projection step"},
                               timer.wall_time());
    }
    ...
    void
    TimeIntBDFPressureCorrection<dim,
                                 Number>::prepare_vectors_for_next_timestep()
    {
      ...
      // We also have to care about the history of pressure Dirichlet boundary
      // conditions.
      if (extra_pressure_gradient.get_order() > 0)
        {
          push_back(pressure_dbc);

          // no need to move the mesh here since we still have the mesh
          // Omega_{n+1} at this point!
          pde_operator->interpolate_pressure_dirichlet_bc(
            pressure_dbc[0], this->get_next_time());
        }
    }
      ...
      if (this->param.convergence_criterion_steady_problem ==
          ConvergenceCriterionSteadyProblem::SolutionIncrement)
        {
          ...
          while (not(converged) and
                 this->time < (this->end_time - this->eps) and
                 this->get_time_step_number() <=
                   this->param.max_number_of_time_steps)
            {
              ...
              if (this->print_solver_info())
                {
                  this->pcout << std::endl
                              ...
                              << "  ||incr_abs|| = " << std::scientific
                              << std::setprecision(10) << incr << std::endl
                              << "  ||incr_rel|| = " << std::scientific
                              << std::setprecision(10) << incr_rel << std::endl;
                }

              // check convergence
              if (incr < this->param.abs_tol_steady or
    ...
    TimeIntBDFPressureCorrection<dim, Number>::evaluate_residual()
    {
      ...
      if (this->print_solver_info())
        {
          this->pcout << std::endl
                      << "Norm of residual of steady Navier-Stokes equations:"
                      << std::endl
                      << "  ||r|| = " << std::scientific
                      << std::setprecision(10) << residual << std::endl;
        }
      ...
      if (this->param.nonlinear_problem_has_to_be_solved())
        {
          ...
          iterations_avg[3] = (double)iterations_pressure.second /
                              std::max(1., (double)iterations_pressure.first);
          iterations_avg[4] = (double)iterations_projection.second /
                              std::max(1., (double)iterations_projection.first);
        }
      else // linear problem
        {
          ...
          iterations_avg[0] = (double)std::get<1>(iterations_momentum.second) /
                              std::max(1., (double)iterations_momentum.first);
          iterations_avg[1] = (double)iterations_pressure.second /
                              std::max(1., (double)iterations_pressure.first);
          iterations_avg[2] = (double)iterations_projection.second /
                              std::max(1., (double)iterations_projection.first);
        }
