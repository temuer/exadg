...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    {
      double time = 0.0;
      pde_operator->prescribe_initial_conditions(solution.block(0),
                                                 solution.block(1),
                                                 time);
    }
    ...
    template <int dim, typename Number>
    void
    DriverSteadyProblems<dim, Number>::solve(double const time,
    ...
    template <int dim, typename Number>
    void
    DriverSteadyProblems<dim, Number>::do_solve(double const time,
                                                bool         unsteady_problem)
    {
      ...
      if (this->param.nonlinear_problem_has_to_be_solved())
        ...
        {
          // linearly implicit convective term does not make sense for steady
          // problems, but we need to pass a vector to the functions
          // rhs_linear_problem() and solve_linear_problem()
    ...
    template <int dim, typename Number>
    bool
    DriverSteadyProblems<dim, Number>::print_solver_info(
      double const time,
      bool         unsteady_problem) const
    {
      return not(unsteady_problem) or
             param.solver_info_data.write(this->global_timer.wall_time(),
