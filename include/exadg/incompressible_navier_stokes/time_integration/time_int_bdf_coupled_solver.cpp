  ...
  namespace IncNS
  {
    ...
    TimeIntBDFCoupled<dim, Number>::setup_derived()
    {
      ...
      characteristic_element_length =
        pde_operator->get_characteristic_element_length();
    }
    ...
    template <int dim, typename Number>
    void
    TimeIntBDFCoupled<dim, Number>::set_velocity(VectorType const  &velocity_in,
    ...
    template <int dim, typename Number>
    void
    TimeIntBDFCoupled<dim, Number>::set_pressure(VectorType const  &pressure_in,
                                                 unsigned int const i)
    {
    ...
    TimeIntBDFCoupled<dim, Number>::do_timestep_solve()
    {
      ...
      // extrapolate old solutions to obtain a good initial guess for the
      // solver, or to update the viscosity model or the penalty parameters
      // based on this extrapolated solution
      ...
      if (this->param.apply_penalty_terms_in_postprocessing_step == false)
        {
          if (this->param.use_divergence_penalty == true)
            {
              pde_operator->update_divergence_penalty_operator(
                solution_np.block(0));
            }
          if (this->param.use_continuity_penalty == true)
            {
              pde_operator->update_continuity_penalty_operator(
                solution_np.block(0));
            }
        }
      ...
      if (this->param.use_scaling_continuity == true)
        {
          ...
          pde_operator->set_scaling_factor_continuity(
      ...
      bool const update_preconditioner =
        ...
        ((this->time_step_number - 1) %
           this->param.update_preconditioner_coupled_every_time_steps ==
         0);
      ...
      if (this->param.convective_problem() and
            ...
            TreatmentOfConvectiveTerm::Explicit)
        {
          if (this->param.ale_formulation)
            {
              // evaluate convective term for all previous times since the mesh
              // has been updated
      ...
      if (this->param.nonlinear_problem_has_to_be_solved())
        {
          ...
          if (this->param.convective_problem() and
              this->param.treatment_of_convective_term ==
                TreatmentOfConvectiveTerm::Explicit)
            {
              // add extrapolation of convective term to the rhs (-> minus
              // sign!)
              for (unsigned int i = 0; i < this->vec_convective_term.size();
                   ++i)
        ...
        {
          // linearly implicit convective term: use extrapolated/stored velocity
          // as transport velocity
          ...
          // calculate rhs vector for the linear problem, with contributions
          // from the convective term for a linearly implicit formulation
          pde_operator->rhs_linear_problem(rhs_vector,
                                           transport_velocity,
                                           this->get_next_time());
          ...
          if (this->param.convective_problem() and
                ...
                TreatmentOfConvectiveTerm::Explicit)
            {
              ...
              for (unsigned int i = 0; i < this->vec_convective_term.size();
                   ++i)
                rhs_vector.block(0).add(-this->extra.get_beta(i),
                                        this->vec_convective_term[i]);
            }

          // apply mass operator to sum_alphai_ui and add to rhs vector
          pde_operator->apply_mass_operator_add(rhs_vector.block(0),
                                                sum_alphai_ui);
      ...
      // reconstruct pressure solution p from auxiliary variable p^{*}: p =
      // scaling_factor * p^{*}
      ...
      pde_operator->adjust_pressure_level_if_undefined(solution_np.block(1),
                                                       this->get_next_time());

      if (this->store_solution and
          this->param.apply_penalty_terms_in_postprocessing_step == true)
        {
          solution_last_iter = solution_np;
        }

      this->timer_tree->insert({"Timeloop", "Coupled system"},
                               timer.wall_time());
    ...
    TimeIntBDFCoupled<dim, Number>::penalty_step()
    {
      ...
      if (this->use_extrapolation)
        {
          ...
          for (unsigned int i = 0; i < solution.size(); ++i)
            velocity_extrapolated.add(this->extra.get_beta(i),
                                      solution[i].block(0));
        }
      else
        {
      ...
      unsigned int n_iter =
        pde_operator->solve_projection(solution_np.block(0),
                                       rhs,
                                       update_preconditioner);
    ...
    TimeIntBDFCoupled<dim, Number>::postprocessing_stability_analysis()
    {
      ...
      AssertThrow(
        solution[0].block(0).l2_norm() < 1.e-15 and
          solution[0].block(1).l2_norm() < 1.e-15,
        dealii::ExcMessage(
