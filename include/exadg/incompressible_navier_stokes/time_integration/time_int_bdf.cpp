...
namespace ExaDG
{
  namespace IncNS
  {
      ...
      , vec_grid_coordinates(param_in.order_time_integrator)
    {
      needs_vector_convective_term =
        ...
        (this->param.treatment_of_convective_term ==
           TreatmentOfConvectiveTerm::Explicit or
      ...
      if (needs_vector_convective_term)
        {
          for (unsigned int i = 0; i < vec_convective_term.size(); ++i)
            this->operator_base->initialize_vector_velocity(
      ...
      if (param.ale_formulation and param.restarted_simulation == false)
        {
          ...
          helpers_ale->fill_grid_coordinates_vector(
            vec_grid_coordinates[0], this->operator_base->get_dof_handler_u());
      ...
      if (needs_vector_convective_term)
        {
          ...
          // from memory.
          if (this->param.ale_formulation == false and
    ...
    TimeIntBDF<dim, Number>::ale_update()
    {
      ...
      // t_{n+1}
      helpers_ale->fill_grid_coordinates_vector(
        grid_coordinates_np, this->operator_base->get_dof_handler_u());
    ...
    template <int dim, typename Number>
    void
    TimeIntBDF<dim, Number>::advance_one_timestep_partitioned_solve(
      ...
      if (this->param.start_with_low_order == false)
        {
          for (unsigned int i = 1; i < vec_convective_term.size(); ++i)
            {
              this->operator_base->evaluate_convective_term(
                vec_convective_term[i],
                get_velocity(i),
    ...
    TimeIntBDF<dim, Number>::calculate_time_step_size()
    {
      ...
      if (param.calculation_of_time_step_size ==
          TimeStepCalculation::UserSpecified)
        {
          ...
          this->pcout << std::endl
                      << "User specified time step size:" << std::endl
                      << std::endl;
      ...
      else if (param.calculation_of_time_step_size == TimeStepCalculation::CFL)
        {
          double time_step_global =
            operator_base->calculate_time_step_cfl_global();
        ...
        {
          time_step =
            operator_base->calculate_time_step_max_efficiency(this->order);
    ...
    template <int dim, typename Number>
    void
    TimeIntBDF<dim, Number>::get_velocities_and_times(
      std::vector<VectorType const *> &velocities,
      std::vector<double>             &times) const
    {
      get_quantities_and_times(velocities, times, [&](auto const i) {
    ...
    template <int dim, typename Number>
    void
    TimeIntBDF<dim, Number>::get_velocities_and_times_np(
      std::vector<VectorType const *> &velocities,
      std::vector<double>             &times) const
    {
      get_quantities_and_times_np(
        velocities,
        times,
        [&](auto const i) { return &get_velocity(i); },
        [&]() { return &get_velocity_np(); });
    }

    template <int dim, typename Number>
    void
    TimeIntBDF<dim, Number>::get_pressures_and_times(
      std::vector<VectorType const *> &pressures,
      std::vector<double>             &times) const
    {
      get_quantities_and_times(pressures, times, [&](auto const i) {
        return &get_pressure(i);
      });
    }

    template <int dim, typename Number>
    void
    TimeIntBDF<dim, Number>::get_pressures_and_times_np(
      std::vector<VectorType const *> &pressures,
      std::vector<double>             &times) const
    {
      get_quantities_and_times_np(
        pressures,
        times,
        [&](auto const i) { return &get_pressure(i); },
    ...
    TimeIntBDF<dim, Number>::postprocessing() const
    {
      ...
      // dealii::VectorTools::integrate_difference() does not take constraints
      // into account like MatrixFree does, hence reading the wrong values.
      // distribute_constraint_u() updates the constrained values for the
      // velocity.
      operator_base->distribute_constraint_u(
        const_cast<VectorType &>(get_velocity(0)));
      ...
      if (standard)
      ...
      else // consider velocity and pressure errors instead
        {
          ...
          operator_base->prescribe_initial_conditions(velocity_error,
                                                      pressure_error,
                                                      this->get_time());
