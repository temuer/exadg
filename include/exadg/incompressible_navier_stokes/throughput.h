...
namespace ExaDG
{
    ...
    try
      {
        ...
        IncNS::get_application<Dim, Number>(input_file, MPI_COMM_WORLD)
          ->add_parameters(prm);
      }
    catch (...)
      {}
      ...
      bool const                                       is_test)
  {
    ...
    std::shared_ptr<IncNS::Driver<dim, Number>> driver =
      std::make_shared<IncNS::Driver<dim, Number>>(mpi_comm,
                                                   application,
                                                   is_test,
                                                   true);
...
main(int argc, char **argv)
{
  ...
  ExaDG::ThroughputParameters<ExaDG::IncNS::OperatorType> throughput(
    input_file);

  ExaDG::IncNS::PressureDegree pressure_degree =
    ExaDG::IncNS::PressureDegree::MixedOrder;
