...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    struct FieldFunctions
    {
      ...
      /*
       * The function analytical_solution_pressure is used to adjust the
       * pressure level in the special case of ...
       *   ... pure Dirichlet boundary conditions (where the pressure is only
      ...
      /*
       * Analytical solution of the velocity. It is e.g. used by
       * TimeIntInterpolateAnalyticalSolution that interpolates the analytical
       * solution instead of computing the solution.
       */
      std::shared_ptr<dealii::Function<dim>> analytical_solution_velocity;

      /*
       * The function right_hand_side is used to evaluate the body force term on
       * the right-hand side of the momentum equation of the incompressible
       * Navier-Stokes equations.
       */
      std::shared_ptr<dealii::Function<dim>> right_hand_side;
      std::shared_ptr<dealii::Function<dim>>
        gravitational_force; // Boussinesq term
    };
