  ...
  namespace IncNS
  {
    ...
    Parameters::Parameters()
      ...
      convergence_criterion_steady_problem(
        ConvergenceCriterionSteadyProblem::Undefined)
      ...
      order_extrapolation_pressure_nbc(
        (order_time_integrator <= 2) ? order_time_integrator : 2)
    ...
    Parameters::check(dealii::ConditionalOStream const &pcout) const
    {
      ...
      AssertThrow(formulation_viscous_term != FormulationViscousTerm::Undefined,
                  dealii::ExcMessage("parameter must be defined"));
      AssertThrow(formulation_convective_term !=
                    FormulationConvectiveTerm::Undefined,
                  dealii::ExcMessage("parameter must be defined"));
      ...
      // In case of a steady solver, the parameter temporal_discretization does
      // not have to be specified, since we always choose a coupled/monolithic
      // solution approach).
      if (solver_type == SolverType::Unsteady)
        {
          AssertThrow(temporal_discretization !=
                        TemporalDiscretization::Undefined,
                      dealii::ExcMessage("parameter must be defined"));
        }

      if (convective_problem())
        {
          AssertThrow(treatment_of_convective_term !=
                        TreatmentOfConvectiveTerm::Undefined,
                      dealii::ExcMessage("parameter must be defined"));

          if (treatment_of_convective_term ==
              TreatmentOfConvectiveTerm::LinearlyImplicit)
            {
              AssertThrow(
      ...
      // For the coupled solution approach, degree_p = 0 is allowed in
      // principle. For projection-type methods, degree_p > 0 has to be
      // fulfilled (the SIPG discretization of the pressure Poisson equation
      // would be inconsistent for degree_p = 0).
      ...
      AssertThrow(IP_formulation_viscous !=
                    InteriorPenaltyFormulation::Undefined,
                  dealii::ExcMessage("parameter must be defined"));

      if (formulation_viscous_term ==
          FormulationViscousTerm::DivergenceFormulation)
        {
          AssertThrow(penalty_term_div_formulation !=
                        PenaltyTermDivergenceFormulation::Undefined,
                      dealii::ExcMessage("parameter must be defined"));
        }

      if (equation_type == EquationType::NavierStokes)
        {
          AssertThrow(upwind_factor >= 0.0,
                      dealii::ExcMessage(
                        "Upwind factor must not be negative."));
        }

      if (use_continuity_penalty == true)
        {
          AssertThrow(continuity_penalty_components !=
                        ContinuityPenaltyComponents::Undefined,
                      dealii::ExcMessage("Parameter must be defined"));

          if (continuity_penalty_use_boundary_data == true)
            {
              if (temporal_discretization ==
      ...
      if (preconditioner_momentum == MomentumPreconditioner::Multigrid)
        {
          AssertThrow(multigrid_operator_type_momentum !=
                        MultigridOperatorType::Undefined,
                      dealii::ExcMessage("Parameter must be defined"));
          ...
          if (treatment_of_convective_term ==
            ...
            {
              AssertThrow(
                multigrid_operator_type_momentum !=
      ...
      if (temporal_discretization ==
          TemporalDiscretization::BDFDualSplittingScheme)
        {
          if (spatial_discretization == SpatialDiscretization::HDIV)
            {
              pcout
                ...
                << std::endl
                << "solution. Use the coupled scheme instead if this is desired."
                << std::endl;
          ...
          AssertThrow(order_extrapolation_pressure_nbc <= order_time_integrator,
                      dealii::ExcMessage(
                        "Invalid parameter order_extrapolation_pressure_nbc!"));
      ...
      if (temporal_discretization ==
          TemporalDiscretization::BDFPressureCorrection)
        {
          ...
          AssertThrow(order_pressure_extrapolation <= order_time_integrator,
                      dealii::ExcMessage(
                        "Invalid parameter order_pressure_extrapolation!"));
        }
      ...
      if (temporal_discretization == TemporalDiscretization::BDFCoupledSolution)
        {
          if (use_scaling_continuity == true)
            AssertThrow(scaling_factor_continuity > 0.0,
                        dealii::ExcMessage("Invalid parameter"));

          if (preconditioner_velocity_block ==
              MomentumPreconditioner::Multigrid)
            {
              ...
              if (equation_type == EquationType::Stokes)
                {
                  AssertThrow(
                    multigrid_operator_type_velocity_block !=
                      MultigridOperatorType::ReactionConvectionDiffusion,
                    dealii::ExcMessage(
    ...
    {
      return (equation_type == EquationType::NavierStokes or
    ...
    {
      return (equation_type == EquationType::Stokes or
              equation_type == EquationType::NavierStokes or
    ...
    {
      return (turbulence_model_data.is_active or
    ...
    {
      return (implicit_nonlinear_convective_problem() or
              nonlinear_viscous_problem());
    }
    ...
    Parameters::involves_h_multigrid() const
    {
      ...
      // TODO: we currently treat
      // TemporalDiscretization::InterpolateAnalyticalSolution as a splitting
      // scheme, since the implementation creates a spatial operator related to
      // the pressure-correction scheme in case of
      // InterpolateAnalyticalSolution. This is only a temporary solution and we
      // need to write a separate class
      // SpatialOperatorInterpolateAnalyticalSolution that does not create
      ...
      if (degree_p == DegreePressure::MixedOrder)
        {
          AssertThrow(
            degree_u > 0,
            dealii::ExcMessage(
              "The polynomial degree of the velocity shape functions"
      ...
      if (ale_formulation)
        {
          ...
          print_parameter(pcout,
                          "NBC with variable normal vector",
                          neumann_with_variable_normal_vector);
        }
    }
      ...
      if (adaptive_time_stepping)
        {
          ...
          print_parameter(pcout,
                          ...
                          adaptive_time_stepping_cfl_type);
        }


      // here we do not print quantities such as max_velocity, cfl,
      // time_step_size because this is done by the time integration scheme (or
      // the functions that calculate the time step size)
      ...
      dealii::ConditionalOStream const &pcout) const
    {
      ...
      if (spatial_discretization == SpatialDiscretization::L2)
        ...
        {
          print_parameter(pcout,
                          "Polynomial degree velocity (normal)",
                          degree_u);
          print_parameter(pcout,
                          "Polynomial degree velocity (tangential)",
                          degree_u);
        }
      else
        {
      ...
      print_parameter(pcout,
                      ...
                      adjust_pressure_level);

      print_parameter(pcout,
                      "Use divergence penalty term",
                      use_divergence_penalty);
      ...
      print_parameter(pcout,
                      ...
                      use_continuity_penalty);

      if (temporal_discretization ==
            TemporalDiscretization::BDFCoupledSolution or
          temporal_discretization ==
            TemporalDiscretization::BDFDualSplittingScheme)
        {
          if (use_divergence_penalty == true or use_continuity_penalty == true)
      ...
      if (use_continuity_penalty == true)
        {
          ...
          print_parameter(pcout,
                          ...
                          continuity_penalty_factor);

          print_parameter(pcout,
                          "Continuity penalty term components",
                          continuity_penalty_components);
        }
      ...
      dealii::ConditionalOStream const &pcout) const
    {
      ...
      print_parameter(pcout,
                      "Quadrature rule linearization",
                      quad_rule_linearization);
    }

    void
    Parameters::print_parameters_pressure_poisson(
      dealii::ConditionalOStream const &pcout) const
    {
      // pressure Poisson equation
      ...
      if (preconditioner_pressure_poisson ==
          PreconditionerPressurePoisson::Multigrid)
        {
    ...
    void
    Parameters::print_parameters_projection_step(
      dealii::ConditionalOStream const &pcout) const
    {
      if (use_divergence_penalty == true)
        {
          ...
          if (use_divergence_penalty == true and use_continuity_penalty == true)
            {
              ...
              if (preconditioner_projection ==
                    PreconditionerProjection::BlockJacobi and
                  implement_block_diagonal_preconditioner_matrix_free)
                {
                  print_parameter(pcout,
    ...
    void
    Parameters::print_parameters_dual_splitting(
      dealii::ConditionalOStream const &pcout) const
    {
      // nothing to print if we bypass the PDE solver by
      // TemporalDiscretization::InterpolateAnalyticalSolution
    ...
    void
    Parameters::print_parameters_momentum_step(
      dealii::ConditionalOStream const &pcout) const
    {
      // Momentum step
      ...
      print_parameter(pcout,
                      ...
                      update_preconditioner_momentum);
    ...
    void
    Parameters::print_parameters_pressure_correction(
      dealii::ConditionalOStream const &pcout) const
    {
      // nothing to print if we bypass the PDE solver by
      // TemporalDiscretization::InterpolateAnalyticalSolution
      ...
      pcout << std::endl
            << "  Formulation of pressure-correction scheme:" << std::endl;
      print_parameter(pcout,
                      "Order of pressure extrapolation",
                      order_pressure_extrapolation);
    ...
    void
    Parameters::print_parameters_coupled_solver(
      dealii::ConditionalOStream const &pcout) const
    {
      // nothing to print if we bypass the PDE solver by
      // TemporalDiscretization::InterpolateAnalyticalSolution
      ...
      print_parameter(pcout,
                      "Use scaling of continuity equation",
                      use_scaling_continuity);
      if (use_scaling_continuity == true)
        print_parameter(pcout,
      ...
      if (preconditioner_velocity_block == MomentumPreconditioner::Multigrid)
        {
          ...
          print_parameter(pcout,
                          ...
                          iterative_solve_of_velocity_block);
      ...
      if (preconditioner_pressure_block ==
            SchurComplementPreconditioner::LaplaceOperator or
          preconditioner_pressure_block ==
            SchurComplementPreconditioner::CahouetChabard or
          preconditioner_pressure_block ==
            SchurComplementPreconditioner::PressureConvectionDiffusion)
        {
      ...
      if (use_divergence_penalty == true or use_continuity_penalty == true)
        {
          if (apply_penalty_terms_in_postprocessing_step == true)
            {
              pcout << std::endl
                    << "Postprocessing of velocity (penalty terms):"
                    << std::endl;
              print_parameters_projection_step(pcout);
            }
        }
    ...
    Parameters::involves_h_multigrid_pressure_step() const
    {
      ...
      if (preconditioner_pressure_poisson ==
            PreconditionerPressurePoisson::Multigrid and
