...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    struct BoundaryDescriptorU
    {
      ...
      // another type of Dirichlet boundary condition where the Dirichlet value
      // comes from the solution on another domain that is in contact with the
      // actual domain of interest at the given boundary (this type of Dirichlet
      // boundary condition is required for fluid-structure interaction
      // problems)
      std::set<dealii::types::boundary_id> dirichlet_cached_bc;

      // Neumann: prescribe all components of the velocity gradient in normal
      // direction
      std::map<dealii::types::boundary_id,
               std::shared_ptr<dealii::Function<dim>>>
        neumann_bc;

      // Symmetry: A boundary condition prescribing "symmetry" for planar
      // surfaces. Using this boundary condition, the velocity normal to
      ...
      //
      // where F(u) = grad(u) (Laplace formulation) or F(u) = grad(u) +
      // grad(u)^T (divergence formulation). For both types of formulation of
      // the viscous term, this implies that the gradient of tangential velocity
      // components in the direction normal to the boundary is set to zero
      //
      //   grad(u)*n - [(grad(u)*n)*n] n = 0 . (2b)
      //
      // For the Laplace formulation, equation (2b) follows immediately from
      // equation (2a). For the divergence formulation, one can show that the
      // grad(u)^T terms in equation (2a) drop out if (u*n)=0 and if the normal
      // vector is constant, i.e. for a planar surface
      //
      //   u*n=0, n=const => grad(u)^T*n - [(grad(u)^T*n)*n] n = 0 => (2a)
      //   implies (2b)
      //
      // Note that the user does not have to prescribe data for this symmetry
      // boundary condition; so one can simply use ZeroFunction<dim> (the
      // function is not relevant because it will not be evaluated by the code).
      std::map<dealii::types::boundary_id,
               std::shared_ptr<dealii::Function<dim>>>
        symmetry_bc;
                                     ...
                                     &periodic_boundary_ids) const
      {
        ...
        if (this->dirichlet_cached_bc.find(boundary_id) !=
            this->dirichlet_cached_bc.end())
          counter++;
    ...
    private:
      mutable std::shared_ptr<ContainerInterfaceData<1, dim, double> const>
    ...
    struct BoundaryDescriptorP
    {
      ...
      // Neumann: only the boundary IDs are stored but no inhomogeneous boundary
