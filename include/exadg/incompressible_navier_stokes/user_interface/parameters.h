...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    private:
      ...
      void
      print_parameters_pressure_poisson(
        dealii::ConditionalOStream const &pcout) const;

      void
      print_parameters_projection_step(
        dealii::ConditionalOStream const &pcout) const;

      void
      print_parameters_momentum_step(
        dealii::ConditionalOStream const &pcout) const;

      void
      print_parameters_dual_splitting(
        dealii::ConditionalOStream const &pcout) const;

      void
      print_parameters_pressure_correction(
        dealii::ConditionalOStream const &pcout) const;

      void
      print_parameters_coupled_solver(
        dealii::ConditionalOStream const &pcout) const;
    ...
    public:
      ...
      // if the body force vector on the right-hand side of the momentum
      // equation of the Navier-Stokes equations is unequal zero, set
      // right_hand_side = true This parameter also has to be true when
      // considering the Boussinesq term.
      bool right_hand_side;

      // Boussinesq term (natural convection through buoyancy forces)
      bool boussinesq_term;

      // If Boussinesq term is activated: solves only for dynamic pressure
      // variations if true, and includes hydrostatic component if false.
      ...
      // use true to activate ALE formulation, otherwise the standard Eulerian
      // formulation with fixed mesh will be used
      ...
      // density (not required by fluid solver which is formulated in terms of
      // the kinematic viscosity only, but for the calculation of the fluid
      // stress for FSI problems)
      double density;
      ...
      // This parameter defines by which factor the time step size is allowed to
      // increase or to decrease in case of adaptive time step, e.g., if one
      // wants to avoid large jumps in the time step size. A factor of 1 implies
      // that the time step size can not change at all, while a factor towards
      // infinity implies that arbitrary changes in the time step size are
      // allowed from one time step to the next.
      double adaptive_time_stepping_limiting_factor;

      // specify a maximum time step size in case of adaptive time stepping
      // since the adaptive time stepping algorithm would choose arbitrarily
      // large time step sizes if the velocity field is close to zero. This
      // variable is only used for adaptive time stepping.
      double time_step_size_max;

      // Different variants are available for calculating the time step size
      // based on a local CFL criterion.
      CFLConditionType adaptive_time_stepping_cfl_type;

      // maximum velocity needed when calculating the time step according to
      // cfl-condition
      double max_velocity;
      ...
      // C_eff: constant that has to be specified for time step calculation
      // method MaxEfficiency, which means that the time step is selected such
      // that the errors of the temporal and spatial discretization are
      // comparable
      double c_eff;
      ...
      // Pseudo-timestepping for steady-state problems: These tolerances are
      // only relevant when using an unsteady solver to solve the steady
      ...
      // show solver performance (wall time, number of iterations) every ...
      // timesteps
      ...
      // Polynomial degree of velocity shape functions. In case of H-div
      // Raviart-Thomas 'degree_u' is the polynomial degree in normal direction
      // and (degree_u - 1) is the degree in tangential direction.
      unsigned int degree_u;

      // Polynomial degree of pressure shape functions
      DegreePressure degree_p;

      // convective term: upwind factor describes the scaling factor in front of
      // the stabilization term (which is strictly dissipative) of the numerical
      // function of the convective term. For the divergence formulation of the
      // convective term with local Lax-Friedrichs flux, a value of
      // upwind_factor = 1.0 corresponds to the theoretical value (e.g., maximum
      // eigenvalue of the flux Jacobian, lambda = 2 |u*n|) but a lower value
      // (e.g., upwind_factor = 0.5, lambda = |u*n|) might be much more
      // advantages in terms of computational costs by allowing significantly
      ...
      // interior penalty parameter scaling factor for Helmholtz equation of
      // viscous step
      ...
      // For certain setups and types of boundary conditions, the pressure level
      // is undefined, e.g., if the velocity is prescribed on the whole boundary
      // or in case of periodic boundary conditions. This variable defines the
      // method used to adjust the pressure level in or to obtain a well-defined
      // pressure solution. If you observe convergence in the velocity error,
      // but no convergence in the pressure error, this might likely be related
      // to the fact that this parameter has not been set as intended.
      ...
      // Divergence and continuity penalty terms are applied in a postprocessing
      // step if set to true. Otherwise, penalty terms are added to the
      // monolithic systems of equations in case of the monolithic solver, or to
      // the projection step in case of the dual splitting scheme. For the
      // pressure-correction scheme, this parameter is irrelevant since the
      // projection step is the last step of the splitting scheme anyway.
      bool apply_penalty_terms_in_postprocessing_step;

      // specify which components of the velocity field to penalize, i.e.,
      // normal components only or all components
      ContinuityPenaltyComponents continuity_penalty_components;

      // specify whether boundary conditions prescribed for the velocity should
      // be used in continuity penalty operator. Otherwise, boundary faces are
      // ignored.
      ...
      // Implement block diagonal (block Jacobi) preconditioner in a matrix-free
      // way by solving the block Jacobi problems elementwise using iterative
      // solvers and matrix-free operator evaluation. By default, this variable
      // should be set to true because the matrix-based variant (which is used
      // otherwise) is very slow and the matrix-free variant can be expected to
      // be much faster. Only in case that convergence problems occur or for
      // reasons of testing/debugging the matrix-based variant should be used.
      ...
      // Solver data for block Jacobi preconditioner. Accordingly, this
      // parameter is only relevant if the block diagonal preconditioner is
      // implemented in a matrix-free way using an elementwise iterative
      // solution procedure for which solver tolerances have to be provided by
      // the user. It was found that rather coarse relative solver  tolerances
      // of around 1.e-2 are enough and for lower tolerances one would
      // 'over-solve' the local preconditioning problems without a further
      // benefit in global iteration counts.
      SolverData solver_data_block_diagonal;

      // Quadrature rule used to integrate the linearized convective term. This
      // parameter is therefore only relevant if linear systems of equations
      // have to be solved involving the convective term. The quadrature rule
      // specified by this parameter is also used in case of a linearly implicit
      // formulation of the convective term. Note that if the convective term is
      // not involved in the momentum operator, this parameter does not have any
      // effect and the standard quadrature rule will be used for the momentum
      // operator. For reasons of computational efficiency, it might be
      // advantageous to use a standard quadrature rule for the linear(ized)
      // momentum operator in order to speed up the computation. However, it was
      // found that choosing a lower order quadrature rule for the linearized
      // problem only, increases the number of iterations significantly. It was
      // found that the quadrature rules used for the nonlinear and linear
      // problems should be the same. Hence, although this parameter speeds up
      // the operator evaluation (i.e. the wall time per iteration), it is
      // unclear whether a lower order quadrature rule really allows to achieve
      // a more efficient method overall.
      ...
      /*                                                                                    */
      /**************************************************************************************/
      // These parameters are relevant if the inverse mass can not be realized
      // as a matrix-free operator evaluation. The typical use case is a DG
      // formulation with non hypercube elements (e.g. simplex).

      InverseMassParameters inverse_mass_operator;

      // This parameter is only relevant if an H(div)-conforming formulation is
      // chosen.
      ...
      // This parameter is used for all inverse mass preconditioners used for
      // incompressible Navier-Stokes solvers. Note that there is a separate
      // parameter above for the inverse mass operator (which needs to be
      // inverted "exactly" for reasons of accuracy). This parameter is only
      // relevant if the mass operator is block diagonal and if the inverse mass
      // operator can not be realized as a matrix-free operator evaluation. The
      // typical use case is a DG formulation with non hypercube elements (e.g.
      // simplex). In these cases, however, you should think about replacing the
      // inverse mass preconditioner by a simple point Jacobi preconditioner as
      // an efficient alternative to the (exact) inverse mass preconditioner.
      // This type of preconditioner is not available for an H(div)-conforming
      // formulation.
      ...
      // description: see enum declaration (only relevant if block diagonal is
      // used as preconditioner)
      Elementwise::Preconditioner preconditioner_block_diagonal_projection;

      // solver data for block Jacobi preconditioner (only relevant if
      // elementwise iterative solution procedure is used for block diagonal
      // preconditioner)
      ...
      // nonlinear problems, i.e., if the convective term is formulated
      // implicitly) This variable is only used if update_preconditioner_coupled
      // = true.
      ...
      // order of extrapolation of viscous term and convective term in pressure
      // Neumann BC
      unsigned int order_extrapolation_pressure_nbc;

      // description: see enum declaration
      // The formulation of the convective term in the boundary conditions for
      // the dual splitting scheme (there are two occurrences, the g_u_hat term
      // arising from the divergence term on the right-hand side of the pressure
      // Poisson equation as well as the pressure NBC for the dual splitting
      // scheme)can be chosen independently from the type of formulation used
      // for the discretization of the convective term in the Navier-Stokes
      // equations. As a default parameter,
      // FormulationConvectiveTerm::ConvectiveFormulation is used (exploiting
      // that div(u)=0 holds in the continuous case).
