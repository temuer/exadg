  ...
  namespace FTI
  {
    ...
    public:
            ...
            std::vector<std::string> const                  &subsection_names)
      {
        ...
        // boundary conditions
        boundary_descriptor =
          std::make_shared<IncNS::BoundaryDescriptor<dim>>();
    ...
    public:
            ...
            Grid<dim> const                &grid)
      {
        ...
        // set degree of shape functions (note that the refinement level is
        // identical to fluid domain)
        param.degree = degree;

        set_parameters();
        param.check();

        param.print(this->pcout,
                    "List of parameters for quantity " +
                      subsection_names.back() + ":");
    ...
    public:
      ...
      add_parameters(dealii::ParameterHandler &prm)
      {
        AssertThrow(fluid.get(),
                    dealii::ExcMessage("fluid has not been initialized."));
            ...
            std::shared_ptr<MultigridMappings<dim, Number>> &multigrid_mappings)
      {
        ...
        // The fluid field is defined as the field that creates the grid and the
        // mapping, while all scalar fields use the same grid/mapping
        ...
        for (unsigned int i = 0; i < scalars.size(); ++i)
          {
            ...
            AssertThrow(
              scalars[i]->get_parameters().ale_formulation ==
