...
namespace ExaDG
{
  namespace FTI
  {
    ...
    Driver<dim, Number>::setup()
    {
      ...
      pcout << std::endl
            << "Setting up incompressible flow with scalar transport solver:"
            << std::endl;
      ...
      // to neglect the inertial term in the fluid equations. The overall
      // coupled flow-transport problem is then still a transient/unsteady
      // problem.
      ...
      if (ale) // moving mesh
        {
          ...
          ale_multigrid_mappings =
            std::make_shared<MultigridMappings<dim, Number>>(
              ale_mapping,
              application->fluid->get_parameters().mapping_degree_coarse_grids);
          ...
          helpers_ale->update_pde_operator_after_grid_motion = [&]() {
            // Since we use the same MatrixFree object for the fluid field and
            // the scalar transport field, we need to update MatrixFree here in
            // the Driver.
            matrix_free->update_mapping(*ale_mapping->get_mapping());

            fluid_operator->update_after_grid_motion(
              false /* update_matrix_free */);
            for (unsigned int i = 0; i < application->scalars.size(); ++i)
          ...
          helpers_ale->fill_grid_coordinates_vector =
            [&](VectorType                    &grid_coordinates,
              ...
              ale_mapping->fill_grid_coordinates_vector(grid_coordinates,
                                                        dof_handler);
            };
        }
      ...
      if (application->fluid->get_parameters().solver_type ==
      ...
      else if (application->fluid->get_parameters().solver_type ==
               IncNS::SolverType::Steady)
        {
          fluid_operator =
            std::make_shared<IncNS::OperatorCoupled<dim, Number>>(
              grid,
              dynamic_mapping,
              ale ? ale_multigrid_mappings : multigrid_mappings,
              application->fluid->get_boundary_descriptor(),
      ...
      for (unsigned int i = 0; i < n_scalars; ++i)
        {
          scalar_operator[i] =
            std::make_shared<ConvDiff::Operator<dim, Number>>(
              grid,
              dynamic_mapping,
              ale ? ale_multigrid_mappings : multigrid_mappings,
      ...
      for (unsigned int i = 0; i < n_scalars; ++i)
        {
          AssertThrow(
            application->scalars[i]
                ->get_parameters()
                .use_cell_based_face_loops ==
      ...
      if (application->fluid->get_parameters().boussinesq_term)
        {
          // assume that the first scalar field with index 0 is the active
          // scalar that couples to the incompressible Navier-Stokes equations
      ...
      // Initialize DoF-vector temperature in case of transport->fluid coupling
      // with Boussinesq term:
      if (application->fluid->get_parameters().boussinesq_term)
        {
          // assume that the first scalar quantity with index 0 is the active
          // scalar coupled to the incompressible Navier-Stokes equations via
          // the Boussinesq term
      ...
      for (unsigned int i = 0; i < n_scalars; ++i)
        {
          scalar_postprocessor[i] =
            application->scalars[i]->create_postprocessor();
      ...
      if (application->fluid->get_parameters().solver_type ==
          IncNS::SolverType::Unsteady)
        {
          ...
          if (application->fluid->get_parameters().restarted_simulation ==
              false)
            {
              // The parameter start_with_low_order for BDF time integration has
              // to be true. This is due to the fact that the setup function of
              // the time integrator initializes the solution at previous time
              // instants t_0 - dt, t_0 - 2*dt, ... in case of
              // start_with_low_order == false. However, the combined time step
              // size is not known at this point since we have to first
              // communicate the time step size in order to find the minimum
              // time step size. Overwriting the time step size would imply that
              // the time step sizes are non constant for a time integrator, but
              // the time integration constants are initialized based on the
              // assumption of constant time step size. Hence, the easiest way
              // to avoid this kind of inconsistency is to preclude the case
              // start_with_low_order == false. In case of a restart,
              // start_with_low_order = false is possible since it has been
              // enforced that all previous time steps are identical for fluid
              // and scalar transport.
              AssertThrow(
                application->fluid->get_parameters().start_with_low_order ==
                  true,
                dealii::ExcMessage(
                  "start_with_low_order has to be true for this solver."));
            }

          fluid_time_integrator->setup(
            application->fluid->get_parameters().restarted_simulation);
        }
      else if (application->fluid->get_parameters().solver_type ==
               IncNS::SolverType::Steady)
        {
          std::shared_ptr<IncNS::OperatorCoupled<dim, Number>>
            fluid_operator_coupled =
              std::dynamic_pointer_cast<IncNS::OperatorCoupled<dim, Number>>(
                fluid_operator);

          // initialize driver for steady state problem that depends on
          // fluid_operator
          fluid_driver_steady =
            std::make_shared<DriverSteady>(fluid_operator_coupled,
                                           fluid_postprocessor,
                                           application->fluid->get_parameters(),
                                           mpi_comm,
                                           is_test);
      ...
      for (unsigned int i = 0; i < n_scalars; ++i)
        {
          ...
          scalar_time_integrator[i] =
            ConvDiff::create_time_integrator<dim, Number>(
              scalar_operator[i],
              helpers_ale,
              scalar_postprocessor[i],
              application->scalars[i]->get_parameters(),
          ...
          if (application->scalars[i]->get_parameters().restarted_simulation ==
                false and
              application->scalars[i]
                  ->get_parameters()
          ...
          AssertThrow(
            application->scalars[i]
                ->get_parameters()
                .analytical_velocity_field == false,
            dealii::ExcMessage(
    ...
    Driver<dim, Number>::synchronize_time_step_size() const
    {
      ...
      if (application->fluid->get_parameters().solver_type ==
          IncNS::SolverType::Unsteady)
        {
          ...
          IncNS::Parameters const &fluid_param =
            application->fluid->get_parameters();

          // fluid
          if (fluid_time_integrator->get_time() >
              fluid_param.start_time - EPSILON)
          ...
          if (use_adaptive_time_stepping == false)
            {
              ...
              time_step_size_fluid =
                ...
                (1 + int((fluid_param.end_time - fluid_param.start_time) /
                         time_step_size_fluid));
            }
      ...
      for (unsigned int i = 0; i < application->scalars.size(); ++i)
        {
          ConvDiff::Parameters const &scalar_param_i =
            application->scalars[i]->get_parameters();

          double time_step_size_scalar = std::numeric_limits<double>::max();
          if (scalar_time_integrator[i]->get_time() >
              scalar_param_i.start_time - EPSILON)
            time_step_size_scalar =
              scalar_time_integrator[i]->get_time_step_size();
          ...
          if (use_adaptive_time_stepping == false)
            {
              ...
              time_step_size_scalar =
                ...
                (1 + int((scalar_param_i.end_time - scalar_param_i.start_time) /
                         time_step_size_scalar));
            }
      ...
      if (use_adaptive_time_stepping == false)
        {
          pcout << std::endl
                << "Combined time step size dt = " << time_step_size
                << std::endl;
        }
    ...
    {
      // We need to communicate between fluid solver and scalar transport
      // solver, i.e., ask the scalar transport solver (scalar 0 by definition)
      // for the temperature and hand it over to the fluid solver.
      if (application->fluid->get_parameters().boussinesq_term)
        {
          // assume that the first scalar quantity with index 0 is the active
          // scalar coupled to the incompressible Navier-Stokes equations via
          // the Boussinesq term
          if (application->scalars[0]
                ->get_parameters()
                .temporal_discretization ==
              ConvDiff::TemporalDiscretization::ExplRK)
            {
              std::shared_ptr<ConvDiff::TimeIntExplRK<Number>> time_int_scalar =
                std::dynamic_pointer_cast<ConvDiff::TimeIntExplRK<Number>>(
                  scalar_time_integrator[0]);
              time_int_scalar->extrapolate_solution(temperature);
            }
          else if (application->scalars[0]
                     ->get_parameters()
                     .temporal_discretization ==
                   ConvDiff::TemporalDiscretization::BDF)
            {
              std::shared_ptr<ConvDiff::TimeIntBDF<dim, Number>>
                time_int_scalar =
                  std::dynamic_pointer_cast<ConvDiff::TimeIntBDF<dim, Number>>(
                    scalar_time_integrator[0]);
              time_int_scalar->extrapolate_solution(temperature);
            }
          else
    ...
    {
      // We need to communicate between fluid solver and scalar transport
      // solver, i.e., ask the fluid solver for the velocity field and hand it
      // over to all scalar transport solvers.
      std::vector<dealii::LinearAlgebra::distributed::Vector<Number> const *>
                          velocities;
      ...
      if (application->fluid->get_parameters().solver_type ==
               ...
               IncNS::SolverType::Steady)
        {
          ...
          AssertThrow(scalar_time_integrator[0].get() != nullptr,
                      dealii::ExcMessage("Not implemented."));
    ...
    Driver<dim, Number>::ale_update() const
    {
      ...
      timer_tree.insert({"Flow + transport", "ALE", "Reinit mapping"},
                        sub_timer.wall_time());
      ...
      timer_tree.insert({"Flow + transport",
                         "ALE",
                         "Update matrix-free / PDE operators"},
                        sub_timer.wall_time());
      ...
      while (not finished)
        {
          ...
          if (application->fluid->get_parameters().solver_type ==
              IncNS::SolverType::Unsteady)
            {
              fluid_time_integrator->advance_one_timestep_solve();
                   ...
                   IncNS::SolverType::Steady)
            {
              fluid_driver_steady->solve(
                scalar_time_integrator[0]->get_next_time(), true /*unsteady*/);
            }
          else
            {
              AssertThrow(false, dealii::ExcMessage("Not implemented."));
          ...
          // Both solvers have already calculated the new, adaptive time step
          // size individually in function advance_one_timestep(). Here, we have
          // to synchronize the time step size.
      ...
      double const total_time) const
    {
      ...
      pcout << "Performance results for coupled flow-transport solver:"
            << std::endl;
      ...
      // Fluid
      this->pcout << std::endl
                  << "Incompressible Navier-Stokes solver:" << std::endl;
      ...
      for (unsigned int i = 0; i < application->scalars.size(); ++i)
        {
          this->pcout << std::endl
                      << "Convection-diffusion solver for scalar " << i << ":"
                      << std::endl;
          ...
          if (application->scalars[i]
            ...
            {
              this->pcout
                << "  Explicit solver (no systems of equations have to be solved)"
                << std::endl;
            }
          else
            {
      ...
      if (application->fluid->get_parameters().solver_type ==
        ...
        {
          timer_tree.insert({"Flow + transport"},
                            fluid_time_integrator->get_timings(),
                            "Timeloop fluid");
        }
      else if (application->fluid->get_parameters().solver_type ==
               IncNS::SolverType::Steady)
        {
          timer_tree.insert({"Flow + transport"},
                            fluid_driver_steady->get_timings(),
                            "Timeloop fluid");
        }
      else
        {
      ...
      dealii::types::global_dof_index DoFs =
        this->fluid_operator->get_number_of_dofs();
