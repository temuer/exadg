...
namespace ExaDG
{
  namespace Structure
  {
    ...
    DriverQuasiStatic<dim, Number>::do_solve()
    {
      ...
      // Step 0 is a pre step with a smaller load factor in order to make
      // solving step 1 easier.
      step_number = 0;
      // In the first load step, we can not extrapolate the solution, so we
      // solve the problem for a much smaller load factor and afterwards
      // extrapolate the solution to the actual load factor in order to solve
      // the first load step.
              ...
              try
                {
                  ...
                  solution.add(load_increment / last_load_increment,
              ...
              catch (...)
                {
                  ...
                  pcout
                    << std::endl
                    << "  Could not solve non-linear problem. Reduce load increment to "
                    << load_increment << "." << std::endl
                    << std::flush;
                }
            }
    ...
    template <int dim, typename Number>
    void
    DriverQuasiStatic<dim, Number>::output_solver_info_header(
    ...
    DriverQuasiStatic<dim, Number>::postprocessing() const
    {
      ...
      timer_tree->insert({"DriverQuasiStatic", "Postprocessing"},
                         timer.wall_time());
    }
