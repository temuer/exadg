...
namespace ExaDG
{
  namespace Structure
  {
    ...
    struct BoundaryDescriptor
    {
      ...
      // This data structure will only be used if the initial_acceleration is
      // not set in FieldFunctions. Moreover, this data structure will only be
      // used for unsteady problems.
      std::map<dealii::types::boundary_id,
               std::shared_ptr<dealii::Function<dim>>>
        dirichlet_bc_initial_acceleration;

      // ComponentMask
      // If a certain boundary ID is not inserted into this map, it is assumed
      // that all components are active, in analogy to the default constructor
      // of dealii::ComponentMask.
      ...
      // another type of Neumann boundary condition where the traction force
      // comes from the solution on another domain that is in contact with the
      // actual domain of interest at the given boundary (this type of Neumann
        ...
        get_boundary_type(dealii::types::boundary_id const &boundary_id) const
      {
        ...
        else if (this->neumann_cached_bc.find(boundary_id) !=
                 this->neumann_cached_bc.end())
        ...
        if (dirichlet_bc.find(boundary_id) != dirichlet_bc.end())
          {
            ...
            AssertThrow(
              dirichlet_bc_component_mask.find(boundary_id) !=
                dirichlet_bc_component_mask.end(),
              dealii::ExcMessage(
