...
namespace ExaDG
{
  namespace Structure
  {
    ...
    public:
      ...
      // are large deformations to be expected, than compute with non linear
      // method
      bool large_deformation;

      // For nonlinear problems with large deformations, it is important to
      // specify whether the body forces are formulated with respect to the
      // current deformed configuration or the reference configuration.
      //
      // Option 1: pull_back_body_force = false
      // In this case, the body force is specified as force per undeformed
      // volume. A typical use case are density-proportional forces such as the
      // gravitational force. The body is then directly described in reference
      // space, b_0 = rho_0 * g, and the pull-back to the reference
      ...
      // For nonlinear problems with large deformations, it is important to
      // specify whether the traction Neumann boundary condition is formulated
      // with respect to the current deformed configuration or the reference
      // configuration. Both cases appear in practice, so it needs to be
      // specified by the user which formulation is to be used.
      //
      // Option 1: pull_back_traction = false
      // In this case, the traction is specified as a force per undeformed area,
      // e.g., a force of fixed amount distributed uniformly over a surface of
      // the body. The force per deformed area is an unknown. Hence, it is more
      // natural to specify the traction in the reference configuration and
      // deactivate the pull-back from the current to the reference
      // configuration.
      //
      // Option 2: pull_back_traction = true
      // The traction is known as a force per area of the deformed body. In this
      // case, the traction needs to be pulled-back to the reference
      // configuration, i.e., t_0 = da/dA * t, where the surface area ratio
      // da/dA depends on the current state of deformation. A typical use case
      // would be fluid-structure-interaction problems where the fluid stresses
      // are applied as traction boundary conditions for the structure. Note
      // that the direction of the traction vector does not change by this
      // pull-back operation.
      ...
      // density rho_0 in initial configuration (only relevant for unsteady
      // problems)
      double density;

      // linear weak damping coefficient (mass proportional) for unsteady
      // problems
      ...
      // configure printing of solver performance (wall time, number of
      // iterations)
      ...
      // Applies to time-dependent OR nonlinear problems: update of
      // preconditioner

      // Should the preconditioner be updated at all (set to false to never
      // update the preconditioner)?
      bool update_preconditioner;
      // If the above option is set to true, one can specify in more detail when
      // to update the preconditioner exactly:
      ...
      // - every ... Newton iterations (first update is invoked in the first
      // Newton iteration)
