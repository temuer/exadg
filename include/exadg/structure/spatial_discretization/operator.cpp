...
namespace ExaDG
{
  namespace Structure
  {
    ...
    Operator<dim, Number>::Operator(
      ...
      , pcout(std::cout,
              dealii::Utilities::MPI::this_mpi_process(mpi_comm_in) == 0)
    {
    ...
    Operator<dim, Number>::initialize_dof_handler_and_constraints()
    {
      ...
      // copy periodicity and hanging node constraints, and add further
      // constraints stemming from Dirichlet boundary conditions
      affine_constraints.copy_from(
        affine_constraints_periodicity_and_hanging_nodes);

      // use all the component masks defined by the user
      std::map<dealii::types::boundary_id, dealii::ComponentMask>
        map_bid_to_mask = boundary_descriptor->dirichlet_bc_component_mask;
      ...
      fill_keys_of_map_into_set(all_dirichlet_bids,
                                boundary_descriptor->dirichlet_bc);

      // fill with default mask if no mask has been defined
      fill_map_bid_to_mask_with_default_mask(map_bid_to_mask,
                                             all_dirichlet_bids);

      // call deal.II utility function to add Dirichlet constraints
      add_homogeneous_dirichlet_constraints(affine_constraints,
                                            dof_handler,
                                            map_bid_to_mask);

      // compress constraints *once after* complete setup, since affine
      // constraints to copy from need to be non-closed to add further
      // constraints
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::fill_matrix_free_data(
      MatrixFreeData<dim, Number> &matrix_free_data) const
    {
      if (param.large_deformation)
        matrix_free_data.append_mapping_flags(
          NonLinearOperator<dim, Number>::get_mapping_flags());
      else
        matrix_free_data.append_mapping_flags(
          LinearOperator<dim, Number>::get_mapping_flags());
      ...
      // inhomogeneous Dirichlet boundary conditions: use additional
      // AffineConstraints object, but the same DoFHandler
      matrix_free_data.insert_dof_handler(
        &dof_handler, get_dof_name_periodicity_and_hanging_node_constraints());
      ...
      // Create a Gauss-Lobatto quadrature rule for DirichletCached boundary
      // conditions. These quadrature points coincide with the nodes of the
      // discretization, so that the values stored in the DirichletCached
      // boundary condition can be directly injected into the DoF vector. This
      // allows to set constrained degrees of freedom in case of continuous
      // Galerkin discretizations with DirichletCached boundary conditions.
      if (not(boundary_descriptor->dirichlet_cached_bc.empty()))
        {
          AssertThrow(
            this->grid->triangulation->all_reference_cells_are_hyper_cube(),
            ExcNotImplemented());
      ...
      if (not(boundary_descriptor->dirichlet_cached_bc.empty()))
        {
          ...
          // Gauss-Lobatto quadrature rule for DirichletCached boundary
          // conditions!
          ...
          boundary_descriptor->set_dirichlet_cached_data(
            interface_data_dirichlet_cached);
        }

      if (not(boundary_descriptor->neumann_cached_bc.empty()))
        {
          interface_data_neumann_cached =
            std::make_shared<ContainerInterfaceData<1, dim, double>>();
          std::vector<unsigned int> quad_indices;
          quad_indices.emplace_back(get_quad_index());

          interface_data_neumann_cached->setup(
            *matrix_free,
            get_dof_index(),
            quad_indices,
            boundary_descriptor->neumann_cached_bc);

          boundary_descriptor->set_neumann_cached_data(
            interface_data_neumann_cached);
        }
    }
    ...
    Operator<dim, Number>::setup_operators()
    {
      ...
      operator_data.use_matrix_based_vmult =
        param.use_matrix_based_implementation;
      ...
      if (not(boundary_descriptor->dirichlet_cached_bc.empty()))
        {
          ...
          operator_data.quad_index_gauss_lobatto =
            get_quad_index_gauss_lobatto();
        }
      ...
      if (param.problem_type == ProblemType::Unsteady)
        {
          ...
          mass_data.dof_index_inhomogeneous =
            get_dof_index_periodicity_and_hanging_node_constraints();
    ...
    Operator<dim, Number>::setup_preconditioner()
    {
      if (param.problem_type == ProblemType::Unsteady)
        {
          mass_preconditioner = std::make_shared<
            JacobiPreconditioner<Structure::MassOperator<dim, Number>>>(
            mass_operator, true);
        }
      ...
      if (param.preconditioner == Preconditioner::None)
          ...
          if (param.large_deformation)
            {
              preconditioner = std::make_shared<
                JacobiPreconditioner<NonLinearOperator<dim, Number>>>(
                elasticity_operator_nonlinear, false);
            }
          else
            {
              preconditioner = std::make_shared<
                JacobiPreconditioner<LinearOperator<dim, Number>>>(
      ...
      else if (param.preconditioner == Preconditioner::AdditiveSchwarz)
        {
          if (param.large_deformation)
            {
              preconditioner = std::make_shared<
                AdditiveSchwarzPreconditioner<NonLinearOperator<dim, Number>>>(
                elasticity_operator_nonlinear, false);
            }
          else
            {
              preconditioner = std::make_shared<
                AdditiveSchwarzPreconditioner<LinearOperator<dim, Number>>>(
                elasticity_operator_linear, false);
            }
        }
          ...
          if (param.large_deformation)
            {
              ...
              Map_DBC_ComponentMask dirichlet_bc_component_mask =
                elasticity_operator_nonlinear.get_data()
                  .bc->dirichlet_bc_component_mask;
              ...
              // DirichletCached BCs.
              for (auto iter : elasticity_operator_nonlinear.get_data()
                                 .bc->dirichlet_cached_bc)
                {
                  typedef
                    typename std::pair<dealii::types::boundary_id,
                                       std::shared_ptr<dealii::Function<dim>>>
                      pair;
                  ...
                  dirichlet_bc_component_mask.insert(
                    pair_mask(iter, default_mask));
                }
          ...
          else
            {
              ...
              typedef std::map<dealii::types::boundary_id,
                               dealii::ComponentMask>
                                    Map_DBC_ComponentMask;
              Map_DBC_ComponentMask dirichlet_bc_component_mask =
                elasticity_operator_linear.get_data()
                  .bc->dirichlet_bc_component_mask;
              ...
              // fill the map with dealii::Functions::ZeroFunction for
              // DirichletCached BCs.
              for (auto iter : elasticity_operator_linear.get_data()
                                 .bc->dirichlet_cached_bc)
                {
                  typedef
                    typename std::pair<dealii::types::boundary_id,
      ...
      else if (param.preconditioner == Preconditioner::AMG)
        {
          if (param.large_deformation)
            {
              typedef PreconditionerAMG<NonLinearOperator<dim, Number>, Number>
                AMG;
              preconditioner = std::make_shared<AMG>(
                elasticity_operator_nonlinear,
                false /* initialize */,
            ...
            {
              typedef PreconditionerAMG<LinearOperator<dim, Number>, Number>
                AMG;
              preconditioner = std::make_shared<AMG>(
                elasticity_operator_linear,
                false /* initialize */,
      ...
      else
        {
          AssertThrow(false,
                      dealii::ExcMessage(
                        "Specified preconditioner is not implemented!"));
        }
    }
      ...
      if (param.problem_type == ProblemType::Unsteady)
        {
          ...
          if (param.large_deformation)
            {
              solver_data.solver_tolerance_abs =
                param.newton_solver_data.abs_tol;
              solver_data.solver_tolerance_rel =
                param.newton_solver_data.rel_tol;
          ...
          typedef Krylov::SolverCG<Structure::MassOperator<dim, Number>,
                                   PreconditionerBase<Number>,
                                   VectorType>
            CG;
          mass_solver = std::make_shared<CG>(mass_operator,
                                             *mass_preconditioner,
                                             solver_data);
        }
      ...
      if (param.solver == Solver::CG)
        {
          ...
          if (param.large_deformation)
            {
              typedef Krylov::SolverCG<NonLinearOperator<dim, Number>,
                                       PreconditionerBase<Number>,
                                       VectorType>
                CG;
              linear_solver =
                std::make_shared<CG>(elasticity_operator_nonlinear,
                                     *preconditioner,
                                     solver_data);
            }
          else
            {
              typedef Krylov::SolverCG<LinearOperator<dim, Number>,
                                       PreconditionerBase<Number>,
                                       VectorType>
                CG;
              linear_solver = std::make_shared<CG>(elasticity_operator_linear,
                                                   *preconditioner,
                                                   solver_data);
            }
        }
      else if (param.solver == Solver::FGMRES)
        {
          ...
          if (param.large_deformation)
            {
              typedef Krylov::SolverFGMRES<NonLinearOperator<dim, Number>,
                                           PreconditionerBase<Number>,
                                           VectorType>
                FGMRES;
              linear_solver =
                std::make_shared<FGMRES>(elasticity_operator_nonlinear,
                                         *preconditioner,
                                         solver_data);
            }
          else
            {
              typedef Krylov::SolverFGMRES<LinearOperator<dim, Number>,
                                           PreconditionerBase<Number>,
                                           VectorType>
                FGMRES;
              linear_solver =
                std::make_shared<FGMRES>(elasticity_operator_linear,
                                         *preconditioner,
                                         solver_data);
            }
        }
        ...
        {
          AssertThrow(
            false, dealii::ExcMessage("Specified solver is not implemented!"));
        }
      ...
      if (param.large_deformation)
        {
          ...
          newton_solver =
            std::make_shared<NewtonSolver>(param.newton_solver_data,
                                           residual_operator,
                                           linearized_operator,
                                           *linear_solver);
        }
    ...
    std::string
    Operator<dim,
             Number>::get_dof_name_periodicity_and_hanging_node_constraints()
      const
    {
    ...
    unsigned int
    Operator<dim,
             Number>::get_dof_index_periodicity_and_hanging_node_constraints()
      const
    {
      return matrix_free_data->get_dof_index(
        get_dof_name_periodicity_and_hanging_node_constraints());
    }
      ...
      double const scaling_factor_velocity) const
    {
      ...
      if (param.weak_damping_active)
        {
          scaling_factor_mass +=
            param.weak_damping_coefficient * scaling_factor_velocity;
        }
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::prescribe_initial_displacement(
      VectorType  &displacement,
      double const time) const
    {
      Utilities::interpolate(dof_handler,
                             *field_functions->initial_displacement,
                             displacement,
                             time);
    }
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::prescribe_initial_velocity(VectorType  &velocity,
                                                      double const time) const
    {
      Utilities::interpolate(dof_handler,
                             *field_functions->initial_velocity,
                             velocity,
                             time);
    }
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::compute_initial_acceleration(
      VectorType       &initial_acceleration,
      ...
      double const      time) const
    {
      if (field_functions->initial_acceleration.get())
      ...
      else
        {
          ...
          if (param.large_deformation) // nonlinear case
            {
              ...
              double const scaling_factor_mass =
                elasticity_operator_nonlinear
                  .get_scaling_factor_mass_operator();
              elasticity_operator_nonlinear.set_scaling_factor_mass_operator(
                0.0);

              // evaluate elasticity operator including inhomogeneous
              // Dirichlet/Neumann boundary conditions: Note that we do not have
              // to set inhomogeneous Dirichlet degrees of freedom explicitly
              // since the function prescribe_initial_displacement() sets the
              // initial displacement for all dofs (including Dirichlet dofs)
              // and since the initial condition for the displacements needs to
              // be consistent with the Dirichlet boundary data g(t=t0) at
              // initial time, i.e. the vector initial_displacement already
              // contains the correct Dirichlet data.
              elasticity_operator_nonlinear.set_time(time);
              elasticity_operator_nonlinear.evaluate_nonlinear(
                rhs, initial_displacement);
              ...
              if (param.body_force)
                {
                  body_force_operator.evaluate_add(rhs,
          ...
          // Shift inhomogeneous part of mass matrix operator (i.e. mass matrix
          // applied to a dof vector with the initial acceleration in Dirichlet
          // degrees of freedom) to the right-hand side
          mass_operator.rhs_add(rhs);

          // invert mass operator to get acceleration
          mass_solver->solve(initial_acceleration, rhs);

          // Set initial acceleration for the Dirichlet degrees of freedom so
          // that the initial acceleration is also correct on the Dirichlet
          // boundary
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::evaluate_mass_operator(VectorType       &dst,
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::apply_add_damping_operator(
      VectorType       &dst,
      VectorType const &src) const
    {
      if (param.weak_damping_active)
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::evaluate_nonlinear_residual(
      VectorType       &dst,
      ...
      double const      time) const
    {
      // elasticity operator: make sure that constrained degrees of freedom have
      // been set correctly before evaluating the elasticity operator.
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::set_solution_linearization(
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::apply_elasticity_operator(
      VectorType       &dst,
      VectorType const &src) const
    {
      if (param.large_deformation)
    ...
    Operator<dim, Number>::solve_nonlinear(
      VectorType       &sol,
      VectorType const &const_vector,
      double const      scaling_factor_acceleration,
      ...
      bool const        update_preconditioner) const
    {
      // update operators
      double const scaling_factor_mass =
        compute_scaling_factor_mass(scaling_factor_acceleration,
      ...
      // set inhomogeneous Dirichlet values in order to evaluate the nonlinear
      // residual correctly
      elasticity_operator_nonlinear.set_time(time);
      ...
      update.update_every_newton_iter =
        param.update_preconditioner_every_newton_iterations;
      update.update_once_converged =
        param.update_preconditioner_once_newton_converged;

      // solve nonlinear problem
      auto const iter = newton_solver->solve(sol, update);

      // TODO
      // This call should not be necessary: The constraints have already been
      // set before the nonlinear solver is called and no contributions to the
      // constrained degrees of freedom should be added in the Newton solver by
      // the linearized solver (because the residual vector forming the rhs of
      // the linearized problem is zero for constrained degrees of freedom, the
      // initial solution of the linearized solver is also zero, and the
      // linearized operator contains values of 1 on the diagonal for
    ...
    template <int dim, typename Number>
    unsigned int
    Operator<dim, Number>::solve_linear(
      VectorType       &sol,
      ...
      bool const        update_preconditioner) const
    {
      ...
      // Set Dirichlet degrees of freedom according to Dirichlet boundary
      // condition.
      elasticity_operator_linear.set_time(time);
