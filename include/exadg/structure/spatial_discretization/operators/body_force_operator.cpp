...
namespace ExaDG
{
  namespace Structure
  {
    ...
    BodyForceOperator<dim, Number>::BodyForceOperator()
      : matrix_free(nullptr)
      , time(0.0)
    {}
    ...
    template <int dim, typename Number>
    void
    BodyForceOperator<dim, Number>::initialize(
                                                 ...
                                                 double const      time) const
    {
      ...
      matrix_free->cell_loop(
        &This::cell_loop, this, dst, src, false /*zero_dst_vector*/);
    }

    template <int dim, typename Number>
    void
    BodyForceOperator<dim, Number>::cell_loop(
      dealii::MatrixFree<dim, Number> const &matrix_free,
      VectorType                            &dst,
      VectorType const                      &src,
          ...
          for (unsigned int q = 0; q < integrator.n_q_points; ++q)
            {
              ...
              auto b =
                FunctionEvaluator<1, dim, Number>::value(*(data.function),
                                                         q_points,
                                                         time);
