...
namespace ExaDG
{
  namespace Structure
  {
      ...
      OperatorData<dim> const                 &data)
    {
      ...
      integrator_lin = std::make_shared<IntegratorCell>(
        *this->matrix_free,
      ...
      // dof_index_inhomogeneous
      this->matrix_free->initialize_dof_vector(displacement_lin,
    ...
    template <int dim, typename Number>
    void
    NonLinearOperator<dim, Number>::evaluate_nonlinear(
      VectorType       &dst,
    ...
    template <int dim, typename Number>
    bool
    NonLinearOperator<dim, Number>::valid_deformation(
      VectorType const &displacement) const
    {
      Number dst = 0.0;
    ...
    {
      // Only update linearized operator if deformation state is valid. It is
      // better to continue with an old deformation state in the linearized
      // operator than with an invalid one.
    ...
    template <int dim, typename Number>
    void
    NonLinearOperator<dim, Number>::reinit_cell_nonlinear(
      ...
      Range const                           &range) const
    {
      ...
      auto const unsteady_flag = this->operator_data.unsteady ?
      ...
      for (auto cell = range.first; cell < range.second; ++cell)
        {
          ...
          integrator_inhom.gather_evaluate(
            src, unsteady_flag | dealii::EvaluationFlags::gradients);

          do_cell_integral_nonlinear(integrator_inhom);

          // make sure that we do not write into Dirichlet degrees of freedom
          integrator_inhom.integrate(unsteady_flag |
      ...
      for (unsigned int face = range.first; face < range.second; face++)
        {
          ...
          if (this->operator_data.pull_back_traction)
            {
              integrator_m_inhom.gather_evaluate(
                src, dealii::EvaluationFlags::gradients);
            }
    ...
    template <int dim, typename Number>
    void
    NonLinearOperator<dim, Number>::do_cell_integral_nonlinear(
      IntegratorCell &integrator) const
    {
      std::shared_ptr<Material<dim, Number>> material =
        this->material_handler.get_material();
      ...
      for (unsigned int q = 0; q < integrator.n_q_points; ++q)
        {
          ...
          tensor const S = material->second_piola_kirchhoff_stress(
            Grad_d, integrator.get_current_cell_index(), q);
          ...
          if (this->operator_data.unsteady)
            {
              integrator.submit_value(this->scaling_factor_mass *
                                        this->operator_data.density *
      ...
      dealii::types::boundary_id const &boundary_id) const
    {
      BoundaryType boundary_type =
        this->operator_data.bc->get_boundary_type(boundary_id);

      for (unsigned int q = 0; q < integrator.n_q_points; ++q)
        {
          auto traction =
            calculate_neumann_value<dim, Number>(q,
                                                 integrator,
                                                 boundary_type,
                                                 boundary_id,
    ...
    template <int dim, typename Number>
    void
    NonLinearOperator<dim, Number>::reinit_cell_derived(
      IntegratorCell    &integrator,
      unsigned int const cell) const
    {
    ...
    template <int dim, typename Number>
    void
    NonLinearOperator<dim, Number>::do_cell_integral(
      IntegratorCell &integrator) const
    {
      std::shared_ptr<Material<dim, Number>> material =
        this->material_handler.get_material();
      ...
      for (unsigned int q = 0; q < integrator.n_q_points; ++q)
        {
          ...
          // 2nd Piola-Kirchhoff stresses
          tensor const S_lin = material->second_piola_kirchhoff_stress(
            Grad_d_lin, integrator.get_current_cell_index(), q);
