...
namespace ExaDG
{
  namespace Structure
  {
    ...
    public:
       ...
       *
       * This function ensures that constrained degrees of freedom are set to
       * zero in the dst vector. Note, however, that this function does not
       * ensure that Dirichlet degrees of freedom are set to the prescribed
       * inhomogeneous boundary data in the src vector. This needs to be done
    ...
    private:
       ...
       */
      void
      reinit_cell_nonlinear(IntegratorCell    &integrator,
      ...
      /*
       * A cell loop that checks whether the Jacobian determinant is positive
       * for all q-points. Prior to calling this function, inhomogeneous
       * Dirichlet degrees of freedom need to be set correctly.
       */
      void
      cell_loop_valid_deformation(
        dealii::MatrixFree<dim, Number> const &matrix_free,
        Number                                &dst,
        VectorType const                      &src,
        Range const                           &range) const;
       ...
       *  t_0 = da/dA t .
       *
       * If the traction is specified as force per surface area of the
       * underformed body, the specified traction t is interpreted as t_0 = t,
       * and no pull-back is necessary.
       */
      void
       ...
       *
       * with the directional derivative of the 1st Piola-Kirchhoff stress
       * tensor P_h
       *
       ...
       *
       *  Note that a dependency of the Neumann BC on the displacements d
       * through the area ratio da/dA = function(d) is neglected in the
       * linearization.
       */
      void
