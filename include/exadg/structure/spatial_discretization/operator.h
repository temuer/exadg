...
namespace ExaDG
{
  namespace Structure
  {
    ...
    /*
     * This operator provides the interface required by the non-linear Newton
     * solver. Requests to evaluate the residual for example are simply handed
     * over to the operators that implement the physics.
     */
    template <int dim, typename Number>
    ...
    public:
      ...
      evaluate_residual(VectorType &dst, VectorType const &src) const
      {
        pde_operator->evaluate_nonlinear_residual(
          dst, src, *const_vector, scaling_factor_mass, time);
      }
    ...
    /*
     * This operator implements the interface required by the non-linear Newton
     * solver. Requests to apply the linearized operator are simply handed over
     * to the operators that implement the physics.
     */
    template <int dim, typename Number>
    ...
    public:
      ...
      /*
       * The implementation of linear solvers in deal.ii requires that a
       * function called 'vmult' is provided.
       */
      void
    ...
    public:
      ...
      /*
       * Prescribe initial conditions using a specified initial solution
       * function.
       */
      void
      prescribe_initial_displacement(VectorType  &displacement,
                                     double const time) const final;

      void
      prescribe_initial_velocity(VectorType  &velocity,
                                 double const time) const final;

      /*
       * This computes the initial acceleration field by evaluating all PDE
       * terms for the given initial condition, shifting all terms to the
       * right-hand side of the equations, and solving a mass matrix system to
      ...
      void
      evaluate_mass_operator(VectorType       &dst,
                             VectorType const &src) const final;

      void
      apply_add_damping_operator(VectorType       &dst,
                                 VectorType const &src) const final;

      /*
       * This function evaluates the nonlinear residual which is required by the
       * Newton solver. In order to evaluate inhomogeneous Dirichlet boundary
       * conditions correctly, inhomogeneous Dirichlet degrees of freedom need
       * to be set correctly in the src-vector prior to calling this function.
       */
      void
      ...
      /*
       * This function solves the system of equations for nonlinear problems.
       * This function needs to make sure that Dirichlet degrees of freedom are
       * filled correctly with their inhomogeneous boundary data before calling
      ...
      /*
       * This function calculates the right-hand side of the linear system of
       * equations for linear elasticity problems.
       */
      void
       ...
       *
       * Before calling this function, make sure that the function rhs() has
       * been called.
       */
      unsigned int
    ...
    private:
      ...
      /**
       * Scaling factor for mass matrix assuming a weak damping operator leading
       * to a scaled mass matrix.
       */
      double
      ...
      // AffineConstraints object as needed by iterative solvers and
      // preconditioners for linear systems of equations. This constraint object
      // contains additional constraints from Dirichlet boundary conditions as
      // compared to the constraint object below. Note that the present
      // constraint object can treat Dirichlet boundaries only in a homogeneous
      // manner.
      dealii::AffineConstraints<Number> affine_constraints;

      // To treat inhomogeneous Dirichlet BCs correctly in the context of
      // matrix-free operator evaluation using dealii::MatrixFree/FEEvaluation,
      // we need a separate AffineConstraints object containing only periodicity
      // and hanging node constraints. When using the standard AffineConstraints
      // object including Dirichlet boundary conditions, inhomogeneous boundary
      // data would be ignored by dealii::FEEvaluation::read_dof_values(). While
      // dealii::FEEvaluation::read_dof_values_plain() would take into account
      // inhomogeneous Dirichlet data using the standard AffineConstraints
      // object, hanging-node constraints would not be resolved correctly. The
      // solution/workaround is to use dealii::FEEvaluation::read_dof_values()
      // for a correct handling of hanging nodes, but to exclude Dirichlet
      // degrees of freedom from the AffineConstraints object so that it is
      // possible to read inhomogeneous boundary data when calling
      // dealii::FEEvaluation::read_dof_values(). This inhomogeneous boundary
      // data needs to be set beforehand in separate routines.
      ...
      mutable std::shared_ptr<ContainerInterfaceData<1, dim, double>>
