  ...
  namespace Structure
  {
    ...
    public:
      typedef std::pair<dealii::types::material_id,
        ...
        Pair;
      typedef std::map<dealii::types::material_id,
                       std::shared_ptr<Material<dim, Number>>>
        Materials;
                 ...
                 bool const                                large_deformation)
      {
        ...
        for (auto iter = material_descriptor->begin();
             iter != material_descriptor->end();
             ++iter)
          {
            ...
            switch (type)
              {
                case MaterialType::Undefined:
                  {
                    AssertThrow(
                      false, dealii::ExcMessage("Material type is undefined."));
                    break;
                  }
                case MaterialType::StVenantKirchhoff:
                  {
                    std::shared_ptr<StVenantKirchhoffData<dim>> data_svk =
                      std::static_pointer_cast<StVenantKirchhoffData<dim>>(
                        data);
      ...
      {
        auto mid =
          matrix_free.get_cell_iterator(cell, 0, dof_index)->material_id();
        ...
        for (unsigned int v = 1;
             ...
             v++)
          AssertThrow(
            mid == matrix_free.get_cell_iterator(cell, v)->material_id(),
            dealii::ExcMessage(
