...
namespace ExaDG
{
  namespace Structure
  {
    ...
    MultigridPreconditioner<dim, Number>::update()
    {
      ...
      if (data.unsteady)
        {
          this->for_all_levels([&](unsigned int const level) {
            if (nonlinear)
              {
                this->get_operator_nonlinear(level)->set_time(
                  pde_operator->get_time());
                this->get_operator_nonlinear(level)
                  ->set_scaling_factor_mass_operator(
                    pde_operator->get_scaling_factor_mass_operator());
              }
            else
              {
                this->get_operator_linear(level)->set_time(
                  pde_operator->get_time());
                this->get_operator_linear(level)
                  ->set_scaling_factor_mass_operator(
                    pde_operator->get_scaling_factor_mass_operator());
              }
          });
      ...
      if (nonlinear)
        {
          ...
          VectorType const &vector_linearization =
            pde_operator_nonlinear->get_solution_linearization();
          ...
          // Note: This function also re-assembles the sparse matrix in case a
          // matrix-based implementation is used
          ...
          this->transfer_from_fine_to_coarse_levels(
            [&](unsigned int const fine_level,
              ...
              this->transfers->interpolate(fine_level,
                                           vector_coarse_level,
                                           vector_fine_level);
              // Note: This function also re-assembles the sparse matrix in case
              // a matrix-based implementation is used
              this->get_operator_nonlinear(coarse_level)
      ...
      // Update the smoothers and the coarse grid solver. This is generic
        ...
        Map_DBC_ComponentMask const &dirichlet_bc_component_mask)
    {
      ...
      // Due to the interface of the base class MultigridPreconditionerBase, we
      // also need to set up additional DoFHandler objects.
      if (nonlinear)
        {
          Map_DBC               dirichlet_bc_empty;
          Map_DBC_ComponentMask dirichlet_bc_empty_component_mask;
          this->do_initialize_dof_handler_and_constraints(
            false,
            n_components,
            dirichlet_bc_empty,
      ...
      unsigned int const                    dealii_tria_level)
    {
      ...
      if (nonlinear)
        matrix_free_data.append_mapping_flags(
          PDEOperatorNonlinear::get_mapping_flags());
      else // linear
        matrix_free_data.append_mapping_flags(
          PDEOperatorLinear::get_mapping_flags());

      matrix_free_data.insert_dof_handler(&(*this->dof_handlers[level]),
                                          "elasticity_dof_index");
      ...
      if (nonlinear)
        {
          matrix_free_data.insert_dof_handler(
            &(*dof_handlers_inhomogeneous[level]),
    ...
    MultigridPreconditioner<dim, Number>::get_operator_nonlinear(
      unsigned int level)
    {
    ...
    std::shared_ptr<LinearOperator<
      dim,
      typename MultigridPreconditionerBase<dim, Number>::MultigridNumber>>
    MultigridPreconditioner<dim, Number>::get_operator_linear(
      unsigned int level)
    {
      ...
      unsigned int const level)
    {
      ...
      data.dof_index = this->matrix_free_data_objects[level]->get_dof_index(
        "elasticity_dof_index");
      if (nonlinear)
        {
          data.dof_index_inhomogeneous =
            this->matrix_free_data_objects[level]->get_dof_index(
              "elasticity_dof_index_inhomogeneous");
        }
      data.quad_index = this->matrix_free_data_objects[level]->get_quad_index(
        "elasticity_quadrature");

      if (nonlinear)
        {
          std::shared_ptr<PDEOperatorNonlinearMG> pde_operator_level(
            new PDEOperatorNonlinearMG());
          ...
          mg_operator_level =
            std::make_shared<MGOperatorNonlinear>(pde_operator_level);
        }
      else // linear
        {
          std::shared_ptr<PDEOperatorLinearMG> pde_operator_level(
            new PDEOperatorLinearMG());
          ...
          mg_operator_level =
            std::make_shared<MGOperatorLinear>(pde_operator_level);
        }
