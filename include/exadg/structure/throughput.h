...
namespace ExaDG
{
    ...
    try
      {
        ...
        Structure::get_application<Dim, Number>(input_file, MPI_COMM_WORLD)
          ->add_parameters(prm);
      }
    catch (...)
      {}
      ...
      bool const                                                  is_test)
  {
    ...
    std::shared_ptr<Structure::Driver<dim, Number>> driver =
      std::make_shared<Structure::Driver<dim, Number>>(mpi_comm,
                                                       application,
                                                       is_test,
                                                       true);
...
main(int argc, char **argv)
{
  ...
  ExaDG::ThroughputParameters<ExaDG::Structure::OperatorType> throughput(
    input_file);
