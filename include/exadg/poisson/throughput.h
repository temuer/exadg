...
namespace ExaDG
{
    ...
    try
      {
        ...
        Poisson::get_application<Dim, 1, Number>(input_file, MPI_COMM_WORLD)
          ->add_parameters(prm);
      }
    catch (...)
      {}
      ...
      bool const                                         is_test)
  {
    ...
    application->set_parameters_throughput_study(degree,
                                                 refine_space,
                                                 n_cells_1d);

    std::shared_ptr<Poisson::Driver<dim, Number>> driver =
      std::make_shared<Poisson::Driver<dim, Number>>(mpi_comm,
                                                     application,
                                                     is_test,
                                                     true);
    ...
    std::tuple<unsigned int, dealii::types::global_dof_index, double>
      wall_time = driver->apply_operator(throughput.operator_type,
...
main(int argc, char **argv)
{
  ...
  if (argc == 1)
    {
      if (dealii::Utilities::MPI::this_mpi_process(mpi_comm) == 0)
        {
          std::cout
            << "To run the program, use:      ./throughput input_file"
            << std::endl
            << "To setup the input file, use: ./throughput input_file --help"
            << std::endl;
        }
  ...
  ExaDG::ThroughputParameters<ExaDG::Poisson::OperatorType> throughput(
    input_file);
  ...
  auto const lambda_get_dofs_per_element =
    [&](unsigned int const       dim,
      ...
      return ExaDG::get_dofs_per_element(
        element_type,
        spatial_discretization == ExaDG::Poisson::SpatialDiscretization::DG,
        1 /* n_components */,
        degree,
        dim);
    };
  ...
  for (auto iter = resolution.resolutions.begin();
       iter != resolution.resolutions.end();
       ++iter)
    {
