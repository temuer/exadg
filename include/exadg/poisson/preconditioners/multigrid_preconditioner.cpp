  ...
  namespace Poisson
  {
      ...
      if (mesh_is_moving)
        {
          ...
          // Once the operators are updated, the update of smoothers and the
          // coarse grid solver is generic functionality implemented in the base
          // class.
      ...
      unsigned int const                    dealii_triangulation_level)
    {
      ...
      matrix_free_data.append_mapping_flags(
        Operators::LaplaceKernel<dim, Number>::get_mapping_flags(
    ...
    template <int dim, typename Number, int n_components>
    std::shared_ptr<LaplaceOperator<
      dim,
      typename MultigridPreconditionerBase<dim, Number>::MultigridNumber,
      n_components>>
    MultigridPreconditioner<dim, Number, n_components>::get_operator(
      unsigned int level)
    {
