...
namespace ExaDG
{
  namespace Poisson
  {
    ...
    {
      AssertThrow(this->is_dg,
                  dealii::ExcMessage(
                    "This function is only implemented for DG."));
    ...
    template <int dim, typename Number, int n_components>
    void
    LaplaceOperator<dim, Number, n_components>::reinit_face_derived(
    ...
    template <int dim, typename Number, int n_components>
    void
    LaplaceOperator<dim, Number, n_components>::do_cell_integral(
    ...
    template <int dim, typename Number, int n_components>
    void
    LaplaceOperator<dim, Number, n_components>::do_face_integral(
      IntegratorFace &integrator_m,
      ...
      for (unsigned int q = 0; q < integrator_p.n_q_points; ++q)
        {
          ...
          value gradient_flux =
            kernel.calculate_gradient_flux(value_p, value_m);

          // minus sign to get the correct normal vector n⁺ = -n⁻
          value normal_gradient_p = -integrator_p.get_normal_derivative(q);
          value normal_gradient_m = value();

          value value_flux = kernel.calculate_value_flux(normal_gradient_p,
                                                         normal_gradient_m,
                                                         value_p,
                                                         value_m);

          integrator_p.submit_normal_derivative(
            -gradient_flux, q); // opposite sign since n⁺ = -n⁻
      ...
      dealii::types::boundary_id const &boundary_id) const
    {
      ...
      for (unsigned int q = 0; q < integrator_m.n_q_points; ++q)
        {
          value value_m =
            calculate_interior_value<dim, Number, n_components, rank>(
              q, integrator_m, operator_type);
          value value_p =
            calculate_exterior_value<dim, Number, n_components, rank>(
              value_m,
              q,
              integrator_m,
              operator_type,
          ...
          value normal_gradient_m =
            calculate_interior_normal_gradient<dim, Number, n_components, rank>(
              q, integrator_m, operator_type);
          value normal_gradient_p =
            calculate_exterior_normal_gradient<dim, Number, n_components, rank>(
              normal_gradient_m,
              q,
              integrator_m,
              operator_type,
      ...
      for (unsigned int face = range.first; face < range.second; face++)
        {
          ...
          // deviating from the standard function
          // boundary_face_loop_inhom_operator() because the boundary condition
          // comes from the vector src
      ...
      for (unsigned int q = 0; q < integrator_m.n_q_points; ++q)
        {
          ...
          // deviating from the standard boundary_face_loop_inhom_operator()
          // function, because the boundary condition comes from the vector src
          Assert(
          ...
          if (boundary_type == BoundaryType::Dirichlet)
            {
              // The desired boundary value g is obtained as
      ...
      dealii::types::boundary_id const &boundary_id) const
    {
      ...
      for (unsigned int q = 0; q < integrator_m.n_q_points; ++q)
        {
          value neumann_value =
            calculate_neumann_value<dim, Number, n_components, rank>(
              q,
              integrator_m,
              boundary_type,
              boundary_id,
    ...
    void
    LaplaceOperator<dim, Number, n_components>::
      set_inhomogeneous_boundary_values(VectorType &dst) const
    {
      ...
      for (auto dbc : operator_data.bc->dirichlet_bc)
        {
          ...
          auto                  dbc_mask =
            operator_data.bc->dirichlet_bc_component_mask.find(dbc.first);
                  ...
                  for (unsigned int q = 0; q < integrator.n_q_points; ++q)
                    {
                      ...
                      unsigned int const index =
                        this->matrix_free->get_shape_info(dof_index, quad_index)
                      ...
                      if (boundary_type == BoundaryType::DirichletCached)
                        {
                          auto bc =
                            operator_data.bc->get_dirichlet_cached_data();

                          g = FunctionEvaluator<rank, dim, Number>::value(
                            *bc, face, q, quad_index);
                        }
                      else
                        {
                          AssertThrow(false,
