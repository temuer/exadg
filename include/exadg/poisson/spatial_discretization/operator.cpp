...
namespace ExaDG
{
  namespace Poisson
  {
    ...
    template <int dim, int n_components, typename Number>
    void
    Operator<dim, n_components, Number>::
      initialize_dof_handler_and_constraints()
    {
      fe = create_finite_element<dim>(param.grid.element_type,
                                      param.spatial_discretization ==
      ...
      // Affine constraints are only relevant for continuous Galerkin
      // discretizations.
      ...
      MatrixFreeData<dim, Number> &matrix_free_data) const
    {
      ...
      // for continuous FE discretizations, we need to evaluate inhomogeneous
      // Neumann boundary conditions or set constrained Dirichlet values, which
      // is why the second argument is always true
      ...
      if (param.right_hand_side)
        {
          matrix_free_data.append_mapping_flags(
            ExaDG::Operators::RHSKernel<dim, Number, n_components>::
              get_mapping_flags());
        }
      ...
      // Galerkin discretizations with DirichletCached boundary conditions. This
      // is not needed in case of discontinuous Galerkin discretizations where
      // boundary conditions are imposed weakly via integrals over the domain
      // boundaries.
      ...
      if (not(boundary_descriptor->dirichlet_cached_bc.empty()))
        {
          interface_data_dirichlet_cached =
            std::make_shared<ContainerInterfaceData<rank, dim, double>>();
    ...
    Operator<dim, n_components, Number>::setup_preconditioner_and_solver()
    {
      ...
      if (param.preconditioner == Poisson::Preconditioner::None)
        ...
        {
          preconditioner =
            std::make_shared<JacobiPreconditioner<Laplace>>(laplace_operator,
                                                            true);
        }
      else if (param.preconditioner == Poisson::Preconditioner::BlockJacobi)
        {
          preconditioner = std::make_shared<BlockJacobiPreconditioner<Laplace>>(
            laplace_operator, true);
        }
      else if (param.preconditioner == Poisson::Preconditioner::AdditiveSchwarz)
        {
          preconditioner =
            std::make_shared<AdditiveSchwarzPreconditioner<Laplace>>(
              laplace_operator, true);
        }
      ...
      if (param.solver == LinearSolver::CG)
        {
          ...
          solver_data.compute_performance_metrics =
            param.compute_performance_metrics;
      ...
      else if (param.solver == LinearSolver::FGMRES)
        {
          ...
          // initialize solver
          iterative_solver =
            std::make_shared<Krylov::SolverFGMRES<Laplace,
    ...
    template <int dim, int n_components, typename Number>
    void
    Operator<dim, n_components, Number>::initialize_dof_vector(
    ...
    template <int dim, int n_components, typename Number>
    void
    Operator<dim, n_components, Number>::prescribe_initial_conditions(
      VectorType &src) const
    {
    ...
    template <int dim, int n_components, typename Number>
    void
    Operator<dim, n_components, Number>::vmult(VectorType       &dst,
      ...
      if (false)
        {
          ...
          CheckMultigrid<dim, Number, Laplace, Multigrid> check_multigrid(
    ...
    template <int dim, int n_components, typename Number>
    std::shared_ptr<
      ContainerInterfaceData<Operator<dim, n_components, Number>::rank,
                             dim,
                             double>>
