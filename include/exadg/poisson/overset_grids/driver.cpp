...
namespace ExaDG
{
  namespace Poisson
  {
    namespace OversetGrids
    {
      ...
      {
        pcout << std::endl
              << "Setting up Poisson solver for overset grids:" << std::endl;

        // setup application
        AssertThrow(application->domain1.get(),
                    dealii::ExcMessage("Domain 1 is uninitialized."));
        AssertThrow(application->domain2.get(),
                    dealii::ExcMessage("Domain 2 is uninitialized."));

        application->domain1->setup_pre(grid1,
                                        mapping1,
                                        multigrid_mappings1,
                                        {"Domain1"});
        application->domain2->setup_pre(grid2,
                                        mapping2,
                                        multigrid_mappings2,
                                        {"Domain2"});
        ...
        poisson1->setup(
          application->domain1, grid1, mapping1, multigrid_mappings1, mpi_comm);
        poisson2->setup(
          application->domain2, grid2, mapping2, multigrid_mappings2, mpi_comm);

        // setup interface coupling
        {
          // domain 1 to domain 2
          pcout << std::endl
                << "Setup interface coupling first -> second ..." << std::endl;

          first_to_second =
            std::make_shared<InterfaceCoupling<rank, dim, Number>>();
          // No map of boundary IDs can be provided to make the search more
          // efficient. The reason behind is that the two domains are not
          // connected along boundaries but are overlapping instead. To resolve
          // this, the implementation of InterfaceCoupling needs to be
          // generalized.
          first_to_second->setup(
            poisson2->pde_operator->get_container_interface_data(),
          ...
          pcout << std::endl
                << "Setup interface coupling second -> first ..." << std::endl;
