...
namespace ExaDG
{
  namespace Poisson
  {
    ...
    {
      /**
       * This function determines which faces of the dst triangulation are
       * inside the src-triangulation. A face is considered inside, if all
       * vertices of the face are inside. Then, the boundary ID is set to bid
       * for all the faces of the dst-triangulation in the overlap region.
       */
      template <int dim>
      void
      set_boundary_ids_overlap_region(
        dealii::Triangulation<dim> const &tria_dst,
        ...
        dealii::Triangulation<dim> const &tria_src)
      {
        ...
        using Id =
          std::tuple<CellIteratorType /* cell */, unsigned int /*face*/>;
        std::vector<std::pair<Id, unsigned int /* first_point_in_vector */>>
          id_to_vector_index;
        ...
        // check which points have been found and whether a face on dst-side is
        // located inside the src triangulation
        for (auto iter = id_to_vector_index.begin();
             iter != id_to_vector_index.end();
             ++iter)
          {
            unsigned int const begin = iter->second;
            unsigned int const end   = (iter + 1 != id_to_vector_index.end()) ?
                                         (iter + 1)->second :
                                         points.size();
      ...
      public:
        ...
        setup_post(std::shared_ptr<Grid<dim> const> const &grid)
        {
          // boundary conditions
          boundary_descriptor =
            std::make_shared<BoundaryDescriptor<rank, dim>>();
      ...
      public:
        ...
        add_parameters(dealii::ParameterHandler &prm)
        {
          AssertThrow(domain1.get(),
                      dealii::ExcMessage("Domain 1 is uninitialized."));
          AssertThrow(domain2.get(),
