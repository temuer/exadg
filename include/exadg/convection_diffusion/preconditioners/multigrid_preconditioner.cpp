...
namespace ExaDG
{
  namespace ConvDiff
  {
      ...
      Map_DBC_ComponentMask const &dirichlet_bc_component_mask)
    {
      ...
      // When solving the reaction-convection-diffusion equations, it might be
      ...
      if (this->mg_operator_type == MultigridOperatorType::ReactionDiffusion)
        {
          ...
          data.diffusive_problem  = true;
               ...
               MultigridOperatorType::ReactionConvection)
        {
    ...
    MultigridPreconditioner<dim, Number>::update()
    {
      ...
      if (data.convective_problem and
          data.convective_kernel_data.velocity_type ==
            TypeVelocityField::DoFVector)
        {
          // If necessary, interpolate fine level velocity field to coarser
          // levels and set velocity for all operators
          ...
          this->transfer_from_fine_to_coarse_levels(
            [&](unsigned int const fine_level,
              ...
              transfers_velocity->interpolate(fine_level,
                                              vector_coarse_level,
                                              vector_fine_level);
      ...
      unsigned int const                    dealii_tria_level)
    {
      ...
      if (data.convective_problem)
        {
          if (data.convective_kernel_data.velocity_type ==
              TypeVelocityField::Function)
            {
              // do nothing
            }
          else if (data.convective_kernel_data.velocity_type ==
                   TypeVelocityField::DoFVector)
            {
              matrix_free_data.insert_dof_handler(
                &(*dof_handlers_velocity[level]), "velocity_dof_handler");
              matrix_free_data.insert_constraint(
                &(*constraints_velocity[level]), "velocity_dof_handler");
            }
          else
            {
              AssertThrow(false, dealii::ExcMessage("Not implemented."));
      ...
      unsigned int const level)
    {
      ...
      std::shared_ptr<MGOperator> mg_operator_level(
      ...
      if (data.convective_problem and
            ...
            TypeVelocityField::DoFVector)
        {
          std::shared_ptr<dealii::FiniteElement<dim>> fe_velocity =
            create_finite_element<dim>(get_element_type(
    ...
    template <int dim, typename Number>
    std::shared_ptr<CombinedOperator<
      dim,
      typename MultigridPreconditionerBase<dim, Number>::MultigridNumber>>
