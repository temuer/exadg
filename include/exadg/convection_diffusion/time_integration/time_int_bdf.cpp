  ...
  namespace ConvDiff
  {
    ...
    TimeIntBDF<dim, Number>::setup_derived()
    {
      ...
      if (param.ale_formulation and param.restarted_simulation == false)
        {
          // compute the grid coordinates at start time (and at previous times
          // in case of start_with_low_order == false)

          helpers_ale->move_grid(this->get_time());
          helpers_ale->fill_grid_coordinates_vector(
            vec_grid_coordinates[0], pde_operator->get_dof_handler_velocity());
      ...
      // Initialize vec_convective_term: Note that this function has to be
      // called after the solution has been initialized because the solution is
      // evaluated in this function.
      if (param.convective_problem() and param.treatment_of_convective_term ==
                                           TreatmentOfConvectiveTerm::Explicit)
        {
          // vec_convective_term does not have to be initialized in ALE case
          // (the convective term is recomputed in each time step for all
          // previous times on the new mesh). vec_convective_term does not have
          // to be initialized in case of a restart, where the vectors are read
          // from memory.
          if (param.ale_formulation == false and
      ...
      if (param.ale_formulation == true)
        {
          ...
          for (unsigned int i = 0; i < vec_grid_coordinates.size(); ++i)
            pde_operator->initialize_dof_vector_velocity(
              vec_grid_coordinates[i]);
        }
    }
    ...
    {
      // Start with i=1 since we only want to initialize the solution at former
      // instants of time.
      for (unsigned int i = 1; i < solution.size(); ++i)
        {
          ...
          pde_operator->prescribe_initial_conditions(
            solution[i], this->get_previous_time(i));
        }
    }
    ...
    TimeIntBDF<dim, Number>::calculate_time_step_size()
    {
      ...
      if (param.calculation_of_time_step_size ==
        ...
        {
          time_step =
            calculate_const_time_step(param.time_step_size, refine_steps_time);

          this->pcout << std::endl
                      << "Calculation of time step size (user-specified):"
                      << std::endl
                      << std::endl;
      ...
      else if (param.calculation_of_time_step_size == TimeStepCalculation::CFL)
        {
          AssertThrow(
            ...
            dealii::ExcMessage(
              "Specified type of time step calculation does not make sense!"));

          double time_step_global =
            pde_operator->calculate_time_step_cfl_global(this->get_time());
          ...
          if (this->adaptive_time_stepping == true)
            {
              ...
              // Note that in the ALE case there is no possibility to know the
              // grid velocity at this point and to use it for the calculation
              // of the time step size.

              if (param.analytical_velocity_field)
                {
                  time_step_adap =
                    pde_operator->calculate_time_step_cfl_analytical_velocity(
                      this->get_time());
                  time_step_adap *= cfl;
                }
              else
              ...
              // use adaptive time step size only if it is smaller, otherwise
              // use global time step size
              time_step = std::min(time_step_adap, time_step_global);

              // make sure that the maximum allowable time step size is not
              // exceeded
              time_step = std::min(time_step, param.time_step_size_max);

              print_parameter(this->pcout,
                              "Time step size (CFL adaptive)",
                              time_step);
            }
          else // constant time step size
            {
              time_step = adjust_time_step_to_hit_end_time(param.start_time,
                                                           param.end_time,
                                                           time_step_global);
              ...
              print_parameter(this->pcout, "Time step size", time_step);
               ...
               TimeStepCalculation::MaxEfficiency)
        {
          time_step =
            pde_operator->calculate_time_step_max_efficiency(this->order);
          ...
          this->pcout << std::endl
                      << "Calculation of time step size (max efficiency):"
                      << std::endl
                      << std::endl;
      ...
      else
        {
          AssertThrow(
            false,
            dealii::ExcMessage(
              "Specified type of time step calculation is not implemented."));
        }
      ...
      if (param.analytical_velocity_field)
      ...
      else // numerical velocity field
        {
          AssertThrow(velocities[0] != nullptr,
                      dealii::ExcMessage(
                        "Pointer velocities[0] is not initialized."));
          ...
          new_time_step_size =
            pde_operator->calculate_time_step_cfl_numerical_velocity(
              u_relative);
          new_time_step_size *= cfl;
        }

      // make sure that time step size does not exceed maximum allowable time
      // step size
      new_time_step_size =
        std::min(new_time_step_size, param.time_step_size_max);
      ...
      if (use_limiter)
        {
          ...
          limit_time_step_change(new_time_step_size,
                                 last_time_step_size,
                                 factor);
        }
    ...
    {
      // and compute grid coordinates at the end of the current time step
      // t_{n+1}
      helpers_ale->fill_grid_coordinates_vector(
        grid_coordinates_np, pde_operator->get_dof_handler_velocity());
    ...
    template <int dim, typename Number>
    void
    TimeIntBDF<dim, Number>::write_restart_vectors(
    ...
    TimeIntBDF<dim, Number>::do_timestep_solve()
    {
          ...
          if (param.get_type_velocity_field() == TypeVelocityField::DoFVector)
            {
              ...
              if (param.analytical_velocity_field)
                {
                  pde_operator->project_velocity(velocity_np,
                                                 this->get_next_time());
                }
              else
                {
                  AssertThrow(std::abs(times[0] - this->get_next_time()) <
                                1.e-12 * param.end_time,
                              dealii::ExcMessage("Invalid assumption."));
                  AssertThrow(
                    velocities[0] != nullptr,
                    dealii::ExcMessage(
                      "Pointer velocities[0] is not correctly initialized."));
      ...
      if (param.convective_problem() and param.treatment_of_convective_term ==
                                           TreatmentOfConvectiveTerm::Explicit)
        {
          // recompute convective term on new mesh for all previous time
          // instants in case of ALE formulation
          if (param.ale_formulation == true)
            {
              for (unsigned int i = 0; i < vec_convective_term.size(); ++i)
                {
      ...
      // evaluate convective term at end time t_{n+1} at which we know the
      // boundary condition g_u(t_{n+1})
      if (param.convective_problem() and param.treatment_of_convective_term ==
                                           TreatmentOfConvectiveTerm::Explicit)
        {
          if (param.ale_formulation == false)
      ...
      if (print_solver_info() and not(this->is_test))
        {
          this->pcout << std::endl
                      << "Solve scalar convection-diffusion equation:";
    ...
    TimeIntBDF<dim, Number>::postprocessing() const
    {
      ...
      // To allow a computation of errors at start_time (= if time step number
      // is 1 and if the simulation is not a restarted one), the mesh has to be
    ...
    TimeIntBDF<dim, Number>::print_iterations() const
    {
      ...
      iterations_avg[0] =
        (double)iterations.second / std::max(1., (double)iterations.first);
