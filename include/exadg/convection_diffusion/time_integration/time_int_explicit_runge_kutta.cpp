...
namespace ExaDG
{
  namespace ConvDiff
  {
    ...
    TimeIntExplRK<Number>::TimeIntExplRK(
      ...
      , diffusion_number(param.diffusion_number /
      ...
      if (param.calculation_of_time_step_size ==
          TimeStepCalculation::UserSpecified)
        {
          ...
          print_parameter(this->pcout, "time step size", this->time_step);
                 ...
                 TimeStepCalculation::CFL or
               param.calculation_of_time_step_size ==
                 TimeStepCalculation::CFLAndDiffusion)
        {
          double time_step_conv =
          ...
          if (this->adaptive_time_stepping)
            {
              ...
              // exceeded
              time_step_conv =
                std::min(time_step_conv, param.time_step_size_max);
          ...
          if (param.calculation_of_time_step_size ==
              TimeStepCalculation::CFLAndDiffusion)
            {
              ...
              this->pcout
                << std::endl
                << "Calculation of time step size according to Diffusion condition:"
                << std::endl
                << std::endl;
              print_parameter(this->pcout,
                              "Diffusion number",
                              diffusion_number);
              print_parameter(this->pcout,
                              "Time step size (diffusion)",
                              time_step_diff);

              time_step_conv = std::min(time_step_conv, time_step_diff);

              this->pcout << std::endl
                          << "Use minimum time step size:" << std::endl
                          << std::endl;
              print_parameter(this->pcout,
                              "Time step size (CFL and diffusion)",
                              time_step_conv);
            }
          ...
          if (this->adaptive_time_stepping == false)
            {
              time_step_conv =
                adjust_time_step_to_hit_end_time(this->start_time,
                                                 this->end_time,
                                                 time_step_conv);
          ...
          this->time_step = time_step_conv;
               ...
               TimeStepCalculation::Diffusion)
        {
          // calculate time step according to Diffusion number condition
          time_step_diff = pde_operator->calculate_time_step_diffusion();
          time_step_diff *= diffusion_number;

          this->time_step = adjust_time_step_to_hit_end_time(this->start_time,
                                                             this->end_time,
                                                             time_step_diff);
               ...
               TimeStepCalculation::MaxEfficiency)
        {
          ...
          this->time_step =
            pde_operator->calculate_time_step_max_efficiency(order);
    ...
    {
      AssertThrow(
        param.calculation_of_time_step_size == TimeStepCalculation::CFL or
          param.calculation_of_time_step_size ==
            TimeStepCalculation::CFLAndDiffusion,
        dealii::ExcMessage(
      ...
      if (print_solver_info() and not(this->is_test))
        {
          this->pcout
            << std::endl
            << "Solve scalar convection-diffusion equation explicitly:";
          print_wall_time(this->pcout, timer.wall_time());
        }

      this->timer_tree->insert({"Timeloop", "Solve-explicit"},
                               timer.wall_time());
    }
