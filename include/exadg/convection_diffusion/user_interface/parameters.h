...
namespace ExaDG
{
  namespace ConvDiff
  {
    ...
    public:
      ...
      // Use true if an analytical function is used to prescribe the velocity
      // field
      ...
      // description: see enum declaration (only relevant for explicit time
      // integration)
      TimeIntegratorRK time_integrator_rk;

      // order of time integration scheme (only relevant for BDF time
      // integration)
      ...
      // description: see enum declaration (this parameter is ignored for steady
      // problems or unsteady problems with explicit Runge-Kutta time
      // integration scheme). In case of a purely diffusive problem, one also
      // does not have to specify this parameter.
      ...
      // cfl number ("global" CFL number, can be larger than critical CFL in
      // case of operator-integration-factor splitting)
      double cfl;
      ...
      // exponent of fe_degree used in the calculation of the convective time
      // step size
      double exponent_fe_degree_convection;

      // exponent of fe_degree used in the calculation of the diffusion time
      // step size
      ...
      // diffusive term: Symmetric interior penalty discretization Galerkin
      // (SIPG) interior penalty parameter scaling factor: default value is 1.0
      double IP_factor;
      ...
      /*                                                                                    */
      /**************************************************************************************/
      // These parameters are only relevant if the inverse mass can not be
      // realized as a matrix-free operator evaluation. The typical use case is
      // a DG formulation with non hypercube elements (e.g. simplex elements).
      ...
      // This parameter is used for the inverse mass preconditioner. Note that
      // there is a separate parameter above for the inverse mass operator
      // (which needs to be inverted "exactly" for reasons of accuracy). This
      // parameter is only relevant if the inversion of the block-diagonal mass
      ...
      // simplex). In this case, however, you should think about replacing the
      ...
      // update preconditioner every ... time step. Only relevant if update
      // preconditioner is set to true.
      ...
      // solvers and matrix-free operator evaluation
      ...
      // solver data for block Jacobi preconditioner (only relevant for
      ...
      // Evaluate convective term and diffusive term at once instead of
      // implementing each operator separately and subsequently looping over all
      // operators. This parameter is only relevant in case of fully explicit
      // time stepping. In case of semi-implicit or fully implicit time
      // integration the combined operator will always be used.
      bool use_combined_operator;

      // In case that the velocity field is prescribed analytically, it might be
      // advantageous from the point of view of computational costs to store the
      // velocity field in a DoF vector instead of repeatedly calling
      // dealii::Function<dim>::value() whenever evaluating the operator in
      // iterative solvers. In other words, depending on the computer hardware
      // it might be more efficient to load a DoF vector and interpolate into
      // the quadrature points than calculating the velocity field by means of
      // dealii::Function<dim>::value(). This strategy makes only sense in case
      // of steady state problems or unsteady problems with an implicit
      // treatment of the convective term, i.e., in cases where the convective
