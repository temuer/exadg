...
namespace ExaDG
{
  namespace ConvDiff
  {
    Parameters::Parameters()
      ...
      , numerical_flux_convective_operator(
    ...
    Parameters::check() const
    {
      ...
      if (ale_formulation)
        {
          AssertThrow(
            ...
            dealii::ExcMessage(
              "Problem type has to be Unsteady when using ALE formulation."));

          AssertThrow(
            temporal_discretization == TemporalDiscretization::BDF and
              (treatment_of_convective_term ==
                 TreatmentOfConvectiveTerm::Implicit or
               treatment_of_convective_term ==
      ...
      if (equation_type == EquationType::Diffusion or
          equation_type == EquationType::ConvectionDiffusion)
        {
          AssertThrow(diffusivity >
      ...
      if (problem_type == ProblemType::Unsteady)
        {
          ...
          if (calculation_of_time_step_size ==
              TimeStepCalculation::UserSpecified)
            AssertThrow(time_step_size > 0.0,
                        dealii::ExcMessage("parameter must be defined"));
          ...
          if (calculation_of_time_step_size == TimeStepCalculation::Diffusion)
            {
              ...
              AssertThrow(diffusion_number > 0.,
                          dealii::ExcMessage("parameter must be defined"));
            }
          ...
          if (adaptive_time_stepping == true)
            {
              AssertThrow(
                calculation_of_time_step_size == TimeStepCalculation::CFL or
                  calculation_of_time_step_size ==
                    TimeStepCalculation::CFLAndDiffusion,
                dealii::ExcMessage(
          ...
          if (temporal_discretization == TemporalDiscretization::ExplRK)
            {
              AssertThrow(
                order_time_integrator >= 1 and order_time_integrator <= 4,
                dealii::ExcMessage(
                  "Specified order of time integrator ExplRK not implemented!"));
            }
          ...
          if (temporal_discretization == TemporalDiscretization::BDF)
            {
              AssertThrow(
                order_time_integrator >= 1 and order_time_integrator <= 4,
                dealii::ExcMessage(
                  "Specified order of time integrator BDF not implemented!"));
            }
        }
    ...
    {
      if (linear_system_has_to_be_solved() and
          preconditioner == Preconditioner::Multigrid and
    ...
    {
      if (analytical_velocity_field == true and
          store_analytical_velocity_in_dof_vector == false)
      ...
      dealii::ConditionalOStream const &pcout) const
    {
      ...
      // here we do not print quantities such as cfl, diffusion_number,
      ...
      dealii::ConditionalOStream const &pcout) const
    {
      ...
      if (implement_block_diagonal_preconditioner_matrix_free)
        {
          print_parameter(pcout,
                          "Solver block diagonal",
                          solver_block_diagonal);
