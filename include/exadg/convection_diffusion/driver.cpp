...
namespace ExaDG
{
  namespace ConvDiff
  {
    ...
    Driver<dim, Number>::setup()
    {
      ...
      pcout << std::endl
            << "Setting up scalar convection-diffusion solver:" << std::endl;
      ...
      if (not is_throughput_study)
        {
          ...
          if (application->get_parameters().problem_type ==
    ...
    template <int dim, typename Number>
    void
    Driver<dim, Number>::mark_cells_coarsening_and_refinement(
    ...
    {
      limit_coarsening_and_refinement(*grid->triangulation,
                                      application->get_parameters().amr_data);
              ...
              GridUtilities::
                create_coarse_triangulations_after_coarsening_and_refinement(
                  *grid->triangulation,
                  ...
                  application->get_parameters()
                    .amr_data.preserve_boundary_cells);
            }
              ...
              do
                {
                  ...
                  // Adapt the mesh before post_solve(), in order to recalculate
                  // the time step size based on the new mesh.
                  if (trigger_coarsening_and_refinement_now(
                        application->get_parameters()
                          .amr_data.trigger_every_n_time_steps,
                        time_integrator->get_number_of_time_steps()))
                    {
                      // AMR is only implemented for implicit timestepping.
                      std::shared_ptr<TimeIntBDF<dim, Number>>
                        bdf_time_integrator =
                          std::dynamic_pointer_cast<TimeIntBDF<dim, Number>>(
                            time_integrator);
                      ...
                      mark_cells_coarsening_and_refinement(
                        *grid->triangulation,
      ...
      double const total_time) const
    {
      ...
      this->pcout << "Performance results for convection-diffusion solver:"
                  << std::endl;

      // Averaged number of iterations are only relevant for BDF time integrator
      if (application->get_parameters().problem_type ==
            ProblemType::Unsteady and
          application->get_parameters().temporal_discretization ==
          ...
          if (application->get_parameters().temporal_discretization ==
              TemporalDiscretization::ExplRK)
            {
              std::shared_ptr<TimeIntExplRK<Number>> time_integrator_rk =
                std::dynamic_pointer_cast<TimeIntExplRK<Number>>(
                  time_integrator);
              timer_tree.insert({"Convection-diffusion"},
                                time_integrator_rk->get_timings());
            }
                   ...
                   TemporalDiscretization::BDF)
            {
              std::shared_ptr<TimeIntBDF<dim, Number>> time_integrator_bdf =
                std::dynamic_pointer_cast<TimeIntBDF<dim, Number>>(
                  time_integrator);
              timer_tree.insert({"Convection-diffusion"},
                                time_integrator_bdf->get_timings());
            }
          else
            {
              AssertThrow(false, dealii::ExcMessage("Not implemented."));
