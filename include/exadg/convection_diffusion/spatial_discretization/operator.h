...
namespace ExaDG
{
  namespace ConvDiff
  {
    ...
    public:
       ...
       * formulation. Note that you need to call the function
       * fill_matrix_free_data() beforehand in order to correctly initialize
      ...
      /*
       * Initialization of velocity dof-vector (in case of numerical velocity
       * field).
       */
      void
      ...
      /*
       * Obtain velocity dof-vector by interpolation of specified analytical
       * velocity field.
       */
      void
      ...
      /*
       * Obtain velocity dof-vector by L2-projection using the specified
      ...
      prescribe_initial_conditions(VectorType  &src,
                                   double const evaluation_time) const final;

      /*
       * This function is used in case of explicit time integration:
       *
       * It evaluates the right-hand side operator, the convective and diffusive
       * terms (subsequently multiplied by -1.0 in order to shift these terms to
       * the right-hand side of the equations) and finally applies the inverse
       * mass operator.
       */
      void
      ...
      /*
       * This function evaluates the convective term which is needed when using
       * an explicit formulation for the convective term.
       */
      void
       ...
       * arising e.g. from inhomogeneous boundary conditions or the solution at
       * previous instants of time occurring in the discrete time derivative
       * term.
       *
       * Note that the convective operator only has a contribution to the
       * right-hand side if it is formulated implicitly in time. In case of an
       * explicit treatment the whole convective operator (call function
       * evaluate_convective_term() instead of rhs()) has to be added to the
      ...
      /*
       * This function applies the mass operator to the src-vector and writes
       * the result to the dst-vector.
       */
      void
      apply_mass_operator(VectorType &dst, VectorType const &src) const;

      /*
       * This function applies the mass operator to the src-vector and adds the
       * result to the dst-vector.
       */
      void
      apply_mass_operator_add(VectorType &dst, VectorType const &src) const;

      /*
       * This function applies the convective operator to the src-vector and
       * writes the result to the dst-vector. It is needed for throughput
       * measurements of the matrix-free implementation.
       */
      void
      apply_convective_term(VectorType &dst, VectorType const &src) const;

      void
      update_convective_term(double const      evaluation_time,
                             VectorType const *velocity = nullptr) const;

      /*
       * This function applies the diffusive operator to the src-vector and
       * writes the result to the dst-vector. It is needed for throughput
       * measurements of the matrix-free implementation.
       */
      void
      ...
      /*
       * This function applies the combined mass-convection-diffusion operator
       * to the src-vector and writes the result to the dst-vector. It is needed
       * for throughput measurements of the matrix-free implementation.
       */
      void
      ...
      /*
       * This function solves the linear system of equations in case of implicit
       * time integration or steady-state problems (potentially involving the
       * mass, convective, and diffusive operators).
       */
      unsigned int
      ...
      calculate_time_step_max_efficiency(
        unsigned int const order_time_integrator) const final;
      ...
      calculate_time_step_cfl_numerical_velocity(
        VectorType const &velocity) const final;
    ...
    private:
       ...
       */
      std::shared_ptr<ExaDG::SolutionTransfer<dim, VectorType>>
        solution_transfer;
      ...
      std::string const quad_index_overintegration =
