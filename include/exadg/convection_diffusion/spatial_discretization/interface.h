  ...
  namespace ConvDiff
  {
    namespace Interface
    {
      ...
      public:
        ...
        // implicit time integration: calculate right-hand side of linear system
        // of equations
        virtual void
        rhs(VectorType       &dst,
        ...
        calculate_time_step_max_efficiency(
          unsigned int const order_time_integrator) const = 0;
        ...
        calculate_time_step_cfl_numerical_velocity(
          VectorType const &velocity) const = 0;
    ...
    public:
      ...
      OperatorExplRK(
        std::shared_ptr<ConvDiff::Interface::Operator<Number>> operator_in,
        bool const numerical_velocity_field_in)
        : pde_operator(operator_in)
        , numerical_velocity_field(numerical_velocity_field_in)
      {
               ...
               double const      evaluation_time) const
      {
        if (numerical_velocity_field)
          {
            interpolate(velocity_interpolated,
                        evaluation_time,
                        velocities,
                        times);

            pde_operator->evaluate_explicit_time_int(dst,
                                                     src,
                                                     evaluation_time,
