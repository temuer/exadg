      ...
      public:
               ...
               bool const                             use_own_velocity_storage)
        {
          ...
          if (data.velocity_type == TypeVelocityField::DoFVector)
            {
              integrator_velocity = std::make_shared<CellIntegratorVelocity>(
                matrix_free, data.dof_index_velocity, quad_index);

              integrator_velocity_m = std::make_shared<FaceIntegratorVelocity>(
                matrix_free, true, data.dof_index_velocity, quad_index);

              integrator_velocity_p = std::make_shared<FaceIntegratorVelocity>(
                matrix_free, false, data.dof_index_velocity, quad_index);

              if (use_own_velocity_storage)
                {
                  velocity.reset();
                  matrix_free.initialize_dof_vector(velocity.own(),
        ...
        get_mapping_flags()
        {
          ...
          flags.cells =
            ...
            dealii::update_quadrature_points; // q-points due to analytical
                                              // velocity field
          flags.inner_faces =
            dealii::update_JxW_values | dealii::update_quadrature_points |
            dealii::update_normal_vectors; // q-points due to analytical
                                           // velocity field
          ...
          if (data.velocity_type == TypeVelocityField::DoFVector)
            {
              ...
              if (boundary_id ==
                ...
                {
                  // TODO: Matrix-free implementation in deal.II does currently
                  // not allow to access data of the neighboring element in case
                  // of cell-based face loops.
                  //      integrator_velocity_p->reinit(cell, face);
                  //      integrator_velocity_p->gather_evaluate(*velocity,
        ...
        {
          return 0.5 *
                 (normal_velocity_m * value_m + normal_velocity_p * value_p);
        }

        /*
         * This function calculates the numerical flux using the Lax-Friedrichs
         * flux.
         */
        inline DEAL_II_ALWAYS_INLINE //
          scalar
                                        ...
                                        scalar const &normal_velocity_p) const
        {
          scalar jump_value = value_m - value_p;
          scalar lambda =
            std::max(std::abs(normal_velocity_m), std::abs(normal_velocity_p));
        ...
        /*
         * This function calculates the numerical flux where the type of the
         * numerical flux depends on the specified parameter. This function
         * handles both analytical and numerical velocity fields.
         */
        inline DEAL_II_ALWAYS_INLINE //
          scalar
                         ...
                         bool const         exterior_velocity_available) const
        {
          ...
          if (data.velocity_type == TypeVelocityField::Function)
            {
              dealii::Point<dim, scalar> q_points =
                integrator.quadrature_point(q);

              vector velocity =
                FunctionEvaluator<1, dim, Number>::value(*(data.velocity),
                                                         q_points,
                                                         time);

              scalar normal_velocity = velocity * normal_m;

              // flux functions are the same for DivergenceFormulation and
              // ConvectiveFormulation

              if (data.numerical_flux_formulation ==
                  NumericalFluxConvectiveOperator::CentralFlux)
                {
                  flux =
                    calculate_central_flux(value_m, value_p, normal_velocity);
                }
              else if (data.numerical_flux_formulation ==
                       NumericalFluxConvectiveOperator::LaxFriedrichsFlux)
                {
                  flux = calculate_lax_friedrichs_flux(value_m,
                                                       value_p,
                                                       normal_velocity);
                }
            }
          else if (data.velocity_type == TypeVelocityField::DoFVector)
            {
              ...
              vector velocity_p = exterior_velocity_available ?
                                    integrator_velocity_p->get_value(q) :
                                    velocity_m;
              ...
              if (data.formulation ==
                  FormulationConvectiveTerm::DivergenceFormulation)
                {
                  if (data.numerical_flux_formulation ==
                      NumericalFluxConvectiveOperator::CentralFlux)
                    {
                      flux = calculate_central_flux(value_m,
                                                    value_p,
                                                    normal_velocity_m,
                                                    normal_velocity_p);
                    }
                           ...
                           NumericalFluxConvectiveOperator::LaxFriedrichsFlux)
                    {
                      flux = calculate_lax_friedrichs_flux(value_m,
                       ...
                       FormulationConvectiveTerm::ConvectiveFormulation)
                {
                  scalar normal_velocity =
                    0.5 * (normal_velocity_m + normal_velocity_p);
        ...
        inline DEAL_II_ALWAYS_INLINE //
          ...
          calculate_flux_interior_and_neighbor(
            unsigned int const q,
            IntegratorFace    &integrator,
            scalar const      &value_m,
            scalar const      &value_p,
            ...
            bool const         exterior_velocity_available) const
        {
          ...
          if (data.formulation ==
              FormulationConvectiveTerm::ConvectiveFormulation)
            {
              vector velocity = calculate_average_velocity(
                q, integrator, time, exterior_velocity_available);
              scalar normal_velocity = velocity * normal_m;
                                          ...
                                          Number const      &time) const
        {
          ...
          if (data.velocity_type == TypeVelocityField::Function)
            {
              velocity = FunctionEvaluator<1, dim, Number>::value(
                *(data.velocity), integrator.quadrature_point(q), time);
            }
          else if (data.velocity_type == TypeVelocityField::DoFVector)
            {
