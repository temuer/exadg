          ...
          if (operator_data.kernel_data.formulation ==
              FormulationConvectiveTerm::DivergenceFormulation)
            {
              ...
              integrator.submit_gradient(
                kernel->get_volume_flux_divergence_form(
                   ...
                   FormulationConvectiveTerm::ConvectiveFormulation)
            {
              ...
              integrator.submit_value(kernel->get_volume_flux_convective_form(
      ...
      for (unsigned int q = 0; q < integrator_m.n_q_points; ++q)
        {
          ...
          std::tuple<scalar, scalar> flux =
            kernel->calculate_flux_interior_and_neighbor(
      ...
      for (unsigned int q = 0; q < integrator_m.n_q_points; ++q)
        {
          ...
          // TODO
          // The matrix-free implementation in deal.II does currently not allow
          // to access neighboring data in case of cell-based face loops. We
          // therefore have to use integrator_velocity_m twice to avoid the
          // problem of accessing data of the neighboring element. Note that
          // this variant calculates the diagonal and block-diagonal only
          // approximately. The theoretically correct version using
          // integrator_velocity_p is currently not implemented in deal.II.
          bool exterior_velocity_available =
            false; // TODO -> set to true once functionality is available
          scalar flux =
            kernel->calculate_flux_interior(q,
                                            integrator_m,
                                            value_m,
                                            value_p,
                                            normal_m,
