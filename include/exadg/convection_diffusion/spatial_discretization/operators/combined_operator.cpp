...
namespace ExaDG
{
  namespace ConvDiff
  {
    ...
    CombinedOperator<dim, Number>::CombinedOperator()
      ...
      CombinedOperatorData<dim> const         &data)
    {
      ...
      if (operator_data.unsteady_problem)
        this->integrator_flags =
          this->integrator_flags | mass_kernel->get_integrator_flags();

      if (operator_data.convective_problem)
        this->integrator_flags =
          this->integrator_flags | convective_kernel->get_integrator_flags();

      if (operator_data.diffusive_problem)
        this->integrator_flags =
          this->integrator_flags | diffusive_kernel->get_integrator_flags();
    }
    ...
    CombinedOperator<dim, Number>::initialize(
      ...
      std::shared_ptr<Operators::ConvectiveKernel<dim, Number>>
        convective_kernel_in,
      std::shared_ptr<Operators::DiffusiveKernel<dim, Number>>
        diffusive_kernel_in)
    {
      operator_data = data;
    ...
    template <int dim, typename Number>
    void
    CombinedOperator<dim, Number>::set_velocity_copy(
    ...
    template <int dim, typename Number>
    void
    CombinedOperator<dim, Number>::set_velocity_ptr(
      VectorType const &velocity) const
    {
      if (operator_data.convective_problem)
    ...
    template <int dim, typename Number>
    void
    CombinedOperator<dim, Number>::set_scaling_factor_mass_operator(
      Number const &scaling_factor)
    {
    ...
    template <int dim, typename Number>
    void
    CombinedOperator<dim, Number>::reinit_cell_derived(
    ...
    template <int dim, typename Number>
    void
    CombinedOperator<dim, Number>::reinit_face_derived(
      ...
      unsigned int const face) const
    {
      ...
      if (operator_data.diffusive_problem)
        diffusive_kernel->reinit_face(integrator_m,
                                      integrator_p,
                                      operator_data.dof_index);
    }
    ...
    template <int dim, typename Number>
    void
    CombinedOperator<dim, Number>::reinit_boundary_face_derived(
      IntegratorFace    &integrator_m,
      unsigned int const face) const
    {
      if (operator_data.convective_problem)
      ...
      if (operator_data.diffusive_problem)
        diffusive_kernel->reinit_boundary_face(integrator_m,
                                               operator_data.dof_index);
    }
    ...
    template <int dim, typename Number>
    void
    CombinedOperator<dim, Number>::do_cell_integral(
          ...
          if (operator_data.convective_problem)
            {
              if (operator_data.convective_kernel_data.formulation ==
                ...
                {
                  gradient_flux +=
                    convective_kernel->get_volume_flux_divergence_form(
                      value, integrator, q, this->time);
                }
              else if (operator_data.convective_kernel_data.formulation ==
                       FormulationConvectiveTerm::ConvectiveFormulation)
                {
                  value_flux +=
                    convective_kernel->get_volume_flux_convective_form(
                      gradient, integrator, q, this->time);
                }
              else
                {
    ...
    template <int dim, typename Number>
    void
    CombinedOperator<dim, Number>::do_face_integral(
      IntegratorFace &integrator_m,
      ...
      for (unsigned int q = 0; q < integrator_m.n_q_points; ++q)
        {
          ...
          if (operator_data.convective_problem)
            {
              std::tuple<scalar, scalar> flux =
                convective_kernel->calculate_flux_interior_and_neighbor(
                  q,
                  integrator_m,
                  value_m,
                  value_p,
          ...
          if (operator_data.diffusive_problem)
            {
              scalar gradient_flux =
                diffusive_kernel->calculate_gradient_flux(value_m, value_p);
    ...
    template <int dim, typename Number>
    void
    CombinedOperator<dim, Number>::do_face_int_integral(
      IntegratorFace &integrator_m,
      IntegratorFace &integrator_p) const
    {
      (void)integrator_p;
    ...
    // TODO can be removed later once matrix-free evaluation allows accessing
    // neighboring data for cell-based face loops
    template <int dim, typename Number>
    void
    CombinedOperator<dim, Number>::do_face_int_integral_cell_based(
      IntegratorFace &integrator_m,
      IntegratorFace &integrator_p) const
    {
      (void)integrator_p;
      ...
      for (unsigned int q = 0; q < integrator_m.n_q_points; ++q)
        {
          ...
          if (operator_data.convective_problem)
            {
              // TODO
              // The matrix-free implementation in deal.II does currently not
              // allow to access neighboring data in case of cell-based face
              // loops. We therefore have to use integrator_velocity_m twice to
              // avoid the problem of accessing data of the neighboring element.
              // Note that this variant calculates the diagonal and
              // block-diagonal only approximately. The theoretically correct
              // version using integrator_velocity_p is currently not
    ...
    template <int dim, typename Number>
    void
    CombinedOperator<dim, Number>::do_face_ext_integral(
      IntegratorFace &integrator_m,
      IntegratorFace &integrator_p) const
    {
      (void)integrator_m;
      ...
      for (unsigned int q = 0; q < integrator_p.n_q_points; ++q)
        {
          ...
          if (operator_data.diffusive_problem)
            {
              ...
              value_flux += -diffusive_kernel->calculate_value_flux(
                normal_gradient_p, normal_gradient_m, value_p, value_m);
            }
          ...
          if (operator_data.diffusive_problem)
            {
              scalar gradient_flux =
                diffusive_kernel->calculate_gradient_flux(value_p, value_m);
      ...
      dealii::types::boundary_id const &boundary_id) const
    {
      ...
      for (unsigned int q = 0; q < integrator_m.n_q_points; ++q)
        {
          scalar value_m =
            calculate_interior_value(q, integrator_m, operator_type);
          ...
          if (operator_data.diffusive_problem)
            {
              scalar normal_gradient_m =
                calculate_interior_normal_gradient(q,
                                                   integrator_m,
                                                   operator_type);
              scalar normal_gradient_p =
