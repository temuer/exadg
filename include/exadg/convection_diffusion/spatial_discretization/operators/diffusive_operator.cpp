...
namespace ExaDG
{
  namespace ConvDiff
  {
    ...
    {
      kernel->calculate_penalty_parameter(*this->matrix_free,
                                          operator_data.dof_index);
    }

    template <int dim, typename Number>
    void
    DiffusiveOperator<dim, Number>::reinit_face_derived(
    ...
    template <int dim, typename Number>
    void
    DiffusiveOperator<dim, Number>::reinit_boundary_face_derived(
      IntegratorFace    &integrator_m,
      unsigned int const face) const
    {
      (void)face;
    ...
    template <int dim, typename Number>
    void
    DiffusiveOperator<dim, Number>::do_cell_integral(
    ...
    template <int dim, typename Number>
    void
    DiffusiveOperator<dim, Number>::do_face_integral(
      IntegratorFace &integrator_m,
      ...
      for (unsigned int q = 0; q < integrator_m.n_q_points; ++q)
        {
          ...
          integrator_p.submit_value(value_flux,
                                    q); // opposite sign since n⁺ = -n⁻
        }
    }

    template <int dim, typename Number>
    void
    DiffusiveOperator<dim, Number>::do_face_int_integral(
      IntegratorFace &integrator_m,
      IntegratorFace &integrator_p) const
    {
      (void)integrator_p;
    ...
    template <int dim, typename Number>
    void
    DiffusiveOperator<dim, Number>::do_face_ext_integral(
      IntegratorFace &integrator_m,
      IntegratorFace &integrator_p) const
    {
      (void)integrator_m;
      ...
      for (unsigned int q = 0; q < integrator_p.n_q_points; ++q)
        {
          ...
          scalar value_flux = kernel->calculate_value_flux(normal_gradient_p,
                                                           normal_gradient_m,
                                                           value_p,
                                                           value_m);
