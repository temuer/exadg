...
namespace ExaDG
{
  ...
  {
    /*
     *  The DoF vector contains the vector of conserved quantities (rho, rho u,
     * rho E). This class allows to transfer these quantities into the derived
     * variables (p, u, T) by using L2-projections.
     */
    ...
    public:
      ...
      void
      compute_pressure(VectorType       &pressure,
                       VectorType const &solution_conserved) const
      {
        ...
        matrix_free->cell_loop(&This::local_apply_pressure,
                               this,
                               pressure,
                               solution_conserved);
      }
      ...
      void
      compute_velocity(VectorType       &velocity,
                       VectorType const &solution_conserved) const
      {
        matrix_free->cell_loop(&This::local_apply_velocity,
                               this,
                               velocity,
                               solution_conserved);
      }
      ...
      void
      compute_temperature(VectorType       &temperature,
                          VectorType const &solution_conserved) const
      {
        AssertThrow(heat_capacity_ratio > 0.0,
                    dealii::ExcMessage(
        ...
        matrix_free->cell_loop(&This::local_apply_temperature,
                               this,
                               temperature,
                               solution_conserved);
      }
    ...
    private:
        ...
        std::pair<unsigned int, unsigned int> const &cell_range) const
      {
        ...
        // dst-vector
        CellIntegratorScalar pressure(matrix_free,
                                      dof_index_scalar,
                                      quad_index,
                                      0);
            ...
            for (unsigned int q = 0; q < energy.n_q_points; ++q)
              {
                ...
                scalar p = dealii::make_vectorized_array<Number>(
                             heat_capacity_ratio - 1.0) *
        ...
        std::pair<unsigned int, unsigned int> const &cell_range) const
      {
        ...
        // dst-vector
        CellIntegratorVector velocity(matrix_free,
                                      dof_index_vector,
                                      quad_index,
                                      0);
        ...
        std::pair<unsigned int, unsigned int> const &cell_range) const
      {
        ...
        // dst-vector
        CellIntegratorScalar temperature(matrix_free,
                                         dof_index_scalar,
                                         quad_index,
                                         0);
            ...
            for (unsigned int q = 0; q < energy.n_q_points; ++q)
              {
                ...
                scalar T =
                  dealii::make_vectorized_array<Number>(
                    (heat_capacity_ratio - 1.0) / specific_gas_constant) *
