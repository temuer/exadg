  ...
  namespace CompNS
  {
    ...
    Operator<dim, Number>::initialize_dof_handler_and_constraints()
    {
      fe        = create_finite_element<dim>(ElementType::Hypercube,
                                      ...
                                      dim + 2,
                                      param.degree);
      fe_vector = create_finite_element<dim>(ElementType::Hypercube,
                                             true,
                                             dim,
                                             param.degree);
      fe_scalar = create_finite_element<dim>(ElementType::Hypercube,
                                             true,
                                             1,
                                             param.degree);
      ...
      MatrixFreeData<dim, Number> &matrix_free_data) const
    {
      ...
      matrix_free_data.insert_dof_handler(&dof_handler_vector,
                                          field + dof_index_vector);
      matrix_free_data.insert_dof_handler(&dof_handler_scalar,
                                          field + dof_index_scalar);
      ...
      if (param.n_q_points_convective == QuadratureRule::Standard)
      ...
      else if (param.n_q_points_convective ==
               QuadratureRule::Overintegration32k)
        ...
        AssertThrow(false,
                    dealii::ExcMessage(
                      "Specified quadrature rule is not implemented."));
      ...
      matrix_free_data.insert_quadrature(*quadrature_standard,
      ...
      matrix_free_data.insert_quadrature(*quadrature_conv,
                                         field +
      ...
      matrix_free_data.insert_quadrature(*quadrature_vis,
                                         field +
                                           quad_index_overintegration_vis);
    }
    ...
    Operator<dim, Number>::setup_operators()
    {
      ...
      inverse_mass_operator_data_vector.parameters =
      ...
      inverse_mass_operator_data_scalar.parameters =
        param.inverse_mass_operator;
      ...
      body_force_operator_data.rhs_rho =
        field_functions->right_hand_side_density;
      body_force_operator_data.rhs_u =
        field_functions->right_hand_side_velocity;
      ...
      if (param.use_combined_operator == true)
        {
          AssertThrow(
            param.n_q_points_convective == param.n_q_points_viscous,
            dealii::ExcMessage(
              "Use the same number of quadrature points for convective term "
          ...
          combined_operator_data.quad_index =
            get_quad_index_overintegration_vis();
    ...
    {
      pcout << std::endl
            << "Setup compressible Navier-Stokes DG operator ..." << std::endl;
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::initialize_dof_vector_dim_components(
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::evaluate(VectorType       &dst,
                                    ...
                                    Number const      time) const
    {
      ...
      // shift viscous and convective terms to the right-hand side of the
      // equation
      dst *= -1.0;
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::evaluate_convective_and_viscous(
      VectorType       &dst,
      VectorType const &src,
      Number const      time) const
    {
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::apply_inverse_mass(VectorType       &dst,
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::compute_pressure(VectorType       &dst,
                                            VectorType const &src) const
    {
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::compute_velocity(VectorType       &dst,
                                            VectorType const &src) const
    {
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::compute_temperature(VectorType       &dst,
                                               VectorType const &src) const
    {
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::compute_vorticity(VectorType       &dst,
                                             VectorType const &src) const
    {
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::compute_divergence(VectorType       &dst,
                                              VectorType const &src) const
    {
    ...
    template <int dim, typename Number>
    void
    Operator<dim, Number>::compute_shear_rate(VectorType       &dst,
                                              VectorType const &src) const
    {
    ...
    Operator<dim, Number>::calculate_time_step_cfl_global() const
    {
      ...
      double const speed_of_sound =
        sqrt(param.heat_capacity_ratio * param.specific_gas_constant *
    ...
    {
      double const h_min =
        calculate_minimum_vertex_distance(dof_handler.get_triangulation(),
                                          get_mapping(),
                                          mpi_comm);
