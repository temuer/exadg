...
namespace ExaDG
{
  namespace CompNS
  {
    ...
    calculate_energy(
      dealii::VectorizedArray<Number> const                         &T,
      dealii::Tensor<1, dim, dealii::VectorizedArray<Number>> const &u,
      Number const                                                  &c_v)
    {
    ...
    inline DEAL_II_ALWAYS_INLINE //
      ...
      calculate_grad_E(
        dealii::VectorizedArray<Number> const &rho_inverse,
        dealii::VectorizedArray<Number> const &rho_E,
        dealii::Tensor<1, dim, dealii::VectorizedArray<Number>> const &grad_rho,
        dealii::Tensor<1, dim, dealii::VectorizedArray<Number>> const
          &grad_rho_E)
    {
    ...
    inline DEAL_II_ALWAYS_INLINE //
      dealii::Tensor<1, dim, dealii::VectorizedArray<Number>>
      calculate_grad_T(
        dealii::Tensor<1, dim, dealii::VectorizedArray<Number>> const &grad_E,
        dealii::Tensor<1, dim, dealii::VectorizedArray<Number>> const &u,
    ...
    /*
     * Calculates exterior state "+" for a scalar/vectorial quantity depending
     * on interior state "-" and boundary conditions.
     */
    template <int dim, typename Number, int rank>
    inline DEAL_II_ALWAYS_INLINE //
      dealii::Tensor<rank, dim, dealii::VectorizedArray<Number>>
      calculate_exterior_value(
        dealii::Tensor<rank, dim, dealii::VectorizedArray<Number>> const
                                         &value_m,
      ...
      if (boundary_type == BoundaryType::Dirichlet)
        {
          ...
          auto g =
            FunctionEvaluator<rank, dim, Number>::value(*bc, q_point, time);

          value_p =
            -value_m +
            dealii::Tensor<rank, dim, dealii::VectorizedArray<Number>>(2.0 * g);
        }
    ...
    /*
     * Calculates exterior state of normal gradient (Neumann type boundary
     * conditions) depending on interior data and boundary conditions.
     */
    template <int dim, typename Number, int rank>
    inline DEAL_II_ALWAYS_INLINE //
      dealii::Tensor<rank, dim, dealii::VectorizedArray<Number>>
        ...
        Number const                                              &time)
    {
      ...
      if (boundary_type == BoundaryType::Dirichlet)
      ...
      else if (boundary_type == BoundaryType::Neumann)
        {
          ...
          grad_P_normal =
            -grad_M_normal +
            dealii::Tensor<rank, dim, dealii::VectorizedArray<Number>>(2.0 * h);
        }
      else
        {
          AssertThrow(false, dealii::ExcMessage("Not implemented."));
    ...
    /*
     * This function calculates the Lax-Friedrichs flux for the momentum
     * equation
     */
    template <int dim, typename Number>
    inline DEAL_II_ALWAYS_INLINE //
        ...
        dealii::Tensor<1, dim, dealii::VectorizedArray<Number>> const &normal)
    {
      ...
      for (unsigned int d = 0; d < dim; ++d)
        {
          dealii::VectorizedArray<Number> sum =
            dealii::VectorizedArray<Number>();
          for (unsigned int e = 0; e < dim; ++e)
    ...
    /*
     * This function calculates the Lax-Friedrichs flux for scalar quantities
     * (density/energy)
     */
    template <int dim, typename Number>
    inline DEAL_II_ALWAYS_INLINE //
      dealii::VectorizedArray<Number>
      calculate_flux(
        dealii::Tensor<1, dim, dealii::VectorizedArray<Number>> const &flux_M,
        ...
        dealii::Tensor<1, dim, dealii::VectorizedArray<Number>> const &normal)
    {
      dealii::Tensor<1, dim, dealii::VectorizedArray<Number>> average_flux =
        0.5 * (flux_M + flux_P);
     ...
     *   lambda = max( |u_M| + sqrt(|gamma * p_M / rho_M|) , |u_P| + sqrt(|gamma
     * * p_P / rho_P|) )
     */
    template <int dim, typename Number>
    inline DEAL_II_ALWAYS_INLINE //
      dealii::VectorizedArray<Number>
      calculate_lambda(
        dealii::VectorizedArray<Number> const                         &rho_m,
        ...
        Number const                                                  &gamma)
    {
      dealii::VectorizedArray<Number> lambda_m =
        u_m.norm() + std::sqrt(std::abs(gamma * p_m / rho_m));
      dealii::VectorizedArray<Number> lambda_p =
        u_p.norm() + std::sqrt(std::abs(gamma * p_p / rho_p));
    ...
    public:
                        ...
                        unsigned int const    q) const
      {
        ...
        scalar rhs_density =
          FunctionEvaluator<0, dim, Number>::value(*(data.rhs_rho),
                                                   q_points,
                                                   eval_time);
        vector rhs_momentum =
          FunctionEvaluator<1, dim, Number>::value(*(data.rhs_u),
                                                   q_points,
                                                   eval_time);
        scalar rhs_energy =
          FunctionEvaluator<0, dim, Number>::value(*(data.rhs_E),
                                                   q_points,
                                                   eval_time);

        return std::make_tuple(rhs_density,
                               rhs_momentum,
                               rhs_momentum * u + rhs_energy);
      }
    ...
    private:
      ...
      {
        CellIntegratorScalar density(matrix_free,
                                     data.dof_index,
                                     data.quad_index,
                                     0);
        CellIntegratorVector momentum(matrix_free,
                                      data.dof_index,
                                      data.quad_index,
                                      1);
        CellIntegratorScalar energy(matrix_free,
                                    data.dof_index,
                                    data.quad_index,
                                    1 + dim);
            ...
            for (unsigned int q = 0; q < density.n_q_points; ++q)
              {
                std::tuple<scalar, vector, scalar> flux =
                  get_volume_flux(density, momentum, q);
    ...
    public:
                 ...
                 unsigned int const    q) const
      {
        ...
        // calculate lambda
        scalar lambda =
          calculate_lambda(rho_M, rho_P, u_M, u_P, p_M, p_P, gamma);

        // flux density
        scalar flux_density =
          calculate_flux(rho_u_M, rho_u_P, rho_M, rho_P, lambda, normal);
        ...
        vector flux_momentum = calculate_flux(
          momentum_flux_M, momentum_flux_P, rho_u_M, rho_u_P, lambda, normal);

        // flux energy
        vector energy_flux_M = (rho_E_M + p_M) * u_M;
        vector energy_flux_P = (rho_E_P + p_P) * u_P;

        scalar flux_energy = calculate_flux(
          energy_flux_M, energy_flux_P, rho_E_M, rho_E_P, lambda, normal);
                          ...
                          unsigned int const                q) const
      {
        ...
        // calculate rho_P
        scalar rho_P =
          calculate_exterior_value<dim, Number, 0>(rho_M,
        ...
        // calculate u_P
        vector u_P =
          calculate_exterior_value<dim, Number, 1>(u_M,
        ...
        // calculate p_P
        scalar p_P =
          calculate_exterior_value<dim, Number, 0>(p_M,
        ...
        if (boundary_variable == EnergyBoundaryVariable::Energy)
          {
            E_P =
              calculate_exterior_value<dim, Number, 0>(E_M,
                                                       boundary_type_energy,
                                                       data.bc->energy,
                                                       boundary_id,
                                                       energy.quadrature_point(
                                                         q),
                                                       this->eval_time);
          }
        else if (boundary_variable == EnergyBoundaryVariable::Temperature)
          {
            ...
            scalar T_P =
              calculate_exterior_value<dim, Number, 0>(T_M,
                                                       boundary_type_energy,
                                                       data.bc->energy,
                                                       boundary_id,
                                                       energy.quadrature_point(
    ...
    private:
            ...
            for (unsigned int q = 0; q < momentum.n_q_points; ++q)
              {
                std::tuple<vector, tensor, vector> flux =
                  get_volume_flux(density, momentum, energy, q);
      ...
      {
        FaceIntegratorScalar density_m(
          matrix_free, true, data.dof_index, data.quad_index, 0);
        FaceIntegratorScalar density_p(
          matrix_free, false, data.dof_index, data.quad_index, 0);

        FaceIntegratorVector momentum_m(
          matrix_free, true, data.dof_index, data.quad_index, 1);
        FaceIntegratorVector momentum_p(
          matrix_free, false, data.dof_index, data.quad_index, 1);

        FaceIntegratorScalar energy_m(
          matrix_free, true, data.dof_index, data.quad_index, 1 + dim);
        FaceIntegratorScalar energy_p(
          matrix_free, false, data.dof_index, data.quad_index, 1 + dim);
        ...
        for (unsigned int face = face_range.first; face < face_range.second;
             face++)
          {
            ...
            for (unsigned int q = 0; q < momentum_m.n_q_points; ++q)
              {
                std::tuple<scalar, vector, scalar> flux = get_flux(density_m,
                                                                   density_p,
                                                                   momentum_m,
                                                                   momentum_p,
                                                                   energy_m,
        ...
        std::pair<unsigned int, unsigned int> const &face_range) const
      {
        ...
        for (unsigned int face = face_range.first; face < face_range.second;
             face++)
          {
            ...
            BoundaryType boundary_type_density =
              data.bc->density.get_boundary_type(boundary_id);
            BoundaryType boundary_type_velocity =
              data.bc->velocity.get_boundary_type(boundary_id);
            BoundaryType boundary_type_pressure =
              data.bc->pressure.get_boundary_type(boundary_id);
            BoundaryType boundary_type_energy =
              data.bc->energy.get_boundary_type(boundary_id);
    ...
    public:
                 ...
                 ViscousOperatorData<dim> const        &data_in)
      {
        ...
        dealii::FiniteElement<dim> const &fe =
          matrix_free->get_dof_handler(data.dof_index).get_fe();
        degree = fe.degree;
      ...
      inline DEAL_II_ALWAYS_INLINE //
        scalar
        get_penalty_parameter(FaceIntegratorScalar &fe_eval_m,
                              FaceIntegratorScalar &fe_eval_p) const
      {
        scalar tau =
          ...
          IP::get_penalty_factor<dim, Number>(
            degree,
            get_element_type(fe_eval_m.get_matrix_free()
                               .get_dof_handler(data.dof_index)
                               .get_triangulation()),
            data.IP_factor) *
          nu;
        ...
        get_penalty_parameter(FaceIntegratorScalar &fe_eval) const
      {
        scalar tau = fe_eval.read_cell_data(array_penalty_parameter) *
                     IP::get_penalty_factor<dim, Number>(
                       degree,
                       get_element_type(fe_eval.get_matrix_free()
                                          .get_dof_handler(data.dof_index)
                                          .get_triangulation()),
                       data.IP_factor) *
                     nu;
                          ...
                          unsigned int const    q) const
      {
        ...
        scalar gradient_flux_energy =
          flux_energy_average * normal - tau_IP * jump_energy;

        return std::make_tuple(gradient_flux_density,
                               gradient_flux_momentum,
                               gradient_flux_energy);
      }


      inline DEAL_II_ALWAYS_INLINE //
        std::tuple<scalar, vector, scalar>
        get_gradient_flux_boundary(
          FaceIntegratorScalar             &density,
          FaceIntegratorVector             &momentum,
          FaceIntegratorScalar             &energy,
          scalar const                     &tau_IP,
          ...
          unsigned int const                q) const
      {
        ...
        vector tau_P_normal = calculate_exterior_normal_grad<dim, Number, 1>(
          tau_M * normal,
          boundary_type_velocity,
          data.bc->velocity,
          boundary_id,
        ...
        scalar grad_T_P_normal = calculate_exterior_normal_grad<dim, Number, 0>(
          grad_T_M_normal,
          boundary_type_energy,
          data.bc->energy,
          boundary_id,
        ...
        scalar jump_energy = rho_E_M - rho_E_P;
        scalar gradient_flux_energy =
          0.5 * (u_M * tau_M * normal + u_P * tau_P_normal +
                       ...
                       unsigned int const    q) const
      {
        ...
        // value flux momentum
        tensor grad_u_using_jumps_M =
          calculate_grad_u(rho_inv_M,
                           rho_u_M,
                           jump_rho /*instead of grad_rho*/,
                           jump_rho_u /*instead of grad_rho_u*/);

        tensor tau_using_jumps_M =
          calculate_stress_tensor(grad_u_using_jumps_M, mu);
        tensor value_flux_momentum_M = -0.5 * tau_using_jumps_M;

        tensor grad_u_using_jumps_P =
          calculate_grad_u(rho_inv_P,
                           rho_u_P,
                           jump_rho /*instead of grad_rho*/,
                           jump_rho_u /*instead of grad_rho_u*/);

        tensor tau_using_jumps_P =
          calculate_stress_tensor(grad_u_using_jumps_P, mu);
        tensor value_flux_momentum_P = -0.5 * tau_using_jumps_P;

        // value flux energy
        vector grad_E_using_jumps_M =
          calculate_grad_E(rho_inv_M,
                           rho_E_M,
                           jump_rho /*instead of grad_rho*/,
                           jump_rho_E /*instead of grad_rho_E*/);

        vector grad_T_using_jumps_M = calculate_grad_T(
          grad_E_using_jumps_M, u_M, grad_u_using_jumps_M, gamma, R);
        vector value_flux_energy_M =
          -0.5 * (tau_using_jumps_M * u_M + lambda * grad_T_using_jumps_M);

        vector grad_E_using_jumps_P =
          calculate_grad_E(rho_inv_P,
                           rho_E_P,
                           jump_rho /*instead of grad_rho*/,
                           jump_rho_E /*instead of grad_rho_E*/);

        vector grad_T_using_jumps_P = calculate_grad_T(
          grad_E_using_jumps_P, u_P, grad_u_using_jumps_P, gamma, R);
        vector value_flux_energy_P =
            ...
            for (unsigned int q = 0; q < density_m.n_q_points; ++q)
              {
                ...
                std::tuple<vector, tensor, vector, vector, tensor, vector>
                  value_flux = get_value_flux(density_m,
                                              density_p,
                                              momentum_m,
                                              momentum_p,
                                              energy_m,
        ...
        for (unsigned int face = face_range.first; face < face_range.second;
             face++)
          {
            ...
            momentum.integrate_scatter(dealii::EvaluationFlags::values |
                                         dealii::EvaluationFlags::gradients,
                                       dst);
            energy.integrate_scatter(dealii::EvaluationFlags::values |
                                       dealii::EvaluationFlags::gradients,
                                     dst);
          }
      }
            ...
            for (unsigned int q = 0; q < momentum.n_q_points; ++q)
              {
                std::tuple<vector, tensor, vector> conv_flux =
                  convective_operator->get_volume_flux(density,
                                                       momentum,
                                                       energy,
                                                       q);

                std::tuple<vector, tensor, vector> visc_flux =
                  viscous_operator->get_volume_flux(density,
                                                    momentum,
                                                    energy,
                                                    q);

                density.submit_gradient(-std::get<0>(conv_flux), q);
                momentum.submit_gradient(-std::get<1>(conv_flux) +
                                           std::get<1>(visc_flux),
                                         q);
                energy.submit_gradient(-std::get<2>(conv_flux) +
                                         std::get<2>(visc_flux),
                                       q);
              }
                ...
                std::pair<unsigned int, unsigned int> const &face_range) const
      {
        ...
        for (unsigned int face = face_range.first; face < face_range.second;
             face++)
          {
            ...
            scalar tau_IP =
              viscous_operator->get_penalty_parameter(density_m, density_p);

            for (unsigned int q = 0; q < density_m.n_q_points; ++q)
              {
                std::tuple<scalar, vector, scalar> conv_flux =
                  convective_operator->get_flux(density_m,
                                                density_p,
                                                momentum_m,
                                                momentum_p,
                                                energy_m,
                ...
                std::tuple<scalar, vector, scalar> visc_grad_flux =
                  viscous_operator->get_gradient_flux(density_m,
                                                      density_p,
                                                      momentum_m,
                                                      momentum_p,
                                                      energy_m,
                ...
                std::tuple<vector, tensor, vector, vector, tensor, vector>
                  visc_value_flux = viscous_operator->get_value_flux(density_m,
                                                                     density_p,
                                                                     momentum_m,
                                                                     momentum_p,
                                                                     energy_m,
                ...
                density_m.submit_value(std::get<0>(conv_flux) -
                                         std::get<0>(visc_grad_flux),
                                       q);
                // - sign since n⁺ = -n⁻
                density_p.submit_value(-std::get<0>(conv_flux) +
                                         std::get<0>(visc_grad_flux),
                                       q);

                momentum_m.submit_value(std::get<1>(conv_flux) -
                                          std::get<1>(visc_grad_flux),
                                        q);
                // - sign since n⁺ = -n⁻
                momentum_p.submit_value(-std::get<1>(conv_flux) +
                                          std::get<1>(visc_grad_flux),
                                        q);
                ...
                energy_m.submit_value(std::get<2>(conv_flux) -
                                        std::get<2>(visc_grad_flux),
                                      q);
                // - sign since n⁺ = -n⁻
                energy_p.submit_value(-std::get<2>(conv_flux) +
                                        std::get<2>(visc_grad_flux),
                                      q);
            ...
            for (unsigned int q = 0; q < density.n_q_points; ++q)
              {
                ...
                std::tuple<scalar, vector, scalar> visc_grad_flux =
                  viscous_operator->get_gradient_flux_boundary(
                    density,
                    momentum,
                    energy,
                    tau_IP,
                ...
                std::tuple<vector, tensor, vector> visc_value_flux =
                  viscous_operator->get_value_flux_boundary(
                    density,
                    momentum,
                    energy,
                    boundary_type_density,
                ...
                density.submit_value(std::get<0>(conv_flux) -
                                       std::get<0>(visc_grad_flux),
                                     q);

                momentum.submit_value(std::get<1>(conv_flux) -
                                        std::get<1>(visc_grad_flux),
                                      q);
                momentum.submit_gradient(std::get<1>(visc_value_flux), q);

                energy.submit_value(std::get<2>(conv_flux) -
                                      std::get<2>(visc_grad_flux),
                                    q);
