...
namespace ExaDG
{
  namespace CompNS
  {
    ...
    PostProcessor<dim, Number>::setup(Operator<dim, Number> const &pde_operator)
    {
      ...
      pressure_difference_calculator.setup(
        pde_operator.get_dof_handler_scalar(),
        pde_operator.get_mapping(),
      ...
      types::time_step const time_step_number)
    {
      ...
      if (pointwise_output_generator.time_control.needs_evaluation(
        ...
        {
          pointwise_output_generator.evaluate(
      ...
      if (pressure_difference_calculator.time_control.needs_evaluation(
            time, time_step_number))
        pressure_difference_calculator.evaluate(pressure.evaluate_get(solution),
                                                time);
      ...
      if (kinetic_energy_spectrum_calculator.time_control.needs_evaluation(
            time, time_step_number))
        {
          kinetic_energy_spectrum_calculator.evaluate(
            velocity.evaluate_get(solution),
            time,
    ...
    {
      if (pp_data.output_data.write_pressure or
      ...
      // velocity
      if (pp_data.output_data.write_velocity or
          pp_data.output_data.write_vorticity or
          pp_data.output_data.write_divergence or
          pp_data.output_data.write_shear_rate or
          pp_data.lift_and_drag_data.time_control_data.is_active or
      ...
      if (pp_data.output_data.write_temperature)
        {
          ...
          temperature.recompute_solution_field = [&](VectorType       &dst,
                                                     VectorType const &src) {
