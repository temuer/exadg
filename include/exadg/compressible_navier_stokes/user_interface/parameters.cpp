  ...
  namespace CompNS
  {
    ...
    Parameters::check() const
    {
      ...
      if (temporal_discretization == TemporalDiscretization::SSPRK)
        {
          AssertThrow(stages >= 1,
                      dealii::ExcMessage("Specify number of RK stages!"));
        }

      if (calculation_of_time_step_size == TimeStepCalculation::CFLAndDiffusion)
        {
          AssertThrow(max_velocity >= 0.0,
                      dealii::ExcMessage("Invalid parameter max_velocity."));
          AssertThrow(cfl_number > 0.0,
                      dealii::ExcMessage("parameter must be defined"));
          AssertThrow(diffusion_number > 0.0,
                      dealii::ExcMessage("parameter must be defined"));
        }
      ...
      dealii::ConditionalOStream const &pcout) const
    {
      ...
      // here we do not print quantities such as cfl_number, diffusion_number,
      ...
      dealii::ConditionalOStream const &pcout) const
    {
      ...
      print_parameter(pcout,
                      "Quadrature rule convective term",
                      n_q_points_convective);
      print_parameter(pcout,
                      "Quadrature rule viscous term",
                      n_q_points_viscous);
