...
namespace ExaDG
{
  namespace CompNS
  {
    ...
    public:
      ...
      // reference density needed to calculate the kinematic viscosity from the
      // specified dynamic viscosity
      ...
      // maximum temperature (needed to calculate time step size according to
      // CFL condition)
      ...
      // diffusive term: Symmetric interior penalty Galerkin (SIPG)
      // discretization interior penalty parameter scaling factor: default value
      // is 1.0
      double IP_factor;
      ...
      // detect instabilities (norm of solution vector grows by a large factor
      // from one time step to the next
      bool detect_instabilities;

      // use combined operator for viscous term and convective term in order to
      // improve run time
