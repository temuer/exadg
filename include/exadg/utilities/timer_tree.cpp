...
namespace ExaDG
{
    ...
    if (this->id == "") // the tree is currently empty
      {
        AssertThrow(sub_trees.empty(),
                    dealii::ExcMessage("invalid state found. aborting."));
      ...
      {
        AssertThrow(false,
                    dealii::ExcMessage("The name provided is " + ids[0] +
  ...
  void
  TimerTree::print_level(dealii::ConditionalOStream const &pcout,
                         unsigned int const                level) const
  {
    ...
    if (level <= max_level)
      ...
      {
        pcout << std::endl
              << "Timings can not be printed for level = " << level << ","
              << std::endl
              << "since the maximum level of the timer tree is max_level = "
              << max_level << "." << std::endl;
      }
  }
