...
namespace ExaDG
{
  ...
  print_throughput(
    std::vector<
      std::tuple<unsigned int, dealii::types::global_dof_index, double>> const
                      &wall_times,
...

  {
    double const N_iter_linear_avg =
      (N_iter_nonlinear > 0) ?
        double(N_iter_linear) / double(N_iter_nonlinear) :
        N_iter_linear;
  ...
  inline void
  print_wall_time(dealii::ConditionalOStream const &pcout,
                  double const                      wall_time)
                           ...
                           std::vector<double> const        &iterations_avg)
  {
    ...
    for (unsigned int i = 0; i < iterations_avg.size(); ++i)
      {
        pcout << "  " << std::setw(length + 2) << std::left << names[i]
              << std::fixed << std::setprecision(2) << std::right
              << std::setw(6) << iterations_avg[i] << std::endl;
      }
  }
  ...
  struct SolverResult
  {
    ...
    print(dealii::ConditionalOStream const &pcout) const
    {
      ...
      pcout << std::setw(15) << std::scientific << std::setprecision(2)
            << tau_10;
      pcout << std::setw(15) << std::scientific << std::setprecision(2)
            << 1.0 / tau_10;
      pcout << std::endl;
  ...
  inline void
  print_results(std::vector<SolverResult> const &results,
                MPI_Comm const                  &mpi_comm)
  {
    ...
    for (std::vector<SolverResult>::const_iterator it = results.begin();
         it != results.end();
         ++it)
      it->print(pcout);
