    ...
    private:
                    ...
                    &multigrid_mappings) final
      {
        auto const lambda_create_triangulation = [&](
          ...
          dealii::GridGenerator::hyper_ball(tria_2d,
                                            dealii::Point<2>(),
                                            R_INNER);
              ...
              for (auto const &f : cell->face_indices())
                {
                  ...
                  AssertThrow(
                    BOUNDARY_ID_FSI == 0,
                    dealii::ExcMessage(
                      "Boundary ID of fluid-structure interface is invalid."));
                }
            }
                      ...
                      for (auto const &v : cell->face(f)->vertex_indices())
                        {
                          dealii::Point<dim> point =
                            dealii::Point<dim>(cell->face(f)->vertex(v)[0],
                                               cell->face(f)->vertex(v)[1],
                                               0);
      ...
      set_boundary_descriptor_ale_poisson() final
      {
        ...
        boundary_descriptor->dirichlet_bc_component_mask.insert(
          pair_mask(BOUNDARY_ID_INFLOW, mask));
        ...
        boundary_descriptor->dirichlet_bc.insert(
          pair(BOUNDARY_ID_OUTFLOW,
               new dealii::Functions::ZeroFunction<dim>(dim)));
        boundary_descriptor->dirichlet_bc_component_mask.insert(
          pair_mask(BOUNDARY_ID_OUTFLOW, mask));
      ...
      set_material_descriptor_ale_elasticity() final
      {
        ...
        material_descriptor->insert(
          Pair(0,
               new StVenantKirchhoffData<dim>(type, E, poisson, two_dim_type)));
      }
