...
#define TESTCASE 2
  ...
  public:
          ...
          unsigned int const        component = 0) const final
    {
      ...
      if (component == 0)
        result =
          U_X_MAX * (y * (H - y) / std::pow(H / 2.0, 2.0)) *
  ...
  public:
          ...
          unsigned int const        component = 0) const final
    {
      ...
      double const value  = (std::abs(p[1] - H / 2.) < H / 8.) ?
                              std::cos(4.0 * p[1] / H * dealii::numbers::PI) :
                              0.0;
      ...
      create_triangulation(dealii::Triangulation<2> &tria)
      {
        ...
        dealii::GridGenerator::general_cell(
          ...
          {dealii::Point<2>(X_0, 0.0),
           ...
           dealii::Point<2>(X_C - R / std::sqrt(2.0),
                            Y_C + R / std::sqrt(2.0))});
        ...
        dealii::GridGenerator::general_cell(
          ...
          {dealii::Point<2>(X_C + R / std::sqrt(2.0), Y_C - R / std::sqrt(2.0)),
           dealii::Point<2>(X_2, 0.0),
           dealii::Point<2>(X_C + R * std::cos(std::asin(T / (2.0 * R))),
                            Y_C - T / 2.0),
        ...
        std::vector<dealii::Triangulation<2> const *> tria_vec_ptr(
          tria_vec.size());
              ...
              for (auto const &f : cell->face_indices())
                {
                  ...
                  if (std::fabs(cell->face(f)->center().distance(center)) <
                      R + TOL)
                    {
                      cell->face(f)->set_boundary_id(BOUNDARY_ID_CYLINDER);
                    }

                  if (std::fabs(y - (Y_C - T / 2.0)) < TOL or
                      std::fabs(y - (Y_C + T / 2.0)) < TOL or
                      (std::fabs(y - Y_C) < T / 2.0 + TOL and
                       std::fabs(x - (X_C + R + L_FLAG)) < TOL))
                    {
                  ...
                  if (cell->face(f)->at_boundary())
                    {
                      ...
                      for (auto const &v : cell->face(f)->vertex_indices())
                        {
                          if (std::abs(
                                center.distance(cell->face(f)->vertex(v)) - R) >
                              TOL)
                            {
                      ...
                      if (face_at_sphere_boundary)
                        {
                          ...
                          unsigned int manifold_id =
                            manifold_id_start + manifold_ids.size() + 1;
