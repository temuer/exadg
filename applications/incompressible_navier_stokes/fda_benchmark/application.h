...
namespace ExaDG
{
  namespace IncNS
  {
    namespace Precursor
    {
      // set the throat Reynolds number Re_throat = U_{mean,throat} * (2
      // R_throat) / nu
      ...
      double const area_inflow =
        FDANozzle::R_OUTER * FDANozzle::R_OUTER * dealii::numbers::PI;
      double const area_throat =
        FDANozzle::R_INNER * FDANozzle::R_INNER * dealii::numbers::PI;

      double const mean_velocity_throat =
        Re * viscosity / (2.0 * FDANozzle::R_INNER);
      ...
      // use prescribed velocity profile at inflow superimposed by random
      // perturbations (white noise)? If yes, specify amplitude of perturbations
      // relative to maximum velocity on centerline. Can be used with and
      // without precursor approach
      bool const   use_random_perturbations    = false;
      double const factor_random_perturbations = 0.02;

      // set this variable to true in order to switch off the precursor and
      // simulate on the actual domain only
      ...
      // sampling interval should last over (100-200) * T_0 according to
      // preliminary results.
      double const sample_start_time = 50.0 * T_0; // let the flow develop
      double const sample_end_time =
        end_time; // that's the only reasonable choice
      ...
      do_set_parameters(Parameters &param, bool const is_precursor = false)
      {
        ...
        param.turbulence_model_data.turbulence_model =
        ...
        param.solver_data_pressure_poisson =
          SolverData(1000, 1.e-12, 1.e-3, 100);
        ...
        param.order_pressure_extrapolation =
          1; // use 0 for non-incremental formulation
        param.rotational_formulation =
          true; // use false for standard formulation
        ...
        if (param.temporal_discretization ==
            TemporalDiscretization::BDFPressureCorrection)
          {
            ...
            param.newton_solver_data_momentum =
              Newton::SolverData(100, 1.e-12, 1.e-3);
        ...
        param.newton_solver_data_coupled =
          Newton::SolverData(100, 1.e-20, 1.e-3);
        ...
        param.preconditioner_pressure_block = SchurComplementPreconditioner::
          CahouetChabard; // PressureConvectionDiffusion;

        // Chebyshev moother
        param.multigrid_data_pressure_block.smoother_data.smoother =
          MultigridSmoother::Chebyshev;
      ...
      public:
                      ...
                      &multigrid_mappings) final
        {
          auto const lambda_create_triangulation = [&](
            ...
            FDANozzle::create_grid_and_set_boundary_ids_precursor(
              tria, global_refinements, periodic_face_pairs);
          };
        ...
        set_field_functions() final
        {
          ...
          // prescribe body force for the turbulent pipe flow (precursor) to
          // adjust the desired flow rate
          flow_rate_controller.reset(
            new FlowRateController(target_flow_rate,
                                   viscosity,
                                   max_velocity,
                                   FDANozzle::R_OUTER,
          ...
          this->field_functions->right_hand_side.reset(
            new RightHandSide<dim>(*flow_rate_controller));
        }
        ...
        create_postprocessor() final
        {
          ...
          pp_data.output_data.time_control_data.start_time =
            output_start_time_precursor;
          ...
          pp_data.output_data.filename =
            this->output_parameters.filename + "_precursor";
          ...
          // as weak Dirichlet boundary condition at the left boundary of the
          // nozzle domain
          AssertThrow(inflow_data_storage.get(),
                      dealii::ExcMessage(
                        "inflow_data_storage is uninitialized."));
          ...
          pp_data_fda.inflow_data.n_points_z =
            inflow_data_storage->n_points_phi;
          ...
          pp_data_fda.mean_velocity_data.directory =
      ...
      public:
                      ...
                      &multigrid_mappings) final
        {
          auto const lambda_create_triangulation = [&](
            ...
            FDANozzle::create_grid_and_set_boundary_ids_nozzle(
              tria, global_refinements, periodic_face_pairs);
          };
        ...
        set_boundary_descriptor() final
        {
          ...
          // inflow boundary condition at left boundary with ID=1: prescribe
          // velocity profile which is obtained as the results of the simulation
          // on the precursor domain
          AssertThrow(inflow_data_storage.get(),
                      dealii::ExcMessage(
                        "inflow_data_storage is not initialized."));
          ...
          // outflow boundary condition at right boundary with ID=2: set
          // pressure to zero
        ...
        create_postprocessor() final
        {
          ...
          pp_data.output_data.time_control_data.is_active =
            this->output_parameters.write;
          pp_data.output_data.time_control_data.start_time =
            output_start_time_nozzle;
          pp_data.output_data.time_control_data.trigger_interval =
            output_interval_time;
          pp_data.output_data.directory =
            this->output_parameters.directory + "vtu/";
          pp_data.output_data.filename =
            this->output_parameters.filename + "_nozzle";
          pp_data.output_data.write_divergence         = true;
          pp_data.output_data.write_processor_id       = true;
          pp_data.output_data.mean_velocity.is_active  = true;
          pp_data.output_data.mean_velocity.start_time = sample_start_time;
          ...
          pp_data_fda.line_plot_data.time_control_data_statistics
            .time_control_data.is_active = true;
          pp_data_fda.line_plot_data.time_control_data_statistics
            .time_control_data.start_time = sample_start_time;
          pp_data_fda.line_plot_data.time_control_data_statistics
            .time_control_data.end_time = end_time;
          pp_data_fda.line_plot_data.time_control_data_statistics
            .time_control_data.trigger_every_time_steps =
            sample_every_timesteps;
          pp_data_fda.line_plot_data.time_control_data_statistics
            .write_preliminary_results_every_nth_time_step =
            sample_every_timesteps * 100;
          pp_data_fda.line_plot_data.directory =
            this->output_parameters.directory;

          // lines
          std::shared_ptr<LineCircumferentialAveraging<dim>> axial_profile,
            radial_profile_z1, radial_profile_z2, radial_profile_z3,
            radial_profile_z4, radial_profile_z5, radial_profile_z6,
            radial_profile_z7, radial_profile_z8, radial_profile_z9,
          ...
          double z_1 = -0.088, z_2 = -0.064, z_3 = -0.048, z_4 = -0.02,
                 z_5 = -0.008, z_6 = 0.0, z_7 = 0.008, z_8 = 0.016, z_9 = 0.024,
                 z_10 = 0.032, z_11 = 0.06, z_12 = 0.08;
          ...
          radial_profile_z1->end =
            dealii::Point<dim>(FDANozzle::radius_function(z_1), 0, z_1);
          radial_profile_z2->begin = dealii::Point<dim>(0, 0, z_2);
          radial_profile_z2->end =
            dealii::Point<dim>(FDANozzle::radius_function(z_2), 0, z_2);
          radial_profile_z3->begin = dealii::Point<dim>(0, 0, z_3);
          radial_profile_z3->end =
            dealii::Point<dim>(FDANozzle::radius_function(z_3), 0, z_3);
          radial_profile_z4->begin = dealii::Point<dim>(0, 0, z_4);
          radial_profile_z4->end =
            dealii::Point<dim>(FDANozzle::radius_function(z_4), 0, z_4);
          radial_profile_z5->begin = dealii::Point<dim>(0, 0, z_5);
          radial_profile_z5->end =
            dealii::Point<dim>(FDANozzle::radius_function(z_5), 0, z_5);
          radial_profile_z6->begin = dealii::Point<dim>(0, 0, z_6);
          radial_profile_z6->end =
            dealii::Point<dim>(FDANozzle::radius_function(z_6), 0, z_6);
          radial_profile_z7->begin = dealii::Point<dim>(0, 0, z_7);
          radial_profile_z7->end =
            dealii::Point<dim>(FDANozzle::radius_function(z_7), 0, z_7);
          radial_profile_z8->begin = dealii::Point<dim>(0, 0, z_8);
          radial_profile_z8->end =
            dealii::Point<dim>(FDANozzle::radius_function(z_8), 0, z_8);
          radial_profile_z9->begin = dealii::Point<dim>(0, 0, z_9);
          radial_profile_z9->end =
            dealii::Point<dim>(FDANozzle::radius_function(z_9), 0, z_9);
          radial_profile_z10->begin = dealii::Point<dim>(0, 0, z_10);
          radial_profile_z10->end =
            dealii::Point<dim>(FDANozzle::radius_function(z_10), 0, z_10);
          radial_profile_z11->begin = dealii::Point<dim>(0, 0, z_11);
          radial_profile_z11->end =
            dealii::Point<dim>(FDANozzle::radius_function(z_11), 0, z_11);
          radial_profile_z12->begin = dealii::Point<dim>(0, 0, z_12);
          radial_profile_z12->end =
            dealii::Point<dim>(FDANozzle::radius_function(z_12), 0, z_12);
          ...
          radial_profile_z1->n_points_circumferential =
            n_points_line_circumferential;
          radial_profile_z2->n_points_circumferential =
            n_points_line_circumferential;
          radial_profile_z3->n_points_circumferential =
            n_points_line_circumferential;
          radial_profile_z4->n_points_circumferential =
            n_points_line_circumferential;
          radial_profile_z5->n_points_circumferential =
            n_points_line_circumferential;
          radial_profile_z6->n_points_circumferential =
            n_points_line_circumferential;
          radial_profile_z7->n_points_circumferential =
            n_points_line_circumferential;
          radial_profile_z8->n_points_circumferential =
            n_points_line_circumferential;
          radial_profile_z9->n_points_circumferential =
            n_points_line_circumferential;
          radial_profile_z10->n_points_circumferential =
            n_points_line_circumferential;
          radial_profile_z11->n_points_circumferential =
            n_points_line_circumferential;
          radial_profile_z12->n_points_circumferential =
            n_points_line_circumferential;
      ...
      public:
          ...
          : Precursor::ApplicationBase<dim, Number>(input_file, comm)
        {
          ...
          // compute number of points for inflow data array depending on spatial
          ...
          unsigned int const n_points =
            20 * (resolution_main.degree + 1) *
            dealii::Utilities::pow(2, resolution_main.refine_space);

          inflow_data_storage.reset(
            new InflowDataStorage<dim>(n_points,
                                       FDANozzle::R_OUTER,
                                       max_velocity,
                                       use_random_perturbations,
          ...
          this->precursor =
            std::make_shared<PrecursorDomain<dim, Number>>(input_file,
                                                           comm,
                                                           inflow_data_storage);
          this->main = std::make_shared<MainDomain<dim, Number>>(
            input_file, comm, inflow_data_storage, this->precursor_is_active());
        }
