  ...
  namespace IncNS
  {
        ...
        if (component == 2)
          {
            ...
            double const max_velocity_z =
              max_velocity * std::pow(FDANozzle::R_OUTER / R, 2.0);
            ...
            // (otherwise, the perturbations will be damped and the flow becomes
            // laminar). According to first numerical results, the perturbed
            // flow returns to a laminar steady state in the precursor domain
            // for Reynolds numbers Re_t = 500, 2000, 3500, 5000, and 6500.
            if (p[2] <= FDANozzle::Z2_PRECURSOR)
              {
                ...
                double perturbation =
                  factor * max_velocity_z * std::sin(4.0 * phi) *
                    std::sin(8.0 * dealii::numbers::PI * p[2] /
                             FDANozzle::LENGTH_PRECURSOR) +
                  factor * max_velocity_z * ((double)rand() / RAND_MAX - 0.5) /
                    0.5;

                // the perturbations should fulfill the Dirichlet boundary
                // conditions
    ...
    public:
            ...
            unsigned int const        component = 0) const final
      {
        ...
        double const result =
          linear_interpolation_2d_cylindrical(r,
                                              phi,
                                              data.r_values,
                                              data.phi_values,
    ...
    /*
     *  Right-hand side function: Implements the body force vector occurring on
     * the right-hand side of the momentum equation of the Navier-Stokes
     * equations. Only relevant for precursor simulation.
     */
    template <int dim>
