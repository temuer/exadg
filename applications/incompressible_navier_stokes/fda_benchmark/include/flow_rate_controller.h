  ...
  namespace IncNS
  {
    ...
    public:
      FlowRateController(double const target_flow_rate,
        ...
        // under the assumption of a parabolic velocity profile in radial
        // direction
        f(4.0 * viscosity * max_velocity / std::pow(R_outer, 2.0))
        , // f(t=t_0) = f_0
      ...
      update_body_force(double const flow_rate_, double const time_)
      {
        ...
        // use an I-controller to asymptotically reach the desired target flow
        // rate

        // dimensional analysis: [k] = 1/(m^2 s^2) -> k = const *
        // U_{mean,inflow}^2 / D^4 constant: choose a default value of 1
        double const k =
          1.0e0 * std::pow(mean_velocity_inflow, 2.0) / std::pow(D, 4.0);
