...
namespace ExaDG
{
  namespace FDANozzle
  {
    ...
    double const LENGTH_CONE =
      (R_OUTER - R_INNER) / std::tan(20.0 / 2.0 * dealii::numbers::PI / 180.0);
    ...
    double const Z2_PRECURSOR =
      -LENGTH_THROAT - LENGTH_CONE - LENGTH_INFLOW - OFFSET;
    ...
    radius_function(double const z)
    {
      ...
      if (z >= Z1_INFLOW and z <= Z2_INFLOW)
        ...
        radius = R_OUTER * (1.0 - (z - Z1_CONE) / (Z2_CONE - Z1_CONE) *
        ...
        & /*periodic_faces*/)
    {
      ...
      dealii::GridGenerator::hyper_ball(tria_2d_inflow,
                                        dealii::Point<2>(),
                                        R_OUTER);
      ...
      dealii::GridGenerator::hyper_ball(tria_2d_cone,
                                        dealii::Point<2>(),
                                        R_OUTER);
      ...
      // apply conical geometry: stretch vertex positions according to
      // z-coordinate
              ...
              if (cell->vertex(v)[2] > Z1_CONE + 1.e-10)
                {
                  ...
                  // note that this value is only valid for the current dealii
                  // implementation of hyper_ball!
                  if (std::abs((cell->vertex(v) - point_2d).norm() -
                               2.485281374239e-03 / 6.0e-3 * R_OUTER) <
                        1.e-10 or
                      std::abs((cell->vertex(v) - point_2d).norm() - R_OUTER) <
                        1.e-10)
                    {
                      cell->vertex(v)[0] *=
                        1.0 - (cell->vertex(v)[2] - Z1_CONE) /
                                (Z2_CONE - Z1_CONE) * (R_OUTER - R_INNER) /
                                R_OUTER;
                      cell->vertex(v)[1] *=
                        1.0 - (cell->vertex(v)[2] - Z1_CONE) /
                                (Z2_CONE - Z1_CONE) * (R_OUTER - R_INNER) /
      ...
      dealii::GridGenerator::hyper_ball(tria_2d_throat,
                                        dealii::Point<2>(),
                                        R_INNER);
      ...
      dealii::Triangulation<2> tria_2d_outflow_inner, circle_1, circle_2,
        circle_3, tria_tmp_2d_1, tria_tmp_2d_2, tria_2d_outflow;
      dealii::GridGenerator::hyper_ball(tria_2d_outflow_inner,
                                        dealii::Point<2>(),
                                        R_INNER);
      ...
      dealii::GridGenerator::hyper_shell(
        circle_2, dealii::Point<2>(), R_1, R_2, n_cells_circle, true);
      ...
      dealii::GridGenerator::merge_triangulations(tria_2d_outflow_inner,
                                                  circle_1,
                                                  tria_tmp_2d_1);
      dealii::GridGenerator::merge_triangulations(circle_2,
                                                  circle_3,
                                                  tria_tmp_2d_2);
      dealii::GridGenerator::merge_triangulations(tria_tmp_2d_1,
                                                  tria_tmp_2d_2,
                                                  tria_2d_outflow);
      ...
      dealii::GridGenerator::merge_triangulations(tria_inflow,
                                                  tria_cone,
                                                  tria_tmp);
      dealii::GridGenerator::merge_triangulations(tria_tmp,
                                                  tria_throat,
                                                  tria_tmp2);
      dealii::GridGenerator::merge_triangulations(tria_tmp2,
                                                  tria_outflow,
                                                  triangulation);
      ...
      for (auto cell : current_tria->cell_iterators())
        {
                      ...
                      for (auto const &v : cell->face(f)->vertex_indices())
                        {
                          ...
                          if (std::abs(
                                (cell->face(f)->vertex(v) - point).norm() -
                                R_OUTER) > 1e-12)
                            {
                      ...
                      for (auto const &v : cell->face(f)->vertex_indices())
                        {
                          ...
                          dealii::Point<dim> point =
                            dealii::Point<dim>(0, 0, z);
                          if (std::abs(
                                (cell->face(f)->vertex(v) - point).norm() -
                                radius_function(z)) > 1e-12)
                            {
                              face_at_sphere_boundary = false;
                              break;
                            }
                      ...
                      if (face_at_sphere_boundary)
                        {
                          ...
                          unsigned int manifold_id = MANIFOLD_ID_OFFSET_CONE +
                                                     manifold_ids_cone.size() +
                                                     1;
                          cell->set_all_manifold_ids(manifold_id);
          ...
          // THROAT
          else if (cell->center()[2] > Z1_THROAT and
                   cell->center()[2] < Z2_THROAT)
            {
              for (auto const &f : cell->face_indices())
                {
                  if (cell->face(f)->at_boundary())
          ...
          // OUTFLOW
          else if (cell->center()[2] > Z1_OUTFLOW and
                   cell->center()[2] < Z2_OUTFLOW)
            {
              dealii::Point<dim> point2 =
                dealii::Point<dim>(0, 0, cell->center()[2]);
                      ...
                      for (auto const &v : cell->face(f)->vertex_indices())
                        {
                          ...
                          if (std::abs(
                                (cell->face(f)->vertex(v) - point).norm() -
                                R_INNER) > 1e-12 or
                              (cell->center() - point2).norm() >
                                R_INNER / std::sqrt(2.0))
                            {
                              face_at_sphere_boundary = false;
                              break;
                            }
      ...
      // generate vector of manifolds and apply manifold to all cells that have
      // been marked
                  ...
                  manifold_vec[i] = std::shared_ptr<dealii::Manifold<dim>>(
                    static_cast<dealii::Manifold<dim> *>(
                      new OneSidedCylindricalManifold<dim>(
                        *current_tria, cell, face_ids[i], center)));
                  current_tria->set_manifold(manifold_ids[i],
                                             *(manifold_vec[i]));
                }
            }
        }
      ...
      // conical manifold
      static std::vector<std::shared_ptr<dealii::Manifold<dim>>>
        manifold_vec_cone;
              ...
              if (cell->manifold_id() == manifold_ids_cone[i])
                {
                  ...
                  manifold_vec_cone[i] = std::shared_ptr<dealii::Manifold<dim>>(
                    static_cast<dealii::Manifold<dim> *>(
                      new OneSidedConicalManifold<dim>(*current_tria,
                                                       cell,
                                                       face_ids_cone[i],
                                                       center,
                                                       radius_0_cone[i],
                                                       radius_1_cone[i])));
                  current_tria->set_manifold(manifold_ids_cone[i],
                                             *(manifold_vec_cone[i]));
                }
            }
        }
        ...
        &periodic_face_pairs)
    {
          ...
          for (auto const &f : cell->face_indices())
            {
              ...
              for (auto const &v : cell->face(f)->vertex_indices())
                {
                  dealii::Point<dim> point =
                    dealii::Point<dim>(0, 0, cell->face(f)->vertex(v)[2]);

                  if (std::abs((cell->face(f)->vertex(v) - point).norm() -
                               R_OUTER) > 1e-12)
                    face_at_sphere_boundary = false;
                }
              if (face_at_sphere_boundary)
      ...
      for (auto cell : tria.cell_iterators())
        {
          for (auto const &face : cell->face_indices())
            {
              // left boundary
              if ((std::fabs(cell->face(face)->center()[2] - Z1_PRECURSOR) <
                   1e-12))
                {
              ...
              // right boundary
              if ((std::fabs(cell->face(face)->center()[2] - Z2_PRECURSOR) <
                   1e-12))
                {
      ...
      dealii::GridTools::collect_periodic_faces(
        tria, 0 + 10, 1 + 10, 2, periodic_face_pairs);
