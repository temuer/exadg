...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    struct InflowDataStorage
    {
      ...
      initialize_r_and_phi_values()
      {
        AssertThrow(n_points_r >= 2,
                    dealii::ExcMessage("Variable n_points_r is invalid"));
        AssertThrow(n_points_phi >= 2,
                    dealii::ExcMessage("Variable n_points_phi is invalid"));
        ...
        for (unsigned int i = 0; i < n_points_phi; ++i)
          phi_values[i] = -dealii::numbers::PI + double(i) /
                                                   double(n_points_phi - 1) *
            ...
            for (unsigned int iz = 0; iz < n_points_phi; ++iz)
              {
                ...
                // flow in z-direction
                velocity[2] =
                  max_velocity * (1.0 - std::pow(r_values[iy] / R, 2.0));

                if (use_random_perturbations == true)
                  {
                    // Add random perturbation
                    double perturbation =
                      factor_random_perturbations * velocity[2] *
        ...
        for (unsigned int iy = 0; iy < n_points_r; ++iy)
          {
            for (unsigned int iz = 0; iz < n_points_phi; ++iz)
              {
                // Add random perturbation
                double perturbation = factor_random_perturbations *
                                      ((double)rand() / RAND_MAX - 0.5) / 0.5;
