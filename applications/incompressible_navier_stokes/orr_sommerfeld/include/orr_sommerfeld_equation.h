 ...
 */
                      ...
                      dealii::FE_DGQ<dim>               &fe)
  {
    ...
    dealii::Table<2, std::complex<double>> A(dof_handler.n_dofs(),
                                             dof_handler.n_dofs());
    dealii::Table<2, std::complex<double>> B(dof_handler.n_dofs(),
                                             dof_handler.n_dofs());
            ...
            for (unsigned int q = 0; q < n_q_points; ++q)
              {
                ...
                // 1/Re * [(phi'', psi'') + 2*alpha^2*(phi',psi') +
                // alpha^4*(phi,psi)]
                ...
                sum_a_imag -=
                  (phi * alpha *
                   (2 * psi + (1 - y * y) * (ddpsi - alpha * alpha * psi))) *
                  fe_values.JxW(q);
    ...
    */
    ...
    auto eliminate_constraints =
      [](dealii::Table<2, std::complex<double>> const &M,
         unsigned int const                            m,
         dealii::LAPACKFullMatrix<double> const       &constraints,
      ...
      for (unsigned int i = 0; i < m - 4; ++i)
        {
          for (unsigned int j = 0; j < m - 4; ++j)
            {
              MM(i, j) =
                M(i + 2, j + 2) + constraints(2, j) * M(i + 2, 1) +
                constraints(3, j) * M(i + 2, m - 2) +
                constraints(2, i) * M(1, j + 2) +
        ...
        if (eigval.real() > 0)
          {
            ...
            // output
            // pcout << eigval <<  "  [" << eigval_alpha[i] << " " <<
            // eigval_beta[i] << "]   " << std::endl;
            pcout << std::scientific << std::setprecision(precision) << eigval
                  << std::endl;
    ...
    if (positive_i != -1)
      {
        ...
        for (int i=0; i<=n_output; ++i)
        {
          ...
          for (int j=0; j<m; ++j)
            evaluated += eigenvector[j] *
        fe.shape_value(j,dealii::Point<dim>(y_unit));
        ...
        */
        ...
        for (int i=0; i<m; ++i)
          derivative += eigenvector[i] *
        fe.shape_grad(i,dealii::Point<dim>(0.))[0]; pcout << "Correctness of
        constraint: " << derivative << std::endl;
        */
      }
  }
