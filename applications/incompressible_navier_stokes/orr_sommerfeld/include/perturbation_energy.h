    ...
    private:
      ...
      do_evaluate(VectorType const &velocity, double const time)
      {
        ...
        unsigned int l = matrix_free->get_dof_handler(dof_index)
            ...
            std::string filename = data.directory + data.filename + "_l" +
                                   dealii::Utilities::int_to_string(l);

            std::ofstream f;
            if (clear_files == true)
              {
                f.open(filename.c_str(), std::ios::trunc);
                f << "Perturbation energy: E = (1,(u-u_base)^2)_Omega"
                  << std::endl
                  << "Error:               e = |exp(2*omega_i*t) - E(t)/E(0)|"
                  << std::endl;
            ...
            Number const rel =
              perturbation_energy / initial_perturbation_energy;
            Number const error =
              std::abs(std::exp(2 * data.omega_i * time) - rel);

            unsigned int const precision = 12;
            f << std::scientific << std::setprecision(precision)
              << std::setw(precision + 8) << time << std::setw(precision + 8)
              << perturbation_energy << std::setw(precision + 8) << error
              << std::endl;
          }
      }
      ...
      {
        CellIntegrator<dim, dim, Number> integrator(matrix_free,
                                                    dof_index,
                                                    quad_index);
        ...
        for (unsigned int cell = cell_range.first; cell < cell_range.second;
             ++cell)
          {
            ...
            dealii::VectorizedArray<Number> energy_vec =
            ...
            for (unsigned int q = 0; q < integrator.n_q_points; ++q)
              {
                ...
                energy_vec += integrator.JxW(q) * (velocity - velocity_base) *
