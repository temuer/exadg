...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    public:
            ...
            unsigned int const        component = 0) const final
      {
        ...
        Assert(eta <= 1.0 + 1.e-12 and eta >= 0.0 - 1.e-12,
               dealii::ExcMessage(
                 "Point in reference coordinates is invalid."));
        ...
        if (component == 0)
          {
            ...
            // multiply by d(eta)/dy to obtain derivative d(psi)/dy in physical
            // space
            dpsi *= 0.5 / H;

            std::complex<double> perturbation_complex =
              dpsi * exp * amplification;
        ...
        else
          {
            Assert(false,
                   dealii::ExcMessage(
                     "Orr-Sommerfeld problem is a 2-dimensional problem."));
          }
    ...
    /*
     *  Right-hand side function: Implements the body force vector occuring on
     * the right-hand side of the momentum equation of the Navier-Stokes
     * equations
     */
    template <int dim>
    ...
    public:
            ...
            unsigned int const component = 0) const final
      {
        ...
        //   f * L * 2H = tau * 2 * L (2H = height, L = length, factor 2 = upper
        //   and lower wall)
        // with tau = nu du/dy|_{y=-H} = nu * U_max * (-2y/H^2)|_{y=-H} = 2 * nu
        // * U_max / H
        if (component == 0)
    ...
    private:
      ...
      set_parameters() final
      {
        ...
        this->param.right_hand_side =
          true; // prescribe body force in x-direction
        ...
        this->param.solver_data_pressure_poisson =
          SolverData(1000, 1.e-14, 1.e-14, 100);
        ...
        if (this->param.temporal_discretization ==
            TemporalDiscretization::BDFPressureCorrection)
          {
            ...
            this->param.newton_solver_data_momentum =
              Newton::SolverData(100, 1.e-14, 1.e-14);

            // linear solver
            this->param.solver_momentum = SolverMomentum::GMRES;
            this->param.solver_data_momentum =
              SolverData(1e4, 1.e-14, 1.e-14, 100);
            this->param.preconditioner_momentum =
              MomentumPreconditioner::InverseMassMatrix;
          }
        ...
        this->boundary_descriptor->velocity->dirichlet_bc.insert(
          pair(0,
               new AnalyticalSolutionVelocity<dim>(
                 H, MAX_VELOCITY, ALPHA, EPSILON, FE, OMEGA, EIG_VEC)));
      ...
      create_postprocessor() final
      {
        ...
        pp_data.output_data.filename = this->output_parameters.filename + "_k" +
        ...
        pp_data_os.energy_data.filename  = this->output_parameters.filename +
                                          "_perturbation_energy" + "_k" +
                                          std::to_string(this->param.degree_u);
      ...
      double const ALPHA        = 1.0;
      double const EPSILON =
        1.0e-5; // perturbations are small (<< 1, linearization)
      ...
      // eigenvector (EIG_VEC). do not use more than 300 due to conditioning of
      // polynomials
      ...
      // the time the Tollmien-Schlichting-waves need to travel through the
      // domain (depends on solution of Orr-Sommerfeld equation)
      double       t0         = 0.0;
