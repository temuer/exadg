...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    {
    public:
      AnalyticalSolutionVelocity(double const lambda)
            ...
            unsigned int const        component = 0) const final
      {
        ...
        if (component == 0)
          ...
          result = lambda / 2.0 / pi * std::exp(lambda * p[0]) *
                   std::sin(2 * pi * p[1]);
    ...
    {
    public:
      NeumannBoundaryVelocity(FormulationViscousTerm const &formulation_viscous,
                              double const                  lambda)
        : dealii::Function<dim>(dim, 0.0)
        ...
        if (formulation_viscous == FormulationViscousTerm::LaplaceFormulation)
          {
            if (component == 0)
              ...
              result = std::pow(lambda, 2.0) / 2 / pi * std::exp(lambda) *
                       std::sin(2 * pi * p[1]);
          }
                 ...
                 FormulationViscousTerm::DivergenceFormulation)
          {
            if (component == 0)
              result =
                -2.0 * lambda * std::exp(lambda) * std::cos(2 * pi * p[1]);
            else if (component == 1)
              result = (std::pow(lambda, 2.0) / 2 / pi + 2.0 * pi) *
                       std::exp(lambda) * std::sin(2 * pi * p[1]);
          }
        else
          {
            AssertThrow(
    ...
    private:
        ...
        if (this->param.temporal_discretization ==
            TemporalDiscretization::BDFPressureCorrection)
          {
            ...
            this->param.newton_solver_data_momentum =
              Newton::SolverData(100, 1.e-20, 1.e-6);

            // linear solver
            this->param.solver_momentum = SolverMomentum::GMRES;
            this->param.solver_data_momentum =
              SolverData(1e4, 1.e-20, 1.e-4, 100);
            this->param.preconditioner_momentum =
      ...
      set_boundary_descriptor() final
      {
        ...
        this->boundary_descriptor->velocity->neumann_bc.insert(
          pair(1,
               new NeumannBoundaryVelocity<dim>(formulation_viscous, lambda)));
      ...
      double const lambda =
        0.5 / viscosity - std::pow(0.25 / std::pow(viscosity, 2.0) +
                                     4.0 * std::pow(dealii::numbers::PI, 2.0),
                                   0.5);
