...
namespace ExaDG
{
  namespace FlowPastSphere
  {
    ...
    class SphericalManifoldBoundaryLayer
      : public dealii::Manifold<dim, spacedim>
    {
    public:
      SphericalManifoldBoundaryLayer(double const radius,
      ...
      {
        return std::make_unique<SphericalManifoldBoundaryLayer<dim, spacedim>>(
          radius, radius_next);
      }

      virtual dealii::Point<spacedim>
      get_intermediate_point(dealii::Point<spacedim> const &p1,
                             dealii::Point<spacedim> const &p2,
                             double const                   w) const override
      {
        return push_forward(
          spherical.get_intermediate_point(pull_back(p1), pull_back(p2), w));
      }

      virtual dealii::Point<spacedim>
      get_new_point(
        dealii::ArrayView<const dealii::Point<spacedim>> const &vertices,
        dealii::ArrayView<const double> const &weights) const override
      {
        boost::container::small_vector<dealii::Point<spacedim>, 100>
          pulled_back_vertices;
        ...
        return push_forward(spherical.get_new_point(
          dealii::ArrayView<const dealii::Point<spacedim>>(
      ...
      virtual void
      get_new_points(
        dealii::ArrayView<const dealii::Point<spacedim>> const
                                                  &surrounding_points,
        dealii::Table<2, double> const            &weights,
    ...
    private:
      ...
      push_forward(dealii::Point<spacedim> const &p) const
      {
        ...
        double const r = radius + (R - stretch_factor * radius) /
                                    (radius_next - stretch_factor * radius) *
                                    (R - radius);
        return p * (r / R);
      }
      ...
      pull_back(dealii::Point<spacedim> const &p) const
      {
        ...
        double const r = p.norm();
        double const c = stretch_factor * radius * radius -
                         (r - radius) * (radius_next - stretch_factor * radius);
                       ...
                       TriangulationType const    &triangulation_type)
    {
      ...
      dealii::GridGenerator::hyper_shell(
        tria1, dealii::Point<dim>(), radius, radius_next, 6);
      dealii::GridGenerator::hyper_shell(
      ...
      dealii::GridGenerator::merge_triangulations(
        {&tria1, &tria2, &tria3, &tria4}, tria_ser);

      // Set cells near sphere to spherical manifold for first round of
      // refinement
      ...
      SphericalManifoldBoundaryLayer<dim> const spherical_manifold(radius,
                                                                   radius_next);
      ...
      dealii::GridGenerator::create_triangulation_with_removed_cells(
        tria_rectangle, cells_to_remove, tria_outer);

      dealii::GridGenerator::merge_triangulations({&tria_inner, &tria_outer},
                                                  tria);
          ...
          if (cell->index() < 6 * 8)
            {
              ...
              for (unsigned int v : cell->vertex_indices())
                AssertThrow(cell->vertex(v).norm() < radius_next + 1e-10,
          ...
          if (cell->at_boundary(f))
            {
              ...
              // inflow -> id 1
              else if (std::abs(cell->face(f)->center()[0] + 3. * outer) <
                       1e-10)
                cell->face(f)->set_boundary_id(1);
              // symmetry -> id 0
              else if (2. * outer - std::abs(cell->face(f)->center()[1]) <
                         1e-10 or
                       (dim == 3 and
                        (2. * outer - std::abs(cell->face(f)->center()[2]) <
                         1e-10)))
                cell->face(f)->set_boundary_id(0);
              // outflow -> id 2
              else
                {
                  AssertThrow(std::abs(cell->face(f)->center()[0] -
      ...
      if (n_refinements > 0)
        {
          // In the first round, only refine cells in the inner part of the duct
          // as the outer cells carry little information
          for (auto const &cell : tria.active_cell_iterators())
            ...
            if (center[0] > 0 and center[0] < 4.5 * outer)
              {
                ...
                double const radius_factor = center[0] < 0 ? 2.3 : 2.8;
                center[0]                  = 0;
