  ...
  namespace IncNS
  {
    ...
    public:
                        ...
                        int const    time_step_number)
      {
        ...
        // use an I-controller with damping (D) to asymptotically reach the
        // desired target flow rate

        // dimensional analysis: [k_I] = 1/(m^2 s^2) -> k_I = const * u_b^2 /
        // H^4
        double const C_I = 100.0;
        double const k_I =
          C_I * std::pow(bulk_velocity, 2.0) / std::pow(length_scale, 4.0);
