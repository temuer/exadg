...
namespace ExaDG
{
  ...
  /*
   * This function returns the distance by which points are shifted in
   * y-direction due to the hill, i.e., a value of 0 is returned at x=0H, 9H,
   * and a value of -H at x=4.5H.
   */
  double
  ...
  public:
    ...
    push_forward(dealii::Point<dim> const &xi) const final
    {
      ...
      double       xi_1_hat =
        std::tanh(gamma * (2.0 * xi_1_normalized - 1.0)) / std::tanh(gamma);
      x[1] = H + (xi_1_hat + 1.0) / 2.0 * HEIGHT +
             (1.0 - xi_1_hat) / 2.0 * f(xi[0], H, LENGTH);
    ...
    pull_back(dealii::Point<dim> const &x) const final
    {
      ...
      double const xi_1_hat =
        (2.0 * x[1] - 2.0 * H - HEIGHT - f_x) / (HEIGHT - f_x);
      double const gamma = GRID_STRETCH_FAC;
      xi[1] =
        HEIGHT / 2.0 * (std::atanh(xi_1_hat * std::tanh(gamma)) / gamma + 1) +
        H;
    ...
    clone() const final
    {
      return std::make_unique<PeriodicHillManifold<dim>>(H,
                                                         LENGTH,
                                                         HEIGHT,
                                                         GRID_STRETCH_FAC);
    }
