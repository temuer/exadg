  ...
  namespace IncNS
  {
        ...
        if (component == 0)
          {
            ...
            if (p[1] > H and p[1] < (H + height))
              result = bulk_velocity * (p[1] - H) * ((H + height) - p[1]) /
                       std::pow(height / 2.0, 2.0);
    ...
    public:
      RightHandSide(FlowRateController const &flow_rate_controller)
        : dealii::Function<dim>(dim, 0.0)
        , flow_rate_controller(flow_rate_controller)
      {}
    ...
    public:
      ...
      {
        flow_rate_controller.reset(new FlowRateController(
          bulk_velocity, target_flow_rate, H, start_time));
      }
    ...
    private:
      ...
      parse_parameters() final
      {
        ...
        // viscosity needs to be recomputed since the parameters inviscid, Re
        ...
        sample_start_time =
          double(sample_start_time_multiples) * flow_through_time;

        // sample end time is equal to end time, which is read from the input
        // file
                    ...
                    &multigrid_mappings) final
      {
        auto const lambda_create_triangulation = [&](
          ...
          // use 2 cells in x-direction on coarsest grid and 1 cell in y- and
          // z-directions
          ...
          // create hill by shifting y-coordinates of middle vertices by -H in
          // y-direction
          ...
          // periodicity in x-direction (add 10 to avoid conflicts with
          // dirichlet boundary, which is 0) make use of the fact that the mesh
          ...
          if (dim == 3)
            {
              ...
              dealii::GridTools::collect_periodic_faces(
                tria, 4 + 10, 5 + 10, 2, periodic_face_pairs);
            }
      ...
      create_postprocessor() final
      {
        ...
        std::shared_ptr<LineHomogeneousAveraging<dim>> vel_0, vel_1, vel_2,
          vel_3, vel_4, vel_5, vel_6, vel_7, vel_8;
        ...
        vel_0->begin =
          dealii::Point<dim>(0 * H, H + f(0 * H, H, length) + eps, 0);
        vel_0->end = dealii::Point<dim>(0 * H, H + height - eps, 0);
        vel_1->begin =
          dealii::Point<dim>(1 * H, H + f(1 * H, H, length) + eps, 0);
        vel_1->end = dealii::Point<dim>(1 * H, H + height - eps, 0);
        vel_2->begin =
          dealii::Point<dim>(2 * H, H + f(2 * H, H, length) + eps, 0);
        vel_2->end = dealii::Point<dim>(2 * H, H + height - eps, 0);
        vel_3->begin =
          dealii::Point<dim>(3 * H, H + f(3 * H, H, length) + eps, 0);
        vel_3->end = dealii::Point<dim>(3 * H, H + height - eps, 0);
        vel_4->begin =
          dealii::Point<dim>(4 * H, H + f(4 * H, H, length) + eps, 0);
        vel_4->end = dealii::Point<dim>(4 * H, H + height - eps, 0);
        vel_5->begin =
          dealii::Point<dim>(5 * H, H + f(5 * H, H, length) + eps, 0);
        vel_5->end = dealii::Point<dim>(5 * H, H + height - eps, 0);
        vel_6->begin =
          dealii::Point<dim>(6 * H, H + f(6 * H, H, length) + eps, 0);
        vel_6->end = dealii::Point<dim>(6 * H, H + height - eps, 0);
        vel_7->begin =
          dealii::Point<dim>(7 * H, H + f(7 * H, H, length) + eps, 0);
        vel_7->end = dealii::Point<dim>(7 * H, H + height - eps, 0);
        vel_8->begin =
          dealii::Point<dim>(8 * H, H + f(8 * H, H, length) + eps, 0);
        ...
        my_pp_data.line_plot_data.time_control_data_statistics.time_control_data
          .is_active = calculate_statistics;
        my_pp_data.line_plot_data.time_control_data_statistics.time_control_data
        ...
        pp.reset(new MyPostProcessor<dim, Number>(
          my_pp_data, this->mpi_comm, length, *flow_rate_controller));
