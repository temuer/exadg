  ...
  namespace IncNS
  {
    ...
    public:
            ...
            unsigned int const        component = 0) const final
      {
        ...
        if (component == 0)
          result =
            V_0 * std::sin(p[0] / L) * std::cos(p[1] / L) * std::cos(p[2] / L);
        else if (component == 1)
          result =
            -V_0 * std::cos(p[0] / L) * std::sin(p[1] / L) * std::cos(p[2] / L);
        else if (component == 2)
          result = 0.0;
    ...
    {
    public:
      InitialSolutionPressure(double const V_0,
                              double const L,
                              double const p_0)
    ...
    private:
          ...
          if (exploit_symmetry == false) // periodic box
          ...
          else // symmetric box
            {
              dealii::GridGenerator::subdivided_hyper_cube(
                tria, n_subdivisions_1d_hypercube, 0.0, right);
              ...
              if (mesh_type == MeshType::Curvilinear)
                {
                  ...
                  apply_deformed_cube_manifold(
                    tria, 0.0, right, deformation, frequency);
                }
        ...
        if (exploit_symmetry)
          {
            ...
            this->boundary_descriptor->velocity->symmetry_bc.insert(
              pair(0,
                   new dealii::Functions::ZeroFunction<dim>(
                     dim))); // function will not be used
      ...
      create_postprocessor() final
      {
        ...
        pp_data.kinetic_energy_spectrum_data.time_control_data.start_time =
          start_time;
        pp_data.kinetic_energy_spectrum_data.do_fftw =
          do_fftw_during_simulation;
        pp_data.kinetic_energy_spectrum_data.write_raw_data_to_files =
          not do_fftw_during_simulation;
        pp_data.kinetic_energy_spectrum_data.directory =
          this->output_parameters.directory;
        pp_data.kinetic_energy_spectrum_data.filename =
          name + "_energy_spectrum";
        ...
        pp_data.kinetic_energy_spectrum_data.exploit_symmetry =
          exploit_symmetry;
        pp_data.kinetic_energy_spectrum_data.n_cells_1d_coarse_grid =
          n_subdivisions_1d_hypercube;
        pp_data.kinetic_energy_spectrum_data.refine_level =
      ...
      double const left                = -dealii::numbers::PI * L,
                   right               = dealii::numbers::PI * L;
