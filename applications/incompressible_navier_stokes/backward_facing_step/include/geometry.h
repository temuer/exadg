...
namespace ExaDG
{
  namespace IncNS
  {
    namespace Geometry
    {
      ...
      double const X1_COORDINATE_OUTFLOW_CHANNEL =
        -LENGTH_BFS_UP - GAP_CHANNEL_BFS;
      ...
      grid_transform_y(double const &eta)
      {
        ...
        y =
          xi * (1.0 - (std::tanh(gamma * (xi - eta)) / std::tanh(gamma * xi)));
        return y;
      }
      ...
      inverse_grid_transform_y(double const &y)
      {
        ...
        eta = xi - (1.0 / gamma) *
                     std::atanh((1.0 - y / xi) * std::tanh(gamma * xi));
        return eta;
      }
          ...
          typename dealii::Triangulation<dim>::cell_iterator>> &periodic_faces)
      {
        ...
        dealii::GridGenerator::merge_triangulations(tmp1,
                                                    tria_3,
                                                    triangulation);
            ...
            for (auto const &f : cell->face_indices())
              {
                ...
                if ((std::fabs(cell->face(f)->center()(0) -
                               X1_COORDINATE_OUTFLOW) < 1.e-12))
                  cell->face(f)->set_boundary_id(1);
                // inflow boundary on the left has ID = 2
                if ((std::fabs(cell->face(f)->center()(0) -
                               X1_COORDINATE_INFLOW) < 1.e-12))
                  cell->face(f)->set_boundary_id(2);

                // periodicity in z-direction
                if ((std::fabs(cell->face(f)->center()(2) - WIDTH_BFS / 2.0) <
                     1.e-12))
        ...
        if (use_grid_stretching_in_y_direction == true)
          {
            ...
            static MyManifold<dim> const manifold;
            ...
            for (auto const &f : cell->face_indices())
              {
                ...
                if (std::fabs(cell->face(f)->center()(0) -
                              (center[0] - dimensions[0] / 2.0)) < 1.e-12)
                ...
                if (std::fabs(cell->face(f)->center()(2) -
                              (center[2] - dimensions[2] / 2.0)) < 1.e-12)
                  cell->face(f)->set_all_boundary_ids(2 + 10);
                if (std::fabs(cell->face(f)->center()(2) -
                              (center[2] + dimensions[2] / 2.0)) < 1.e-12)
                  cell->face(f)->set_all_boundary_ids(3 + 10);
