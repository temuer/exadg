...
namespace ExaDG
{
  namespace IncNS
  {
    ...
    do_set_parameters(Parameters &param, bool const is_precursor = false)
    {
      ...
      // convective term
      if (param.formulation_convective_term ==
          FormulationConvectiveTerm::DivergenceFormulation)
        param.upwind_factor =
    ...
    namespace Precursor
    {
      ...
      public:
                      ...
                      &multigrid_mappings) final
        {
          auto const lambda_create_triangulation = [&](
            ...
            Geometry::create_grid_precursor(tria,
                                            global_refinements,
                                            periodic_face_pairs);
          };
        ...
        create_postprocessor() final
        {
          ...
          pp_data_bfs.turb_ch_data.cells_are_stretched =
            Geometry::use_grid_stretching_in_y_direction;
          pp_data_bfs.turb_ch_data.viscosity = viscosity;
          pp_data_bfs.turb_ch_data.directory =
            this->output_parameters.directory;
          pp_data_bfs.turb_ch_data.filename =
            this->output_parameters.filename + "_precursor";

          // use precursor results to prescribe inflow velocity for backward
          // facing step domain
          ...
          pp_data_bfs.inflow_data.normal_coordinate =
            Geometry::X1_COORDINATE_OUTFLOW_CHANNEL;
          ...
          pp.reset(
            new PostProcessorBFS<dim, Number>(pp_data_bfs, this->mpi_comm));
      ...
      public:
        ...
        set_boundary_descriptor() final
        {
          ...
          // inflow boundary condition at left boundary with ID=2: prescribe
          // velocity profile which is obtained as the results of the precursor
          // simulation
          AssertThrow(inflow_data_storage.get(),
                      dealii::ExcMessage(
                        "inflow_data_storage is not initialized."));
          ...
          // the inflow boundary condition is time dependent (du/dt != 0) but,
          // for simplicity, we assume that this is negligible when using the
          // dual splitting scheme
          this->boundary_descriptor->pressure->neumann_bc.insert(2);

          // outflow boundary condition at right boundary with ID=1: set
          // pressure to zero
        ...
        create_postprocessor() final
        {
          ...
          pp_data_bfs.line_plot_data.directory =
            this->output_parameters.directory;
          ...
          // lines
          std::shared_ptr<LineHomogeneousAveraging<dim>> vel_0, vel_1, vel_2,
            vel_3, vel_4, vel_5, vel_6, vel_7, vel_8, vel_9, vel_10, vel_11,
            Cp_1, Cp_2, Cf;
          ...
          vel_0->begin =
            dealii::Point<dim>(Geometry::X1_COORDINATE_INFLOW + eps, 0, 0);
          vel_0->end =
            dealii::Point<dim>(Geometry::X1_COORDINATE_INFLOW + eps, 2 * H, 0);
          ...
          Cp_2->end =
            dealii::Point<dim>(Geometry::X1_COORDINATE_OUTFLOW, -H, 0);
      ...
      public:
          ...
          : ApplicationBase<dim, Number>(input_file, comm)
        {
          inflow_data_storage.reset(
            new InflowDataStorage<dim>(n_points_inflow));
          ...
          this->precursor =
            std::make_shared<PrecursorDomain<dim, Number>>(input_file,
                                                           ...
                                                           inflow_data_storage);
          this->main =
            std::make_shared<MainDomain<dim, Number>>(input_file,
                                                      comm,
                                                      inflow_data_storage);
        }
