...
namespace ExaDG
{
  ...
  {
    /*
     * Manufactured solution for incompressible flow of a generalized Newtonian
     * fluid in a hypercube. (Navier-)Stokes equations, where the convective
     * term may be disabled u     ... velocity vector p     ... kinematic
     * pressure nu    ... kinematic viscosity rho   ... density f     ... body
     * force vector sigma ... Cauchy stress
     *
     ...
     *
     * d/dt(u) + (grad(u)) * u + grad(p) - nu * div(grad(u)) - 2 * sym_grad(u) *
     * grad(nu) = f
     *
     ...
     *
     * shear_rate := sqrt(2 * sym_grad(u) : sym_grad(u)) = 2 cos(t) * cos(x) *
     * cos(y)
     *
     ...
     *
     * to derive a suitable body force vector f from the momentum balance
     * equation.
     */
    ...
    public:
            ...
            unsigned int const        component = 0) const final
      {
        ...
        if (component == 0)
          ...
          {
            AssertThrow(component < 2,
                        dealii::ExcMessage(
                          "Manufactured solution only valid for 2D."));
            return 0.0;
          }
      }
    };
    ...
    public:
            ...
            unsigned int const        component = 0) const final
      {
         ...
         * parts of the stress term yielding for the stress-divergence
         * formulation
         *
        ...
        // nu = nu_oo + (nu_0 - nu_oo) * [kappa +
        // (lambda*shear_rate)^a]^[(n-1)/a]
    ...
    public:
            ...
            unsigned int const        component = 0) const final
      {
        ...
        // velocity Laplacian respecting the rheological law to obtain the
        // kinematic viscosity
        double const shear_rate = 2.0 * cos_t * cos_x * cos_y;
        double const kinematic_viscosity =
          nu_oo +
        ...
        grad_nu *=
          data.lambda * (data.n - 1) *
          std::pow(data.lambda * shear_rate, data.a - 1.0) *
          data.viscosity_margin *
          std::pow(data.kappa + std::pow(data.lambda * shear_rate, data.a),
                   (data.n - 2.0) / data.a);
      ...
      add_parameters(dealii::ParameterHandler &prm) final
      {
        ...
        prm.enter_subsection("Application");
        {
          ...
          prm.add_parameter("TreatmentOfConvectiveTermImplicit",
                            ...
                            dealii::Patterns::Bool());
          prm.add_parameter(
            "TreatmentOfVariableViscosityImplicit",
    ...
    private:
      ...
      set_parameters() final
      {
        ...
        this->param.generalized_newtonian_model_data =
        ...
        if (this->param.temporal_discretization ==
            TemporalDiscretization::BDFPressureCorrection)
          {
            ...
            this->param.newton_solver_data_momentum =
              Newton::SolverData(100, 1.e-14, 1.e-6);

            // linear solver
            this->param.solver_momentum = SolverMomentum::GMRES;
            this->param.solver_data_momentum =
              SolverData(1e4, 1.e-12, 1.e-8, 100);
            this->param.preconditioner_momentum =
              MomentumPreconditioner::InverseMassMatrix;
      ...
      set_boundary_descriptor() final
      {
        AssertThrow(dim == 2,
                    dealii::ExcMessage(
                      "Manufactured solution for dim == 2 implemented only."));
            ...
            if (i == 0 && pure_dirichlet_problem == false)
              {
                ...
                this->boundary_descriptor->velocity->neumann_bc.insert(
                  pair(i,
                       new NeumannBoundaryVelocity<dim>(
                         this->param.formulation_viscous_term,
                         normal_x,
                         normal_y,
                         kinematic_viscosity,
