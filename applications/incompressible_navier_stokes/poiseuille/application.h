  ...
  namespace IncNS
  {
    ...
    public:
            ...
            unsigned int const /*component*/) const final
      {
        ...
        double pressure_gradient =
          -2. * viscosity * max_velocity / std::pow(H / 2., 2.0);
    ...
    public:
            ...
            unsigned int const        component = 0) const final
      {
        ...
        // The Neumann velocity boundary condition that is consistent with the
        // analytical solution (in case of a parabolic inflow profile) is (grad
        // U)*n = 0.

        // Hence:
        // If the viscous term is written in Laplace formulation, prescribe
        // result = 0 as Neumann BC If the viscous term is written in Divergence
        // formulation, the following boundary condition has to be used to
        // ensure that (grad U)*n = 0: (grad U + (grad U)^T)*n = (grad U)^T * n

        if (formulation == FormulationViscousTerm::DivergenceFormulation)
          {
            if (component == 1)
              result =
                -max_velocity * 2.0 * p[1] / std::pow(H / 2., 2.0) * normal;
          }
    ...
    {
    public:
      RightHandSide(double const viscosity,
                    double const max_velocity,
                    double const H)
        : dealii::Function<dim>(dim, 0.0)
        , viscosity(viscosity)
    ...
    private:
      ...
      set_parameters() final
      {
        ...
        this->param.convergence_criterion_steady_problem =
          ConvergenceCriterionSteadyProblem::
            SolutionIncrement; // ResidualSteadyNavierStokes;
        ...
        this->param.solver_data_pressure_poisson =
          SolverData(1000, 1.e-20, 1.e-6, 100);
        ...
        if (this->param.temporal_discretization ==
            TemporalDiscretization::BDFPressureCorrection)
          {
            ...
            this->param.solver_data_momentum =
              SolverData(1e4, 1.e-20, 1.e-6, 100);
            this->param.preconditioner_momentum =
              MomentumPreconditioner::InverseMassMatrix;
              ...
              for (auto const &face : cell->face_indices())
                {
                  ...
                  if (apply_symmetry_bc) // upper wall
                    if ((std::fabs(cell->face(face)->center()(1) - y_upper) <
                         1e-12))
          ...
          if (boundary_condition == BoundaryCondition::Periodic)
            {
              ...
              dealii::GridTools::collect_periodic_faces(
                tria, 1, 2, 0, periodic_face_pairs);
      ...
      set_boundary_descriptor() final
      {
                ...
                this->boundary_descriptor->velocity->neumann_bc.insert(
                  pair(1,
                       new NeumannBoundaryVelocity<dim>(
                         formulation_viscous_term, max_velocity, H, -1.0)));
              }
            else
              {
        ...
        if (apply_symmetry_bc)
          {
            // On symmetry boundaries, a Neumann BC is prescribed for the
            // pressure.
            // -> prescribe dudt for dual-splitting scheme, which is equal to
            // zero since (du/dt)*n = d(u*n)/dt = d(0)/dt = 0, i.e., the time
            // derivative term is multiplied by the normal vector and the normal
            // velocity is zero (= symmetry boundary condition).
