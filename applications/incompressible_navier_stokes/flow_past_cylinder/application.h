        ...
        if (component == 0)
          {
            ...
            double const T     = 1.0;
            double coefficient = dealii::Utilities::fixed_power<dim - 1>(4.) *
                                 Um /
            ...
            if (test_case == 1)
              ...
              result = coefficient * x[1] * (H - x[1]) *
                       ((t / T) < 1.0 ? std::sin(pi / 2. * t / T) : 1.0);
            else if (test_case == 3)
              result =
                coefficient * x[1] * (H - x[1]) * std::sin(pi * t / end_time);
            else
            ...
            if (use_random_perturbations)
              {
                ...
                if (dim == 2)
                ...
                else if (dim == 3)
                  {
                    ...
                    perturbation = linear_interpolation_2d_cartesian(
                      point_3d, y_vector, z_vector, u_vector, component);
                  }
                else
                  AssertThrow(false, dealii::ExcMessage("Not implemented."));
      ...
      add_parameters(dealii::ParameterHandler &prm) final
      {
        ...
        prm.enter_subsection("Application");
        {
          ...
          prm.add_parameter("CFL",
                            cfl_number,
                            "CFL number.",
                            dealii::Patterns::Double(0.0, 1.0e6),
                            true);
        }
    ...
    private:
      ...
      set_parameters() final
      {
        ...
        this->param.solver_data_pressure_poisson =
          SolverData(1000, ABS_TOL, REL_TOL, 30);
        ...
        this->param.multigrid_data_pressure_poisson.smoother_data.iterations =
          5;
        ...
        if (this->param.temporal_discretization ==
            TemporalDiscretization::BDFPressureCorrection)
          {
            ...
            this->param.multigrid_data_momentum.smoother_data
        ...
        this->param.multigrid_data_velocity_block.coarse_problem.solver =
          MultigridCoarseGridSolver::GMRES;
        this->param.multigrid_data_velocity_block.coarse_problem
          .preconditioner = MultigridCoarseGridPreconditioner::BlockJacobi;
      ...
      create_postprocessor() final
      {
        ...
        std::string name = this->output_parameters.filename + "_l" +
                           std::to_string(this->param.grid.n_refine_global) +
        ...
        pp_data.lift_and_drag_data.time_control_data.trigger_every_time_steps =
          1;
        ...
        if (dim == 2)
          ...
          pp_data.lift_and_drag_data.reference_value =
            1.0 / 2.0 * pow(U, 2.0) * D * H;

        // surface for calculation of lift and drag coefficients has boundary_ID
        // = 2
        pp_data.lift_and_drag_data.boundary_IDs.insert(2);

        pp_data.lift_and_drag_data.directory =
        ...
        pp_data.pressure_difference_data.time_control_data
          .trigger_every_time_steps = 1;
        pp_data.pressure_difference_data.time_control_data.start_time =
          start_time;
        if (dim == 2)
          {
            dealii::Point<dim> point_1_2D((X_C - D / 2.0), Y_C),
              point_2_2D((X_C + D / 2.0), Y_C);
        ...
        pp_data.pressure_difference_data.directory =
          this->output_parameters.directory;
        pp_data.pressure_difference_data.filename =
          name + "_pressure_difference";
      ...
      initialize_y_and_z_values()
      {
        AssertThrow(n_points_y >= 2,
                    dealii::ExcMessage("Variable n_points_y is invalid"));
        if (dim == 3)
          AssertThrow(n_points_z >= 2,
                      dealii::ExcMessage("Variable n_points_z is invalid"));
                ...
                if (use_perturbation == true)
                  {
                    ...
                    double       coefficient =
                      dealii::Utilities::fixed_power<dim - 1>(4.) * Um /
                      dealii::Utilities::fixed_power<2 * dim - 2>(H);
                    double perturbation = amplitude_perturbation * coefficient *
      ...
      // select test case according to Schaefer and Turek benchmark definition:
      // 2D-1/2/3, 3D-1/2/3
      unsigned int test_case = 3; // 1, 2 or 3

      ProblemType  problem_type = ProblemType::Unsteady;
      double const Um           = (dim == 2 ? (test_case == 1 ? 0.3 : 1.5) :
                                              (test_case == 1 ? 0.45 : 2.25));
      ...
      // in order to not stop pseudo-timestepping approach before having
      // converged
