  ...
  namespace IncNS
  {
    ...
    public:
            ...
            unsigned int const        component = 0) const final
      {
        ...
        AssertThrow(dim == 3,
                    dealii::ExcMessage("Dimension has to be dim==3."));

        double result = 0.0;
        // use turbulent-like profile with superimposed vortices and random
        // noise to initiate a turbulent flow
        if (component == 0)
    ...
    public:
      ...
      setup(Operator const &pde_operator) final
      {
        ...
        statistics_turb_ch.reset(
          new StatisticsManager<dim, Number>(pde_operator.get_dof_handler_u(),
        ...
        if (statistics_turb_ch->time_control_statistics.time_control
              .needs_evaluation(time, time_step_number))
          {
            statistics_turb_ch->evaluate(
              velocity, Utilities::is_unsteady_timestep(time_step_number));
          }
    ...
    private:
      ...
      set_field_functions() final
      {
        this->field_functions->initial_solution_velocity.reset(
          new InitialSolutionVelocity<dim>());
      ...
      create_postprocessor() final
      {
        ...
        pp_data_turb_ch.turb_ch_data.time_control_data_statistics
          .time_control_data.trigger_every_time_steps = SAMPLE_EVERY_TIME_STEPS;
        pp_data_turb_ch.turb_ch_data.time_control_data_statistics
          .write_preliminary_results_every_nth_time_step =
          SAMPLE_EVERY_TIME_STEPS * 100;
