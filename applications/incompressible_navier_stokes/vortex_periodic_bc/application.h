  ...
  namespace IncNS
  {
    ...
    public:
            ...
            unsigned int const        component = 0) const final
      {
        ...
        double result = 0.0;
        if (component == 0)
          result = (-std::cos(pi * p[0]) * std::sin(pi * p[1])) *
                   std::exp(-2.0 * pi * pi * t * nu);
        else if (component == 1)
          result = (+std::sin(pi * p[0]) * std::cos(pi * p[1])) *
                   std::exp(-2.0 * pi * pi * t * nu);
    ...
    public:
            ...
            unsigned int const /*component*/) const final
      {
        ...
        double const pi = dealii::numbers::PI;
        double const result =
          -0.25 * (std::cos(2 * pi * p[0]) + std::cos(2 * pi * p[1])) *
    ...
    private:
      ...
      set_parameters() final
      {
        ...
        this->param.newton_solver_data_coupled =
          Newton::SolverData(1000, ABS_TOL, REL_TOL);
        ...
        this->param.inverse_mass_operator_hdiv.solver_data =
          SolverData(1000, ABS_TOL, REL_TOL);
        this->param.inverse_mass_operator_hdiv.preconditioner =
