...
namespace ExaDG
{
  namespace Acoustics
  {
    ...
    {
    public:
      AnalyticalSolutionPressure(double const xi,
                                 double const alpha,
                                 double const l)
        : dealii::Function<dim>(1, 0.0)
        , xi(xi)
      ...
      value(dealii::Point<dim> const &p, unsigned int const) const final
      {
        ...
        double const result =
          1.0e-6 * 2.0 * xi * alpha * l * (p[0] * cos_xi_t - p[1] * sin_xi_t) *
          std::exp(-1.0 * alpha *
    ...
    {
    public:
      AnalyticalSolutionVelocity(double const xi,
                                 double const alpha,
                                 double const l)
    ...
    private:
      ...
      set_parameters() final
      {
        ...
        this->param.end_time =
          number_of_rotations * compute_rotation_duration();
        ...
        this->param.restart_data.interval_time =
          (this->param.end_time - this->param.start_time) * 0.4;
      ...
      {
        this->boundary_descriptor->velocity_dbc.insert(std::make_pair(
          1, std::make_shared<AnalyticalSolutionVelocity<dim>>(xi, alpha, l)));
      }
