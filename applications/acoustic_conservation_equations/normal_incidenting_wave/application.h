    ...
    private:
      ...
      set_boundary_descriptor() final
      {
        this->boundary_descriptor->admittance_bc.insert(std::make_pair(
          1,
          std::make_shared<dealii::Functions::ConstantFunction<dim>>(admittance,
                                                                     1)));
      }
      ...
      create_postprocessor() final
      {
        ...
        // To be able to compute an analytical solution for testing purposes we
        // have to make multiple assumptions. The application is intended to be
        // used with different admittances which contradicts the assumptions. We
        // compute the analytical solution only if we run with Y=1 and after the
        // wave left the domain.
        if (analytical_solution_is_available)
          {
            // To be able to compute an error and thus test the behavior of the
            // admittance BC for Y=1 (first-oder ABC). We also have to ensure
            // the that the wave left the domain when computing the error.
            AssertThrow(std::abs(admittance - 1.0) < 1.0e-12,
                        dealii::ExcMessage(
                          "The analytical solution only holds for Y=1."));
            AssertThrow(
              this->param.end_time > 1.5 * radius / speed_of_sound,
              dealii::ExcMessage(
                "The end time has to be after the wave left the domain."));

            // calculation of pressure error
            pp_data.error_data_p.time_control_data.is_active = true;
            pp_data.error_data_p.time_control_data.start_time =
              this->param.start_time;
            ...
            pp_data.error_data_p.calculate_relative_errors =
              false; // analytical solution is zero
            pp_data.error_data_p.name = "pressure";

            // calculation of velocity error
            pp_data.error_data_u.time_control_data.is_active = true;
            pp_data.error_data_u.time_control_data.start_time =
              this->param.start_time;
