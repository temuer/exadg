  ...
  namespace Structure
  {
    ...
    public:
      ...
      value(dealii::Point<dim> const &p, unsigned int const c) const final
      {
        ...
        if (unsteady)
          factor = std::pow(std::sin(this->get_time() * 2.0 *
                                     dealii::numbers::PI / end_time),
                            2.0);
    ...
    // TODO: only the time factor is different compared to the above function ->
    // refactor and unify the code
    template <int dim>
        ...
        if (unsteady)
          {
            factor =
              ...
              (1.0 - 2.0 * std::pow(std::sin(this->get_time() * 2.0 *
                                             dealii::numbers::PI / end_time),
                                    2.0));
          }
    ...
    public:
      ...
      add_parameters(dealii::ParameterHandler &prm) final
      {
        ...
        prm.enter_subsection("Application");
        {
          ...
          prm.add_parameter("UseVolumeForce",
                            use_volume_force,
                            "Use volume force.");
    ...
    private:
      ...
      set_parameters() final
      {
        ...
        if (this->param.problem_type == ProblemType::Unsteady and
        ...
        if (this->param.grid.element_type == ElementType::Simplex)
        ...
        else if (this->param.grid.element_type == ElementType::Hypercube)
          {
            ...
            this->param.grid.create_coarse_triangulations =
              false; // can also be set to true if desired
          }
                    ...
                    &multigrid_mappings) final
      {
        auto const lambda_create_triangulation =
          [&](dealii::Triangulation<dim, dim> &tria,
            ...
            if (this->param.grid.element_type == ElementType::Hypercube)
            ...
            else if (this->param.grid.element_type == ElementType::Simplex)
              {
                ...
                dealii::GridGenerator::subdivided_hyper_rectangle(
                  tria_hypercube, repetitions, p1, p2);
             ...
             *                  bid = 3
             *
             * in the 3d case: face at z = 0 has bid = 4, the other face has bid
             * = 0 (as the top face in the figure above).
             *
             */
            for (auto cell : tria)
              {
                for (auto const &face : cell.face_indices())
                  {
                    ...
                    // right face
                    if (std::fabs(cell.face(face)->center()(0) - this->length) <
                        1e-8)
                      {
                    ...
                    if (dim == 3)
                      {
                        if (std::fabs(cell.face(face)->center()(2) - 0.0) <
                            1e-8)
                          {
      ...
      set_boundary_descriptor() final
      {
        ...
        if (boundary_type == BoundaryType::Dirichlet)
          {
            ...
            this->boundary_descriptor->dirichlet_bc_initial_acceleration.insert(
              pair(2,
                   new AccelerationDBC<dim>(displacement,
                                            quasistatic_solver,
                                            unsteady,
                                            200.0 /* end_time */)));

            this->boundary_descriptor->dirichlet_bc_component_mask.insert(
              pair_mask(2, mask_right));
          }
        ...
        else // we need to apply additional boundary conditions to suppress
             // rigid body motion
          {
            if (dim == 2)
              {
                this->boundary_descriptor->dirichlet_bc.insert(
                  pair(3, new dealii::Functions::ZeroFunction<dim>(dim)));
                this->boundary_descriptor->dirichlet_bc_initial_acceleration
                  .insert(
                    pair(3, new dealii::Functions::ZeroFunction<dim>(dim)));
      ...
      create_postprocessor() final
      {
        ...
        if (boundary_type == BoundaryType::Dirichlet)
          {
            pp_data.error_data.analytical_solution.reset(new SolutionDBC<dim>(
              this->length, this->displacement, vol_force, this->E_modul));
          }
        else if (boundary_type == BoundaryType::Neumann)
          {
            pp_data.error_data.analytical_solution.reset(new SolutionNBC<dim>(
              this->length, this->area_force, vol_force, this->E_modul));
          }
        else
          {
            AssertThrow(false, dealii::ExcMessage("not implemented."));
