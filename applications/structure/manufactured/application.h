...
/*
 * Manufactured solution for nonlinear elasticity problem with St.
 * Venant-Kirchhoff material. The test case can be used in both 2d (plane
 * strain!) and 3d, as well as for testing both steady and unsteady solvers.
 *
 ...
 *     \ Z /   \   0       /
 *
 * with function f(t) in time, and g(X) a one-dimensional displacement constant
 * in Y, Z.
 *
 * If g(X) is a linear function, the Green-Lagrange strains and Piola-Kirchhoff
 * stresses are constant in space, so that the stress term vanishes in the
 * balance equation. Moreover, a linear function can be represented exactly by
 * piecewise linear shape functions (or shape functions of higher degree).
 * Hence, this setup is well suited to measure temporal discretization errors.
 */
  ...
  namespace Structure
  {
    ...
    time_derivative(double const time, double const frequency)
    {
      ...
      if (function_type_time == FunctionTypeTime::Sine)
        ...
        time_factor = 2.0 * std::sin(time * frequency) * frequency *
                      std::cos(time * frequency);
      else
        AssertThrow(false, dealii::ExcMessage("not implemented"));
    ...
    time_2nd_derivative(double const time, double const frequency)
    {
      ...
      if (function_type_time == FunctionTypeTime::Sine)
        ...
        time_factor = 2.0 * frequency * frequency *
                      (std::pow(std::cos(time * frequency), 2.0) -
                       std::pow(std::sin(time * frequency), 2.0));
      else
        AssertThrow(false, dealii::ExcMessage("not implemented"));
    ...
    double
    space_function(double const x,
                   double const length,
                   double const max_displacement)
    {
      ...
      if (function_type_space == FunctionTypeSpace::Linear)
        ...
        space_factor =
          max_displacement * std::sin(x * length * 2.0 * dealii::numbers::PI);
      else if (function_type_space == FunctionTypeSpace::Exponential)
        space_factor = max_displacement * (std::exp(x / length) - 1.0) /
                       (std::exp(1.0) - 1.0);
      else
        AssertThrow(false, dealii::ExcMessage("not implemented"));
    ...
    double
    space_derivative(double const x,
                     double const length,
                     double const max_displacement)
    {
      double space_factor = 1.0;
      ...
      if (function_type_space == FunctionTypeSpace::Linear)
        ...
        space_factor = max_displacement / length * (std::exp(x / length)) /
                       (std::exp(1.0) - 1.0);
      else
        AssertThrow(false, dealii::ExcMessage("not implemented"));
    ...
    double
    space_2nd_derivative(double const x,
                         double const length,
                         double const max_displacement)
    {
      double space_factor = 1.0;
      ...
      if (function_type_space == FunctionTypeSpace::Linear)
        ...
        space_factor = -max_displacement *
                       std::pow(2.0 * dealii::numbers::PI / length, 2.0) *
                       std::sin(x / length * 2.0 * dealii::numbers::PI);
      else if (function_type_space == FunctionTypeSpace::Exponential)
        space_factor = max_displacement / (length * length) *
                       (std::exp(x / length)) / (std::exp(1.0) - 1.0);
      else
        AssertThrow(false, dealii::ExcMessage("not implemented"));
        ...
        if (c == 0)
          {
            double const time_factor =
              time_function(this->get_time(), frequency);

            return space_function(p[0], length, max_displacement) *
                   (unsteady ? time_factor : 1.0);
          }
        else
          return 0.0;
      }
    ...
    public:
      ...
      value(dealii::Point<dim> const &p, unsigned int const c) const final
      {
        if (c == 0)
          {
            double const time_2nd =
              time_2nd_derivative(this->get_time(), frequency);
            double const time_factor =
              time_function(this->get_time(), frequency);

            double const acceleration_term =
              density * space_function(p[0], length, max_displacement) *
              (unsteady ? time_2nd : 0.0);
            double const elasticity_term =
              ...
              (3.0 *
               ...
               1.0) *
              (unsteady ? time_factor : 1.0) *
              space_2nd_derivative(p[0], length, max_displacement);
    ...
    private:
      ...
      {
        this->param.problem_type =
          unsteady ? ProblemType::Unsteady : ProblemType::Steady;
        ...
        this->param.newton_solver_data =
          Newton::SolverData(1e4, 1.e-10, 1.e-10);
      ...
      set_boundary_descriptor() final
      {
        ...
        this->boundary_descriptor->dirichlet_bc.insert(pair(
          0, new Solution<dim>(max_displacement, length, unsteady, frequency)));
        this->boundary_descriptor->dirichlet_bc_initial_acceleration.insert(
          pair(0,
               new InitialAcceleration<dim>(
                 max_displacement, length, unsteady, frequency)));
      ...
      {
        this->field_functions->right_hand_side.reset(new VolumeForce<dim>(
          max_displacement, length, density, unsteady, frequency, f0));
        this->field_functions->initial_displacement.reset(
          new dealii::Functions::ZeroFunction<dim>(dim));
        this->field_functions->initial_velocity.reset(new InitialVelocity<dim>(
          max_displacement, length, unsteady, frequency));
      ...
      create_postprocessor() final
      {
        ...
        pp_data.error_data.time_control_data.trigger_interval =
          (end_time - start_time);
      ...
      double const nu = 0.3;
      double const f0 =
        E * (1.0 - nu) / (1 + nu) / (1.0 - 2.0 * nu); // plane strain
