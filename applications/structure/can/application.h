...
namespace ExaDG
{
  namespace Structure
  {
    ...
    {
    public:
      VolumeForce(double volume_force)
        : dealii::Function<dim>(dim)
    ...
    private:
                    ...
                    &multigrid_mappings) final
      {
        AssertThrow(dim == 3,
                    dealii::ExcMessage(
                      "This application only makes sense for dim=3."));
        ...
        auto const lambda_create_triangulation = [&](
          ...
          dealii::GridGenerator::cylinder_shell(tria,
                                                height,
                                                inner_radius,
                                                outer_radius);
              ...
              for (auto const &f : cell->face_indices())
                {
                  if (cell->face(f)->at_boundary())
                    {
                      dealii::Point<dim> const face_center =
                        cell->face(f)->center();

                      // bottom
                      if (dim == 3 and
                          std::abs(face_center[dim - 1] - 0.0) < 1.e-8)
                        {
                          cell->face(f)->set_boundary_id(0);
                        }
                      // top
                      else if (dim == 3 and
                               std::abs(face_center[dim - 1] - height) < 1.e-8)
                        {
      ...
      set_boundary_descriptor() final
      {
        ...
        //     std::vector<bool> mask_lower = {false, false, true}; // let
        //     boundary slide in x-y-plane
        std::vector<bool> mask_lower = {
          true, true, true}; // clamp boundary, i.e., fix all directions
        ...
        if (boundary_type == BoundaryType::Dirichlet)
          {
            std::vector<bool> mask_upper = {
              false, false, true}; // let boundary slide in x-y-plane
            //      std::vector<bool> mask_upper = {true, true, true}; // clamp
            //      boundary, i.e., fix all directions
            this->boundary_descriptor->dirichlet_bc.insert(pair(
              1, new DisplacementDBC<dim>(displacement, incremental_loading)));
            // DisplacementDBC is a linearly increasing function, so the
            // acceleration is zero.
            this->boundary_descriptor->dirichlet_bc_initial_acceleration.insert(
              pair(1, new dealii::Functions::ZeroFunction<dim>(dim)));

            this->boundary_descriptor->dirichlet_bc_component_mask.insert(
              pair_mask(1, mask_upper));
          }
      ...
      set_field_functions() final
      {
        if (use_volume_force)
          this->field_functions->right_hand_side.reset(
            new VolumeForce<dim>(volume_force));
        else
          this->field_functions->right_hand_side.reset(
