  ...
  namespace ConvDiff
  {
    ...
    public:
            ...
            unsigned int const /*component*/) const final
      {
        ...
        double result =
          phi_l + (phi_r - phi_l) *
                    (std::exp(Pe * p[0] / L) - std::exp(-Pe / 2.0)) /
    ...
    private:
      ...
      set_parameters() final
      {
        ...
        this->param.multigrid_data.smoother_data.smoother =
          MultigridSmoother::Jacobi; // Chebyshev;
        // MG smoother data
                ...
                for (auto const &f : cell.face_indices())
                  {
                    if ((std::fabs(cell.face(f)->center()(1) - left) < 1e-12) or
                        (std::fabs(cell.face(f)->center()(1) - right) <
                         1e-12) or
                        ((dim == 3) and ((std::fabs(cell.face(f)->center()(2) -
                                                    left) < 1e-12) or
      ...
      set_field_functions() final
      {
        ...
        this->field_functions->velocity.reset(
          new dealii::Functions::ConstantFunction<dim>(velocity));
      }
