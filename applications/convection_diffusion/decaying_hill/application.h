...
namespace ExaDG
{
  namespace ConvDiff
  {
    ...
    BoundaryConditionType const BOUNDARY_TYPE =
      BoundaryConditionType::HomogeneousDBC;

    bool const RIGHT_HAND_SIDE =
      (BOUNDARY_TYPE == BoundaryConditionType::HomogeneousNBCWithRHS) ? true :
                                                                        false;
    ...
    {
    public:
      Solution(double const diffusivity)
        ...
        if (BOUNDARY_TYPE == BoundaryConditionType::HomogeneousDBC)
          {
            ...
            result *=
              std::exp(-0.5 * diffusivity * pow(dealii::numbers::PI, 2.0) * t);
          }
    ...
    {
    public:
      RightHandSide(double const diffusivity)
        : dealii::Function<dim>(1, 0.0)
        , diffusivity(diffusivity)
      {}
            ...
            unsigned int const /*component*/) const final
      {
        ...
        if (BOUNDARY_TYPE == BoundaryConditionType::HomogeneousDBC or
        ...
        else if (BOUNDARY_TYPE == BoundaryConditionType::HomogeneousNBCWithRHS)
          {
            ...
            result *=
              -std::pow(dealii::numbers::PI, 2.0) * diffusivity *
              std::exp(-2.0 * diffusivity * pow(dealii::numbers::PI, 2.0) * t);
          }
        else
          {
    ...
    private:
      ...
      set_parameters() final
      {
        ...
        // SOLVER
        this->param.solver = Solver::CG; // use FGMRES for elementwise iterative
                                         // block Jacobi type preconditioners
        this->param.solver_data = SolverData(1e4, 1.e-20, 1.e-6, 100);
        this->param.preconditioner =
          Preconditioner::InverseMassMatrix; // BlockJacobi; //Multigrid;
        ...
        this->param.preconditioner_block_diagonal =
        ...
        if (BOUNDARY_TYPE == BoundaryConditionType::HomogeneousDBC)
          {
            this->boundary_descriptor->dirichlet_bc.insert(
              pair(0, new Solution<dim>(diffusivity)));
          }
      ...
      set_field_functions() final
      {
        this->field_functions->initial_solution.reset(
          new Solution<dim>(diffusivity));
        this->field_functions->right_hand_side.reset(
          new RightHandSide<dim>(diffusivity));
        this->field_functions->velocity.reset(
