  ...
  namespace ConvDiff
  {
    ...
    public:
            ...
            unsigned int const /*component*/) const final
      {
        ...
        double result   = std::exp(-50 * pow(p[0] - center_x, 2.0) -
                                 50 * pow(p[1] - center_y, 2.0));
    ...
    {
    public:
      VelocityField(unsigned int const n_components = dim,
                    double const       time         = 0.)
        : dealii::Function<dim>(n_components, time)
      {}

      double
      value(dealii::Point<dim> const &point,
    ...
    private:
      ...
      set_parameters() final
      {
        ...
        //    TemporalDiscretization::ExplRK; this->param.time_integrator_rk =
        //    TimeIntegratorRK::ExplRK3Stage7Reg2;
        ...
        this->param.preconditioner =
          Preconditioner::Multigrid; // None; //InverseMassMatrix;
                                     // //PointJacobi; // BlockJacobi;
                                     // //Multigrid;
        this->param.update_preconditioner = true;

        // BlockJacobi (these parameters are also relevant if used as a smoother
        // in multigrid)
