...
namespace ExaDG
{
    ...
    private:
      ...
      set_parameters() final
      {
        ...
        this->param.start_time =
          n_rotations_before_start *
          CoRotVortexPair::compute_rotation_period(intensity, r_0);
        this->param.end_time =
          n_rotations *
          CoRotVortexPair::compute_rotation_period(intensity, r_0);
      ...
      set_boundary_descriptor() final
      {
        ...
        this->boundary_descriptor->admittance_bc.insert(std::make_pair(
          1, std::make_shared<dealii::Functions::ConstantFunction<dim>>(Y, 1)));
      }
      ...
      create_postprocessor() final
      {
        ...
        pp_data.output_data.filename =
          this->output_parameters.filename + "_acoustic";
  ...
  namespace FluidAeroAcoustic
  {
    ...
    {
    public:
      AnalyticalSolutionVelocity(double const intensity,
                                 double const r_0,
                                 double const r_c)
            ...
            double const       theta) const
      {
        double result = intensity * r /
                        (pi * (std::pow(r * r + r_0 * r_0 + r_c * r_c, 2) -
                               4.0 * r * r * r_0 * r_0 *
                                 std::pow(std::cos(omega * t - theta), 2)));

        if (comp == 0)
          result *= -(r_c * r_c + r * r) * std::sin(theta) +
                    r_0 * r_0 * std::sin(2.0 * omega * t - theta);
        else
          result *= (r_c * r_c + r * r) * std::cos(theta) -
                    r_0 * r_0 * std::cos(2.0 * omega * t - theta);
    ...
    {
    public:
      AnalyticalSolutionPressure(double const intensity,
                                 double const r_0,
                                 double const r_c)
      ...
      value(dealii::Point<dim> const &p, unsigned int const) const final
      {
        ...
        double const dRe_dt =
          ...
          (r_0 * r_0 + r_c * r_c -
           r * r * std::cos(2.0 * omega * t - 2.0 * theta)) /
          (pi * (std::pow(r * r + r_0 * r_0 + r_c * r_c, 2) -
                 4.0 * r * r * r_0 * r_0 *
                   std::pow(std::cos(omega * t - theta), 2)));
    ...
    public:
      ...
      set_parameters() final
      {
        ...
        this->param.viscosity =
          0.0; // analytical solution derived with potential theory
        ...
        this->param.max_velocity =
          intensity * r * ((r_c * r_c + r * r) - r_0 * r_0) /
        ...
        this->param.grid.create_coarse_triangulations =
          additional_refinements_around_source > 0;
        ...
        if (this->param.formulation_convective_term ==
            FormulationConvectiveTerm::DivergenceFormulation)
          this->param.upwind_factor =
            0.5; // allows using larger CFL values for explicit formulations
        ...
        if (this->param.temporal_discretization ==
            TemporalDiscretization::BDFPressureCorrection)
          {
            ...
            if (this->param.treatment_of_convective_term ==
              ...
              this->param.solver_data_momentum =
                SolverData(1e4, ABS_TOL_LINEAR, REL_TOL_LINEAR, 100);
            else
              this->param.solver_data_momentum =
        ...
        if (this->param.treatment_of_convective_term ==
          ...
          this->param.solver_data_coupled =
            SolverData(1e3, ABS_TOL_LINEAR, REL_TOL_LINEAR, 100);
        else
          this->param.solver_data_coupled =
            SolverData(1e3, ABS_TOL, REL_TOL, 100);
        ...
        this->param.multigrid_data_pressure_block.coarse_problem.solver =
          MultigridCoarseGridSolver::CG;
        this->param.multigrid_data_pressure_block.coarse_problem
          .preconditioner = MultigridCoarseGridPreconditioner::AMG;
      }
                    ...
                    &multigrid_mappings) final
      {
        auto const lambda_create_triangulation =
          [&](dealii::Triangulation<dim, dim> &tria,
            ...
            refine_triangulation_around_center(
              tria, additional_refinements_around_source, 1.5 * r_0);
          };
            ...
            for (unsigned int r = 0; r < n_ref; ++r)
              {
                for (auto const &cell : tria.active_cell_iterators())
                  if (cell->is_locally_owned())
                    {
                      for (unsigned int const i :
                           dealii::GeometryInfo<dim>::vertex_indices())
                        {
      ...
      {
        this->field_functions->initial_solution_velocity =
          std::make_shared<AnalyticalSolutionVelocity<dim>>(intensity,
                                                            r_0,
                                                            r_c);

        this->field_functions->initial_solution_pressure =
          std::make_shared<AnalyticalSolutionPressure<dim>>(intensity,
                                                            r_0,
                                                            r_c);

        this->field_functions->analytical_solution_velocity =
          std::make_shared<FluidAeroAcoustic::AnalyticalSolutionVelocity<dim>>(
            intensity, r_0, r_c);

        this->field_functions->analytical_solution_pressure =
          std::make_shared<FluidAeroAcoustic::AnalyticalSolutionPressure<dim>>(
            intensity, r_0, r_c);
  ...
  namespace AeroAcoustic
  {
    ...
    public:
      BlendInFunction(double const blend_in_start, double const blend_in_end)
        : Utilities::SpatialAwareFunction<dim>(1, 0.0)
        , start(blend_in_start)
        , end(blend_in_end)
      {}
    ...
    public:
      ...
      value(dealii::Point<dim> const &p, unsigned int const) const final
      {
        ...
        double const f1 = r_0 * r_0 *
                          (r_0 * r_0 + r_c * r_c -
                           r * r * std::cos(2.0 * omega * t - 2.0 * theta));

        double const f2 = r * r *
                          (std::pow(r_c * r_c + r * r, 2) + std::pow(r_0, 4) -
                           2.0 * r_0 * r_0 * (r * r + r_c * r_c) *
                             std::cos(2.0 * omega * t - 2.0 * theta));

        double const g =
          std::pow(r * r + r_0 * r_0 + r_c * r_c, 2) -
          4.0 * r * r * r_0 * r_0 * std::pow(std::cos(omega * t - theta), 2);

        double const dt_f1 = -2.0 * r * r * r_0 * r_0 * omega *
                             std::sin(2.0 * theta - 2.0 * omega * t);

        double const dt_f2 = -4.0 * r * r * r_0 * r_0 * omega *
                             (r_c * r_c + r * r) *
                             std::sin(2.0 * theta - 2.0 * omega * t);

        double const dt_g = -8.0 * r * r * r_0 * r_0 * omega *
                            std::sin(theta - omega * t) *
                            std::cos(theta - omega * t);

        double const ddt_1 = (dt_f1 * g - f1 * dt_g) / (g * g);
        double const ddt_2 = (dt_f2 * g - 2.0 * f2 * dt_g) / (g * g * g);

        if (not source_term_with_convection)
          return intensity * omega / pi * ddt_1 +
                 0.5 * intensity * intensity / (pi * pi) * ddt_2;

        double const dr_f1 =
          -2.0 * r * r_0 * r_0 * std::cos(2.0 * theta - 2.0 * t * omega);
        double const dtheta_f1 =
          2.0 * r * r * r_0 * r_0 * std::sin(2.0 * theta - 2.0 * t * omega);


        double const dr_f2 =
          2.0 * r *
          (std::pow(r_c, 4) -
           2.0 * r_0 * r_0 * (r_c * r_c + 2.0 * r * r) *
             std::cos(2.0 * theta - 2.0 * t * omega) +
           4.0 * r_c * r_c * r * r + 3.0 * std::pow(r, 4) + std::pow(r_0, 4));
        double const dtheta_f2 = 4.0 * r * r * r_0 * r_0 * (r_c * r_c + r * r) *
                                 std::sin(2.0 * theta - 2.0 * omega * t);

        double const dr_g =
          4.0 * r *
          (r_c * r_c + r * r -
           2.0 * r_0 * r_0 * std::pow(std::cos(theta - omega * t), 2) +
           r_0 * r_0);
        double const dtheta_g = 8.0 * r * r * r_0 * r_0 *
                                std::sin(theta - omega * t) *
                                std::cos(theta - omega * t);

        double const w_r = intensity * r * r_0 * r_0 *
                           std::sin(2.0 * omega * t - 2.0 * theta) / (pi * g);

        double const w_theta =
          intensity * r *
          (r * r + r_c * r_c -
           r_0 * r_0 * std::cos(2.0 * omega * t - 2.0 * theta)) /
          (pi * g);


        double const DDt_1 =
          ddt_1 + (dr_f1 * g - f1 * dr_g) / (g * g) * w_r +
          (dtheta_f1 * g - f1 * dtheta_g) / (r * g * g + 1e-16) * w_theta;

        double const DDt_2 = ddt_2 +
                             (dr_f2 * g - 2.0 * f2 * dr_g) / (g * g * g) * w_r +
                             (dtheta_f2 * g - 2.0 * f2 * dtheta_g) /
                               (r * g * g * g + 1e-16) * w_theta;
    ...
    private:
      void
      set_single_field_solvers(std::string     input_file,
                               MPI_Comm const &comm) final
      {
        this->acoustic =
          std::make_shared<AcousticsAeroAcoustic::Application<dim, Number>>(
            input_file, comm);
        this->fluid =
          std::make_shared<FluidAeroAcoustic::Application<dim, Number>>(
            input_file, comm);
      }
      ...
      set_field_functions() final
      {
        this->field_functions->source_term_blend_in =
          std::make_shared<BlendInFunction<dim>>(
        ...
        this->field_functions->analytical_aero_acoustic_source_term =
          std::make_shared<AnalyticalSourceTerm<dim>>(
            source_term_with_convection, intensity, r_0, r_c);
      }
