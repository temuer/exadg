...
namespace ExaDG
{
  namespace CompNS
  {
    // set parameters according to Wiart et al. ("Assessment of discontinuous
    // Galerkin method for the simulation of vortical flows at high Reynolds
    // number"):
    ...
    // set Ma number -> calculate speed of sound c_0, temperature T_0, pressure
    // p_0
    ...
    {
    public:
      InitialSolution(unsigned int const n_components = dim + 2,
                      double const       time         = 0.)
        : dealii::Function<dim>(n_components, time)
      {}

      double
      value(dealii::Point<dim> const &x,
            unsigned int const        component = 0) const final
      {
        double const u1 =
          V_0 * std::sin(x[0] / L) * std::cos(x[1] / L) * std::cos(x[2] / L);
        double const u2 =
          -V_0 * std::cos(x[0] / L) * std::sin(x[1] / L) * std::cos(x[2] / L);
        double const p =
          p_0 + RHO_0 * V_0 * V_0 / 16.0 *
      ...
      add_parameters(dealii::ParameterHandler &prm) final
      {
        ...
        prm.enter_subsection("Application");
        {
          prm.add_parameter("MeshType",
                            mesh_type,
                            "Type of mesh (Cartesian versus curvilinear).");
        }
      }
    ...
    private:
      ...
      set_parameters() final
      {
        ...
        // Classical RK:
        // classical RK1 (1 stage): CFL_crit = 0.04 - 0.05, costs = stages / CFL
        // = 20 - 25 classical RK2 (2 stages):CFL_crit = 0.31  - 0.32, costs =
        // stages / CFL =  6.3 - 6.5 classical RK3 (3 stages):CFL_crit = 0.35 -
        // 0.4, costs = stages / CFL =  7.5 - 8.5 classical RK4 (4
        // stages):CFL_crit = 0.4  - 0.45, costs = stages / CFL =  8.9 - 10

        // Kennedy et al.:
        // LowStorageRK3Stage3Reg2: CFL_crit = 0.3 - 0.4, costs = stages / CFL
        // =  7.5 - 10 LowStorageRK3Stage4Reg2: CFL_crit = 0.4 - 0.5, costs =
        // stages / CFL =  8 - 10 LowStorageRK4Stage5Reg2: CFL_crit = 0.725 -
        // 0.75, costs = stages / CFL =  6.7 - 6.9 LowStorageRK4Stage5Reg3:
        // CFL_crit = 0.7 - 0.725, costs = stages / CFL =  6.9 - 7.1
        // LowStorageRK5Stage9Reg2: CFL_crit = 0.8 - 0.9, costs = stages / CFL =
        // 10  - 11.3

        // Toulorge & Desmet: 3rd order scheme with 7 stages currently the most
        // efficient scheme LowStorageRK3Stage7Reg2: CFL_crit = 1.3 - 1.35,
        // costs = stages / CFL =  5.2 - 5.4, computational costs = 0.69 CPUh
        // LowStorageRK4Stage8Reg2: CFL_crit = 1.25 - 1.3, costs = stages / CFL
        // =  6.2 - 6.4

        // Kubatko et al.:
        // SSPRK(7,3): CFL_crit = 1.2 - 1.25, costs = stages / CFL = 5.6 - 5.8
        // SSPRK(8,3): CFL_crit = 1.5 - 1.6, costs = stages / CFL = 5.0 - 5.3,
        // computational costs = 0.77 CPUh SSPRK(8,4): CFL_crit = 1.25 - 1.3,
        // costs = stages / CFL = 6.2 - 6.4

        this->param.temporal_discretization =
          TemporalDiscretization::ExplRK3Stage7Reg2;
        ...
        this->param.restart_data.filename = this->output_parameters.directory +
      ...
      create_postprocessor() final
      {
        ...
        pp_data.kinetic_energy_spectrum_data.time_control_data
          .trigger_interval = 0.5;
        pp_data.kinetic_energy_spectrum_data.time_control_data.start_time =
          start_time;


        pp_data.kinetic_energy_spectrum_data.directory =
          this->output_parameters.directory;
        pp_data.kinetic_energy_spectrum_data.filename =
          this->output_parameters.filename + "_spectrum";
        pp_data.kinetic_energy_spectrum_data.degree = this->param.degree;
        pp_data.kinetic_energy_spectrum_data.evaluation_points_per_cell =
          (this->param.degree + 1) * 1;
