...
namespace ExaDG
{
  namespace CompNS
  {
    ...
    // set Re = u_tau * delta / nu, density=1, u_tau=1, delta=1 -> calculate
    ...
    /*
     *  maps eta in [0,1] --> y in [-1,1]*length_y/2.0 (using a hyperbolic mesh
     * stretching)
     */
    double
    grid_transform_y(double const &eta)
    {
      ...
      if (GRID_STRETCH_FAC >= 0)
        y = DIMENSIONS_X2 / 2.0 *
            std::tanh(GRID_STRETCH_FAC * (2. * eta - 1.)) /
    ...
    public:
            ...
            unsigned int const        component = 0) const final
      {
        ...
        double velocity1 = -MAX_VELOCITY * (pow(p[1], 6.0) - 1.0) *
                           (1.0 + ((double)rand() / RAND_MAX - 1.0) * 0.5 -
                            2. / MAX_VELOCITY * std::sin(p[2] * 8.));
        double velocity3 = (pow(p[1], 6.0) - 1.0) * std::sin(p[0] * 8.) * 2.;

        // viscous time step limitations: consider a laminar test case with a
        // large viscosity
    ...
    public:
      ...
      MyPostProcessor(MyPostProcessorData<dim> const &pp_data_turb_channel,
      ...
      setup(Operator<dim, Number> const &pde_operator) final
      {
        ...
        statistics_turb_ch.reset(new StatisticsManager<dim, Number>(
          pde_operator.get_dof_handler_vector(), pde_operator.get_mapping()));
                        ...
                        types::time_step const time_step_number) final
      {
        ...
        if (statistics_turb_ch->time_control_statistics.time_control
              .needs_evaluation(time, time_step_number))
          {
        ...
        if (statistics_turb_ch->time_control_statistics
    ...
    private:
                  ...
                  std::shared_ptr<dealii::Mapping<dim>> &mapping) final
      {
        auto const lambda_create_triangulation = [&](
          ...
          const BoundaryLayerManifold<dim> manifold(dimensions,
                                                    GRID_STRETCH_FAC);
          ...
          dealii::GridTools::collect_periodic_faces(
            tria, 0 + 10, 1 + 10, 0, periodic_face_pairs);
          if (dim == 3)
            dealii::GridTools::collect_periodic_faces(
              tria, 2 + 10, 3 + 10, 2, periodic_face_pairs);
      ...
      set_boundary_descriptor() final
      {
        ...
        // For Neumann boundaries, no value is prescribed (only first derivative
        // of density occurs in equations). Hence the specified function is
        // irrelevant (i.e., it is not used).
      ...
      create_postprocessor() final
      {
        ...
        pp_data_turb_ch.turb_ch_data.time_control_data_statistics
          .time_control_data.end_time = SAMPLE_END_TIME;
        pp_data_turb_ch.turb_ch_data.time_control_data_statistics
          .time_control_data.trigger_every_time_steps = 10;
        pp_data_turb_ch.turb_ch_data.time_control_data_statistics
        ...
        pp_data_turb_ch.turb_ch_data.directory =
          this->output_parameters.directory;
        pp_data_turb_ch.turb_ch_data.filename =
          this->output_parameters.filename;

        std::shared_ptr<PostProcessorBase<dim, Number>> pp;
        pp.reset(
          new MyPostProcessor<dim, Number>(pp_data_turb_ch, this->mpi_comm));
