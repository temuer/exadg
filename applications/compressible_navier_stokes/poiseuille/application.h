...
namespace ExaDG
{
  namespace CompNS
  {
    ...
    parabolic_velocity_profile(double const y, double const t)
    {
      ...
      double result = U_0 * (1.0 - pow(y / (H / 2.0), 2.0)) *
                      (t < T ? std::sin(pi / 2. * t / T) : 1.0);
    ...
    private:
      ...
      set_boundary_descriptor() final
      {
        ...
        // pressure
        this->boundary_descriptor->pressure.dirichlet_bc.insert(pair(
          1, new dealii::Functions::ConstantFunction<dim>(RHO_0 * R * T_0, 1)));
