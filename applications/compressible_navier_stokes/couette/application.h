...
namespace ExaDG
{
  namespace CompNS
  {
    ...
    public:
      ...
      {
        double const T = T_0 + DYN_VISCOSITY * U_0 * U_0 / (2.0 * LAMBDA) *
                                 (p[1] * p[1] / (H * H) - 1.0);
          ...
          for (auto cell : tria)
            {
              for (auto const &face : cell.face_indices())
                {
                  if (std::fabs(cell.face(face)->center()(1) - point2[1]) <
                      1e-12)
                    {
                      cell.face(face)->set_boundary_id(1);
                    }
                  else if (std::fabs(cell.face(face)->center()(1) - 0.0) <
                           1e-12)
                    {
                      cell.face(face)->set_boundary_id(0);
                    }
                  else if (std::fabs(cell.face(face)->center()(0) - 0.0) <
                           1e-12)
                    {
                      cell.face(face)->set_boundary_id(0 + 10);
                    }
                  else if (std::fabs(cell.face(face)->center()(0) - point2[0]) <
                           1e-12)
                    {
