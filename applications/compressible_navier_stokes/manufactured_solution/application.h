 ...
 *  conditions in x_2-direction. The velocity u_2 is zero. The energy is
 * constant. The density and the velocity u_1 are a function of x_1 and time t.
 */
...
namespace ExaDG
{
  namespace CompNS
  {
    ...
    {
    public:
      Solution(unsigned int const n_components = dim + 2,
               double const       time         = 0.)
    ...
    {
    public:
      RightHandSideDensity(unsigned int const n_components = 1,
                           double const       time         = 0.)
        : dealii::Function<dim>(n_components, time)
      {}
    ...
    {
    public:
      RightHandSideVelocity(unsigned int const n_components = dim,
                            double const       time         = 0.)
        : dealii::Function<dim>(n_components, time)
      {}
    ...
    {
    public:
      RightHandSideEnergy(unsigned int const n_components = 1,
                          double const       time         = 0.)
        : dealii::Function<dim>(n_components, time)
      {}
    ...
    else
    {
      AssertThrow(false, dealii::ExcMessage("Not implemented."));
        ...
        for (unsigned int d = 0; d < dim; ++d)
          {
            f_times_u += rhs_velocity.value(p, d) *
                         analytical_solution.value(p, 1 + d) /
    ...
    private:
                  ...
                  std::shared_ptr<dealii::Mapping<dim>> &mapping) final
      {
        auto const lambda_create_triangulation = [&](
              ...
              for (auto const &face : cell.face_indices())
                {
                  ...
                  else if (std::fabs(cell.face(face)->center()(1) - right) <
                           1e-12)
                    {
          ...
          dealii::GridTools::collect_periodic_faces(
            tria, 0 + 10, 1 + 10, 1, periodic_face_pairs);
      ...
      set_boundary_descriptor() final
      {
        ...
        this->boundary_descriptor->density.dirichlet_bc.insert(
          pair(0, new DensityBC<dim>()));
        this->boundary_descriptor->velocity.dirichlet_bc.insert(
          pair(0, new VelocityBC<dim>()));
