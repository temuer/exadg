  ...
  namespace FTI
  {
    ...
    public:
            ...
            unsigned int const /*component = 0*/) const final
      {
        ...
        perturbation *=
          1.0 * delta_T *
          std::pow(std::sin(dealii::numbers::PI * p[0] / (length / 4.)), 2.0);
        if (dim == 3)
          perturbation *=
            std::pow(std::sin(dealii::numbers::PI * p[2] / (length / 4.)), 2.0);
        ...
        auto const lambda_create_triangulation = [&](
          ...
          if (dim == 2)
          ...
          else if (dim == 3)
            {
              ...
              dealii::Point<dim>        point1(-L / 2., 0.0, -L / 2.),
                point2(L / 2., H, L / 2.);
              dealii::GridGenerator::subdivided_hyper_rectangle(tria,
                                                                repetitions,
                                                                point1,
              ...
              for (auto const &f : cell->face_indices())
                {
                  ...
                  if (dim == 3)
                    {
                      if ((std::fabs(cell->face(f)->center()(2) + L / 2.) <
                           1e-12))
          ...
          dealii::GridTools::collect_periodic_faces(
            tria, 10, 11, 0, periodic_face_pairs);
          if (dim == 3)
            dealii::GridTools::collect_periodic_faces(
              tria, 12, 13, 2, periodic_face_pairs);
    ...
    private:
      ...
      set_boundary_descriptor() final
      {
        ...
        this->boundary_descriptor->dirichlet_bc.insert(
          pair(1,
               new TemperatureBC<dim>(T_ref, delta_T, L, characteristic_time)));
      }
