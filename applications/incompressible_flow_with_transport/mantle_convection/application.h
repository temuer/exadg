...
namespace ExaDG
{
  namespace FTI
  {
    ...
    public:
            ...
            unsigned int const /*component = 0*/) const final
      {
        ...
        if (dim == 2)
          perturbation *=
            0.25 * delta_T *
            std::pow(std::sin(dealii::numbers::PI * (p[0] + std::sqrt(2)) /
                              (length / 2.)) *
                       std::sin(dealii::numbers::PI * (p[1] + std::sqrt(3)) /
                                (length / 2.)),
                     2.0);
        else if (dim == 3)
          perturbation *=
            ...
            std::pow(std::sin(dealii::numbers::PI * (p[0] + std::sqrt(2)) /
                       ...
                       std::sin(dealii::numbers::PI * (p[1] + std::sqrt(3)) /
                                (length / 2.)) *
                       std::sin(dealii::numbers::PI * (p[2] + std::sqrt(5)) /
                                (length / 2.)),
                     2.0);
        else
    ...
    double const U =
      std::sqrt(Ra * kinematic_viscosity * thermal_diffusivity / (H * H));
    ...
    private:
      ...
      set_parameters() final
      {
        ...
        this->param.type_penalty_parameter =
        ...
        this->param.solver_data_pressure_poisson =
          SolverData(1000, 1.e-30, reltol, 100);
        ...
        if (this->param.temporal_discretization ==
            TemporalDiscretization::BDFPressureCorrection)
          {
            ...
            this->param.newton_solver_data_momentum =
              Newton::SolverData(100, 1.e-30, reltol);

            // linear solver
            this->param.solver_momentum = SolverMomentum::CG;
            this->param.solver_data_momentum =
              SolverData(1e4, 1.e-30, reltol, 100);
            this->param.preconditioner_momentum =
              MomentumPreconditioner::Multigrid;
            this->param.multigrid_data_momentum.type = MultigridType::cphMG;
            this->param.multigrid_operator_type_momentum =
                    ...
                    &multigrid_mappings) final
      {
        auto const lambda_create_triangulation = [&](
          ...
          dealii::Point<dim> center = dim == 2 ? dealii::Point<dim>(0., 0.) :
                  ...
                  if (cell->face(f)->at_boundary())
                    {
                      ...
                      for (auto const &v : cell->face(f)->vertex_indices())
                        {
                          if (std::abs(
                                center.distance(cell->face(f)->vertex(v)) -
                                R1) > 1e-12 * R1)
                            {
    ...
    private:
      ...
      create_postprocessor() final
      {
        ...
        pp.reset(
          new ConvDiff::PostProcessor<dim, Number>(pp_data, this->mpi_comm));
    ...
    public:
        ...
        : ApplicationBase<dim, Number>(input_file, comm)
      {
        ...
        this->scalars[0] =
          std::make_shared<Scalar<dim, Number>>(input_file, comm);
      }
    };
