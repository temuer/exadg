...
namespace ExaDG
{
  namespace FTI
  {
    ...
    {
    public:
      InitialTemperature(double const T_ref,
            ...
            unsigned int const /*component = 0*/) const final
      {
        ...
        if (dim == 2)
        ...
        else if (dim == 3)
          {
            dealii::Point<dim> center =
              dealii::Point<dim>(0.5 * L, 0.35 * L, 0.5 * L);
            radius = p.distance(center);
          }
        else
          {
        ...
        double const factor =
          radius < r_b ?
            delta_T / 2.0 * (1 + std::cos(dealii::numbers::PI * radius / r_b)) :
            0.0;
    ...
    // solver tolerances
    double const ABS_TOL = 1.e-12; // 1.e-10 was necessary for dual splitting
                                   // scheme and refine level 8
