...
namespace ExaDG
{
  namespace Poisson
  {
    ...
    public:
      ...
      dealii::Tensor<1, dim>
      gradient(dealii::Point<dim> const &p,
               unsigned int const        c = 0) const final
      {
        (void)c;
        (void)p;
      ...
      Number
      value(dealii::Point<dim, Number> const &p) const
      {
        (void)p;
        Number value;
        value = 1;
    ...
    protected:
      ...
      static std::array<dealii::Point<dim>, n_source_centers> const
                          source_centers;
    ...
    template <>
    std::array<dealii::Point<1>, SolutionBase<1>::n_source_centers> const
      SolutionBase<1>::source_centers = {{dealii::Point<1>(-1.0 / 3.0),
                                          dealii::Point<1>(0.0),
                                          dealii::Point<1>(+1.0 / 3.0)}};

    template <>
    std::array<dealii::Point<2>, SolutionBase<2>::n_source_centers> const
      SolutionBase<2>::source_centers = {{dealii::Point<2>(-0.5, +0.5),
                                          dealii::Point<2>(-0.5, -0.5),
                                          dealii::Point<2>(+0.5, -0.5)}};

    template <>
    std::array<dealii::Point<3>, SolutionBase<3>::n_source_centers> const
      SolutionBase<3>::source_centers = {{dealii::Point<3>(-0.5, +0.5, 0.25),
        ...
        for (unsigned int i = 0; i < this->n_source_centers; ++i)
          {
            const dealii::Tensor<1, dim> x_minus_xi =
              p - this->source_centers[i];
            return_value +=
              std::exp(-x_minus_xi.norm_square() / (this->width * this->width));
          }

        return return_value /
               dealii::Utilities::fixed_power<dim>(
    ...
    private:
      ...
      create_postprocessor() final
      {
        ...
        pp.reset(new PostProcessor<dim, n_components, Number>(pp_data,
                                                              this->mpi_comm));
