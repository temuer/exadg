...
namespace ExaDG
{
  namespace Poisson
  {
    ...
    {
    public:
      NeumannBoundary(unsigned int const n_components = 1,
                      double const       time         = 0.)
        : dealii::Function<dim>(n_components, time)
      {}
    ...
    public:
      ...
      add_parameters(dealii::ParameterHandler &prm) final
      {
        ...
        prm.enter_subsection("Application");
        {
          ...
          prm.add_parameter(
            "MultigridCoarseGridPreconditioner",
    ...
    private:
                    ...
                    &multigrid_mappings) final
      {
        auto const lambda_create_triangulation = [&](
          ...
          // choose a coarse grid with at least 2^dim elements to obtain a
          // non-trivial coarse grid problem
          unsigned int const n_cells_1d =
            std::max((unsigned int)2, this->n_subdivisions_1d_hypercube);

          if (read_external_grid)
            {
              GridUtilities::read_external_triangulation<dim>(tria,
                                                              this->param.grid);
            }
          else
            {
          ...
          if (USE_PERIODIC_BOUNDARY)
            {
              AssertThrow(
                USE_NEUMANN_BOUNDARY == false,
                dealii::ExcMessage(
                  "Neumann and periodic boundaries may not be combined."));
          ...
          if (mesh_type == MeshType::Cartesian)
          ...
          else if (mesh_type == MeshType::BoundaryLayer)
            {
              ...
              BoundaryLayerManifold<dim> manifold(dimensions,
                                                  grid_stretch_factor);
      ...
      set_boundary_descriptor() final
      {
        ...
        if (USE_NEUMANN_BOUNDARY)
          this->boundary_descriptor->neumann_bc.insert(
            pair(1, new NeumannBoundary<dim>()));
      }
